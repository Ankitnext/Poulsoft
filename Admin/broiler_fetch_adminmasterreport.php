<?php
//broiler_fetch_adminmasterreport.php $dbname = $_SESSION['dbase'];
...
for($incr = 0;$incr < $row_incr;$incr++){
    ...
    /*if($mfl_col_name[$incr] == "supplier_weeks"){
        $sql_col_name2 = "ALTER TABLE `master_reportfields` DROP COLUMN `supplier_weeks`;"; mysqli_query($sconn,$sql_col_name2);
    }*/
    //$sql_col_name2 = "ALTER TABLE `master_reportfields` DROP COLUMN `supplier_weeks`;"; mysqli_query($sconn,$sql_col_name2);
}
...
while($row = mysqli_fetch_assoc($query)){
    ...
    if($count1 > 0){
        ...
        for($incr = 0;$incr < $row_incr;$incr++){
            ...
            /*if($mfl_col_name[$incr] == "supplier_weeks"){
                $sql_col_name2 = "ALTER TABLE `master_reportfields` CHANGE `supplier_weeks` `supplier_weaks` VARCHAR(15) CHARACTER SET latin1 COLLATE latin1_swedish_ci NULL DEFAULT  'N:0:0';"; mysqli_query($tmp_conn,$sql_col_name2);
            }*/
            //$sql_col_name2 = "ALTER TABLE `master_reportfields` DROP COLUMN `supplier_weeks`;"; mysqli_query($tmp_conn,$sql_col_name2);
        }
    }
    else{
        ...
        if(!mysqli_query($tmp_conn,$sql3)){
            ...
            for($incr = 0;$incr < $row_incr;$incr++){
                $oincr = $incr - 1;
                if(in_array($mfl_col_name[$incr], $existing_col_names, TRUE) == ""){ $sql_col_name2 = "ALTER TABLE `broiler_reportfields` ADD `$mfl_col_name[$incr]` VARCHAR(15) NULL DEFAULT 'N:0:0' AFTER `$mfl_col_name[$oincr]`"; mysqli_query($tmp_conn,$sql_col_name2); }
                //$sql_col_name2 = "ALTER TABLE `broiler_reportfields` CHANGE `$mfl_col_name[$incr]` `$mfl_col_name[$incr]` VARCHAR(15) CHARACTER SET latin1 COLLATE latin1_swedish_ci NULL DEFAULT 'N:0:0';"; mysqli_query($tmp_conn,$sql_col_name2);
            }
        }
    }
}
...
$database_list = array(); $database_list = explode(",",$_GET['database']); $db_all_flag = 0; foreach($database_list as $dlist){ if($dlist == "all"){ $db_all_flag = 1; } }
...
if($db_all_flag == 1){
    ...
    ';
    $sql1 = "SHOW COLUMNS FROM `master_reportfields`"; $query1 = mysqli_query($sconn,$sql1); $existing_col_names = array(); $i = 0;
    while($row1 = mysqli_fetch_assoc($query1)){
        ...
        else{ $existing_col_names[$i] = $row1['Field']; $i++; }
    }
    $sql2 = "SELECT * FROM `master_reportfields` WHERE `id` = '$file_url'"; $query2 = mysqli_query($sconn,$sql2); $c = 0;
    while($row2 = mysqli_fetch_assoc($query2)){
        for($a = 0;$a < $i;$a++){
            $fas_details = explode(":",$row2[$existing_col_names[$a]]);
            if($fas_details[0] == "A" && $fas_details[1] == "1"){
                $col_no = $fas_details[2];
                $sorted_col_name[$col_no] = $existing_col_names[$a];
            }
        }
    }
    ksort($sorted_col_name);
    $sql2 = "SELECT * FROM `master_reportfields` WHERE `id` = '$file_url'"; $query2 = mysqli_query($sconn,$sql2); $c = 0;
    while($row2 = mysqli_fetch_assoc($query2)){
        for($a = 0;$a < $i;$a++){
            $fas_details = explode(":",$row2[$sorted_col_name[$a]]);
            if($fas_details[0] == "A" && $fas_details[1] == "1"){
                $inv_list .= '
                <tr>
                    <td>
                        <input type="text" name="client_col_names[]" id="client_col_names['.$c.']" class="form-control" value="'.$sorted_col_name[$a].'" style="width:220px;text-align:left;" readonly >
                    </td>
                    <td>
                        <input type="text" name="client_ocol_nos[]" id="client_ocol_nos['.$c.']" class="form-control" value="'.$fas_details[2].'" style="width:120px;text-align:left;" readonly >
                    </td>
                    <td>
                        <input type="text" name="client_ncol_nos[]" id="client_ncol_nos['.$c.']" class="form-control" value="'.$fas_details[2].'" style="width:120px;text-align:left;" >
                    </td>
                </tr>
                ';
                $c++;
    ...
    $inv_list .= '</table></div><div class="col-md-3"></div>';
}
else if($db_all_flag == 0){
    if(sizeof($database_list) > 1){
        $inv_list .= '
        <div class="col-md-3"></div>
        <div class="col-md-6">
        <table>
        ...
        ';
        $sql2 = "SELECT * FROM `master_reportfields` WHERE `id` = '$file_url'"; $query2 = mysqli_query($sconn,$sql2);
        while($row2 = mysqli_fetch_assoc($query2)){ $field_href = $row2['field_href']; }

        $sql1 = "SHOW COLUMNS FROM `master_reportfields`"; $query1 = mysqli_query($sconn,$sql1);
        while($row1 = mysqli_fetch_assoc($query1)){
            if($row1['Field'] == "id" || $row1['Field'] == "field_name" || $row1['Field'] == "field_href" || $row1['Field'] == "field_pattern" || $row1['Field'] == "user_access_code" || $row1['Field'] == "column_count" || $row1['Field'] == "active" || $row1['Field'] == "dflag"){ }
            else{ $master_col_names[$row1['Field']] = $row1['Field']; }
        }
        $sql1 = "SELECT * FROM `master_reportfields` WHERE `field_href` LIKE '%$field_href%'"; $query1 = mysqli_query($sconn,$sql1); $msize = 0;
        while($row1 = mysqli_fetch_assoc($query1)){
            foreach($master_col_names as $mcns){
                $master_col_val[$mcns] = $row1[$mcns];
                $msize++;
            }
        }

        $tmp_conn = mysqli_connect($hostname, $db_users, $db_pass, $database_list[0]);

        $sql3 = "SELECT empcode FROM `log_useraccess` WHERE `dblist` LIKE '$database_list[0]' AND `account_access` IN ('BTS','ATS') AND `flag` = '1' AND `id` IN (SELECT MIN(id) as id FROM `log_useraccess` WHERE `dblist` LIKE '$database_list[0]' AND `account_access` IN ('BTS','ATS') AND `flag` = '1')";
        $query3 = mysqli_query($conns,$sql3); $count3 = mysqli_num_rows($query3);
        while($row3 = mysqli_fetch_assoc($query3)){ $admin_code = $row3['empcode']; }

        $sql2 = "SELECT * FROM `broiler_reportfields` WHERE `field_href` LIKE '%$field_href%' AND `user_access_code` = '$admin_code'"; $query2 = mysqli_query($tmp_conn,$sql2); $count2 = mysqli_num_rows($query2);
        if($count2 > 0){
            while($row2 = mysqli_fetch_assoc($query2)){
                foreach($master_col_names as $mcns){
                    $client_col_val[$mcns] = $row2[$mcns];
                }
            }
            $c = 0;
            foreach($master_col_names as $mcns){
                $fas_details = explode(":",$client_col_val[$mcns]);
                $mcv_details = explode(":",$master_col_val[$mcns]);
                if($fas_details[0] == "A" || $mcv_details[0] == "A"){
                    $inv_list .= '
                    <tr>
                        <td>
                            <input type="text" name="client_col_names[]" id="client_col_names['.$c.']" class="form-control" value="'.$mcns.'" style="width:220px;text-align:left;" readonly >
                        </td>
                        <td>
                            <input type="text" name="client_ocol_nos[]" id="client_ocol_nos['.$c.']" class="form-control" value="'.$fas_details[2].'" style="width:120px;text-align:left;" readonly >
                        </td>
