<?php
//broiler_display_vehicle.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['vehicle'] = $cid; } else{ $cid = $_SESSION['vehicle']; }
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                           <div class="float-left"><h3 class="card-title">Vehicle Details</h3></div>
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        $sql = "SELECT * FROM `broiler_vehicle` WHERE `dflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['id'];
                                    ...
                                    <tr>
										...
										<td><?php echo $row['manufacturer_model']; ?></td>
										<td><?php echo $row['engine_number']; ?></td>
										<td><?php echo $row['chassis_number']; ?></td>
										<td><?php echo date("d.m.Y",strtotime($row['insurance_validity'])); ?></td>
