...
for($i = 0; $i < $dsize;$i++){
    $sql ="SELECT * FROM `master_dashboard_links` WHERE `user_code` = '$user_code[$i]' AND `field_name` = '$field_name[$i]' AND `panel_name` = '$panel_name[$i]' AND `dflag` = '0' ORDER BY `id` ASC";
    ...
    if($ccount > 0){ }
        ...
        VALUES ('$field_name[$i]','$panel_name[$i]','$user_code[$i]','$sort_order[$i]','$flag','$active','$dflag','$addedemp','$addedtime','$addedtime')";
