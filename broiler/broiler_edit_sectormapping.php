<?php
//broiler_edit_sectormapping.php
...
<html lang="en">
    ...
    <body class="m-0 hold-transition">
        <?php
		$ids = $_GET['id'];
        $sql = "SELECT * FROM `broiler_secbrch_mapping` WHERE `id` = '$ids' AND `dflag` = '0'";  $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($query)){
            $id = $row['code'];
            $sec_code = $row['sector_code'];
            $brh_code = $row['branch_code'];
        }
		?>
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                            <div class="float-left"><h3 class="card-title">Edit Sector-Branch Mapping</h3></div>
                        </div>
                        <div class="card-body">
                            <div class="col-md-12">
                                <form action="broiler_modify_sectormapping.php" method="post" role="form" onsubmit="return checkval()">
                                    <div class="row">
                                            ...
                                            <table style="width:auto;">
                                                ...
                                                <tbody id="tbody">
                                                    <tr>
                                                        ...
                                                        <td><select name="branch_code" id="branch_code" class="form-control select2" style="width:190px;" onchange="check_duplicate();"><option value="select">select</option><?php foreach($branch_code as $bcode){ ?><option value="<?php echo $bcode; ?>" <?php if($brh_code == $bcode){ echo "selected"; } ?>><?php echo $branch_name[$bcode]; ?></option><?php } ?></select></td>
        ...
        <script>
            ...
            function check_duplicate(){
                ...
                if(sector_code != "select" && branch_code != "select"){
                    ...
                    var method = "GET";
                    var url = "broiler_check_sectormapping_duplicate.php?sector_code="+sector_code+"&branch_code="+branch_code+"&id="+id+"&row_count=0";
                    //window.open(url);
