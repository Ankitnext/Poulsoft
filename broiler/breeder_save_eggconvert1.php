...
$from_item = $to_item = $to_qty = $disposed_qty = $remarks = $stk_qty = $stk_prc = array();
...
$trtype = "eggconvert1";
$trlink = "breeder_display_eggconvert1.php";
...
for($i = 0;$i < $dsize;$i++){
    ...
    $trno_dt1 = generate_transaction_details($date,"eggconvert1","BEC","generate",$_SESSION['dbase']);
    $trno_dt2 = explode("@",$trno_dt1);
    $incr = $trno_dt2[0]; $prefix = $trno_dt2[1]; $trnum = $trno_dt2[2]; $fyear = $trno_dt2[3];

    //Fetch FARM/UNIT/SHED/Batch Details
    $sql = "SELECT * FROM `breeder_shed_allocation` WHERE `code` = '$from_loc' AND `active` = '1' AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql); $f_cnt = mysqli_num_rows($query); $farm_code = $unit_code = $shed_code = $batch_code = $flock_code = "";
    if((int)$f_cnt > 0){ while($row = mysqli_fetch_assoc($query)){ $farm_code = $row['farm_code']; $unit_code = $row['unit_code']; $shed_code = $row['shed_code']; $batch_code = $row['batch_code']; $flock_code = $row['code']; } }
    else{ $farm_code = $locations = $from_loc; }
...
<script>
    ...
    if(x == true){
        window.location.href = "breeder_add_eggconvert1.php";
    }
    else if(x == false) {
        window.location.href = "breeder_display_eggconvert1.php?ccid="+a;
    }
    else {
        window.location.href = "breeder_display_eggconvert1.php?ccid="+a;
    }
