<?php
//broiler_display_suppliercrdrnote.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['suppliercrdrnote'] = $cid; } else{ $cid = $_SESSION['suppliercrdrnote']; }
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM `".$table_name."` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `type` LIKE 'Supplier' AND `dflag` = '0'  $cond_assigned  ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['trnum'];
                                            ...
                                            $print_url2 = "/print/Examples/broiler_supplier_crdrnote.php?trnum=".$row['trnum'];
                                    ?>
                                    <tr>
