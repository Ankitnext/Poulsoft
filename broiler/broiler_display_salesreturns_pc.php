<?php
//broiler_display_salesreturns.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['salesreturns_pc'] = $cid; } else{ $cid = $_SESSION['salesreturns_pc']; }
                    ...
                    <div class="m-0 p-0 card">
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        $sql = "SELECT * FROM `item_details`"; $query = mysqli_query($conn,$sql); // `category` IN ('$bcodes') AND 
                                        ...
                                        $sql = "SELECT * FROM `inv_sectors` WHERE `dflag` = '0' ORDER BY `id` ASC"; $query = mysqli_query($conn,$sql);
                                        while($row = mysqli_fetch_assoc($query)){ $sector_code[$row['code']] = $row['code']; $sector_name[$row['code']] = $row['description']; }
                                        
                                        $delete_url = $delete_link."?utype=delete&trnum=";
                                        $sql = "SELECT * FROM `".$table_name."` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `type` = 'Customer' AND `rtype` = 'SalesReturn'  AND `dflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['trnum'];
                                            $edit_url = $edit_link."?utype=edit&trnum=".$id;
                                            $print_url = "print/Examples/broiler_print_pc_salesreturn.php?utype=print&trnum=".$id;
                                    ...
                                    <tr>
										...
										<td><?php if(!empty($vendor_name[$row['vcode']])){ echo $vendor_name[$row['vcode']]; } ?></td>
                                        <td><?php echo $item_name[$row['itemcode']]; ?></td>
										<td style="text-align:right;"><?php echo round($row['quantity'],3); ?></td>
										<td style="text-align:right;"><?php echo round($row['price']); ?></td>
                                        ...
                                        <?php
                                            ...
                                            if($print_flag == 1){
                                                echo "<a href='".$print_url."' target='_BLANK'><i class='fa fa-print' style='color:brown;' title='Print'></i></a>&ensp;&ensp;";
                                            }
                                        ?>
                                        </td>
                                    </tr>
