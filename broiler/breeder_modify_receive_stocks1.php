...
if($trnum != ""){
    $sql3 = "DELETE FROM `account_summary` WHERE `trnum` = '$ids' AND `etype` NOT IN ('Breeder-Send Stock') AND `dflag` = '0'"; if(!mysqli_query($conn,$sql3)){ die("Error: 1".mysqli_error($conn)); } else{ }
}
...
} else if($sent_qty < $rcv_qty){
    $qntt = (float)$rcv_qty - (float)$sent_qty;
