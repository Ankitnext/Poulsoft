<?php
//broiler_display_itembrands.php
include "newConfig.php";
include "number_format_ind.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['itembrands'] = $cid; } else{ $cid = $_SESSION['itembrands']; }
    ...
    <body class="m-0 hold-transition sidebar-mini">
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM `broiler_item_brands` WHERE `trtype` = 'itembrands' AND `trlink` = 'broiler_display_itembrands.php' AND `dflag` = '0' ORDER BY `id` DESC";
        ...
        <script>
			function checkdelete(x){
                ...
                var id = val1[0];
                var code = val1[1];
                var description = val1[2];
                if(id != "" && code != "" && description != ""){
                    var inv_items = new XMLHttpRequest();
                    var method = "GET";
                    var url = "broiler_check_itembrands.php?code="+code;
                    //window.open(url);
                    ...
                    inv_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(parseInt(count) > 0){
                                alert("You can't delete the Brand: "+val+", as Brand is already in use!");
                            }
                            else{
                                var b = "<?php echo $delete_url; ?>"+id;
                                var c = confirm("are you sure you want to delete the Brand: "+description+"?");
                                if(c == true){
