<?php
//broiler_edit_inventoryissued.php
...
if($link_active_flag > 0){
    ...
    if($acount == 1){
        ...
        $sql = "SELECT * FROM `extra_access` WHERE `field_name` LIKE 'Stock Issue' AND `field_function` LIKE 'Bags' AND `flag` = '1'"; $query = mysqli_query($conn,$sql); $bag_access_flag = mysqli_num_rows($query);
...
<html lang="en">
    ...
    <body class="m-0 hold-transition">
        <?php
        $trnum = $_GET['trnum'];
        $sql = "SELECT * FROM `broiler_inv_intermediate_issued` WHERE `trnum` = '$trnum'"; $query = mysqli_query($conn,$sql); $i = 0;
        while($row = mysqli_fetch_assoc($query)){
            $date = $row['date'];
            $doc_no = $row['doc_no'];
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                            <div class="float-left"><h3 class="card-title">Edit Stock Issued</h3></div>
                        </div>
                        <div class="m-0 p-2 card-body">
                            <div class="col-md-18">
                                <form action="broiler_modify_inventoryissued.php" method="post" role="form" onsubmit="return checkval()">
                                    <div class="row">
        ...
        <script>
            ...
            function fetch_stock_master(){
                ...
                if(sector != "select" && item_code != "select"){
                    ...
                    var method = "GET";
                    var url = "broiler_fetch_itemstockmaster_lsfi.php?sector="+sector+"&item_code="+item_code+"&date="+date+"&id="+idvalue+"&trtype=stkissue&etype=stkissue";
                    //window.open(url);
