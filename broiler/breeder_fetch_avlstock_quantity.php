<?php
//breeder_fetch_avlstock_quantity.php
...
if($itype == "feed" || $ftype == "brd_mventry"){
    ...
    $sql = "SELECT * FROM `breeder_shed_allocation` WHERE `code` = '$flock_code'".$shed_fltr." AND `active` = '1' AND `dflag` = '0'";
    ...
    if((float)$stk_aqty <= 0){
        $err_flag = 1;
        $err_msg = "Breeder Item Stock not available. Please check and try again.";
    }
...
if($ftype == "stk_transfer"){
    ...
    //check and fetch Flock/Sector
    $sql = "SELECT * FROM `breeder_shed_allocation` WHERE `code` = '$fsector' AND `active` = '1' AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql); $f_cnt = mysqli_num_rows($query);
    if((int)$f_cnt > 0){
        $sql = "SELECT * FROM `account_summary` WHERE `date` <= '$today' AND `coa_code` IN ('$icat_iac') AND `item_code` IN ('$item_code') AND `flock_code` = '$fsector'".$trno_fltr." AND `active` = '1' AND `dflag` = '0' ORDER BY `date` ASC,`crdr` DESC";
    }
    else{
...
if($ftype == "egg_stock"){
    ...
    if((float)$stk_aqty <= 0){
        $err_flag = 1;
        $err_msg = "Breeder Egg Stock not available. Please check and try again.";
    }
    echo $err_flag."[@$&]".$err_msg."[@$&]".$rows."[@$&]".$stk_aqty."[@$&]".$stk_aprc;
}
if($ftype == "bird_transfer"){
    ...
    while($row = mysqli_fetch_assoc($query)){
        if(strtolower($row['description']) == "female birds"){ $fbird_code = $row['code']; }
        else if(strtolower($row['description']) == "male birds"){ $mbird_code = $row['code']; }
        else{ }
    ...
    $sql = "SELECT * FROM `breeder_shed_allocation` WHERE `code` = '$fflock' AND `active` = '1' AND `dflag` = '0'";
