...
$date = $dcno = $code = $quantity = $price = $towarehouse = $remarks = $item_sqty = $item_sprc = array();
...
$trtype = "stocktransfer2";
$trlink = "layer_display_stocktransfer2.php";
...
for($i = 0;$i < $dsize;$i++){
    ...
    $trno_dt1 = generate_transaction_details($date[$i],"stocktransfer2","BFMV","generate",$_SESSION['dbase']);
    ...
    VALUES('$incr','$prefix','$trnum','$date[$i]','$dcno[$i]','$fromwarehouse','$code[$i]','$quantity[$i]','$price[$i]','$amount','$towarehouse[$i]','$remarks[$i]','$flag','$active','$dflag','$trtype','$trlink','$addedemp','$addedtime','$addedtime')";
    if(!mysqli_query($conn,$sql)){ die("Error 2:-".mysqli_error($conn)); }
        ...
        VALUES ('CR','$coa_Cr','$date[$i]','$trnum','$code[$i]','$quantity[$i]','$price[$i]','$amount','$fromwarehouse','$remarks[$i]','0','layer-Stock Transfer Out','$flag','$active','$dflag','$addedemp','$addedtime','$addedtime')";
        if(!mysqli_query($conn,$from_post)){ die("Error 3:-".mysqli_error($conn)); } else{ }
        $from_post = "INSERT INTO `account_summary` (`crdr`,`coa_code`,`date`,`trnum`,`item_code`,`quantity`,`price`,`amount`,`location`,`remarks`,`gc_flag`,`etype`,`flag`,`active`,`dflag`,`addedemp`,`addedtime`,`updatedtime`) 
        VALUES ('DR','$coa_Dr','$date[$i]','$trnum','$code[$i]','$quantity[$i]','$price[$i]','$amount','$towarehouse[$i]','$remarks[$i]','0','layer-Stock Transfer In','$flag','$active','$dflag','$addedemp','$addedtime','$addedtime')";
        if(!mysqli_query($conn,$from_post)){ die("Error 3:-".mysqli_error($conn)); } else{ }
    }
}
?>
<script>
    ...
    if(x == true){
        window.location.href = "layer_add_stocktransfer2.php";
    }
    else if(x == false) {
        window.location.href = "layer_display_stocktransfer2.php?ccid="+a;
    }
    else {
        window.location.href = "layer_display_stocktransfer2.php?ccid="+a;
    }
