<?php
//broiler_add_generalsales4.php
include "newConfig.php";
include "broiler_featch_trnum.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $ccid = $_SESSION['generalsales4'];
$trnum = featch_trnum(date('Y-m-d'),'sales','transactions','sales');
...
if($link_active_flag > 0){
    ...
    if($acount == 1){
        ...
        $sql = "SELECT * FROM `item_category` WHERE (`description` LIKE '%READY ITEM%' OR `description` LIKE '%BAG%') AND `active` = '1' AND `dflag` = '0'";
        $query = mysqli_query($conn,$sql); $icat_alist = array();
        while($row = mysqli_fetch_assoc($query)){ $icat_alist[$row['code']] = $row['code']; }
        
        $item_list = implode("','",$icat_alist);
		$sql = "SELECT * FROM `item_details` WHERE `category` IN ('$item_list') AND `active` = '1' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
        ...
        if($ccount3 > 0){ while($row3 = mysqli_fetch_assoc($query3)){ $bag_access_flag = $row3['flag']; } }
        else { mysqli_query($conn, "INSERT INTO `extra_access` ( `field_name`, `field_function`, `user_access`, `flag`) VALUES ( 'Feed Sale', 'Bags', 'all', '0')"); $bag_access_flag =  0; }
		...
		$sql = "SELECT * FROM `acc_coa` WHERE `driver_flag` = '1' AND `active` = '1' AND `dflag` = '0'";
    ...
    <body class="m-0 hold-transition">
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        ...
                        <div class="m-0 p-2 card-body">
                            <div class="col-md-18">
                                <form action="broiler_save_generalsales4.php" method="post" role="form" onSubmit="return checkval()">
                                    <div class="row">
                                        <div class="form-group">
                                            <label>Trnum: </label>
                                            <input type="text" value="<?php echo $trnum; ?>" readonly class="form-control" style="width:150px;" >
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="vcode" id="vcode" class="form-control select2" style="width:260px;">
                                        ...
                                        <div class="form-group">
                                            <label>Driver</label>
                                            <select name="driver_code" id="driver_code" class="form-control select2" style="width:260px;">
                                                <option value="select">select</option>
                                                <?php foreach($driver_code as $dcode){ ?><option value="<?php echo $dcode; ?>"><?php echo $driver_name[$dcode]; ?></option><?php } ?>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Driver Price</label>
                                            <input type="text" name="driver_price" id="driver_price" class="form-control" value="1" style="width:50px;" onkeyup="validatenum(this.id);" onchange="validateamount(this.id);" />
                                        </div>
                                    ...
                                    <table>
                                        <thead>
                                            <tr style="text-align:center;">
                                                ...
                                                <th title="Available Stock">Available Stock</th>
                                                <th title="Stock Price">Stock Price</th>
                                                <th>Sale Rate</th>
                                                <th>Amount</th>
                                                <th></th>
                                                <th></th>
                                                <th style="visibility:hidden;"></th>
                                                <th style="visibility:hidden;">F</th>
                                                <th style="visibility:hidden;">B</th>
                                            </tr>
                                        </thead>
                                        <tbody id="row_body">
                                            <tr>
                                                <td><select name="icode[]" id="icode[0]" class="form-control select2" style="width:280px;" onchange="fetch_stock_master(this.id);fetch_item_prices(this.id);"><option value="select">select</option><?php foreach($item_code as $prod_code){ ?><option value="<?php echo $prod_code; ?>"><?php echo $item_name[$prod_code]; ?></option><?php } ?></select></td>
                                                ...
                                                <td><input type="text" name="available_stock[]" id="available_stock[0]" class="form-control" placeholder="0.00" style="width:130px;" readonly ></td>
                                                <td><input type="text" name="avg_price[]" id="avg_price[0]" class="form-control" placeholder="0.00" style="width:130px;" readonly ></td>
                                                ...
                                                <!--<td style="visibility:hidden;"><input type="text" name="avg_price[]" id="avg_price[0]" class="form-control" placeholder="0.00" style="width:20px;" readonly ></td>-->
                                                <td style="visibility:hidden;"><input type="text" name="avg_amount[]" id="avg_amount[0]" class="form-control" placeholder="0.00" style="width:20px;" readonly ></td>
                                                <td style="visibility:hidden;"><input type="text" name="farm_mnu_name[]" id="farm_mnu_name[0]" class="form-control" style="width:20px;" /></td>
                                                <td style="visibility:hidden;"><input type="text" name="batch_mnu_name[]" id="batch_mnu_name[0]" class="form-control" style="width:20px;" /></td>
                                            </tr>
                                        </tbody>
                                    </table><br/>
                                    <div class="col-md-12">
        ...
        <script>
            function return_back(){
                ...
                window.location.href = 'broiler_display_generalsales4.php?ccid='+ccid;
            }
            function checkval(){
            ...
            function create_row(a){
                ...
                html += '<tr id="row_no['+d+']">';
                html += '<td><select name="icode[]" id="icode['+d+']" class="form-control select2" style="width:280px;" onchange="fetch_stock_master(this.id);fetch_item_prices(this.id);"><option value="select">select</option><?php foreach($item_code as $prod_code){ ?><option value="<?php echo $prod_code; ?>"><?php echo $item_name[$prod_code]; ?></option><?php } ?></select></td>';
                ...
                html += '<td><input type="text" name="available_stock[]" id="available_stock['+d+']" class="form-control" placeholder="0.00" style="width:130px;" readonly ></td>';
                html += '<td><input type="text" name="avg_price[]" id="avg_price['+d+']" class="form-control" placeholder="0.00" style="width:130px;" readonly ></td>';
                ...
                //html += '<td style="visibility:hidden;"><input type="text" name="avg_price[]" id="avg_price['+d+']" class="form-control" placeholder="0.00" style="width:20px;" readonly ></td>';
                html += '<td style="visibility:hidden;"><input type="text" name="avg_amount[]" id="avg_amount['+d+']" class="form-control" placeholder="0.00" style="width:20px;" readonly ></td>';
                html += '<td style="visibility:hidden;"><input type="text" name="farm_mnu_name[]" id="farm_mnu_name['+d+']" class="form-control" style="width:20px;" /></td>';
                html += '<td style="visibility:hidden;"><input type="text" name="batch_mnu_name[]" id="batch_mnu_name['+d+']" class="form-control" style="width:20px;" ></td>';
                html += '</tr>';
            ...
            function fetch_item_prices(a){
                ...
                var fetch_items = new XMLHttpRequest();
				var method = "GET";
				var url = "broiler_fetch_itemprices.php?date="+date+"&item_code="+icode+"&row_count="+d;
                //window.open(url);
				var asynchronous = true;
				fetch_items.open(method, url, asynchronous);
				fetch_items.send();
				fetch_items.onreadystatechange = function(){
					if(this.readyState == 4 && this.status == 200){
						var item_dt1 = this.responseText;
                        var item_dt2 = item_dt1.split("@");
                        var item_prc = item_dt2[0]; if(item_prc == ""){ item_prc = 0; }
                        var rows = item_dt2[1];
                        document.getElementById("rate["+rows+"]").value = parseFloat(item_prc).toFixed(2);
                    }
                }
            }
