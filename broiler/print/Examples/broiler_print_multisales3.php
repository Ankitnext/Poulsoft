...
$sql = "SELECT * FROM `extra_access` WHERE `field_name` = 'broiler_print_multisales3.php' AND `field_function` = 'Display Invoice Number' AND `flag` = '1'";
$query = mysqli_query($conn,$sql); while($row = mysqli_fetch_assoc($query)){} $dinv_flag = mysqli_num_rows($query);
...
//$amt = 322450.00;
//echo "<br/><br/><br/>".convert_number_to_words($amt);
...
$disclaimer = "";
if($count100 > 0){
	$sql = "SELECT * FROM `main_disclaimer` WHERE `type` LIKE 'broiler_print_multisales3.php' AND `active` = '1' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
...
foreach($day_ccode as $cuscode){
	...
	if($cuscode != ""){
		$id = $cuscode;
		$ven_filter = ""; if($stype == "FS"){ $ven_filter = " AND `warehouse` LIKE '$id'"; } else{ $ven_filter = " AND `vcode` LIKE '$id'"; }
		$slno = $iname = $bird = $qty = $price = $amt = $billno = $vehicle_code = $inv_alist = array();
		$tot_birds = $tot_qty = $post = $camt = $oinv = $ocdn = $obdramt = $orct = $occn = $obcramt = $ob_mortality = $ob_returns = 0;
		$sql = "SELECT * FROM `broiler_sales` WHERE `date` = '$day_date[0]'".$ven_filter." AND `active` = '1' AND `dflag` = '0' ";
		...
		while($row = mysqli_fetch_assoc($query)){
			...
			$tot_qty += $qty[$c] = $row['rcd_qty'];
			$price[$c] = $row['rate'];
			$amt[$c] = $row['item_tamt'];
			$tcdsper = $row['tcdsper'];
			$tcdsamt = $row['tcdsamt'];
			$roundoff = $row['roundoff'];
			$inv_alist[$row['trnum']] = $row['trnum'];
			//$famt = $row['finaltotal'];
			$inv = $row['trnum'];
			$ccode = $row['vcode'];
			$odate = $row['date'];
			...
			$billno[$c] = $row['billno'];
			$vehicle_code[$c] = $row['vehicle_code'];
		}
		$inv_list = implode(", ",$inv_alist);
		...
		if($count98 > 0){
			...
			while($row = mysqli_fetch_assoc($query)){
				...
				$baddress[$row['code']] = $row['baddress'];
				if($row['obtype'] == "Cr") { $obcramt = $row['obamt']; $obdramt = 0; }
				else { $obdramt = $row['obamt']; $obcramt = 0; }
			}
		}
		$old_inv = "";
		...
		$html .= '<div align="center" style="text-align:center;">INVOICE</div>';
		...
		$html .= '<b style="padding-left: 10px;text-align:left;">Billing Address: '.$baddress[$ccode].'</b><br/>';
		$html .= '</th>';	
		$html .= '</tr>';
		if((int)$dinv_flag > 0){
			$html .= '<tr>';
			$html .= '<th colspan="7" style="padding-left: 10px;text-align:left;">';
			$html .= '<b style="padding-left: 10px;text-align:left;">Invoice No.: '.$inv_list.'</b><br/>';
			$html .= '</th>';	
			$html .= '</tr>';
		}
		$html .= '</table>';
		
		$slnos = $inames =  $birds = $qtys = $prices = $amts = $billnos = $vehicle_codes = "<br/><br/>";
		for($i = 1;$i <= $c;$i++){
			if($i <$c){
				...
				$billnos = $billnos."".$billno[$i]."<br/><br/>";
				$vehicle_codes = $vehicle_codes."".$vehicle_code[$i]."<br/><br/>";
			}
			else {
				$br = "";
				//$k = 9 - $c;
				...
				$billnos = $billnos."".$billno[$i]."&nbsp;&nbsp;".$br;
				$vehicle_codes = $vehicle_codes."".$vehicle_code[$i]."&nbsp;&nbsp;".$br;
			}
            $famt = $famt + $amt[$i];
		}
		$camt = $post + $famt;
		...
		$html .= '<tr>';
		$html .= '<th colspan="1" style="border: 1px solid black;"><b>DC No</b></th>';
		$html .= '<th colspan="2" style="border: 1px solid black;"><b>Vehicle</b></th>';
		$html .= '<th colspan="2" style="border: 1px solid black;"><b>Items</b></th>';
		$html .= '<th colspan="1" style="border: 1px solid black;"><b>Birds</b></th>';
		$html .= '<th colspan="1" style="border: 1px solid black;"><b>Kgs</b></th>';
		$html .= '<th colspan="1" style="border: 1px solid black;"><b>Rate</b></th>';
		$html .= '<th colspan="2" style="border: 1px solid black;"><b>Amount</b></th>';
		$html .= '</tr>';
		$html .= '<tr>';
			$html .= '<td colspan="1" style="padding:5px;text-align:center;">'.$billnos.'</td>';
			$html .= '<td colspan="2" style="padding:5px;text-align:center;">'.$vehicle_codes.'</td>';
