...
if(in_array("cus_ccode", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `main_contactdetails` ADD `cus_code` VARCHAR(100) NOT NULL DEFAULT '0' COMMENT 'Customer Code' AFTER `code`"; mysqli_query($conn,$sql); }
...
while($row = mysqli_fetch_assoc($query)){ $old_grpcode = $row['groupcode']; $code = $row['code']; }
...
$query = mysqli_query($conn,$sql); $updoc_flag = mysqli_num_rows($query); $link_path1 = $link_path2 = $link_path3 = $link_path4 = $link_path5 = $upd_flist = "";
if($updoc_flag > 0){
    ...
    if(!empty($_FILES["vdoc_link1"]["name"])) {
        ...
        $upd_flist .= ",`vdoc_path1` = '".$link_path1."'";
    }
    if(!empty($_FILES["vdoc_link2"]["name"])) {
        ...
        $upd_flist .= ",`vdoc_path2` = '".$link_path2."'";
    }
    if(!empty($_FILES["vdoc_link3"]["name"])) {
        ...
        $upd_flist .= ",`vdoc_path3` = '".$link_path3."'";
    }
    if(!empty($_FILES["vdoc_link4"]["name"])) {
        ...
        $upd_flist .= ",`vdoc_path4` = '".$link_path4."'";
    }
    if(!empty($_FILES["vdoc_link5"]["name"])) {
        ...
        $upd_flist .= ",`vdoc_path5` = '".$link_path5."'";
    }
}

$sql = "UPDATE `main_contactdetails` SET `cus_prefix` = '$cus_prefix',`name` = '$name',`cus_ccode`='$cus_ccode',`mobile1` = '$mobile1',`mobile2` = '$mobile2',`emails` = '$emails',`pan_no` = '$pan_no',`aadhar_no` = '$aadhar_no',`birth_date` = '$birth_date',`contacttype` = '$contacttype',`groupcode` = '$groupcode',`gstinno` = '$gstinno',`company_details` = '$company_details',`state_code` = '$state_code',`branch_code` = '$branch_code',`baddress` = '$baddress',`saddress` = '$saddress',`obamt` = '$obamt',`obtype` = '$obtype',`obdate` = '$obdate',`obremarks` = '$obremarks',`creditdays` = '$creditdays',`creditamt` = '$creditamt',`bank_accno` = '$bank_accno',`bank` = '$bank',`flag` = '0',`active` = '1',`processing_flag` = '$processing_flag',`dflag` = '0',`updatedemp` = '$addedemp',`updatedtime` = '$addedtime'".$upd_flist." WHERE `id` = '$id'";
if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); }
    ...
    if($opn_trnum != ""){
        if($obamt != "" && (float)$obamt != 0){
            ...
            if($obtype == "Cr"){
                $coa_Cr = $control_acc_group[$new_grpcode]; $coa_Dr = $opn_code;
            }
            if($obtype == "Dr"){
                $coa_Dr = $control_acc_group[$new_grpcode]; $coa_Cr = $opn_code;
            }
            $sql = "DELETE FROM `account_summary` WHERE `trnum` = '$opn_trnum'";
            if(!mysqli_query($conn,$sql)){ die("Error-1: ".mysqli_error($conn)); }
                ...
                VALUES ('CR','$coa_Cr','$obdate','$code','','$opn_trnum','$obamt','$obremarks','0','Customer Openings','0','1','0','$addedemp','$addedtime','$addedtime')";
                if(!mysqli_query($conn,$from_post)){ die("Error 2:-".mysqli_error($conn)); }
                    ...
                    VALUES ('DR','$coa_Dr','$obdate','$code','','$opn_trnum','$obamt','$obremarks','0','Customer Openings','0','1','0','$addedemp','$addedtime','$addedtime')";
            ...
            $sql = "DELETE FROM `account_summary` WHERE `trnum` = '$opn_trnum'"; mysqli_query($conn,$sql);
        }
    }
    else{
        if($obamt != "" && (float)$obamt != 0){
    ...
    if($old_grpcode != $new_grpcode){
        if($contacttype == "C"){
            $coa_code = $control_acc_group[$old_grpcode];
            $sql = "SELECT * FROM `account_summary` WHERE `coa_code` = '$coa_code' AND `vendor` = '$code' AND `active` = '1' AND `dflag` = '0'";
            $query = mysqli_query($conn, $sql); $count = mysqli_num_rows($query);
            if($count > 0){
                $new_code = $control_acc_group[$new_grpcode];
                $sql = "UPDATE `account_summary` SET `coa_code` = '$new_code' WHERE `coa_code` = '$coa_code' AND `vendor` = '$code' AND `active` = '1' AND `dflag` = '0'";
                if(!mysqli_query($conn,$to_post)){ die("Error 4:-".mysqli_error($conn)); } else{ }
            }
        }
    }
