...
if($db == ''){
    ...
    $form_path = "layer_medvacschedule.php";
}
else{
    ...
    include "header_head.php";
    $form_path = "layer_medvacschedule.php?db=$db&userid=".$user_code;
}
...
if(in_array("layer_breed_details", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.layer_breed_details LIKE poulso6_admin_layer_layermaster.layer_breed_details;"; mysqli_query($conn,$sql1); }
if(in_array("broiler_vaccineschedule", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.broiler_vaccineschedule LIKE poulso6_admin_layer_layermaster.broiler_vaccineschedule;"; mysqli_query($conn,$sql1); }
if(in_array("item_details", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.item_details LIKE poulso6_admin_layer_layermaster.item_details;"; mysqli_query($conn,$sql1); }

$file_name = "Vaccination Schedule Report";
...
while($row = mysqli_fetch_assoc($query)){ $it_code[$row['code']] = $row['code']; $it_name[$row['code']] = $row['description']; }
...
<html>
    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            $nhtml .= '<th>Vaccine Name</th>'; $fhtml .= '<th id="order">Vaccine Code</th>';
            $nhtml .= '<th>Vaccine Name</th>'; $fhtml .= '<th id="order">Vaccine Name</th>';
            ...
            if(isset($_POST['submit_report']) == true){
                ...
                $sql = "SELECT * FROM `broiler_vaccineschedule` WHERE `active` = '1'".$brd_fltr." AND `dflag` = '0' ORDER BY `age` ASC";
                $query = mysqli_query($conn,$sql);
                while($row = mysqli_fetch_assoc($query)){
                 
                    $bv_age = $row['age'];
                    $vaccine_code = $row['vaccine_code'];
                    $vaccine_name = $it_name[$row['vaccine_code']];
                   
                    $html .= '<tr>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($bv_age,5))).'</td>';
                    $html .= '<td>'.$vaccine_code.'</td>';
                    $html .= '<td>'.$vaccine_name.'</td>';
                    $html .= '</tr>';
                }
                $html .= '</tbody>';
