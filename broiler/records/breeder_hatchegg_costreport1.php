<?php
//breeder_hatchegg_costreport1.php
...
if($db == ''){
    ...
    global $page_title; $page_title = "Breeder Hatch Eggs Cost Report";
    include "header_head.php";
    $form_path = "breeder_hatchegg_costreport1.php";
}
else{
    ...
    global $page_title; $page_title = "Breeder Hatch Eggs Cost Report";
    include "header_head.php";
    $form_path = "breeder_hatchegg_costreport1.php?db=$db&userid=".$user_code;
}
...
$sql = "SELECT * FROM `item_category` WHERE `dflag` = '0' AND (`bffeed_flag` = '1' OR `bmfeed_flag` = '1') ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql); $cfeed_code = array();
while($row = mysqli_fetch_assoc($query)){ $cfeed_code[$row['code']] = $row['code']; $icat_iac[$row['code']] = $row['iac']; } $feed_list = implode("','", $cfeed_code);
$sql = "SELECT * FROM `item_details` WHERE `category` IN ('$feed_list') AND `dflag` = '0' ORDER BY `sort_order`,`description` ASC"; $query = mysqli_query($conn,$sql); $feed_code = $feed_name = array();
while($row = mysqli_fetch_assoc($query)){ $feed_code[$row['code']] = $row['code']; $feed_name[$row['code']] = $row['description']; }

//Breeder MedVac Details
$sql = "SELECT * FROM `item_category` WHERE `dflag` = '0' AND `bmv_flag` = '1' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql); $cmv_code = array();
while($row = mysqli_fetch_assoc($query)){ $cmv_code[$row['code']] = $row['code']; $icat_iac[$row['code']] = $row['iac']; } $mv_list = implode("','", $cmv_code);
$sql = "SELECT * FROM `item_details` WHERE `category` IN ('$mv_list') AND `dflag` = '0' ORDER BY `sort_order`,`description` ASC"; $query = mysqli_query($conn,$sql); $mv_code = $mv_name = array();
while($row = mysqli_fetch_assoc($query)){ $mv_code[$row['code']] = $row['code']; $mv_name[$row['code']] = $row['description']; }
...
<html>
    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            $nhtml .= '<th colspan="4"></th>'; $fhtml .= '<th colspan="4"></th>';
            $nhtml .= '<th colspan="6">Including Chick & Grower</th>'; $fhtml .= '<th colspan="6">Including Chick & Grower</th>';
            $nhtml .= '<th colspan="6">Excluding Chick & Grower</th>'; $fhtml .= '<th colspan="6">Excluding Chick & Grower</th>';
            ...
            $nhtml .= '<th>Eggs Produced</th>'; $fhtml .= '<th id="order_num">Eggs Produced</th>';
            $nhtml .= '<th>Over Head Cost</th>'; $fhtml .= '<th id="order_num">Over Head Cost</th>';

            $nhtml .= '<th>Feed Consumed(Kgs)</th>'; $fhtml .= '<th id="order_num">Feed Consumed (Kgs)</th>';
            $nhtml .= '<th>Rate</th>'; $fhtml .= '<th id="order_num">Rate</th>';
            $nhtml .= '<th>Feed Cost</th>'; $fhtml .= '<th id="order_num">Feed Cost</th>';
            $nhtml .= '<th>Medicine Cost</th>'; $fhtml .= '<th id="order_num">Medicine Cost</th>';
            $nhtml .= '<th>Total Cost</th>'; $fhtml .= '<th id="order_num">Total Cost</th>';
            $nhtml .= '<th>Cost / Egg</th>'; $fhtml .= '<th id="order_num">Cost / Egg</th>';
            ...
            if(isset($_POST['submit_report']) == true && sizeof($flock_alist) > 0){
                $flock_list = implode("','", $flock_alist); $sdate = $edate = "";
                //Opening Details
                $sql = "SELECT * FROM `breeder_shed_allocation` WHERE `code` IN ('$flock_list') AND `dflag` = '0'";
                $query = mysqli_query($conn,$sql); $opn_fbirds = $opn_mbirds = $breed_code = array();
                while($row = mysqli_fetch_assoc($query)){
                    //$bird_wage = age_in_weeks($row['start_age']); $weeks = fetch_cweek($bird_wage);
                    $iflock = $row['code'];
                    $opn_fbirds[$iflock] += $row['opn_fbirds'];
                    $opn_mbirds[$iflock] += $row['opn_mbirds'];
                    $breed_code[$iflock] = $batch_breed[$row['batch_code']];
                }
                //Egg production
                $sql = "SELECT * FROM `breeder_dayentry_produced` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `flock_code` IN ('$flock_list') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`flock_code` ASC";
                $query = mysqli_query($conn,$sql); $date_alist = array();
                while($row = mysqli_fetch_assoc($query)){
                    $idate = $row['date']; $iflock = $row['flock_code'];
                    $egg_pqty[$idate] += $row['quantity'];
                    if(empty($date_alist[$iflock]) || $date_alist[$iflock] == "" || strtotime($date_alist[$iflock]) >= strtotime($idate)){ $date_alist[$iflock] = $idate; }
                }
                
                $coa_list = implode("','",$icat_iac);
                $sql = "SELECT * FROM `account_summary` WHERE `coa_code` IN ('$coa_list') AND `date` >= '$fdate' AND `date` <= '$tdate' AND `flock_code` IN ('$flock_list') AND `active` = '1' AND `dflag` = '0' ORDER BY `date` ASC,`crdr` DESC";
                $query = mysqli_query($conn,$sql); $feedi_cqty = $feede_cqty = $feedi_camt = $feede_camt = $mdvi_cqty = $mdve_cqty = $mdvi_camt = $mdve_camt = array();
                while($row = mysqli_fetch_assoc($query)){
                    $icrdr = $row['crdr']; $idate = $row['date']; $icode = $row['item_code']; $iqty = $row['quantity']; $iamt = $row['amount'];
                    $ietype = $row['etype']; $iflock = $row['flock_code'];
                    ...
                    //Feed Consumed
                    if(empty($date_alist[$iflock]) || $date_alist[$iflock] == "" || strtotime($date_alist[$iflock]) > strtotime($idate)){
                        if($icrdr == "CR" && $ietype == "Breeder-Female Feed-1 Consumed"){ $feedi_cqty[$idate] += (float)$iqty; $feedi_camt[$idate] += (float)$iamt; }
                        if($icrdr == "CR" && $ietype == "Breeder-Female Feed-2 Consumed"){ $feedi_cqty[$idate] += (float)$iqty; $feedi_camt[$idate] += (float)$iamt; }
                        if($icrdr == "CR" && $ietype == "Breeder-Male Feed-1 Consumed"){ $feedi_cqty[$idate] += (float)$iqty; $feedi_camt[$idate] += (float)$iamt; }
                        if($icrdr == "CR" && $ietype == "Breeder-Male Feed-2 Consumed"){ $feedi_cqty[$idate] += (float)$iqty; $feedi_camt[$idate] += (float)$iamt; }
                        if($icrdr == "CR" && $ietype == "Breeder-MedVac Consumed"){ $mdvi_cqty[$idate] += (float)$iqty; $mdvi_camt[$idate] += (float)$iamt; }
                    }
                    else{
                        if($icrdr == "CR" && $ietype == "Breeder-Female Feed-1 Consumed"){ $feedi_cqty[$idate] += (float)$iqty; $feede_cqty[$idate] += (float)$iqty; $feedi_camt[$idate] += (float)$iamt; $feede_camt[$idate] += (float)$iamt; }
                        if($icrdr == "CR" && $ietype == "Breeder-Female Feed-2 Consumed"){ $feedi_cqty[$idate] += (float)$iqty; $feede_cqty[$idate] += (float)$iqty; $feedi_camt[$idate] += (float)$iamt; $feede_camt[$idate] += (float)$iamt; }
                        if($icrdr == "CR" && $ietype == "Breeder-Male Feed-1 Consumed"){ $feedi_cqty[$idate] += (float)$iqty; $feede_cqty[$idate] += (float)$iqty; $feedi_camt[$idate] += (float)$iamt; $feede_camt[$idate] += (float)$iamt; }
                        if($icrdr == "CR" && $ietype == "Breeder-Male Feed-2 Consumed"){ $feedi_cqty[$idate] += (float)$iqty; $feede_cqty[$idate] += (float)$iqty; $feedi_camt[$idate] += (float)$iamt; $feede_camt[$idate] += (float)$iamt; }
                        if($icrdr == "CR" && $ietype == "Breeder-MedVac Consumed"){ $mdvi_cqty[$idate] += (float)$iqty; $mdve_cqty[$idate] += (float)$iqty; $mdvi_camt[$idate] += (float)$iamt; $mdve_camt[$idate] += (float)$iamt; }
                    }
                }

                $slno = 0;
                for ($cdate = strtotime($fdate); $cdate <= strtotime($tdate); $cdate += (86400)){
                    $adate = date("Y-m-d",$cdate); $key1 = $adate;

                    $ep_qty = 0; if(empty($egg_pqty[$key1]) || $egg_pqty[$key1] == ""){ } else{ $ep_qty = $egg_pqty[$key1]; }
                    $oh_cst = 0;
                    $fi_qty = 0; if(empty($feedi_cqty[$key1]) || $feedi_cqty[$key1] == ""){ } else{ $fi_qty = $feedi_cqty[$key1]; }
                    $fi_amt = 0; if(empty($feedi_camt[$key1]) || $feedi_camt[$key1] == ""){ } else{ $fi_amt = $feedi_camt[$key1]; }
                    $fi_prc = 0; if((float)$fi_qty != 0){ $fi_prc = (float)$fi_amt / (float)$fi_qty; }
                    $mi_amt = 0; if(empty($mdvi_camt[$key1]) || $mdvi_camt[$key1] == ""){ } else{ $mi_amt = $mdvi_camt[$key1]; }
                    $ti_amt = 0; $ti_amt = (float)$oh_cst + (float)$fi_amt + (float)$mi_amt;
                    $ei_prc = 0; if((float)$fi_qty != 0){ $ei_prc = (float)$ti_amt / (float)$ep_qty; }

                    $fe_qty = 0; if(empty($feede_cqty[$key1]) || $feede_cqty[$key1] == ""){ } else{ $fe_qty = $feede_cqty[$key1]; }
                    $fe_amt = 0; if(empty($feede_cqty[$key1]) || $feede_cqty[$key1] == ""){ } else{ $fe_amt = $feede_cqty[$key1]; }
                    $fe_prc = 0; if((float)$fe_qty != 0){ $fe_prc = (float)$fe_amt / (float)$fe_qty; }
                    $me_amt = 0; if(empty($mdve_camt[$key1]) || $mdve_camt[$key1] == ""){ } else{ $me_amt = $mdve_camt[$key1]; }
                    $te_amt = 0; $te_amt = (float)$oh_cst + (float)$fe_amt + (float)$me_amt;
                    $ee_prc = 0; if((float)$fe_qty != 0){ $ee_prc = (float)$te_amt / (float)$ep_qty; }

                    $slno++;
                    $html .= '<tr>';
                    $html .= '<td style="text-align:center;">'.$slno.'</td>';
                    $html .= '<td style="text-align:center;" class="dates">'.date("d.m.Y",strtotime($adate)).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($ep_qty,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($oh_cst,2).'</td>';

                    $html .= '<td style="text-align:right;">'.decimal_adjustments($fi_qty,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($fi_prc,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($fi_amt,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($mi_amt,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($ti_amt,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($ei_prc,2).'</td>';

                    $html .= '<td style="text-align:right;">'.decimal_adjustments($fe_qty,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($fe_prc,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($fe_amt,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($me_amt,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($te_amt,2).'</td>';
                    $html .= '<td style="text-align:right;">'.decimal_adjustments($ee_prc,2).'</td>';
                    $html .= '</tr>';

                    //Totals
                    $tep_qty += (float)$ep_qty;
                    $toh_cst += (float)$oh_cst;

                    $tfi_qty += (float)$fi_qty;
                    $tfi_amt += (float)$fi_amt;
                    $tmi_amt += (float)$mi_amt;
                    $tti_amt += (float)$ti_amt;

                    $tfe_qty += (float)$fe_qty;
                    $tfe_amt += (float)$fe_amt;
                    $tme_amt += (float)$me_amt;
                    $tte_amt += (float)$te_amt;

                }
                $tfi_prc = 0; if((float)$tfi_qty != 0){ $tfi_prc = (float)$tfi_amt / (float)$tfi_qty; }
                $tei_prc = 0; if((float)$tep_qty != 0){ $tei_prc = (float)$tti_amt / (float)$tep_qty; }
                $tfe_prc = 0; if((float)$tfe_qty != 0){ $tfe_prc = (float)$tfe_amt / (float)$tfe_qty; }
                $tee_prc = 0; if((float)$tep_qty != 0){ $tee_prc = (float)$tte_amt / (float)$tep_qty; }

                $html .= '</tbody>';
                $html .= '<tfoot class="thead3">';
                $html .= '<tr>';
                $html .= '<th style="text-align:left;" colspan="2">Total</th>';
                $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tep_qty,5))).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($toh_cst,2).'</th>';

                $html .= '<th style="text-align:right;">'.decimal_adjustments($tfi_qty,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tfi_prc,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tfi_amt,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tmi_amt,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tti_amt,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tei_prc,2).'</th>';
                
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tfe_qty,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tfe_prc,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tfe_amt,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tme_amt,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tte_amt,2).'</th>';
                $html .= '<th style="text-align:right;">'.decimal_adjustments($tee_prc,2).'</th>';
