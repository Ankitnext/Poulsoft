...
if($db == ''){
    ...
    global $page_title; $page_title = "Broiler Profit And Loss Report";
    include "header_head.php";
    $form_path = "broiler_pandl_report11.php";
}
else{
...
$file_name = "Broiler Profit And Loss Report";
...
while($row = mysqli_fetch_assoc($query)){ $sector_code[$row['code']] = $row['code']; $sector_name[$row['code']] = $row['description']; $frmw[$row['code']] = $row['code']; $frmwn[$row['code']] = $row['description'];  }
...
$sql = "SELECT * FROM `broiler_farm` WHERE `active` = '1' AND `dflag` = '0' ORDER BY `description` ASC";
...
while($row = mysqli_fetch_assoc($query)){
    ...
    $frmw[$row['code']] = $row['code'];
    $frmwn[$row['code']] = $row['description'];
}
...
if(isset($_POST['submit_report']) == true){
    ...
    $farmfltr = ""; $wafltr = ""; $bchfltr = "" ; 
 
    if($farms != "all"){ $farmfltr = " AND `warehouse` = '$farms'"; $sector_from_search = " AND `fromwarehouse` = '$farms'"; $sector_to_search = " AND `towarehouse` = '$farms'"; }else{ $farmfltr = ""; $sector_from_search = ""; $sector_to_search = ""; }
    echo $farmfltr; 
    // if( $farms != "all"){ 
    //     $sql = "SELECT * FROM `broiler_farm` WHERE `code` IN ('$farms') AND `dflag` = '0' AND `active` = '1' ORDER BY `description` ASC";
    //     $query = mysqli_query($conn,$sql); $rowcount=mysqli_num_rows($query);
    ...
    //         while($row = mysqli_fetch_assoc($query)){
    //             $farm_alist[$row['code']] = $row['code'];
    //         }
    //         $farmcode1 = implode("','", $farm_alist);
    //         $sql = "SELECT * FROM `broiler_batch` WHERE `farm_code` IN ('$farmcode1') AND `dflag` = '0' AND `active` = '1'";
    //         echo $sql;
    //         $query = mysqli_query($conn,$sql); $batch_alist1 = array();
    //         while($row = mysqli_fetch_assoc($query)){
    //             $batch_alist1[$row['code']] = $row['code'];
    //         }

    //     $bchfltr = implode("','",$batch_alist1);
    //     }else{
    //     $wafltr = $farms;
    //     }
    // }else{
    //      $allfarm = implode("','",$farm_code);
    //      $sql = "SELECT * FROM `broiler_batch` WHERE `farm_code` IN ('$allfarm') AND `dflag` = '0' AND `active` = '1'";
    //         $query = mysqli_query($conn,$sql); $allbatch_alist = array();
    //         while($row = mysqli_fetch_assoc($query)){
    //             $allbatch_alist[$row['code']] = $row['code'];
    //         }
    //         $bchfltr = implode("','",$allbatch_alist);

    //     $wafltr = implode("','",$sector_code);
...
<html>
    ...
    <body align="center">
                            ...
                            <div class="row">
                                ...
                                <div class="m-2 form-group">
                                    ...
                                    <select name="farms" id="farms" class="form-control select2">
                                        ...
                                        <?php foreach($frmw as $fcode){ if($frmwn[$fcode] != ""){ ?>
                                        <option value="<?php echo $fcode; ?>" <?php if($farms == $fcode){ echo "selected"; } ?>><?php echo $frmwn[$fcode]; ?></option>
                                        <?php } } ?>
                                    </select>
                                </div>
                            </div>
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            if(isset($_POST['submit_report']) == true){
                ...
                $sql_record = "SELECT * FROM `broiler_purchases` WHERE `date` < '$fdate'".$farmfltr." AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
               // echo $sql_record; die();
                ...
                while($row = mysqli_fetch_assoc($query)){
                 ...
                 // echo  $hg =  ($row['rcd_qty'] + $row['fre_qty']) * $row['rate'];
                    if(strtotime($idate) < strtotime($fdate)){
                       ...
                       $op_amt[$key_code] += $row['item_tamt'];;
                    } else if((strtotime($idate) > strtotime($fdate)) && (strtotime($idate) < strtotime($tdate))){
                        $pur_qty[$key_code] += $row['rcd_qty'] + $row['fre_qty'];
                         $pur_amt[$key_code] += $row['item_tamt'];;
                    } else {
                        $cls_qty[$key_code] += $row['rcd_qty'] + $row['fre_qty'];
                        $cls_amt[$key_code] += $row['item_tamt'];;
                    }
    
                }
               

                $sql_record = "SELECT * FROM `broiler_purchases` WHERE `date` >= '$fdate' AND  `date` <= '$tdate'".$farmfltr." AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
               ...
               // echo $sql_record;
                ...
                while($row = mysqli_fetch_assoc($query)){
                    $key_code = $item_dcode[$row['icode']];
                    $idate = $row['date'];
                    $totpur[$key_code] += $row['item_tamt'];
                  //  echo $key_code."@".$row['finl_amt'];
                 // echo  $hg =  ($row['rcd_qty'] + $row['fre_qty']) * $row['rate'];
                    if(strtotime($idate) < strtotime($fdate)){
                       $op_qty[$key_code] += $row['rcd_qty'] + $row['fre_qty'];
                       $op_amt[$key_code] += $row['item_tamt'];
                ...
                $sql_record = "SELECT * FROM `broiler_sales` WHERE `date` < '$fdate' ".$farmfltr." AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
                ...
                while($row = mysqli_fetch_assoc($query)){
                    ...
                    if(strtotime($idate) < strtotime($fdate)){
                        ...
                        $op_samt[$key_code] += $row['item_tamt'];   
                     } else if((strtotime($idate) > strtotime($fdate)) && (strtotime($idate) < strtotime($tdate))){
                        $sale_qty[$key_code] += $row['rcd_qty'] + $row['fre_qty'];
                        $sale_amt[$key_code] += $row['item_tamt'];   
                     } else {
                         $cls_qty[$key_code] += $row['rcd_qty'] + $row['fre_qty'];
                         $cls_samt[$key_code] += $row['item_tamt'];         
                     }
                ...
                $sql_record = "SELECT * FROM `broiler_sales` WHERE `date` >= '$fdate' AND  `date` <= '$tdate' ".$farmfltr." AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
              //   echo "<br/>".$sql_record;
                $query = mysqli_query($conn,$sql_record); $i = 1;
                while($row = mysqli_fetch_assoc($query)){
                    $key_code = $item_dcode[$row['icode']];
                     $idate = $row['date'];
                     $totsales[$key_code] += $row['item_tamt'];
                  //   echo "<br/>".$row['icode']."@".$row['item_tamt'];
                    if(strtotime($idate) < strtotime($fdate)){
                        $op_sqty[$key_code] += $row['rcd_qty'] + $row['fre_qty'];
                        $op_samt[$key_code] += $row['item_tamt'];
	...
	// $sector_to_search = " AND `towarehouse` IN ('$all_sec_codes')";
                   ...
                   $sql = "SELECT * FROM `item_stocktransfers` WHERE `date` >= '$fdate' AND  `date` <= '$tdate' AND `active` = 1 AND `dflag` = 0".$sector_from_search; $query = mysqli_query($conn,$sql);
                    while($row = mysqli_fetch_assoc($query)){
                        $tout_qty[$row['code']] = $tout_qty[$row['code']] + $row['quantity'];
                        $tout_amt[$row['code']] = $tout_amt[$row['code']] + ($row['quantity'] * $row['price']);
                    }
                    $sql = "SELECT * FROM `item_stocktransfers` WHERE `date` >= '$fdate' AND  `date` <= '$tdate' AND `active` = 1 AND `dflag` = 0 ".$sector_to_search;$query = mysqli_query($conn,$sql);
                    while($row = mysqli_fetch_assoc($query)){
                ...
                while($row = mysqli_fetch_assoc($query)){  $coaname[$row['code']] = $row['description']; $coa_code[$row['code']] = $row['code']; $coa[] = $row['code']; } 
               
                $seq = "SELECT * FROM `account_vouchers` WHERE `date` >= '$fdate' AND  `date` <= '$tdate' AND ".$farmfltr." `active` = '1'";
              ...
              //  echo $sql;
                ...
                $coads = implode("','",$coa_code);
                $seq = "SELECT * FROM `account_vouchers` WHERE `date` >= '$fdate' AND  `date` <= '$tdate' AND `tcoa` IN ('$coads') AND `active` = '1'";
               ...
               // print_r($exp_amt);
                ...
                foreach($catcode as $cat_code){
                            ...
                            $sal_cat_amt[$cat_code] = $sal_cat_amt[$cat_code] + $totsales[$cat_code];
                ...
                echo "</tr>";
                foreach($catcode as $cat_code){
                    if(number_format_ind($ob_cat_amt[$cat_code]) == "0.00" && number_format_ind($totsales[$cat_code]) == "0.00"){
                        ...
                        //echo "<td></td>";
                        echo "<td style='text-align:left;'>".$catname[$cat_code]."</td>";
                        echo "<td>".number_format_ind($totsales[$cat_code])."</td>";
                        echo "</tr>";
                        $tdr_amt = $tdr_amt + $ob_cat_amt[$cat_code] + $pur_cat_amt[$cat_code];
                        $t_ob_amt = $t_ob_amt + $ob_cat_amt[$cat_code];
                        
                        $t_sl_amt = $t_sl_amt + $totsales[$cat_code];
                    }
                }
                ...
                foreach($catcode as $cat_code){
                    ...
                    if($cb_cat_amt[$cat_code] == "0.00" && number_format_ind($totpur[$cat_code]) == "0.00"){
                        ...
                        //echo "<td></td>";
                        echo "<td style='text-align:left;'>".$catname[$cat_code]."</td>";
                        echo "<td>".number_format_ind($totpur[$cat_code])."</td>";
                        //echo "<td></td>";
                        echo "<td style='text-align:left;'>".$catname[$cat_code]."</td>";
                       ...
                       // $tdr_amt = $tdr_amt + $mort_cat_amt[$cat_code];
                        $t_pur_amt = $t_pur_amt + $totpur[$cat_code];
               ...
               // echo "<td>".number_format_ind($t_mt_amt)."</td>";
                ...
                // echo "</tr>";
                // foreach($catcode as $cat_code){
                //    // echo $cat_code."@".$cb_cat_amt[$cat_code]."<br/>";
                //     if($cb_cat_amt[$cat_code] == "0.00" && number_format_ind($cb_cat_amt[$cat_code]) == "0.00"){
