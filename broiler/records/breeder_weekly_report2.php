    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            if(isset($_POST['submit_report']) == true){
                ...
                $sql = "SELECT * FROM `breeder_dayentry_produced` WHERE `flock_code` IN ('$flocks') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`flock_code` ASC";
                ...
                while($row = mysqli_fetch_assoc($query)){
                    ...
                    if($egg_pdate == "" || strtotime($row['date']) < strtotime($egg_pdate)){ } else{ $week_nos[$weeks] = $weeks; }
                }

                $sql = "SELECT * FROM `breeder_dayentry_consumed` WHERE `flock_code` IN ('$flocks') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`flock_code` ASC";
                ...
                if((int)$de_cnt > 0){
                    while($row = mysqli_fetch_assoc($query)){
                        $bird_wage = age_in_weeks($row['breed_age']); $weeks = fetch_cweek($bird_wage);
                        if($egg_pdate == "" || strtotime($row['date']) < strtotime($egg_pdate)){
                    ...
                    $sql = "SELECT * FROM `breeder_bird_transfer` WHERE `to_flock` IN ('$flocks') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`to_flock` ASC";
                    ...
                    $sql = "SELECT * FROM `breeder_bird_transfer` WHERE `from_flock` IN ('$flocks') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`from_flock` ASC";
                    ...
                    //Livability
                    $html .= '<th style="text-align:right;" class="f_std">'.str_replace(".00","",number_format_ind(round($f_slive,5))).'</th>';
                    $html .= '<th style="text-align:right;" class="f_act">'.str_replace(".00","",number_format_ind(round($f_alive,5))).'</th>';
                    $html .= '<th style="text-align:right;" class="m_act">'.str_replace(".00","",number_format_ind(round($m_alive,5))).'</th>';
                    //Egg Production
                    foreach($egg_code as $eggs){
                    ...
                    $html .= '<th style="text-align:right;" class="act">'.str_replace(".00","",number_format_ind(round($act_hep,5))).'</th>';
                    //HHE
                    $html .= '<th style="text-align:right;" class="std"></th>';
                    $html .= '<th style="text-align:right;" class="act"></th>';
                    //HHHE
                    $html .= '<th style="text-align:right;" class="std"></th>';
                    $html .= '<th style="text-align:right;"class="act"></th>';
                    //Feed per Bird
                    $html .= '<th style="text-align:right;" class="std"></th>';
                    $html .= '<th style="text-align:right;" class="act">'.number_format_ind(round($fafp_bird,5)).'</th>';
                    //Egg Weight
                    $html .= '<th style="text-align:right;" class="std"></th>';
                    $html .= '<th style="text-align:right;" class="act"></th>';
                    //Feed / Egg
                    $html .= '<th style="text-align:right;"></th>';
                    //Body Weight
                    $html .= '<th style="text-align:right;" class="f_std"></th>';
                    $html .= '<th style="text-align:right;" class="f_act"></th>';
                    $html .= '<th style="text-align:right;" class="m_std"></th>';
                    $html .= '<th style="text-align:right;" class="m_act"></th>';
