...
$sql = "SELECT * FROM `extra_access` WHERE `field_name` = 'broiler_feed_formula.php' AND `field_function` = 'Quantity lbs' AND `flag` = '1' ORDER BY `id` ASC"; $query = mysqli_query($conn,$sql);
$qty_lbflag = mysqli_num_rows($query); //if($qty_lbflag > 0) { echo "welcome"; }
...
<html>
    ...
    <body align="center">
        <table class="tbl" align="center">
            ...
            <?php } ?>
            <form action="broiler_feed_formula3.php" method="post">
            ...
            <thead class="thead3" align="center">
                <tr align="center">
                    ...
                    <th>Item Name</th>
                    <th>Quantity</th>
                    <?php if($qty_lbflag > 0) { ?><th>Quantity in lbs</th> <?php } ?>
                    <?php if($qty_lbflag > 0) { ?><th>Rate per lbs</th>  <?php } else { ?>
                    <th>Rate</th> <?php } ?>
                    <th>Amount</th>
                </tr>
            </thead>
            ...
            <tbody class="tbody1">
                ...
                while($row = mysqli_fetch_assoc($query)){
                    ...
                    $qty_lb = $feed_qty[$row['item_code']] * $row['item_qty']; 
                     $iamt = round(((float)$qty_lb * (float)$iprc),2);
                ?>
                <tr>
                    <td title="Feed"><?php echo $sector_name[$row['mill_code']]; ?></td>
                    <td title="Desc"><?php echo $row['description']; ?></td>
                    ...
                    <?php if($qty_lbflag > 0) { ?><td title="Birds" style="text-align:right;"><?php echo number_format_ind($qty_lb); ?></td><?php } ?>
                    <td title="Price" style="text-align:right;"><?php echo number_format_ind($iprc); ?></td>
                    <td title="Amount" style="text-align:right;"><?php echo number_format_ind($iamt); ?></td>
                </tr>
                <?php
                    $tot_qty = $tot_qty + $row['item_qty'];
                    $tot_lbqty += (float)$qty_lb;
                    $tot_amt = $tot_amt + (float)$iamt;
                }
                //if((float)$tot_qty != 0){ $avg_prc = round(((float)$tot_amt / (float)$tot_qty),2); } else{ $avg_prc = 0; }
                if($tot_lbqty != 0 || $tot_lbqty != ""){ $tot_prc = (float)$tot_amt/(float)$tot_lbqty; } else { $tot_prc = 0;}
                ?>
            </tbody>
            <tr class="thead4">
                <th colspan="3" style="text-align:center;">Total</th>
                <th style="text-align:right;"><?php echo number_format_ind(round($tot_qty,2)); ?></th>
                <th style="text-align:right;"><?php echo number_format_ind(round($tot_lbqty,2)); ?></th>
                <th style="text-align:right;"><?php echo number_format_ind(round($tot_prc,2)); ?></th>
