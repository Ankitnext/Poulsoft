<?php
//broiler_item_transferreport.php
...
if(isset($_REQUEST['submit_report']) == true){
    ...
    $fromfltr = $tofltr = "";
    $rgn_fltr = ""; if($fregions != "all"){ $rgn_fltr = " AND `region_code` IN ('$fregions')"; }
    $brh_fltr = ""; if($fbranches != "all"){ $brh_fltr = " AND `branch_code` IN ('$fbranches')"; }
    $sql = "SELECT * FROM `broiler_farm` WHERE `active` = '1'".$rgn_fltr."".$brh_fltr." AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql); $floc_alist = $fbrh_alist = array();
    while($row = mysqli_fetch_assoc($query)){ $floc_alist[$row['code']] = $row['code']; $fbrh_alist[$row['branch_code']] = $row['branch_code']; }

    $fbrh_list = implode("','",$fbrh_alist);
    $sql = "SELECT * FROM `broiler_secbrch_mapping` WHERE `branch_code` IN ('$fbrh_list') AND `active` = '1' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($query)){ $floc_alist[$row['sector_code']] = $row['sector_code']; }
    
    $rgn_fltr = ""; if($tregions != "all"){ $rgn_fltr = " AND `region_code` IN ('$tregions')"; }
    $brh_fltr = ""; if($tbranches != "all"){ $brh_fltr = " AND `branch_code` IN ('$tbranches')"; }
    $sql = "SELECT * FROM `broiler_farm` WHERE `active` = '1'".$rgn_fltr."".$brh_fltr." AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql); $tloc_alist = $tbrh_alist = array();
    while($row = mysqli_fetch_assoc($query)){ $tloc_alist[$row['code']] = $row['code']; $tbrh_alist[$row['branch_code']] = $row['branch_code']; }
    
    $tbrh_list = implode("','",$tbrh_alist);
    $sql = "SELECT * FROM `broiler_secbrch_mapping` WHERE `branch_code` IN ('$tbrh_list') AND `active` = '1' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($query)){ $tloc_alist[$row['sector_code']] = $row['sector_code']; }
        
    if($fregions != "all" || $fbranches != "all"){ $floc_list = implode("','",$floc_alist); $fromfltr = " AND `fromwarehouse` IN ('$floc_list')"; }
    if($tregions != "all" || $tbranches != "all"){ $tloc_list = implode("','",$tloc_alist); $tofltr = " AND `towarehouse` IN ('$tloc_list')"; }
    if($loc_sector_from != "all"){ $fromfltr .= " AND `fromwarehouse` IN ('$loc_sector_from')"; }
    if($loc_sector_to != "all"){ $tofltr .= " AND `towarehouse` IN ('$loc_sector_to')"; }
...
<html>
    ...
    <body align="center">
        <table class="tbl" align="center">
            ...
            <?php } ?>
            <form action="broiler_item_transferreport.php" method="post">
