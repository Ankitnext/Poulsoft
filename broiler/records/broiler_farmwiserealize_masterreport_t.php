<?php
//broiler_farmwiserealize_masterreport_t.php
...
if($db == ''){
    ...
    $form_path = "broiler_farmwiserealize_masterreport_t.php";
}
else{
    ...
    include "header_head.php";
    $form_path = "broiler_farmwiserealize_masterreport_t.php?db=$db&userid=".$user_code;
}
...
$query3 = mysqli_query($conn, $sql3); $a_cnt = mysqli_num_rows($query3);
if($a_cnt > 0){ while($row3 = mysqli_fetch_assoc($query3)){ $admincost_include_flag = $row3['flag']; } }
...
$sql2 = "SELECT * FROM `broiler_reportfields` WHERE `field_href` LIKE 'broiler_farmwiserealize_masterreport.php' AND `user_access_code` = '$user_code' AND `active` = '1'";
...
$sql = "SELECT * FROM `broiler_batch` WHERE `active` = '1' AND `dflag` = '0' ORDER BY `description` ASC";
$query = mysqli_query($conn,$sql); $batch_code = $batch_name = $batch_book = $batch_gcflag = array();
...
$query = mysqli_query($conn,$sql); $bstd_body_weight = $bstd_daily_gain = $bstd_avg_daily_gain = $bstd_fcr = $bstd_cum_feed = array();
...
$query = mysqli_query($conn,$sql); $desig_code = "";
while($row = mysqli_fetch_assoc($query)){ if($desig_code == ""){ $desig_code = $row['code']; } else{ $desig_code = $desig_code."','".$row['code']; } }

$sql = "SELECT * FROM `broiler_employee` WHERE `desig_code` IN ('$desig_code') AND `dflag` = '0' ORDER BY `name` ASC";
...
$query = mysqli_query($conn,$sql); $item_cat_coa = array();
...
<html>
    ...
    <body align="center">
        <table class="tbl" align="center" <?php if($excel_type == "print"){ echo ' id="mine"'; } else{ echo 'width="1300px"'; } ?>>
            ...
            <?php } ?>
            <?php if($db == ''){?>
            <form action="broiler_farmwiserealize_masterreport_t.php" method="post">
            <?php } else { ?>
            <form action="broiler_farmwiserealize_masterreport_t.php?db=<?php echo $db; ?>&userid=<?php echo $user_code; ?>" method="post">
            <?php } ?>
            <form action="broiler_farmwiserealize_masterreport_t.php" method="post">
        ...
        <?php } ?>
            ...
            <tbody class="tbody1" id="tbody1">
                ...
                $high_feedin_gc_sup = "";
                $sql = "SELECT * FROM `broiler_rearingcharge` WHERE `active` = '1' AND `dflag` = '0' $date_filter $farm_query $fcr_filter $cfcr_filter $mortp_filter $brand_filter $chkin_filter"; $query = mysqli_query($conn,$sql); $slno = 0;
                ...
                for($i = 1;$i <= $col_count;$i++){
                    ...
                    if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "sl_no"){ echo "<th style='text-align:left; border-right: 0px;'></th>"; }
                        ...
                        if($_SESSION['dbase'] == "poulso6_broiler_wb_sahapoultry"){
                            ...
                            if((float)$tot_sold_weight != 0){ $t2 = round(((float)$tot_rmt / (float)$tot_sold_weight),2); } else{ $t2 = 0; }
                        ...
                        if($_SESSION['dbase'] == "poulso6_broiler_wb_sahapoultry" || $_SESSION['dbase'] == "poulso6_broiler_hr_easyfoods"){
                            if($tot_sold_weight > 0){
