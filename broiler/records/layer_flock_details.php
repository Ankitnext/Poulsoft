...
if($db == ''){
    ...
    $form_path = "layer_flock_details.php";
}
else{
    ...
    include "header_head.php";
    $form_path = "layer_flock_details.php?db=$db&userid=".$user_code;
}
...
if(in_array("layer_sheds", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.layer_sheds LIKE poulso6_admin_layer_layermaster.layer_sheds;"; mysqli_query($conn,$sql1); }
if(in_array("layer_shed_allocation", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.layer_shed_allocation LIKE poulso6_admin_layer_layermaster.layer_shed_allocation;"; mysqli_query($conn,$sql1); }
...
while($row = mysqli_fetch_assoc($query)){ $shed_code[$row['code']] = $row['code']; $shed_name[$row['code']] = $row['description'];  }
 ...
 $farms = $units = $sheds = "all"; $excel_type = "display";
...
<html>
    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            $nhtml .= '<th>Unit Name</th>'; $fhtml .= '<th id="order">Unit Name</th>';
            ...
            $nhtml .= '<th>Age (In Days)</th>'; $fhtml .= '<th id="order_num">Age (In Days)</th>';
            $nhtml .= '<th>Start Date</th>'; $fhtml .= '<th id="order_num">Start Date</th>';
            ...
            if(isset($_POST['submit_report']) == true){
                ...
                $sql = "SELECT * FROM `layer_shed_allocation` WHERE `active` = '1'".$farm_fltr."".$unit_fltr."".$shed_fltr." AND `dflag` = '0' ORDER BY `addedtime` ASC";
                ...
                while($row = mysqli_fetch_assoc($query)){
                    ...
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($opn_birds,5))).'</td>';
                    
                    $html .= '</tr>';

                    $tot_birds += (float)$opn_birds;
                }
                $html .= '</tbody>';
                $html .= '<tfoot class="thead3">';
                $html .= '<tr>';
                $html .= '<th style="text-align:left;" colspan="7">Total</th>';
                $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tot_birds,5))).'</th>';
                $html .= '</tr>';
                $html .= '</tfoot>';
            }
            echo $html;
