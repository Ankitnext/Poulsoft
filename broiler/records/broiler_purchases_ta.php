...
$query3 = mysqli_query($conn, $sql3); $ccount3 = mysqli_num_rows($query3); $bag_column = 0;
if($ccount3 > 0){ while($row3 = mysqli_fetch_assoc($query3)){ $bag_column = $row3['flag']; } }
else{ mysqli_query($conn, "INSERT INTO `extra_access` ( `field_name`, `field_function`, `user_access`, `flag`) VALUES ( 'Purchase Report', 'bag_column', 'all', '0')"); }
if($bag_column == ""){ $bag_column = 0; }*/
...
$query = mysqli_query($conn, $sql); $ccount4 = mysqli_num_rows($query); $downloadmfiles_flag = 0;
if($ccount4 > 0){ while($row3 = mysqli_fetch_assoc($query)){ $downloadmfiles_flag = $row3['flag']; } }
else{ mysqli_query($conn, "INSERT INTO `extra_access` ( `field_name`, `field_function`, `user_access`, `flag`) VALUES ( 'Purchase Report', 'Download Multiple Files', 'all', '0')"); }
if($downloadmfiles_flag == ""){ $downloadmfiles_flag = 0; }
...
while($row = mysqli_fetch_assoc($query)){ $frmr_mobile[$row['code']] = $row['mobile1']; }
        ...
        <table class="tbl" align="center">
            ...
            <thead class="thead1" align="center" style="width:1212px;">
                <tr align="center">
                    ...
                    <th colspan="16" align="center"><?php echo $row['cdetails']; ?><h5>Purchase Report</h5></th>
                </tr>
            </thead>
            <?php } ?>
                    ...
                    <tr>
                        <th colspan="18">
                            <div class="row">
                                ...
                                <div class="m-2 form-group">
                                    <label>Export</label>
                                    <select name="export" id="export" class="form-control select2"  onchange="tableToExcel('main_body', 'Purchase Summary','<?php echo $filename;?>', this.options[this.selectedIndex].value)">
           ...
           <?php
            ...
            <tbody class="tbody1">
                ...
                while($row = mysqli_fetch_assoc($query)){
                    $file_count = $bags_qty = 0;

                    if(!empty($all_feed_bag_capacity[$row['icode']]) && (float)$all_feed_bag_capacity[$row['icode']] != 0){
                        $bags_qty = (((float)$row['rcd_qty'] + (float)$row['fre_qty']) / (float)$all_feed_bag_capacity[$row['icode']]);
                    }
                    else if(!empty($all_feed_bag_capacity['all']) && (float)$all_feed_bag_capacity['all'] != 0){
                        $bags_qty = (((float)$row['rcd_qty'] + (float)$row['fre_qty']) / (float)$all_feed_bag_capacity['all']);
                    }
                    else{ }
                    $tot_bag_qty += (float)$bags_qty;
                ...
                <tr>
                    ...
                    <td title="Bags" style="text-align:right;"><?php echo $bags_qty; ?></td>
                    ...
                    <td title="Farmer Mobile" style="text-align:left;"><?php echo $frmr_mobile[$farm_farmer[$row['warehouse']]]; ?></td>
