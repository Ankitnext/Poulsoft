<?php
//broiler_dailyrates_report1.php
...
if($db == ''){
    ...
    $form_path = "broiler_dailyrates_report1.php";
}
else{
    ...
    include "header_head.php";
    $form_path = "broiler_dailyrates_report1.php?db=$db&userid=".$user_code;
}
...
$fdate = $tdate = date("Y-m-d"); $farms = $units = $sheds = "all"; $excel_type = "display";
...
<html>
    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            $nhtml .= '<th>BLL</th>'; $fhtml .= '<th id="order_num">BLL</th>';
            $nhtml .= '<th>CUT</th>'; $fhtml .= '<th id="order_num">CUT</th>';
            $nhtml .= '<th>BLL FARM</th>'; $fhtml .= '<th id="order_num">BLL FARM</th>';
            $nhtml .= '<th>BHL</th>'; $fhtml .= '<th id="order_num">BHL</th>';
            $nhtml .= '<th>CUT</th>'; $fhtml .= '<th id="order_num">CUT</th>';
            $nhtml .= '<th>BLL FARM</th>'; $fhtml .= '<th id="order_num">BLL FARM</th>';
            ...
            if(isset($_POST['submit_report']) == true){
                ...
                $sql = "SELECT * FROM `broiler_daily_rates` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `active` = '1' AND `dflag` = '0' ORDER BY `date` ASC";
                $query = mysqli_query($conn,$sql); $tot_sqft = $tot_cpty = $tot_emps = 0;
                while($row = mysqli_fetch_assoc($query)){
                 
                    $cdate = date("d-m-Y",strtotime($row['date']));
                    $bll_dr = $row['bll_dr'];
                    $bll_max_cut = $row['bll_max_cut'];
                    ...
                    $html .= '<tr>';
                    $html .= '<td>'.$cdate.'</td>';
                    $html .= '<td style="text-align:right;">'.rtrim(rtrim(number_format_ind(round($bll_dr, 5)), '0'), '.').'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($bll_max_cut,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($diff,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($bhl_dr,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($bhl_max_cut,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($diff2,5))).'</td>';
                    $html .= '</tr>';

                    // $tbll_dr += (float)$bll_dr;
                    // $tbll_max_cut += (float)$bll_max_cut;
                    // $tdiff += (float)$diff;
                    // $tbhl_dr += (float)$bhl_dr;
                    // $tbhl_max_cut += (float)$bhl_max_cut;
                    // $tdiff2 += (float)$diff2;
                }
                $html .= '</tbody>';
                ...
                // $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tbll_dr,5))).'</th>';
                // $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tbll_max_cut,5))).'</th>';
                // $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tdiff,5))).'</th>';
                // $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tbhl_dr,5))).'</th>';
                // $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tbhl_max_cut,5))).'</th>';
                // $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tdiff2,5))).'</th>';
                // $html .= '</tr>';
                // $html .= '</tfoot>';
            }
            echo $html;
