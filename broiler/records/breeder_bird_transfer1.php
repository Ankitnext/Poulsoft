...
if($db == ''){
    ...
    global $page_title; $page_title = "Birds Transfer Report";
    include "header_head.php";
    $form_path = "breeder_bird_transfer1.php";
}
else{
    ...
    global $page_title; $page_title = "Birds Transfer Report";
    include "header_head.php";
    $form_path = "breeder_bird_transfer1.php?db=$db&userid=".$user_code;
}
...
$file_name = "Birds Transfer Report";
...
<html>
    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            $nhtml .= '<th>From Flock</th>'; $fhtml .= '<th id="order">From Flock</th>';
            $nhtml .= '<th>To Flock</th>'; $fhtml .= '<th id="order">To Flock</th>';
            $nhtml .= '<th>F. Birds</th>'; $fhtml .= '<th id="order_num">F. Birds</th>';
            $nhtml .= '<th>Avg Rate</th>'; $fhtml .= '<th id="order_num">Avg Rate</th>';
            $nhtml .= '<th>Bird Cost</th>'; $fhtml .= '<th id="order_num">Bird Cost</th>';
            $nhtml .= '<th>M. Birds</th>'; $fhtml .= '<th id="order_num">M. Birds</th>';
            $nhtml .= '<th>Avg Rate</th>'; $fhtml .= '<th id="order_num">Avg Rate</th>';
            $nhtml .= '<th>Bird Cost</th>'; $fhtml .= '<th id="order_num">Bird Cost</th>';
            ...
            if(isset($_POST['submit_report']) == true){
                // $drv_fltr = ""; if($farms != "all"){ $drv_fltr = " AND `farm_code` = '$farms'"; }
                $flock_list = implode("','",$flock_alist);

                $sql = "SELECT * FROM `breeder_bird_transfer` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `active` = '1' AND `dflag` = '0' AND (from_flock IN ('$flock_list') OR to_flock IN ('$flock_list')) ORDER BY `date`,`trnum` ASC";
                $query = mysqli_query($conn,$sql); $sl = 0;
                while($row = mysqli_fetch_assoc($query)){
                 
                    $sl++;
                    $date1 = date("d.m.Y",strtotime($row['date']));
                    $fl_fname = $flock_name[$row['from_flock']];
                    $fl_tname = $flock_name[$row['to_flock']];
                    $f_bqty = $row['female_bqty'];
                    $f_bprc = $row['female_bprc'];
                    $f_cost = $f_bqty *  $f_bprc;
                    $m_bqty = $row['male_bqty'];
                    $m_bprc = $row['male_bprc'];
                    $m_amt = $m_bqty * $m_bprc;
                    $rem = $row['remarks'];
                    ...
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($f_bqty,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($f_bprc,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($f_cost,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($m_bqty,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($m_bprc,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($m_amt,5))).'</td>';
                    $html .= '<td>'.$rem.'</td>';
                   
                    $html .= '</tr>';

                    $tot_fbqty += (float)$f_bqty;
                    $tot_fbprc += (float)$f_bprc;
                    $tot_fcost += (float)$f_cost;
                    $tot_mbqty += (float)$m_bqty;
                    $tot_mbprc += (float)$m_bprc;
                    $tot_mamt += (float)$m_amt;
                }
                $html .= '</tbody>';
                $html .= '<tfoot class="thead3">';
                $html .= '<tr>';
                $html .= '<th style="text-align:left;" colspan="4">Total</th>';
                $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tot_fbqty,5))).'</th>';
                $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tot_fbprc,5))).'</td>';
                $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tot_fcost,5))).'</td>';
                $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tot_mbqty,5))).'</td>';
                $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tot_mbprc,5))).'</td>';
                $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tot_mamt,5))).'</td>';
                $html .= '<th></th>';
                $html .= '</tr>';
                $html .= '</tfoot>';
            }
