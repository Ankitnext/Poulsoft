<?php
//breeder_livebatch_summary1.php
...
if($db == ""){
    ...
    global $page_title; $page_title = "Breeder Live Batch Summary Report";
    include "header_head.php";
    $form_path = "breeder_livebatch_summary1.php";
}
else{
    ...
    global $page_title; $page_title = "Breeder Live Batch Summary Report";
    include "header_head.php";
    $form_path = "breeder_livebatch_summary1.php?db=$db&userid=".$user_code;
}
include "decimal_adjustments.php";
include "breeder_cal_ageweeks.php";
$file_name = "Breeder Live Batch Summary Report";
...
$fdate = $tdate = date("Y-m-d"); $farms = $units = $sheds = $flocks = $batches = "all"; $excel_type = "display"; $slno_flag = 0;
...
<html>
    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            $nhtml .= '<th colspan="2">Female B.Wt</th>'; $fhtml .= '<th colspan="2">Female B.Wt</th>';
            $nhtml .= '<th colspan="2">Male B.Wt</th>'; $fhtml .= '<th colspan="2">Male B.Wt</th>';
            $nhtml .= '<th colspan="2">Female Feed/Bird</th>'; $fhtml .= '<th colspan="2">Female Feed/Bird</th>';
            $nhtml .= '<th colspan="2">Male Feed/Bird</th>'; $fhtml .= '<th colspan="2">Male Feed/Bird</th>';
            ...
            $nhtml .= '<th>Last Entry Date</th>'; $fhtml .= '<th id="order_date">Last Entry Date</th>';
            ...
            $nhtml .= '<th>F</th>'; $fhtml .= '<th id="order_num">F</th>';
            $nhtml .= '<th>M</th>'; $fhtml .= '<th id="order_num">M</th>';
            $nhtml .= '<th>Std.</th>'; $fhtml .= '<th id="order_num">Std.</th>';
            ...
            if(isset($_POST['submit_report']) == true){
                //Fetch Flock Details
                $flk_list = implode("','",$flock_code); $flk_fltr = ""; if($flocks != "all"){ $flk_fltr = " AND `code` IN ('$flocks')"; }
                $bch_list = implode("','",$batch_code); $bch_fltr = ""; if($batches != "all"){ $bch_fltr = " AND `batch_code` IN ('$batches')"; }
                if(sizeof($flock_code) > 0){
                    $sql = "SELECT * FROM `breeder_shed_allocation` WHERE `code` IN ('$flk_list') AND `batch_code` IN ('$bch_list')".$flk_fltr."".$bch_fltr." AND `dflag` = '0' ORDER BY `start_date` ASC";
                    $query = mysqli_query($conn,$sql);
                    ...
                    while($row = mysqli_fetch_assoc($query)){
                        ...
                        $breed_code[$key1] = $batch_breed[$row['batch_code']];
                        $flk_alist[$key1] = $key1;
                        $flk_farm[$key1] = $row['farm_code'];
                        $flk_unit[$key1] = $row['unit_code'];
                        $flk_shed[$key1] = $row['shed_code'];
                        $flk_batch[$key1] = $row['batch_code'];
                        $flk_name[$key1] = $row['description'];
                    }
                    ...
                    $query1 = mysqli_query($conn,$sql1); $omort_fbds = $ocull_fbds = $ocons_ffeed = $omort_mbds = $ocull_mbds = $ocons_mfeed = $bmort_fbds = $bcull_fbds = $bcons_ffeed = $bmort_mbds = $bcull_mbds = $bcons_mfeed = $bfbds_wht = $bmbds_wht = $begg_wht = $bdentry_date = $bage_fbds = $bwage_fbds = $ble_date = array();
                    while($row1 = mysqli_fetch_assoc($query1)){
                        if(strtotime($row1['date']) < strtotime($fdate)){
                            $key1 = $row1['flock_code'];
                    ...
                    while($row1 = mysqli_fetch_assoc($query1)){
                        if(strtotime($row1['date']) < strtotime($fdate)){ }
                            ...
                            $key1 = $row1['flock_code']."@".$row1['item_code'];
                    ...
                    foreach($flk_alist as $flks){
                        $fk_name = $flk_name[$flks];
                        $key1 = $flks; $slno++;
            
                        //Flock Details
                        $fcode = $flk_farm[$flks]; $fname = $farm_name[$fcode];
                        $ucode = $flk_unit[$flks]; $uname = $unit_name[$ucode];
                        $scode = $flk_shed[$flks]; $sname = $shed_name[$scode];
                        $bcode = $flk_batch[$flks]; $bname = $batch_name[$bcode];
                        $adate = ""; if(!empty($ble_date[$key1]) && $ble_date[$key1] != ""){ $adate = date("d.m.Y",strtotime($ble_date[$key1])); }
        ...
        <script>
            function fetch_flock_details(a){
                ...
                var method = "GET";
                var url = "breeder_fetch_flock_filter_master.php?farms="+farms+"&units="+units+"&sheds="+sheds+"&batches="+batches+"&flocks="+flocks+"&fetch_type=single&flock_type=all";
                //window.open(url);
