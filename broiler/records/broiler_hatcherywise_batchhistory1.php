<?php
//broiler_hatcherywise_batchhistory1.php
...
$file_name = "Hatchery Wise Realization";
...
$fdate = $tdate = date("Y-m-d"); $branches = $lines = $supervisors = $farms = $chick_in_from = $brands = "all";
$fcr_type = $cfcr_type = $mortp_type = $above_fcr = $above_cfcr = $above_mortp = ""; $sort_order = "date";
...
if(isset($_REQUEST['submit_report']) == true){
    ...
    //$url = "../PHPExcel/Examples/broiler_hatcherywise_batchhistory1-Excel.php?branches=".$branches."&report_view=".$report_view."&fdate=".$fdate."&tdate=".$tdate."&lines=".$lines."&supervisors=".$supervisors."&farms=".$farms."&brands=".$brands."&chick_in_from=".$chick_in_from."&href=".$field_href[0]."&above_fcr=".$above_fcr."&above_cfcr=".$above_cfcr;
}
else{
    $url = "";
}
...
<html>
    ...
    <body align="center">
        <table class="tbl" align="center" <?php if($excel_type == "print"){ echo ' id="mine"'; } else{ echo 'width="1300px"'; } ?>>
            ...
            <?php } ?>
            <?php if($db == ''){?>
            <form action="broiler_hatcherywise_batchhistory1.php" method="post">
            <?php } else { ?>
            <form action="broiler_hatcherywise_batchhistory1.php?db=<?php echo $db; ?>&userid=<?php echo $user_code; ?>" method="post">
            <?php } ?>
            <form action="broiler_hatcherywise_batchhistory1.php" method="post">
                        ...
                        <th colspan="<?php echo $tblcol_size; ?>">
                            <div class="row">
                                ...
                                <div class="m-2 form-group">
                                    ...
                                    <select name="sort_order" id="sort_order" class="form-control select2">
                                        <option value="date" <?php if($sort_order == "date"){ echo "selected"; } ?>>Date</option>
                                        <option value="high_chickin_secvcode" <?php if($sort_order == "high_chickin_secvcode"){ echo "selected"; } ?>>Hatchery</option>
                                        <option value="high_feedin_brand_code" <?php if($sort_order == "high_feedin_brand_code"){ echo "selected"; } ?>>Feed</option>
                                    </select>
                                </div>
                                <div class="m-2 form-group">
                                    <label>Export</label>
        ...
        <?php } ?>
            ...
            <tbody class="tbody1" id="tbody1">
                ...
                $query = mysqli_query($conn,$sql); while($row = mysqli_fetch_assoc($query)){ $batch_arr_list[$row['batch_code']] = $row['batch_code']; $soa_cnt[$row[$sort_order]] += 1; }
                ...
                $tot_sold_weight = $other_deduction = $tot_fmrpayable_amt = $tot_fmrpaid_amt = $tot_pl_amt = $tot_rmt = $tot_actpc_amt = $tot_saleinc_amt = $approved_gc_amt = 0;
                
                $sql = "SELECT * FROM `broiler_rearingcharge` WHERE `active` = '1' AND `dflag` = '0' $date_filter $farm_query $fcr_filter $cfcr_filter $mortp_filter $brand_filter $chkin_filter ORDER BY `$sort_order` ASC"; $query = mysqli_query($conn,$sql); $slno = 0;
                while($row = mysqli_fetch_assoc($query)){ $slno++;
                    ...
                    $tot_rmt += ((float)$row['mean_age'] * (float)$row['sold_birds']);
                    ...
                    $pl_on_fpay = 0; $pl_on_fpay = (float)$row['sale_amount']- (float)$tot_fcost; $pl_title = $row['sale_amount']."-".$tot_fcost;
                    $tot_pl_amt += (float)$pl_on_fpay;
                    $incr++;
                    
                    if($incr > 1 && $osf_dt != $row[$sort_order]){
                        $rm_cnt++;
                        $osf_dt = $row[$sort_order];
                        ...
                        for($i = 1;$i <= $col_count;$i++){
                            ...
                            if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "sl_no"){ echo "<th style='text-align:left; border-right: 0px;'></th>"; }
                                ...
                                //if($r_sbirds > 0){ echo "<th style='text-align:right;'>".number_format_ind(round(($mtsb / $r_sbirds),2))."</th>"; }
                                //else{ echo "<th style='text-align:right;'>".number_format_ind(0)."</th>"; }
                                if($_SESSION['dbase'] != "poulso6_broiler_hr_easyfoods"){
                                    if($r_incr > 0){ $mean_age = 0; $mean_age = $r_mage / $r_incr; } else{ $mean_age = 0; }
                                    echo "<th style='text-align:right;'>".number_format_ind($mean_age)."</th>";
                                }
                                else{
                                    if((float)$r_sbirds != 0){ $t2 = round(((float)$r_trmt / (float)$r_sbirds),2); } else{ $t2 = 0; }
                                    echo "<th style='text-align:right;'>".number_format_ind($t2)."</th>";
                                }
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "chick_placed"){ echo "<th style='text-align:right;'>".str_replace(".00","",number_format_ind($r_pchicks))."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mortality_count"){ echo "<th style='text-align:right;'>".str_replace(".00","",number_format_ind($r_mort))."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mortality_per"){
                                if($r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind(round((($r_mort / $r_pchicks) * 100),2))."</th>";
                                }
                                else{
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mortality_1week_count"){ echo "<th style='text-align:right;'>".str_replace(".00","",number_format_ind($r_days7_mcnt))."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mortality_1week_per"){
                                if($r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind(round((($r_days7_mcnt / $r_pchicks) * 100),2))."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mortality_30days_count"){ echo "<th style='text-align:right;'>".str_replace(".00","",number_format_ind($r_days30_mcnt))."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mortality_30days_per"){
                                if($r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind(round((($r_days30_mcnt / $r_pchicks) * 100),2))."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mortality_30more_count"){ echo "<th style='text-align:right;'>".str_replace(".00","",number_format_ind($r_days31_mcnt))."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mortality_30more_per"){
                                if($r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind(round((($r_days31_mcnt / $r_pchicks) * 100),2))."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "bird_shortage_count"){ echo "<th style='text-align:right;'>".str_replace(".00","",$r_short_birds)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "bird_excess_count"){ echo "<th style='text-align:right;'>".str_replace(".00","",$r_exces_birds)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "feedconsumed_count"){ echo "<th style='text-align:right;'>".number_format_ind(round($r_feed_cons))."</th>"; }
                                ...
                                if($r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind(round(($r_feed_cons / $r_pchicks),2))."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                                ...
                                if($r_sweight > 0){
                                    echo "<th style='text-align:right;'>".decimal_adjustments(($r_feed_cons / $r_sweight),3)."</th>";
                                }
                                else{
                                ...
                                if($r_sbirds > 0){
                                    $t1 = 0; $t1 = $r_sweight / $r_sbirds;
                                }
                                else{
                                    $t1 = 0;
                                }
                                if($r_sweight > 0){
                                    $t2 = 0; $t2 = $r_feed_cons / $r_sweight;
                                }
                                else{
                                    $t2 = 0;
                                }
                                ...
                                if($incr > 0){ $t1 = 0; $t1 = $r_dgain / $incr; } else{ $t1 = 0; }
                                echo "<th style='text-align:right;'>".number_format_ind($t1)."</th>";
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "eef"){
                                if($_SESSION['dbase'] != "poulso6_broiler_hr_easyfoods"){
                                    if($r_pchicks > 0){ $t1 = ($r_pchicks - $r_mort) / $r_pchicks; } else{ $t1 = 0; }
                                    if($r_sbirds > 0){ $t2 = $r_sweight / $r_sbirds; } else{ $t2 = 0; }
                                    if($r_sweight > 0){ $t3 = $r_feed_cons / $r_sweight; } else{ $t3 = 0; }
                                    if($r_sbirds > 0){ $t4 = $mean_age; } else{ $t4 = 0; }
                                    $title = "round((((((($r_pchicks - $r_mort) / $r_pchicks) * 100) * (round(($r_sweight / $r_sbirds),3))) * 100) / (($r_feed_cons / $r_sweight) * ($mean_age))))";
                                    ...
                                    $feef = (((100 - (($r_mort / $r_pchicks) * 100)) * ($r_sweight / $r_sbirds))/(((float)$r_trmt / (float)$r_sbirds) * ($r_feed_cons / $r_sweight)) * 100);
                                    echo "<th style='text-align:right;' title='".$title."'>".number_format_ind($feef)."</th>";
                                }                        
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "sold_birdsno"){ echo "<th style='text-align:right;'>".str_replace(".00","",number_format_ind($r_sbirds))."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "sold_birdswt"){ echo "<th style='text-align:right;'>".number_format_ind($r_sweight)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "avg_bodywt"){
                                if($r_sbirds > 0){
                                    echo "<th style='text-align:right;'>".decimal_adjustments(($r_sweight / $r_sbirds),3)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".decimal_adjustments(0,3)."</th>";
                                }
                                ...
                                if($r_sweight > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_samount / $r_sweight)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "sold_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_samount)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "std_chick_perkg_price"){
                                if($r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_camount / $r_pchicks)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "std_chick_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_camount)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "std_feed_price"){
                                if($r_feed_cons > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_famount / $r_feed_cons)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "std_feed_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_famount)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "std_medicine_price"){
                                if($r_sweight > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_mamount / $r_sweight)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "std_medicine_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_mamount)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "std_admin_price"){
                                if($r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_aamount / $r_pchicks)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                                ...
                                if($r_sweight > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_aamount / $r_sweight)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "std_admin_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_aamount)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_production_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_tfpay)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_prodperkg_price"){
                                if($r_sweight > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_tfpay / $r_sweight)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "std_gc_perkg"){ echo "<th style='text-align:right;'></th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_incentive"){ echo "<th style='text-align:right;'></th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_decentives"){ echo "<th style='text-align:right;'></th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_gc_perkg_price"){ echo "<th style='text-align:right;'></th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_gc_perkg_price2"){ echo "<th style='text-align:right;'></th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "rearing_charges"){ echo "<th style='text-align:right;'></th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "total_rearing_charges"){ echo "<th style='text-align:right;'>".number_format_ind($r_tapayable)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_tds_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_tapayable)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "other_deduction"){ echo "<th style='text-align:right;'>".number_format_ind($r_odeduct)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_payable"){ echo "<th style='text-align:right;'>".number_format_ind($r_frmr_pay)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "actual_chick_price"){
                                if($r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_achk_amt / $r_pchicks)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "actual_chick_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_achk_amt)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "actual_feed_price"){
                                if($r_afeed_amt > 0 && $r_feed_cons > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_afeed_amt / $r_feed_cons)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "actual_feed_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_afeed_amt)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "actual_medicine_price"){
                                if($r_amdvc_amt > 0 && $r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_amdvc_amt / $r_pchicks)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "actual_medicine_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_amdvc_amt)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "actual_admin_price"){
                                if($r_aadmn_amt > 0 && $r_pchicks > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_aadmn_amt / $r_pchicks)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "actual_admin_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_aadmn_amt)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "final_farmer_payable"){ echo "<th style='text-align:right;'>".number_format_ind($r_frmr_pay)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "actual_prod_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_tpamt)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mgmt_perkg_price"){
                                if($r_sweight > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind(($r_tpamt / $r_sweight))."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "total_sale_amount"){ echo "<th style='text-align:right;'>".number_format_ind($r_samount)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "profit_and_loss"){
                                $profit_loss = $r_samount - $r_tpamt;
                                if($profit_loss > 0){
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_actpay_paid"){ echo "<th style='text-align:right;'>".number_format_ind($r_fmrpaid_amt)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_actpay_diff"){ echo "<th style='text-align:right;'>".number_format_ind($r_fmrpaid_amt - $r_frmr_pay)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farm_ccode"){ echo "<th style='text-align:left;'></th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "paid_to_farmer"){ echo "<th style='text-align:right;'>".number_format_ind($r_fmrpaid_amt)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "pl_on_fmrpay"){ echo "<th style='text-align:right;'>".number_format_ind($r_tpl_amt)."</th>"; }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "lifting_efficiency"){ echo "<th style='text-align:right;'></th>"; }
                            ...
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "act_gc_amt"){ echo "<th style='text-align:right;'>".number_format_ind($r_actpc_amt)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "gc_sale_inc_prc"){
                                if($r_sweight > 0){
                                    echo "<th style='text-align:right;'>".number_format_ind($r_saleinc_amt / $r_sweight)."</th>";
                                }
                                else{
                                    echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                                }
                            }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "gc_sale_inc_amt"){ echo "<th style='text-align:right;'>".number_format_ind($r_saleinc_amt)."</th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "approved_gc_prc"){ echo "<th style='text-align:right;'></th>"; }
                            else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "approved_gc_amt"){ echo "<th style='text-align:right;'>".number_format_ind($r_agc_amt)."</th>"; }
                            else{ }
                        }
                        echo "</tr>";
                        $r_mage = $r_incr = $r_trmt = $r_sbirds = $r_pchicks = $r_mort = $r_days7_mcnt = $r_short_birds = $r_exces_birds = $r_feed_cons = $r_sweight = $r_dgain = 
                        $r_samount = $r_camount = $r_famount = $r_mamount = $r_aamount = $r_tfpay = $r_tapayable = $r_odeduct = $r_frmr_pay = $r_achk_amt = $r_afeed_amt = 
                        $r_amdvc_amt = $r_aadmn_amt = $r_tpamt = $r_fmrpaid_amt = $tot_fmrpayable_amt = $r_tpl_amt = $r_actpc_amt = $r_saleinc_amt = $r_agc_amt = 0;
                    }
                    if($osf_dt == ""){ $osf_dt = $row[$sort_order]; }

                    $r_mage += (float)$row['mean_age'];
                    $r_incr++;
                    $r_trmt += ((float)$row['mean_age'] * (float)$row['sold_birds']);
                    $r_sbirds += (float)$row['sold_birds'];
                    $r_pchicks += (float)$row['placed_birds'];
                    $r_mort += (float)$row['mortality'];
                    $r_days7_mcnt += (float)$days7_mortcs; $r_days30_mcnt += (float)$days30_mortcs; $r_days31_mcnt += (float)$days31_mortcs;
                    $r_short_birds += (float)$row['shortage'];
                    $r_exces_birds += (float)$row['excess'];
                    $r_feed_cons += (float)$row['feed_consume_kgs'];
                    $r_sweight += (float)$row['sold_weight'];
                    $r_dgain += (float)$row['day_gain'];
                    $r_samount += (float)$row['sale_amount'];
                    $r_camount += (float)$row['chick_cost_amt'];
                    $r_famount += (float)$row['feed_cost_amt'];
                    $r_mamount += (float)$row['medicine_cost_amt'];
                    $r_aamount += (float)$row['admin_cost_amt'];
                    $r_tfpay += (float)$row['total_cost_amt'];
                    $r_tapayable += (float)$row['total_amount_payable'];
                    $r_odeduct += (float)$row['other_deduction'];
                    $r_frmr_pay += (float)$row['farmer_payable'];
                    $r_achk_amt += (float)$row['actual_chick_cost'];
                    $r_afeed_amt += (float)$row['actual_feed_cost'];
                    $r_amdvc_amt += (float)$row['actual_medicine_cost'];
                    $r_aadmn_amt += round((float)$row['admin_cost_amt'],2);
                    $r_tpamt += (float)$total_prod;
                    $tot_sale_amount += (float)$row['sale_amount'];
                    $r_fmrpaid_amt += (float)$farmer_actpay_paid[$row['batch_code']];
                    $tot_fmrpayable_amt += (float)$row['farmer_payable'];
                    $r_tpl_amt += (float)$pl_on_fpay;
                    $r_actpc_amt += (float)$row['actual_charge_exp_amt'];
                    $r_saleinc_amt += (float)$row['sales_incentive_amt'];
                    $r_agc_amt += (float)$row['grow_charge_exp_amt'];
                    ...
                    for($i = 1;$i <= $col_count;$i++){
                        ...
                        if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "sl_no"){ echo "<td title='Sl.No'>".$slno."</td>"; } 
                            ...
                            if(!empty($bstd_cum_feed[round($row['age'])])){
                                if(($bstd_cum_feed[round($row['age'])] / 1000) > ($row['feed_consume_kgs'] / $row['placed_birds'])){
                        ...
                        else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_incentive"){ echo "<td title='Incentives' style='text-align:right;'>".number_format_ind($incentive)."</td>"; }
                        else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "farmer_decentives"){ echo "<td title='Decentives' style='text-align:right;'>".number_format_ind($decentive)."</td>"; }
                        ...
                        else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "rearing_charges"){ echo "<td title='Rearing Charges' style='text-align:right;'>".number_format_ind($row['grow_charge_exp_amt'])."</td>"; }
            ...
            <tfoot>
                ...
                for($i = 1;$i <= $col_count;$i++){
                    ...
                    if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "sl_no"){ echo "<th style='text-align:left; border-right: 0px;'></th>"; }
                    ...
                    else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "liquidation_date"){ echo "<th style='text-align:center; border-left: 0px;'>Grand Total</th>"; }
                    else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "brood_age"){ echo "<th style='text-align:right;'></th>"; }
                    else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mean_age"){
                        //if($tot_sold_birds > 0){ echo "<th style='text-align:right;'>".number_format_ind(round(($mtsb / $tot_sold_birds),2))."</th>"; }
                        //else{ echo "<th style='text-align:right;'>".number_format_ind(0)."</th>"; }
                        if($_SESSION['dbase'] != "poulso6_broiler_hr_easyfoods"){
                            if($incr > 0){ $mean_age = 0; $mean_age = $total_meanage / $incr; } else{ $mean_age = 0; }
                            echo "<th style='text-align:right;'>".number_format_ind($mean_age)."</th>";
                        }
                        else{
                            if((float)$tot_sold_birds != 0){ $t2 = round(((float)$tot_rmt / (float)$tot_sold_birds),2); } else{ $t2 = 0; }
                            echo "<th style='text-align:right;'>".number_format_ind($t2)."</th>";
                        }
                    }
                        ...
                        if($tot_sold_weight > 0){
                            echo "<th style='text-align:right;'>".number_format_ind($tot_fpay / $tot_sold_weight)."</th>";
                        }
                        else{
                            echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                        }
                        ...
                        if($tot_sold_weight > 0){
                            echo "<th style='text-align:right;'>".number_format_ind($tot_saleinc_amt / $tot_sold_weight)."</th>";
                        }
                        else{
                            echo "<th style='text-align:right;'>".number_format_ind(0)."</th>";
                        }
        ...
        <script>
            function fetch_farms_details(a){
                ...
                var bf_flag = lf_flag = sf_flag = ff_flag = 0;
                if(a.match("branches")){ bf_flag = 1; } else if(a.match("lines")){ lf_flag = 1; } else if(a.match("supervisors")){ sf_flag = 1; } else{ ff_flag = 1; }
                    
                var fetch_fltrs = new XMLHttpRequest();
                var method = "GET";
                var url = "broiler_fetch_farm_filter_master.php?branches="+branches+"&lines="+lines+"&supervisors="+supervisors+"&bf_flag="+bf_flag+"&lf_flag="+lf_flag+"&sf_flag="+sf_flag+"&ff_flag="+ff_flag+"&user_code="+user_code;
                //window.open(url);
