<?php
//broiler_feed_production_ta.php
...
while($row = mysqli_fetch_assoc($query)){ $item_cats[$row['code']] = $row['code']; $item_cats_name[$row['code']] = $row['description']; }

$icat_list = implode("','",$item_cats);
$sql = "SELECT * FROM `item_details` WHERE `dflag` = '0' AND `category` IN ('$icat_list') ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql); $item_name = array();
...
$fdate = $tdate = date("Y-m-d"); $sectors =  $Category = "all"; $excel_type = "display";
if (isset($_POST['submit_report'])) {
    ...
    $feedtypes = $_POST['feedtype'] ?? [];

    // Filters
    $mill_filter = ($sectors != "all") ? "`feed_mill` IN ('$sectors')" : "";

    $category_feed_codes = [];
    if ($Category != "all") {
        $cat_sql = "SELECT `code` FROM `item_details` WHERE `dflag` = '0' AND `category` = '$Category'";
        $cat_query = mysqli_query($conn, $cat_sql);
        while ($row = mysqli_fetch_assoc($cat_query)) {
            $category_feed_codes[] = $row['code'];
        }
    }

    $feed_type_codes = array_filter($feedtypes, fn($v) => $v !== "all");

    // Feed code logic
    $feed_code_filter = "";
    if ($category_feed_codes && $feed_type_codes) {
        $common = array_intersect($category_feed_codes, $feed_type_codes);
        $feed_code_filter = $common ? "`feed_code` IN ('" . implode("','", $common) . "')" : "1=0";
    } elseif ($category_feed_codes) {
        $feed_code_filter = "`feed_code` IN ('" . implode("','", $category_feed_codes) . "')";
    } elseif ($feed_type_codes) {
        $feed_code_filter = "`feed_code` IN ('" . implode("','", $feed_type_codes) . "')";
    }
    ...
    $where_clause = implode(" AND ", array_filter($conditions));

    // Final SQL
    // $sql_record = "SELECT * FROM `broiler_feed_production` WHERE $where_clause ORDER BY `date` ASC";
    ...
    $export_sectors = $sector_name[$_POST['sectors']] ?? "All";

    $efeed_type_list = "All";
    if ($feed_type_codes) {
        $efeed_type_list = implode(", ", array_map(fn($c) => $item_name[$c], $feed_type_codes));
    }

    $filename = "Feed Production Report_" . ($export_fdate == $export_tdate ? $export_tdate : $export_fdate . "_to_" . $export_tdate);
...
<html>
    ...
    <body align="center">
        <table class="tbl" align="center">
            ...
            <?php } ?>
            <form action="broiler_feed_production_ta.php" method="post">
                            ...
                            <div class="row">
                                ...
                                <div class="m-2 form-group">
                                    ...
                                    <select name="Category" id="Category" class="form-control select2">
                                        ...
                                        <?php foreach($item_cats as $prod_code){ ?><option value="<?php echo $prod_code; ?>" <?php if($Category == $prod_code){ echo "selected"; } ?>><?php echo $item_cats_name[$prod_code]; ?></option><?php } ?>
                                    </select>
                                </div>
                                <div class="m-2 form-group">
           ...
           <table id="main_body" class="tbl" align="center"  style="width:1300px;">
            ...
            <tbody class="tbody1">
                <?php
                $sql_record = "SELECT * FROM `broiler_feed_production` WHERE $where_clause ORDER BY `date` ASC";
                // $sql_record = "SELECT *  FROM `broiler_feed_production` ".$where_clause." ORDER BY `date` ASC";
