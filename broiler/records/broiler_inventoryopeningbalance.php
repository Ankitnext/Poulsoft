<?php
//broiler_inventoryopeningbalance.php
...
global $page_title; $page_title = "Inventory Opening Balance Report";
...
if(isset($_POST['submit_report']) == true){
    ...
    if($items != "all"){ $item_filter = " AND `type_code` IN ('$items')"; }
        ...
        $item_filter = " AND `type_code` IN ('$icat_list')";
    }
    ...
    $export_items = $item_name[$_POST['items']]; if ($export_items == "") { $export_items = "All"; } 
     ...
     {$filename = "Inventory Opening Balance Report_".$export_tdate; }
      else {
     $filename = "Inventory Opening Balance Report_".$export_fdate."_to_".$export_tdate; }
        ...
        <table class="tbl" align="center">
            ...
            <thead class="thead1" align="center">
                <tr align="center">
                    ...
                    <th colspan="6" align="center" style="border-right:none;"><?php echo $row['cdetails']; ?><h5>Opening Balance Report</h5></th>
                </tr>
            </thead>
            <?php } ?>
            <form action="broiler_inventoryopeningbalance.php" method="post">
                    ...
                    <tr>
                        <th colspan="8">
                            <div class="row">
                                ...
                                <div class="m-2 form-group">
                                    <label>Export</label>
                                    <select name="export" id="export" class="form-control select2"  onchange="tableToExcel('main_body', 'Inventory Opening Balance','<?php echo $filename;?>', this.options[this.selectedIndex].value)">
           ...
           <table id="main_body" class="tbl" align="center"  style="width:1300px;">
            ...
            ?>
                <tr align="center">
                    ...
                    <th colspan="8" align="center" style="border-right:none;"><?php echo $row['cdetails']; ?><h5>Opening Balance Report</h5></th>
            ...
            <tbody class="tbody1">
                ...
                $sql = "SELECT * FROM `broiler_openings` WHERE `date` >= '$fdate' AND `date` <= '$tdate'".$item_filter." AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC"; $query = mysqli_query($conn,$sql);
                while($row = mysqli_fetch_assoc($query)){
                ?>
                <tr>
                ...
                $avg_prc = 0;
                if((float)$total_qty != 0){ $avg_prc = (float)$total_amt / (float)$total_qty; }
                ?>
                <tr class="thead4">
