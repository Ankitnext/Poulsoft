<?php
//broiler_growingcharge_masterreport1.php
...
if($db == ''){
    ...
    $reloadpath = "broiler_growingcharge_masterreport1_ty.php";
}
else{
    ...
    $dbname = $db;
    $reloadpath = "broiler_growingcharge_masterreport1.php?db=".$db."&userid=".$user_code;
    ...
    <body align="center">
        <table class="tbl2" align="center">
            ...
            <?php
                ...
                $sql = "SELECT * FROM `feed_bagcapacity` WHERE `active` = '1' AND `dflag` = '0'";
        ...
        <tbody class="tbody1">
            ...
            <tbody class="tbody1">
                <?php
                $feed_list = implode("','",$feed_code); $feed_bqty = $feedb_bqty = $tot_bags = 0;
                $feed_cum = $tpfin_qty = $tpfin_amt = 0; $feed_purin_qty = $feed_dwpurin_qty = $feedb_dwpurin_qty = array();
                ...
                while($row = mysqli_fetch_assoc($query)){
                    ...
                    if($cpdate > strtotime($row['date'])){
                        $feed_bqty += ((float)$row['rcd_qty'] + (float)$row['fre_qty']);

                        if(!empty($bag_size[$row['icode']]) && (float)$bag_size[$row['icode']] > 0){
                            $feedb_bqty += (float)(round((((float)$row['rcd_qty'] + (float)$row['fre_qty']) / (float)$bag_size[$row['icode']]),2));
                        }
                        else{
                    ...
                    ?>
                        ...
                        <?php
                            if(!empty($bag_size[$row['icode']]) && (float)$bag_size[$row['icode']] > 0){
                                echo number_format_ind(round((($row['rcd_qty'] + $row['fre_qty']) / (float)$bag_size[$row['icode']]),2));
                                $tot_bags += (float)(round((((float)$row['rcd_qty'] + (float)$row['fre_qty']) / (float)$bag_size[$row['icode']]),2));
                            }
                            else{
                                echo number_format_ind(round(($row['rcd_qty'] + $row['fre_qty']) / 50));
                                $tot_bags += (float)(round(($row['rcd_qty'] + $row['fre_qty']) / 50));
                            }
                        ?>
                        </td>
                    ...
                    if(!empty($bag_size[$row['icode']]) && (float)$bag_size[$row['icode']] > 0){
                        $feedb_dwpurin_qty[$row['date']] += (float)(round((((float)$row['rcd_qty'] + (float)$row['fre_qty']) / (float)$bag_size[$row['icode']]),2));
                    }
                    else{
                        $feedb_dwpurin_qty[$row['date']] += (float)(round(($row['rcd_qty'] + $row['fre_qty']) / 50));
                    }
                }
                ?>
            </tbody>
            <tr class="thead4">
                ...
                <th style="text-align:right;" colspan="2"><?php echo number_format_ind(round(($tot_bags))); ?></th>
            ...
            <tbody class="tbody1">
                <?php
                $feed_cum = $tsfin_qty = $tsfin_amt = $tot_bags = 0; $feed_trsin_qty = array();
                ...
                while($row = mysqli_fetch_assoc($query)){
                    ...
                    if($cpdate > strtotime($row['date'])){
                        $feed_bqty += ((float)$row['quantity']);

                        if(!empty($bag_size[$row['code']]) && (float)$bag_size[$row['code']] > 0){
                            $feedb_bqty += (float)(round(((float)$row['quantity'] / (float)$bag_size[$row['code']]),2));
                        }
                        else{
                    ...
                    ?>
                        ...
                        <?php
                            if(!empty($bag_size[$row['code']]) && (float)$bag_size[$row['code']] > 0){
                                echo number_format_ind(round(((float)$row['quantity'] / (float)$bag_size[$row['code']]),2));
                                $tot_bags += (float)(round(((float)$row['quantity'] / (float)$bag_size[$row['code']]),2));
                            }
                            else{
                                echo number_format_ind(round(((float)$row['quantity']) / 50));
                                $tot_bags += (float)(round(((float)$row['quantity']) / 50));
                            }
                        ?>
                        </td>
                    ...
                    if(!empty($bag_size[$row['code']]) && (float)$bag_size[$row['code']] > 0){
                        $feedb_dwpurin_qty[$row['date']] += (float)(round(((float)$row['quantity'] / (float)$bag_size[$row['code']]),2));
                    }
                    else{
                        $feedb_dwpurin_qty[$row['date']] += (float)(round(($row['quantity']) / 50));
                    }
                }
                ?>
            </tbody>
            ...
            <tbody class="tbody1">
                <?php
                $feed_cum = $tsfout_qty = $tsfout_amt = $tot_bags = 0; $feed_trsout_qty = $feed_trfout_qty = array();
                ...
                ?>
                  ...
                  if($count54 > 0){
                    ...
                    ?>
                            ...
                            <td style="text-align:right;" colspan="2">
                            <?php
                            if(!empty($bag_size[$row['itemcode']]) && (float)$bag_size[$row['itemcode']] > 0){
                                echo number_format_ind(round(((float)$row['quantity'] / (float)$bag_size[$row['itemcode']]),2));
                                $tot_bags += (float)(round(((float)$row['quantity'] / (float)$bag_size[$row['itemcode']]),2));
                            }
                            else{
                                echo number_format_ind(round(((float)$row['quantity']) / 50));
                                $tot_bags += (float)(round(((float)$row['quantity']) / 50));
                        ...
                        if(!empty($bag_size[$row['itemcode']]) && (float)$bag_size[$row['itemcode']] > 0){
                            $feedb_dwpurin_qty[$row['date']] -= (float)(round(((float)$row['quantity'] / (float)$bag_size[$row['itemcode']]),2));
                        }
                        else{
                            $feedb_dwpurin_qty[$row['date']] -= (float)(round(($row['quantity']) / 50));
                        }
            ...
            <tbody class="tbody1">
                <?php
                $sql = "SELECT * FROM `broiler_daily_record` WHERE `farm_code` = '$farms' AND `batch_code` = '$batches' AND `active` = '1' AND `dflag` = '0' ORDER BY `date` ASC";
                $query = mysqli_query($conn, $sql); $tsin_bqty = $tfin_bqty = $tfcn_bqty = $tfso_bqty = $tffo_bqty = $tfbl_bqty = 0;
                ...
                <tr align="center">
                    ...
                    <td colspan="2" style="text-align:right;"><?php echo number_format_ind(round($feed_purin_qty[$ficode],2)); ?></td><td colspan="1" style="text-align:right;"><?php if(!empty($bag_size[$ficode]) && (float)$bag_size[$ficode] > 0){ echo number_format_ind(round(((float)$feed_purin_qty[$ficode] / (float)$bag_size[$ficode]),2)); $tsin_bqty += (float)(round(((float)$feed_purin_qty[$ficode] / (float)$bag_size[$ficode]),2)); } else{ echo number_format_ind(round(($feed_purin_qty[$ficode] / 50),2)); $tsin_bqty += (float)(round(($feed_purin_qty[$ficode] / 50),2)); } ?></td>
                    <td colspan="2" style="text-align:right;"><?php echo number_format_ind(round($feed_trsin_qty[$ficode],2)); ?></td><td colspan="1" style="text-align:right;"><?php if(!empty($bag_size[$ficode]) && (float)$bag_size[$ficode] > 0){ echo number_format_ind(round(((float)$feed_trsin_qty[$ficode] / (float)$bag_size[$ficode]),2)); $tfin_bqty += (float)(round(((float)$feed_trsin_qty[$ficode] / (float)$bag_size[$ficode]),2)); } else{ echo number_format_ind(round(($feed_trsin_qty[$ficode] / 50),2)); $tfin_bqty += (float)(round(($feed_trsin_qty[$ficode] / 50),2)); } ?></td>
                    <td colspan="2" style="text-align:right;"><?php echo number_format_ind(round($feed_tcons_qty[$ficode],2)); ?></td><td colspan="1" style="text-align:right;"><?php if(!empty($bag_size[$ficode]) && (float)$bag_size[$ficode] > 0){ echo number_format_ind(round(((float)$feed_tcons_qty[$ficode] / (float)$bag_size[$ficode]),2)); $tfcn_bqty += (float)(round(((float)$feed_tcons_qty[$ficode] / (float)$bag_size[$ficode]),2)); } else{ echo number_format_ind(round(($feed_tcons_qty[$ficode] / 50),2)); $tfcn_bqty += (float)(round(($feed_tcons_qty[$ficode] / 50),2)); } ?></td>
                    <td colspan="2" style="text-align:right;"><?php echo number_format_ind(round($feed_trsout_qty[$ficode],2)); ?></td><td colspan="1" style="text-align:right;"><?php if(!empty($bag_size[$ficode]) && (float)$bag_size[$ficode] > 0){ echo number_format_ind(round(((float)$feed_trsout_qty[$ficode] / (float)$bag_size[$ficode]),2)); $tfso_bqty += (float)(round(((float)$feed_trsout_qty[$ficode] / (float)$bag_size[$ficode]),2)); } else{ echo number_format_ind(round(($feed_trsout_qty[$ficode] / 50),2)); $tfso_bqty += (float)(round(($feed_trsout_qty[$ficode] / 50),2)); } ?></td>
                    <td colspan="2" style="text-align:right;"><?php echo number_format_ind(round($feed_trfout_qty[$ficode],2)); ?></td><td colspan="1" style="text-align:right;"><?php if(!empty($bag_size[$ficode]) && (float)$bag_size[$ficode] > 0){ echo number_format_ind(round(((float)$feed_trfout_qty[$ficode] / (float)$bag_size[$ficode]),2)); $tffo_bqty += (float)(round(((float)$feed_trfout_qty[$ficode] / (float)$bag_size[$ficode]),2)); } else{ echo number_format_ind(round(($feed_trfout_qty[$ficode] / 50),2)); $tffo_bqty += (float)(round(($feed_trfout_qty[$ficode] / 50),2)); } ?></td>
                    <td colspan="4" style="text-align:right;"><?php echo number_format_ind(round($feed_bal_qty,2)); ?></td><td colspan="2" style="text-align:right;"><?php if(!empty($bag_size[$ficode]) && (float)$bag_size[$ficode] > 0){ echo number_format_ind(round(((float)$feed_bal_qty / (float)$bag_size[$ficode]),2)); $tfbl_bqty += (float)(round(((float)$feed_bal_qty / (float)$bag_size[$ficode]),2)); } else{ echo number_format_ind(round(($feed_bal_qty / 50),2)); $tfbl_bqty += (float)(round(($feed_bal_qty / 50),2)); } ?></td>
                </tr>
                <?php
                ...
                <tr class="thead4" align="center">
                    ...
                    <th colspan="2" style="text-align:right;"><?php echo number_format_ind(round($tsin_qty,2)); ?></th><th colspan="1" style="text-align:right;"><?php echo number_format_ind(round(($tsin_bqty),2)); ?></th>
                    <th colspan="2" style="text-align:right;"><?php echo number_format_ind(round($tfin_qty,2)); ?></th><th colspan="1" style="text-align:right;"><?php echo number_format_ind(round(($tfin_bqty),2)); ?></th>
                    <th colspan="2" style="text-align:right;"><?php echo number_format_ind(round($tfcn_qty,2)); ?></th><th colspan="1" style="text-align:right;"><?php echo number_format_ind(round(($tfcn_bqty),2)); ?></th>
                    <th colspan="2" style="text-align:right;"><?php echo number_format_ind(round($tfso_qty,2)); ?></th><th colspan="1" style="text-align:right;"><?php echo number_format_ind(round(($tfso_bqty),2)); ?></th>
                    <th colspan="2" style="text-align:right;"><?php echo number_format_ind(round($tffo_qty,2)); ?></th><th colspan="1" style="text-align:right;"><?php echo number_format_ind(round(($tffo_bqty),2)); ?></th>
                    <th colspan="4" style="text-align:right;"><?php echo number_format_ind(round($tfbl_qty,2)); ?></th><th colspan="2" style="text-align:right;"><?php echo number_format_ind(round(($tfbl_bqty),2)); ?></th>
                </tr>
            </tbody>
            <thead class="report_head" align="center">
                <tr class="report_head">
            ...
            <?php
                ...
                $sale_cum = $avgwt = $sale_wt = $cum_feed = $week_culls = $week_iqty1 = $weekb_iqty1 = $week_mort = $week_sbirds = $week_iqty2 = $weekb_iqty2 = 0; $dstart_date = $mstart_date = "";
                $i = $j = $days7_morts = $days30_mort = $daysge30_mort = $tot_bags1 = $tot_bags2 = 0;
                ...
                while($row = mysqli_fetch_assoc($query)){
                    ...
                    if(!empty($feedb_dwpurin_qty[$row['date']])){ $feedb_bqty += (float)$feedb_dwpurin_qty[$row['date']]; } else{ }
                    //echo "<br/>if(!empty(".$feedb_dwpurin_qty[$row['date']].")){ $feedb_bqty += (float)".$feedb_dwpurin_qty[$row['date']]."; } else{ }";
                    
                    //$title1 = "((float)$feed_bqty - ((float)$iqty1 + (float)$iqty2))";
                    $feed_bqty = ((float)$feed_bqty - ((float)$iqty1 + (float)$iqty2));
                    //$title = "$feed_bqty = ".$title1;
                ...
                <tr align="center">
                    ...
                    <?php
                        $title1 = $title = "";
                        if(!empty($bag_size[$icode1]) && (float)$bag_size[$icode1] > 0){
                            echo number_format_ind(round(((float)$iqty1 / (float)$bag_size[$icode1]),2));
                            $tot_bags1 += (float)(round(((float)$iqty1 / (float)$bag_size[$icode1]),2));
                            $weekb_iqty1 += (float)(round(((float)$iqty1 / (float)$bag_size[$icode1]),2));

                            $title1 = "(float)$feedb_bqty - (float)(round(((float)$iqty1 / (float)$bag_size[$icode1])))";
                            $feedb_bqty -= (float)(round(((float)$iqty1 / (float)$bag_size[$icode1]),2));
                            $title = "$feedb_bqty = ".$title1;
                        }
                        else{
                            echo number_format_ind(round(((float)$iqty1 / 50),2));
                            $tot_bags1 += (float)(round(((float)$iqty1 / 50),2));
                            $weekb_iqty1 += (float)(round(((float)$iqty1 / 50),2));

                            $title1 = "(float)$feedb_bqty - (float)(round(((float)$iqty1 / 50),2))";
                            $feedb_bqty -= (float)(round(((float)$iqty1 / 50),2));
                            $title = "$feedb_bqty = ".$title1;
                        }
                    ?>
                    </td>
                    ...
                    <td style="text-align:right;">
                    <?php
                        if(!empty($bag_size[$icode2]) && (float)$bag_size[$icode2] > 0){
                            echo number_format_ind(round(((float)$iqty2 / (float)$bag_size[$icode2]),2));
                            $tot_bags2 += (float)(round(((float)$iqty2 / (float)$bag_size[$icode2]),2));
                            $weekb_iqty2 += (float)(round(((float)$iqty2 / (float)$bag_size[$icode2]),2));

                            $title1 = "(float)$feedb_bqty - (float)(round(((float)$iqty2 / (float)$bag_size[$icode1])))";
                            $feedb_bqty -= (float)(round(((float)$iqty2 / (float)$bag_size[$icode2]),2));
                            $title .= "\n$feedb_bqty = ".$title1;
                        }
                        else{
                            echo number_format_ind(round(((float)$iqty2 / 50),2));
                            $tot_bags2 += (float)(round(((float)$iqty2 / 50),2));
                            $weekb_iqty2 += (float)(round(((float)$iqty2 / 50),2));

                            $title1 = "(float)$feedb_bqty - (float)(round(((float)$iqty2 / 50),2))";
                            $feedb_bqty -= (float)(round(((float)$iqty2 / 50),2));
                            $title .= "\n$feedb_bqty = ".$title1;
                        }
                    ?>
                    </td>
                ...
                if($i == 7){
                    ...
                    <tr align="center">
                        ...
                        <td style="color:red;font-weight:bold;text-align:right;"><?php echo number_format_ind(round(($weekb_iqty1),2)); ?></td>
                        <td style="color:red;font-weight:bold;text-align:left;"></td>
                        <td style="color:red;font-weight:bold;text-align:right;"><?php echo number_format_ind(round($week_iqty2,2)); ?></td>
                        <td style="color:red;font-weight:bold;text-align:right;"><?php echo number_format_ind(round(($weekb_iqty2),2)); ?></td>
                        ...
                        <td style="color:red;font-weight:bold;text-align:right;" title="<?php echo $title; ?>"><?php echo number_format_ind(round(($feedb_bqty),2)); ?></td>
                        <td style="color:red;font-weight:bold;text-align:left;"></td>
                    </tr>
                    <?php
                    $week_mort = $week_culls = $week_iqty1 = $week_iqty2 = $weekb_iqty1 = $weekb_iqty2 = $week_sbirds = 0;
                }
                    $opn_qty = $cbirds;
                    $tculls += $culls;
                    $tkgs1 += $iqty1;
            ...
            <tr class="thead4" align="center">
                ...
                <th style="text-align:right;"><?php echo number_format_ind(round(($tot_bags1),2)); ?></th>
                <th style="text-align:left;"></th>
                <th style="text-align:right;"><?php echo number_format_ind(round($tkgs2,2)); ?></th>
                <th style="text-align:right;"><?php echo number_format_ind(round(($tot_bags2),2)); ?></th>
                ...
                <th style="text-align:right;"><?php echo number_format_ind(round(($feedb_bqty),2)); ?></th>
                <th style="text-align:left;"></th>
            </tr>
            <?php
            ...
            <tbody class="tbody1">
            <?php
                $trmv_qty = $trmv_prc = $trmv_amt = 0;
                $medvac_list = implode("','",$medvac_code);
                $sql = "SELECT * FROM `item_stocktransfers` WHERE `code` IN ('$medvac_list') AND `fromwarehouse` = '$farms' AND `from_batch` = '$batches' AND `active` = '1' AND `dflag` = '0' ORDER BY `date` ASC";
                $query = mysqli_query($conn, $sql);
                while($row = mysqli_fetch_assoc($query)){
                    $price = 0;
                    if($fetch_type == "farmer"){
            ...
            <thead class="report_head" align="center">
                <tr class="report_head">
                    <td colspan="23" style="height: 25px;color:blue;font-weight:bold;font-size:13px;">Bird Transfer</td>
                </tr>
                <tr align="center">
                    <th colspan="4">Date</th>
                    <th colspan="4">Transaction No.</th>
                    <th colspan="6">To Warehouse</th>
            ...
            <?php  
                    ...
                    $sql = "SELECT * FROM `item_stocktransfers` WHERE `active` = '1' AND `fromwarehouse` = '$farms' AND `from_batch` = '$batches'"; $query = mysqli_query($conn,$sql);
                    while($row = mysqli_fetch_assoc($query)){
                        ?>
                             <tr>
                        <td style="text-align:left;" colspan="4"><?php echo date("d.m.Y",strtotime($row['date'])); ?></td>
                        <td style="text-align:left;" colspan="4"><?php echo $row['trnum']; ?></td>
                        <td style="text-align:left;" colspan="6"><?php echo $sector_name[$row['towarehouse']]; ?></td>
                        <td style="text-align:right;" colspan="4"><?php echo $row['quantity']; ?></td>
                        <td style="text-align:right;" colspan="5"><?php echo $row['price']; ?></td>
            ...
            <?php if($batchdetails_flag == 1){ ?>
                <thead class="report_head" align="center">
                    ...
                    if($rcount > 0){
                        while($row = mysqli_fetch_assoc($query)){
                            if($fetch_type == "farmer"){
                                ...
                                $smr_total_cost_amt = $row['total_cost_amt'];
                        ...
                        $smr_total_cost_amt = $trc_amt + ((float)$tpfin_amt + (float)$tsfin_amt - (float)$tsfout_amt) + (float)$tcmv_amt + (float)$smr_admin_cost_amt;
                    ...
                    <tr align="left">
                        ...
                        <th colspan="4">Totral Production Cost</th><td colspan="4" class="thead4" <?php if($smr_total_cost_amt > $smr_standard_prod_cost){ echo 'style="text-align:right;color:red;font-weight:bold;"'; } else{ echo 'style="text-align:right;color:green;font-weight:bold;"'; } ?>><?php echo number_format_ind(round($smr_total_cost_amt,2)); ?></td>
                    </tr>
                    <tr align="left">
                        ...
                        <th colspan="4">Production Cost Per Kg</th><td colspan="4" class="thead4" <?php if($smr_total_cost_amt > $smr_standard_prod_cost){ echo 'style="text-align:right;color:red;font-weight:bold;"'; } else{ echo 'style="text-align:right;color:green;font-weight:bold;"'; } ?>><?php echo number_format_ind(round($smr_total_cost_unit,2)); ?></td>
                    </tr>
                    <?php
                    //}
                    /*?>
