<?php
//broiler_pending_farmergclist.php
...
include $num_format_file;
    global $page_title; $page_title = "Pending GC Closure List";
...
if(isset($_POST['submit_report']) == true){
   $filename = "Pending GC Closure List"; 
...
<html>
    ...
    <body>
        <table class="tbl" align="center">
            ...
            <thead class="thead1" align="center" style="width:auto;">
                <tr align="center">
                    ...
                    <th colspan="11" align="center" style="border-right:none;"><?php echo $row['cdetails']; ?><h5>Pending GC Closure List Report</h5></th>
                </tr>
            </thead>
            <?php } ?>
            <?php if($db == ''){?>
            <form action="broiler_pending_farmergclist.php" method="post"  onsubmit="return checkval()">
                 <?php } else { ?>
                <form action="broiler_pending_farmergclist.php?db=<?php echo $db; ?>" method="post" onsubmit="return checkval()">
                <?php } ?>
                        ...
                        <th colspan="13">
                            <div class="row">&ensp;&ensp;
                                ...
                                <div class="m-2 form-group">
                                    <label>Export</label>
                                    <select name="export" id="export" class="form-control select2"  onchange="tableToExcel('main_body', 'Pending GC Closure List','<?php echo $filename;?>', this.options[this.selectedIndex].value)">
        ...
        <div class="row" style="padding-left:100px;">
            ...
            ?>
                <tr align="center" >
                    <th colspan="13" align="center" style="border-right:none;"><?php echo $row['cdetails']; ?><h5>Pending GC Closure List Report</h5></th>
                </tr>
            <?php } ?>
            <tr>
                ...
                <thead class="thead3" align="center">
                    <tr align="center" id="header_sorting">
                        ...
                        <th id="order_date">Last Daily Entry</th>
                        <th id="order_date">Last Sale Date</th>
                        <th id="order_date">Latest Feed Return Date</th>
                        <th id="order_num">Feed Balance</th>
                        <th id="order_num">Remaining Birds</th>
                ...
                <tr align="center" style="width:1212px; ">
                        ...
                        <th>Batch</th>
                        <th>Age</th>
                        <th>Last Daily Entry</th>
                        <th>Last Sale Date</th>
                        <th>Latest Feed Return Date</th>
                        <th>Feed Balance</th>
                ...
                <?php
                if(isset($_POST['submit_report']) == true){
                    $icat_code = $feed_code = $batch_code = $batch_name = $chick_in_qty = $chick_out_qty = $chick_placed_date = $sale_max_date = $feed_in_qty = $feed_cons_qty = $feed_out_qty = $dentry_max_date = $chick_mort_qty = $chick_age = $avg_wt = array();
                    ...
                    //Daily Entry
                    $sql = "SELECT SUM(kgs1) as kgs1,SUM(kgs2) as kgs2,SUM(mortality) as mort,SUM(culls) as culls,MAX(brood_age) as age,MAX(date) as date,batch_code FROM `broiler_daily_record` WHERE `batch_code` IN ('$batch_list') AND `active` = '1' AND `dflag` = '0' AND gc_flag = '0' GROUP BY `batch_code` ORDER BY `batch_code` ASC";
                    $query = mysqli_query($conn,$sql);
                    while($row = mysqli_fetch_assoc($query)){
                    ...
                    //Feed Purchase
                    $sql = "SELECT SUM(rcd_qty) as rcd_qty,SUM(fre_qty) as fre_qty,MIN(date) as sdate,icode,farm_batch FROM `broiler_purchases` WHERE `icode` IN ('$feed_list') AND `farm_batch` IN ('$batch_list') AND `active` = '1' AND `dflag` = '0' GROUP BY `farm_batch` ORDER BY `farm_batch` ASC";
                    $query = mysqli_query($conn,$sql);
                    while($row = mysqli_fetch_assoc($query)){
                        $key_code = $row['farm_batch'];
                    ...
                    //Feed Transfer IN
                    $sql_record = "SELECT SUM(quantity) as quantity,MIN(date) as sdate,code,to_batch FROM `item_stocktransfers` WHERE `code` IN ('$feed_list') AND `to_batch` IN ('$batch_list') AND `active` = '1' AND `dflag` = '0' GROUP BY `to_batch` ORDER BY `to_batch` ASC";
                    $query = mysqli_query($conn,$sql_record);
                    while($row = mysqli_fetch_assoc($query)){
                        $key_code = $row['to_batch'];
                    ...
                    //Chick Sale
                    $sql_record = "SELECT SUM(birds) as birds,SUM(rcd_qty) as rcd_qty,SUM(fre_qty) as fre_qty,MAX(date) as edate,icode,farm_batch FROM `broiler_sales` WHERE `icode` = '$bird_code' AND `farm_batch` IN ('$batch_list') AND `active` = '1' AND `dflag` = '0' GROUP BY `farm_batch` ORDER BY `farm_batch` ASC";
                    $query = mysqli_query($conn,$sql_record);
                    while($row = mysqli_fetch_assoc($query)){
                        $key_code = $row['farm_batch'];
                    ...
                    //Feed Sale
                    $sql_record = "SELECT SUM(birds) as birds,SUM(rcd_qty) as rcd_qty,SUM(fre_qty) as fre_qty,MAX(date) as edate,icode,farm_batch FROM `broiler_sales` WHERE `icode` IN ('$feed_list') AND `farm_batch` IN ('$batch_list') AND `active` = '1' AND `dflag` = '0' GROUP BY `farm_batch` ORDER BY `farm_batch` ASC";
                    $query = mysqli_query($conn,$sql_record);
                    while($row = mysqli_fetch_assoc($query)){
                        $key_code = $row['farm_batch'];
                        $feed_out_qty[$key_code] += ((float)$row['birds']);
                    }
                    //Feed Transfer Out
                    $sql_record = "SELECT SUM(quantity) as quantity,MAX(date) as edate,code,from_batch FROM `item_stocktransfers` WHERE `code` IN ('$feed_list') AND `from_batch` IN ('$batch_list') AND `active` = '1' AND `dflag` = '0' GROUP BY `to_batch` ORDER BY `to_batch` ASC";
                    $query = mysqli_query($conn,$sql_record);
                    while($row = mysqli_fetch_assoc($query)){
                        $key_code = $row['from_batch'];
                ...
                foreach($f1 as $fcode){
                    ...
                    ?>
                    <tr>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?> title="Sl. No."><?php echo $slno; ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?> title="Branch Name"><?php echo $branch_name[$farm_branch[$fcode]]; ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?> title="Line"><?php echo $line_name[$farm_line[$fcode]]; ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?> title="Supervisor"><?php echo $supervisor_name[$farm_supervisor[$fcode]]; ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?> title="Farm"><?php echo $farm_name[$fcode]; ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?> title="<?php echo $bcode; ?>"><?php echo $batch_name[$bcode]; ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:center;"'; } else{ echo 'style="text-align:center;"'; } ?> title="Age"><?php echo $age; ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?> title="Last Daily Entry Date"><?php if($last_dentry_date == "01.01.1970"){ echo ""; } else{ echo $last_dentry_date; } ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?> title="Last Sale Date"><?php if($last_sale_date == "01.01.1970"){ echo ""; } else{ echo $last_sale_date; } ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?> title="Last Feed Return Date"><?php if($last_feedout_date == "01.01.1970"){ echo ""; } else{ echo $last_feedout_date; } ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:right;"'; } else{ echo 'style="text-align:right;"'; } ?> title="<?php echo $title2; ?>"><?php echo number_format_ind($display_available_feed); ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:right;"'; } else{ echo 'style="text-align:right;"'; } ?> title="<?php echo $title1; ?>"><?php echo str_replace(".00","",number_format_ind($display_available_birds)); ?></td>
                        <td <?php if((int)$days > 30){ echo 'style="color:red;text-align:right;"'; } else{ echo 'style="text-align:right;"'; } ?> title="Gap Days"><?php echo $days; ?></td>
                    </tr>
                    <?php
                ...
                <thead class="thead2">
                    <tr>
                        ...
                        <td style="text-align:right;"><b><?php echo number_format_ind($tot_feeds); ?></b></td>
                        <td style="text-align:right;"><b><?php echo number_format_ind($tot_birds); ?></b></td>
                        <td></td>
                    </tr>
                </thead>
            <?php
...
<script>
        ...
        <script type="text/javascript">
            ...
            //  return function(table, name, filename, chosen) {
                ...
                html += '<th id="order_date">Last Daily Entry</th>';
                html += '<th id="order_date">Last Sale Date</th>';
                html += '<th id="order_date">Latest Feed Return Date</th>';
                html += '<th id="order_num">Feed Balance</th>';
                html += '<th id="order_num">Remaining Birds</th>';
                html += '<th id="order_num">Gap Days</th>';
