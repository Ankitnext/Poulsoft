<?php
//breeder_batchwise_weeklyreport1.php
...
if($db == ''){
    ...
    global $page_title; $page_title = "Breeder Batch Wise Weekly Report";
    include "header_head.php";
    $form_path = "breeder_batchwise_weeklyreport1.php";
}
else{
    ...
    global $page_title; $page_title = "Breeder Batch Wise Weekly Report";
    include "header_head.php";
    $form_path = "breeder_batchwise_weeklyreport1.php?db=$db&userid=".$user_code;
}
    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            if(isset($_POST['submit_report']) == true){
                ...
                $sql = "SELECT MIN(date) as egg_pdate,flock_code FROM `breeder_dayentry_produced` WHERE `batch_code` IN ('$batches') AND `active` = '1' AND `dflag` = '0' GROUP BY `flock_code` ORDER BY `flock_code` ASC";
                $query = mysqli_query($conn,$sql); $egg_pdate = array();
                while($row = mysqli_fetch_assoc($query)){ $egg_pdate[$row['flock_code']] = $row['egg_pdate']; }

                $sql = "SELECT * FROM `breeder_dayentry_consumed` WHERE `batch_code` IN ('$batches') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`flock_code` ASC";
                ...
                if((int)$de_cnt > 0){
                    while($row = mysqli_fetch_assoc($query)){
                        if(empty($egg_pdate[$row['flock_code']]) || $egg_pdate[$row['flock_code']] == "" || strtotime($egg_pdate[$row['flock_code']] >= strtotime($row['date']))){ }
                        else{
                    ...
                    while($row = mysqli_fetch_assoc($query)){
                        if(empty($egg_pdate[$row['to_flock']]) || $egg_pdate[$row['to_flock']] == "" || strtotime($egg_pdate[$row['to_flock']] >= strtotime($row['date']))){ }
                        else{
                    ...
                    while($row = mysqli_fetch_assoc($query)){
                        if(empty($egg_pdate[$row['from_flock']]) || $egg_pdate[$row['from_flock']] == "" || strtotime($egg_pdate[$row['from_flock']] >= strtotime($row['date']))){ }
                        else{
                            $bird_wage = age_in_weeks($row['bird_age']); $weeks = fetch_cweek($bird_wage);
