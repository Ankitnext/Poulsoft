...
while($row = mysqli_fetch_assoc($query)){ $feed_code[$row['code']] = $row['code']; $feed_qty[$row['code']] = $row['bag_size']; }
...
if(isset($_POST['submit_report']) == true){
	...
	//$url = "../PHPExcel/Examples/broiler_stock_report10-Excel.php?fdate=".$fdate."&tdate=".$tdate."&item_cat=".$item_cat."&sectors=".$sectors;
}
?>
<html>
    <head>
    ...
    <body align="center">
        <table class="tbl" align="center">
            ...
            <?php } ?>
            <form action="broiler_stock_report10.php" method="post">
        ...
        <table class="tbl" align="center" id="main_table">
            ...
            <thead class="thead3" id="head_row3" align="center">
                <tr>
                    <th colspan="4"></th>
                    <th>Bags</th>
                    <th>Us Pounds</th>
                ...
                <?php
                    ...
                    foreach($aitem_code as $ikey){
                        if(number_format_ind($final_opening_stock[$ikey]) != "0.00" || number_format_ind($final_purin_qty[$ikey]) != "0.00" || number_format_ind($final_in_quantity[$ikey]) != "0.00" || number_format_ind($final_consumed_quantity[$ikey]) != "0.00" || number_format_ind($final_sold_quantity[$ikey]) != "0.00" || number_format_ind($final_salereturn_quantity[$ikey]) != "0.00" || number_format_ind($final_closing_stock[$ikey]) != "0.00"){
                            ...
                            if($feed_qty[$ikey] != 0 || $feed_qty[$ikey] != ""){ $tot_feed_qty = $final_opening_stock[$ikey] / $feed_qty[$ikey]; } else { $tot_feed_qty = 0; }
                            if($feed_qty[$ikey] != 0 || $feed_qty[$ikey] != ""){ $tot_purin_qty = $final_purin_qty[$ikey] / $feed_qty[$ikey]; } else { $tot_purin_qty = 0; }
                            if($feed_qty[$ikey] != 0 || $feed_qty[$ikey] != ""){ $tot_cons_qty = $final_consumed_quantity[$ikey] / $feed_qty[$ikey]; } else { $tot_cons_qty = 0; }
                            $total_feed_qty = $total_feed_qty + $tot_feed_qty; if($total_feed_qty == ""){ $total_feed_qty = 0; }
                            $total_purinc_qty = $total_purinc_qty + $tot_purin_qty; if($total_purinc_qty == ""){ $total_purinc_qty = 0; }
                            $total_cons_qty = $total_cons_qty + $tot_cons_qty; if($total_cons_qty == ""){ $total_cons_qty = 0; }
                            echo "<tr>";
                            ...
                            echo "<td title='Opening' style='text-align:right;'>".number_format($tot_feed_qty,3)."</td>";
                            echo "<td title='Opening' style='text-align:right;'>".number_format($final_opening_stock[$ikey],3)."</td>";
                            //Purchases
                            echo "<td title='Purchase/Transfer In' style='text-align:right;'>".number_format($tot_purin_qty,3)."</td>";
                            echo "<td title='Purchase/Transfer In' style='text-align:right;'>".number_format($final_purin_qty[$ikey],3)."</td>";

                            echo "<td title='Consumption/Transferout' style='text-align:right;'>".number_format($tot_cons_qty,3)."</td>";
                            echo "<td title='Consumption/Transferout' style='text-align:right;'>".number_format($final_consumed_quantity[$ikey],3)."</td>";
                            ...
                            if($feed_qty[$ikey] != 0 || $feed_qty[$ikey] != ""){ $tot_cls_qty = $final_closing_stock[$ikey] / $feed_qty[$ikey]; } else { $tot_cls_qty = 0; }
                            $total_cls_qty = $total_cls_qty + $tot_cls_qty; if($total_cls_qty == ""){ $total_cls_qty = 0; }

                            if((int)$lstk_flag[$ikey] == 1 && (float)$final_closing_stock[$ikey] <= (float)$lstk_qty[$ikey]){
                                echo "<td title='Closing' style='text-align:right;color:red;font-weight:bold;'>".number_format($tot_cls_qty,3)."</td>";
                                echo "<td title='Closing' style='text-align:right;color:red;font-weight:bold;'>".number_format($final_closing_stock[$ikey],3)."</td>";
                            }
                            else{
                                echo "<td title='Closing' style='text-align:right;'>".number_format($tot_cls_qty,3)."</td>";
                                echo "<td title='Closing' style='text-align:right;'>".number_format($final_closing_stock[$ikey],3)."</td>";
                            }
                    ...
                    echo "<td  title='Opening' style='text-align:right;font-weight:bold;'>".number_format($total_feed_qty,3)."</td>";
                    echo "<td  title='Opening' style='text-align:right;font-weight:bold;'>".number_format($total_opening_qty,3)."</td>";
                    //Purchases
                    echo "<td  title='Purchase/Transfer In' style='text-align:right;font-weight:bold;'>".number_format($total_purinc_qty,3)."</td>";
                    echo "<td  title='Purchase/Transfer In' style='text-align:right;font-weight:bold;'>".number_format($total_purtrin_qty,3)."</td>";
                    //Consumed
                    echo "<td  title='Consumption/Transferout' style='text-align:right;font-weight:bold;'>".number_format($total_cons_qty,3)."</td>";
                    echo "<td  title='Consumption/Transferout' style='text-align:right;font-weight:bold;'>".number_format($total_consumd_qty,3)."</td>";
                    //Closing
                    echo "<td title='Closing' style='text-align:right;font-weight:bold;'>".number_format($total_cls_qty,3)."</td>";
                    echo "<td title='df' style='text-align:right;font-weight:bold;'>".number_format($total_closed_qty,3)."</td>";
