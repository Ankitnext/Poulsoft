<?php
//broiler_branchwiserealize_masterreport.php
...
//$field_href[0] = "broiler_branchwiserealize_masterreport.php";
...
if(isset($_POST['submit_report']) == true){
    ...
    {$filename = "Branch Wise Realization_".$export_tdate; }
    else {
    $filename = "Branch Wise Realization_".$export_fdate."_to_".$export_tdate; }
    ...
    <body align="center">
        <table class="tbl" align="center"   width="1300px">
            ...
            <thead class="thead1" align="center" width="1212px">
                <tr align="center">
                    ...
                    <th colspan="12" align="center"><?php echo $row['cdetails']; ?><h5>Branch Wise Realization</h5></th>
                </tr>
            </thead>
            <?php } ?>
            <?php if($db == ''){?>
            <form action="broiler_branchwiserealize_masterreport.php" method="post">
            <?php } else { ?>
            <form action="broiler_branchwiserealize_masterreport.php?db=<?php echo $db; ?>" method="post">
            <?php } ?>
        ...
        <div class="row" style="padding-left:100px;">
         ...
         $sql = "SELECT * FROM `main_companyprofile` WHERE `type` = 'Sales Invoice' OR `type` = 'All' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql);
            ...
            ?>
                <tr align="center">
                    <th colspan="14" align="center"><?php echo $row['cdetails']; ?><h5>Branch Wise Realization</h5></th>
                </tr>
        <?php } ?>
        ...
        <tr>
            ...
            <tbody class="tbody1" id="tbody1">
                ...
                $farmcount = $brch_arr_codes = $mage = $chick_placed = $morta_count = $mort7_count = $mort30_count = $mort31g_count = $shortage = 
                ...
                $brh_list = ""; $brh_list = implode("','",$brch_arr_codes);
                $brh_code = array();
                $sql = "SELECT * FROM `location_branch` WHERE `active` = '1' AND `code` IN ('$brh_list') AND `dflag` = '0' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
                while($row = mysqli_fetch_assoc($query)){ $brh_code[$row['code']] = $row['code']; }
                ...
                foreach($brh_code as $key){
                    $slno++;
                    $brch_name = $branch_name[$key];
                    ...
                    for($i = 1;$i <= $col_count;$i++){
                        ...
                        else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "branch_name"){ echo "<td title='Branch' style='text-align:left;'><a href=/records/broiler_farmwiserealize_masterreport.php?submit_report=true&lines=all&fdate=". date("d.m.Y",strtotime($fdate)) ."&tdate=". date("d.m.Y",strtotime($tdate))."&report_view=". $report_view ."&branches=".$key."&lines=all&supervisors=all&farms=all target='_blank'>".$brch_name."</a></td>"; }
        ...
        <script>
            function fetch_farms_details(a){
                ...
                if(a.match("regions")){ rf_flag = 1; }else{ ff_flag = 1; }
                    
                var fetch_fltrs = new XMLHttpRequest();
                var method = "GET";
                var url = "broiler_fetch_farm_filter_master.php?regions="+regions+"&rf_flag="+rf_flag+"&bf_flag="+bf_flag+"&lf_flag="+lf_flag+"&sf_flag="+sf_flag+"&ff_flag="+ff_flag+"&user_code="+user_code;
                //window.open(url);
                var asynchronous = true;
            ...
            function set_auto_selectors(){
                ...
                if(f_cnt <= 1){ setTimeout(set_auto_selectors, 300); }
            }
            set_auto_selectors();
        </script>
    </body>
