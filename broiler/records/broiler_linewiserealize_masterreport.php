<?php
//broiler_linewiserealize_masterreport.php
...
//$field_href[0] = "broiler_linewiserealize_masterreport.php";
...
if(isset($_POST['submit_report']) == true){
    ...
    {$filename = "Line Wise Realization_".$export_tdate; }
    else {
    $filename = "Line Wise Realization_".$export_fdate."_to_".$export_tdate; }
    $excel_type = $_POST['export'];
    
    $ebranches = $_POST['branches'];
    $elines = $_POST['lines'];

    //$export_supervisors = $supervisor_name[$_POST['supervisors']];
    $export_report_view = $POST['report_view'];
    
    if ( $ebranches == "" || $ebranches == "all") { $ebranches = "All"; }
    else {  $ebranches = $branch_name[$_POST['branches']]; }
    if ( $elines == "" || $elines == "all") { $elines = "All"; }
    else {  $elines = $line_name[$_POST['lines']]; }
...
<html>
    ...
    <body align="center">
        <table class="tbl" align="center"   width="1300px">
            ...
            <thead class="thead1" align="center" width="1212px">
                <tr align="center">
                    ...
                    <th colspan="12" align="center"><?php echo $row['cdetails']; ?><h5>Line Wise Realization</h5></th>
                </tr>
            </thead>
            <?php } ?>
            <?php if($db == ''){?>
            <form action="broiler_linewiserealize_masterreport.php" method="post">
            <?php } else { ?>
            <form action="broiler_linewiserealize_masterreport.php?db=<?php echo $db; ?>" method="post">
            <?php } ?>
                    ...
                    <tr>
                        <th colspan="14">
                            <div class="row">
                                ...
                                <div class="m-2 form-group">
                                    <label>Export</label>
                                    <select name="export" id="export" class="form-control select2" onchange="tableToExcel('mine', 'Line Wise Realization_','<?php echo $filename;?>', this.options[this.selectedIndex].value)">
        ...
        ?>
            <tr align="center">
                <th colspan="14" align="center"><?php echo $row['cdetails']; ?><h5>Line Wise Realization</h5></th>
            </tr>
        ...
        <tr>
            ...
            <thead class="thead3" align="center" style="width:1212px;">
                ...
                for($i = 1;$i <= $col_count;$i++){
                    ...
                    else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "branch_name"){ echo "<th id='order'>Branch</th>"; $theadc++; }
                    else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "line_name"){ echo "<th id='order'>Line</th>"; $theadc++; }
                    else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "mean_age"){ echo "<th id='order_num'>Mean Age</th>"; }
                    else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "chick_placed"){ echo "<th id='order_date'>Chicks Placement</th>"; }
            ...
            <tbody class="tbody1" id="tbody1">
                ...
                $farmcount = $brch_arr_codes = $line_arr_codes = $mage = $chick_placed = $morta_count = $mort7_count = $mort30_count = $mort31g_count = $shortage = 
                ...
                foreach($ln_code as $key){
                    $slno++;
                    $brch_name = $branch_name[$brch_arr_codes[$key]];
                    $lne_name = $line_name[$key];
                    ...
                    for($i = 1;$i <= $col_count;$i++){
                        ...
                        else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "branch_name"){ echo "<td title='Branch' style='text-align:left;'>".$brch_name."</td>"; }
                        else if(!empty($act_col_numbs[$key_id]) && $act_col_numbs[$key_id] == "line_name"){ echo "<td title='Line' style='text-align:left;'><a href=/records/broiler_farmwiserealize_masterreport.php?submit_report=true&lines=all&fdate=". date("d.m.Y",strtotime($fdate)) ."&tdate=". date("d.m.Y",strtotime($tdate))."&report_view=". $report_view ."&branches=". $brch_arr_codes[$key] ."&lines=". $key ."&supervisors=all&farms=all target='_blank'>".$lne_name."</a></td>"; }
            ...
            <tfoot class="tfoot1">
                ...
                if($t_act_sbirds == "" || (float)$t_act_sbirds == 0) {
                    ...
                    $t_avg_bwt = round(((float)$t_act_sweight / (float)$t_act_sbirds),3);
                }
        ...
        <script>
            function fetch_farms_details(a){
                ...
                var user_code = '<?php echo $user_code; ?>';
                var rf_flag = bf_flag = 0;
                if(a.match("regions")){ rf_flag = 1; } else if(a.match("branches")){ bf_flag = 1; } else{ ff_flag = 1; }
                    
                var fetch_fltrs = new XMLHttpRequest();
                var method = "GET";
                var url = "broiler_fetch_farm_filter_master.php?regions="+regions+"&branches="+branches+"&rf_flag="+rf_flag+"&bf_flag="+bf_flag+"&user_code="+user_code;
                //window.open(url);
                var asynchronous = true;
