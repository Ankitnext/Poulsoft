...
while($row = mysqli_fetch_assoc($query)){ $sector_code[$row['code']] = $row['code']; $sector_name[$row['code']] = $row['description']; $farm_branch[$row['code']] = $row['branch_code']; }
...
if(isset($_POST['submit_report']) == true){
    ...
    $coa_list = $icat_list = $item_filter = $sector_filter = "";
...
<html>
    ...
    <body align="center">
        <table class="tbl" style="width:auto;" align="center">
            ...
            <?php } ?>
            <form action="broiler_item_summary3.php" method="post" onsubmit="return checkval()">
            ...
            ?>
                <tr align="center">
                    ...
                    <th colspan="14" align="center" style="border-right:none;"><?php echo $row['cdetails']; ?><h5>Item Summary Report</h5></th>
            ...
            <?php
            if(isset($_POST['submit_report']) == true){
                $current_stock = $current_price = $current_amount = $item_list = $sector_list = $between_in_qty = $between_in_amt = $between_out_qty = $between_out_amt = array();
                ...
                while($row = mysqli_fetch_assoc($query)){
                    if($row['item_code'] != ""){
                        $c++; $key = $row['location']."@".$row['item_code'];
                        if($row['crdr'] == "CR"){
                            $current_stock[$key] = $current_stock[$key] - (float)$row['quantity'];
                            $current_amount[$key] = $current_amount[$key] - ($current_price[$key] * (float)$row['quantity']);

                            if(strtotime($row['date']) >= strtotime($fdate)){
                                $between_out_qty[$key] = $between_out_qty[$key] + (float)$row['quantity'];
                                $between_out_amt[$key] = $between_out_amt[$key] + ($current_price[$key] * (float)$row['quantity']);
                            }
                        }
                        else if($row['crdr'] == "DR"){
                            $current_stock[$key] = $current_stock[$key] + (float)$row['quantity'];
                            $current_amount[$key] = $current_amount[$key] + (float)$row['amount'];
                            if((float)$current_stock[$key] != 0){
                                $current_price[$key] = round(((float)$current_amount[$key] / (float)$current_stock[$key]),2);
                            }
                            else{
                                $current_price[$key] = 0;
                            }
                            ...
                            if(strtotime($row['date']) >= strtotime($fdate)){
                                $between_in_qty[$key] = $between_in_qty[$key] + (float)$row['quantity'];
                                $between_in_amt[$key] = $between_in_amt[$key] + (float)$row['amount'];
                            }
                        }
                        else{ }
                        if(strtotime($row['date']) < strtotime($fdate)){ $opening_stk_qty[$key] = $current_stock[$key]; $opening_stk_amt[$key] = $current_amount[$key]; }
                ...
                /* Sector and item category wise filter*/
                $loc_icat_opening_qty = $loc_icat_opening_amt = $loc_icat_btwout_qty = $loc_icat_btwout_amt = $loc_icat_btwin_qty = $loc_icat_btwin_amt = $icat_list = array();
                foreach($sector_list as $slist){
                    foreach($item_list as $ilist){
                        $key = $slist."@".$ilist;
                        if(!empty($farm_branch[$slist])){ $bches = $farm_branch[$slist]; $branch_list[$bches] = $bches; } else{ $bches = $slist; $branch_list[$bches] = $bches; }
                        $ccode = $item_category[$ilist]; $key2 = $bches."@".$ccode; $icat_list[$ccode] = $ccode;
                        ...
                        if(number_format_ind(round($current_stock[$key],2)) == "0.00"){ $current_stock[$key] = $current_amount[$key] = 0; }
            ...
            <tbody>
            <?php
            $total_opening_qty = $total_opening_amt = $total_in_qty = $total_in_amt = $total_out_qty = $total_out_amt = $total_close_qty = $total_close_amt = 0;
            foreach($branch_list as $slist){
                $sector_header[$slist] = 0; $sectorwise_total_open_qty = $sectorwise_total_open_amt = $sectorwise_total_in_qty = $sectorwise_total_in_amt = $sectorwise_total_out_qty = $sectorwise_total_out_amt = $sectorwise_total_close_qty = $sectorwise_total_close_amt = 0;
                foreach($icat_list as $ilist){
                    ...
                    else if($icat_name[$ilist] == ""){ }
                    else{
                        if($sector_header[$slist] == 0){
                            if(!empty($branch_name[$slist])){ $secname = $branch_name[$slist]; } else{ $secname = $sector_name[$slist]; }
                            echo "<tr>";
                            echo "<td>".$secname."</td>";
                            echo "<td>".$icat_name[$ilist]."</td>";
                            echo "<td style='text-align:right;'>".number_format_ind(round($loc_icat_opening_qty[$key],2))."</td>";
                            if((float)$loc_icat_opening_qty[$key] != 0){
                                $loc_icat_opening_prc[$key] = (float)$loc_icat_opening_amt[$key] / (float)$loc_icat_opening_qty[$key];
                            }
                            else{
                                $loc_icat_opening_prc[$key] = 0;
                            }
                            ...
                            if((float)$loc_icat_btwin_qty[$key] != 0){
                                $loc_icat_btwin_prc[$key] = (float)$loc_icat_btwin_amt[$key] / (float)$loc_icat_btwin_qty[$key];
                            }
                            else{
                                $loc_icat_btwin_prc[$key] = 0;
                            }
                            ...
                            if((float)$loc_icat_btwout_qty[$key] != 0){
                                $loc_icat_btwout_prc[$key] = (float)$loc_icat_btwout_amt[$key] / (float)$loc_icat_btwout_qty[$key];
                            }
                            else{
                                $loc_icat_btwout_prc[$key] = 0;
                            }
                            echo "<td style='text-align:right;'>".number_format_ind(round(($loc_icat_btwout_prc[$key]),2))."</td>";
                            echo "<td style='text-align:right;'>".number_format_ind(round($loc_icat_btwout_amt[$key],2))."</td>";
        
                            echo "<td style='text-align:right;'>".number_format_ind(round($loc_icat_btwclose_qty[$key],2))."</td>";
                            if((float)$loc_icat_btwclose_qty[$key] != 0){
                                $loc_icat_btwclose_prc[$key] = (float)$loc_icat_btwclose_amt[$key] / (float)$loc_icat_btwclose_qty[$key];
                            }
                            else{
                                $loc_icat_btwclose_prc[$key] = 0;
                            }
                            echo "<td style='text-align:right;'>".number_format_ind(round(($loc_icat_btwclose_prc[$key]),2))."</td>";
                            echo "<td style='text-align:right;'>".number_format_ind(round($loc_icat_btwclose_amt[$key],2))."</td>";
                            echo "</tr>";
                ...
                <?php
                    if((float)$sectorwise_total_open_qty != 0){
                        $sectorwise_total_open_prc = (float)$sectorwise_total_open_amt / (float)$sectorwise_total_open_qty;
                    }
                    else{
                        $sectorwise_total_open_prc = 0;
                    }
                ?>
                ...
                <?php
                    if((float)$sectorwise_total_in_qty != 0){
                        $sectorwise_total_in_prc = (float)$sectorwise_total_in_amt / (float)$sectorwise_total_in_qty;
                    }
                    else{
                        $sectorwise_total_in_prc = 0;
                    }
                ?>
                ...
                <?php
                    if((float)$sectorwise_total_out_qty != 0){
                        $sectorwise_total_out_prc = (float)$sectorwise_total_out_amt / (float)$sectorwise_total_out_qty;
                    }
                    else{
                        $sectorwise_total_out_prc = 0;
                    }
                ?>
                ...
                <?php
                    if((float)$sectorwise_total_close_qty != 0){
                        $sectorwise_total_close_prc = (float)$sectorwise_total_close_amt / (float)$sectorwise_total_close_qty;
                    }
                    else{
                        $sectorwise_total_close_prc = 0;
                    }
                ?>
            ...
            <tr class="thead4">
                ...
                <?php
                    if((float)$total_opening_qty != 0){
                        $total_opening_prc = (float)$total_opening_amt / (float)$total_opening_qty;
                    }
                    else{
                        $total_opening_prc = 0;
                    }
                ...
                <?php
                    if((float)$total_in_qty != 0){
                        $total_in_prc = (float)$total_in_amt / (float)$total_in_qty;
                    }
                    else{
                        $total_in_prc = 0;
                    }
                ...
                <?php
                    if((float)$total_out_qty != 0){
                        $total_out_prc = (float)$total_out_amt / (float)$total_out_qty;
                    }
                    else{
                        $total_out_prc = 0;
                    }
                ...
                <?php
                    if((float)$total_close_qty != 0){
                        $total_close_prc = (float)$total_close_amt / (float)$total_close_qty;
                    }
                    else{
                        $total_close_prc = 0;
                    }
