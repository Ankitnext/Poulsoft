...
global $page_title; $page_title = "Supplier List Report";
...
$sql = "SELECT * FROM `main_contactdetails` WHERE `contacttype` LIKE '%S%' AND `dflag` = '0' ORDER BY `name` ASC"; $query = mysqli_query($conn,$sql);
while($row = mysqli_fetch_assoc($query)){ $vendor_code[$row['code']] = $row['code']; $vendor_name[$row['code']] = $row['name']; }

$sql = "SELECT * FROM `main_groups` WHERE `gtype` LIKE '%S%' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
...
if(isset($_POST['submit_report']) == true){
	...
	//$url = "../PHPExcel/Examples/SupplierListReport-Excel.php?fromdate=".$fdate."&todate=".$tdate."&vendors=".$vendors."&groups=".$groups;
}
?>
<html>
    <head>
        ...
        <table class="tbl" align="center">
            ...
            <thead class="thead1" align="center">
                <tr align="center">
                    ...
                    <th colspan="19" align="center"><?php echo $row['cdetails']; ?><h5>Supplier List Report</h5></th>
                </tr>
            </thead>
            <?php } ?>
            <form action="broiler_supplier_list.php" method="post">
                    ...
                    <tr>
                        <th colspan="21">
                            <div class="row">
                                ...
                                <div class="m-2 form-group">
                                    <label>Export</label>
                                    <select name="export" id="export" class="form-control select2"  onchange="tableToExcel('main_body', 'Supplier List','<?php echo $filename;?>', this.options[this.selectedIndex].value)">
           ...
           <table id="main_body" class="tbl" align="center"  style="width:1300px;">
            ...
            ?>
                ...
                <th colspan="20" align="center"><?php echo $row['cdetails']; ?><h5>Supplier List Report</h5></th>
            ...
            <tbody class="tbody1">
                <?php
                $sql_record = "SELECT * FROM `main_contactdetails` WHERE `dflag` = '0'".$vcodes."".$gcodes." AND `contacttype` LIKE '%S%'AND `active` = '1' ORDER BY `name` ASC";
