...
if($db == ''){
    ...
    $form_path = "layer_farm_details.php";
}
else{
    ...
    include "header_head.php";
    $form_path = "layer_farm_details.php?db=$db&userid=".$user_code;
}
...
if(in_array("layer_farms", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.layer_farms LIKE poulso6_admin_layer_layermaster.layer_farms;"; mysqli_query($conn,$sql1); }
if(in_array("broiler_employee", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.broiler_employee LIKE poulso6_admin_layer_layermaster.broiler_employee;"; mysqli_query($conn,$sql1); }
if(in_array("layer_units", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.layer_units LIKE poulso6_admin_layer_layermaster.layer_units;"; mysqli_query($conn,$sql1); }

$file_name = "Farm Information Report";
...
<html>
    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            $nhtml .= '<th>Unit Location</th>'; $fhtml .= '<th id="order">Unit Location</th>';
            $nhtml .= '<th>Unit Address</th>'; $fhtml .= '<th id="order">Unit Address</th>';
            $nhtml .= '<th>Water Tank Capacity (Ltrs)</th>'; $fhtml .= '<th id="order_num">Water Tank Capacity (Ltrs)</th>';
            $nhtml .= '<th>Sump Capacity (Ltrs)</th>'; $fhtml .= '<th id="order_num">Sump Capacity (Ltrs)</th>';
            $nhtml .= '<th>Unit Incharge</th>'; $fhtml .= '<th id="order">Unit Incharge</th>';
            ...
            if(isset($_POST['submit_report']) == true){
                ...
                $sql = "SELECT * FROM `layer_units` WHERE `active` = '1'".$drv_fltr." AND `dflag` = '0' ORDER BY `addedtime` ASC";
                $query = mysqli_query($conn,$sql); $tot_wcpt = $tot_scpt = $tot_emps = 0;
                while($row = mysqli_fetch_assoc($query)){
                 
                    $f_code = $farm_ccode[$row['farm_code']];
                    $fname = $farm_name[$row['farm_code']];
                    $u_code = $row['unit_code'];
                    $u_name = $row['description'];
                    $location = $row['location'];
                    $address = $row['address'];
                    $wtank_capacity = $row['wtank_capacity'];
                    $sump_capacity = $row['sump_capacity'];
                    $incharge_emp = $emp_name[$row['incharge_emp']];
                    ...
                    $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($wtank_capacity,5))).'</td>';
                    $html .= '<td style="text-align:right;">'.number_format_ind(round($sump_capacity,5)).'</td>';
                    $html .= '<td>'.$incharge_emp.'</td>';
                    $html .= '<td style="text-align:right;">'.number_format_ind(round($nof_emps,5)).'</td>';
                    
                    $html .= '</tr>';

                    $tot_wcpt += (float)$wtank_capacity;
                    $tot_scpt += (float)$sump_capacity;
                ...
                $html .= '<th style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tot_wcpt,5))).'</th>';
                $html .= '<th style="text-align:right;">'.number_format_ind(round($tot_scpt,5)).'</th>';
                $html .= '<th></th>';
                $html .= '<td style="text-align:right;">'.str_replace(".00","",number_format_ind(round($tot_emps,5))).'</td>';
                $html .= '</tr>';
                $html .= '</tfoot>';
            }
            echo $html;
