<?php
//broiler_dailyentry_gapdays.php
...
include $num_format_file;
    global $page_title; $page_title = "Not Visited Farms List Report";
...
$sql = "SELECT * FROM `sms_master` WHERE `sms_type` = 'BB-NotVisitFarms' AND  `msg_type` IN ('WAPP') AND `active` = '1'"; $query = mysqli_query($conn,$sql); $mob_list = "";
while($row = mysqli_fetch_assoc($query)){ $mob_list = $row['numers']; }

$branches = $lines = $supervisors = $farms = "all"; $excel_type = $vis_type = "display"; $gdays = $days = 1; $report_view = "hd"; $url = "";
...
if($farms != "all"){
    ...
    $wapp_link = "../print/Examples/broiler_dailyentry_gapdayswapp.php?branches=".$branches."&lines=".$lines."&supervisors=".$supervisors."&farms=".$farms."&vis_type=".$vis_type."&mob_list=".$mob_list."&days=".$days;

    $excel_type = $_POST['vis_type'];
    $filename = "Not Visited Farms List Report";

        $export_branches = $branch_name[$_POST['branches']];
        $export_lines = $line_name[$_POST['lines']];
        $export_supervisors = $supervisor_name[$_POST['supervisors']];
        $export_farms = $farm_name[$_POST['farms']];
        $export_gdays = $_POST['days'];
    
    if( $export_branches  == "" ||  $export_branches ="all") { $export_branches  = "All";}
    if( $export_lines  == "" ||  $export_lines ="all") { $export_lines  = "All";}
    if( $export_supervisors  == "" ||  $export_supervisors ="all") { $export_supervisors  = "All";}
    if( $export_farms  == "" ||  $export_farms ="all") { $export_farms  = "All";}
...
<html>
    <head>
        ...
        <?php
          if($excel_type == "print"){
            ...
            .tbody1 tr:hover { background-image: linear-gradient(#FADBD8,#FADBD8); font-weight:bold; }
            #header_sorting2 {  display:none; } </style>';
        }
        else{
    ...
    <body>
        <table class="tbl" align="center">
            ...
            <thead class="thead1" align="center" style="width:auto;">
                <tr align="center">
                    ...
                    <th colspan="8" align="center" style="border-right:none;"><?php echo $row['cdetails']; ?><h5>Not Visited Farms List Report</h5></th>
                </tr>
            </thead>
            <?php } ?>
            <?php if($db == ''){?>
            <form action="broiler_dailyentry_gapdays.php" method="post"  onsubmit="return checkval()">
                 <?php } else { ?>
                <form action="broiler_dailyentry_gapdays.php?db=<?php echo $db; ?>" method="post" onsubmit="return checkval()">
                <?php } ?>
                        ...
                        <th colspan="10">
                            <div class="row">&ensp;&ensp;
                                ...
                                <div class="form-group" style="width:120px;">
                                    ...
                                    <select name="vis_type" id="vis_type" class="form-control select2" style="width:110px;" onchange="test('main_body', 'Not Visited Farms List Report','<?php echo $filename;?>', this.options[this.selectedIndex].value)">
                                        <option value="display" <?php if($vis_type == "display"){ echo "selected"; } ?>>Display</option>
                                        <option value="excel" <?php if($vis_type == "excel"){ echo "selected"; } ?>>Excel</option>
                                        <option value="WhatsApp" <?php if($vis_type == "WhatsApp"){ echo "selected"; } ?>>WhatsApp</option>
                                    </select>
                                </div>
                                <div class="form-group" style="width:150px;">
                                    <label for="days">Days</label>
                                    <input type="text" name="days" id="days" class="form-control" style="width:90px;" value="<?php echo $days; ?>"/>
                                </div>
                                <div class="form-group" style="width:150px;">
        ...
        <?php
            ...
            ?>
                <tr align="center">
                    <th colspan="10" align="center" style="border-right:none;"><?php echo $row['cdetails']; ?><h5>Not Visited Farms List Report</h5></th>
                </tr>
            ...
            <tr>
                       ...
                       <th  colspan="10">
                                       ...
                                       <div class="m-2 form-group">
                                           <label>Line: <?php echo $export_lines; ?></label>
                                       </div>
                                       <div class="m-2 form-group">
                                           <label>Supervisor: <?php echo $export_supervisors; ?></label>
                                       </div>
                                       <div class="m-2 form-group">
                                           <label>Farm: <?php echo $export_farms; ?></label>
                                       </div>
                                       <div class="m-2 form-group">
                                           <label>Days: <?php echo $export_gdays; ?></label>
                                       </div>
                                       <div class="m-2 form-group">
                                           <label><br/></label>
                ...
                <thead class="thead3" align="center" >
                    <tr align="center" id="header_sorting">
                        ...
                        <th id="order_date">Latest Entry Date</th>
                        <th id="order_num">Gap Days</th>
                    </tr>
                </thead>
                <thead class="thead3" align="center" style="width:1212px;  display:none; " id="header_sorting2">
                ...
                if(isset($_POST['submit_report']) == true || isset($_GET['submit']) == true){
                    ...
                    //Daily Entry
                    $sql = "SELECT SUM(mortality) as mort,SUM(culls) as culls,MAX(brood_age) as age,MAX(date) as date,batch_code FROM `broiler_daily_record` WHERE `batch_code` IN ('$batch_list') AND `active` = '1' AND `dflag` = '0' AND gc_flag = '0' GROUP BY `batch_code` ORDER BY `batch_code` ASC";
                    $query = mysqli_query($conn,$sql);
                    while($row = mysqli_fetch_assoc($query)){
                    ...
                    while($row = mysqli_fetch_assoc($query)){
                        ...
                        $chickin_sdate[$row['farm_batch']] = $row['sdate'];
                    ...
                    while($row = mysqli_fetch_assoc($query)){
                        ...
                        $chickin_sdate[$row['to_batch']] = $row['sdate'];
                ...
                foreach($f1 as $fcode){
                    ...
                    if(!empty($dentry_max_date[$bcode])){
                        $latest_entry_date = date("d.m.Y",(strtotime($dentry_max_date[$bcode])));
                    }
                    else if(!empty($chickin_sdate[$bcode])){
                        $latest_entry_date = date("d.m.Y",(strtotime($chickin_sdate[$bcode])));
                    }
                    else{
                        $latest_entry_date = date("d.m.Y",(strtotime("2001-01-01")));
                    }
                    $ldays = (INT)((strtotime($today) - strtotime($latest_entry_date)) / 60 / 60 / 24);
                    if((INT)$ldays >= (INT)$gdays){
                        ...
                        if(number_format_ind($display_available_birds) != "0.00" || number_format_ind($chick_in_qty[$bcode]) != "0.00"){
                            if((float)$display_available_birds > 50){
                            $slno++;
                        ...
                        <tr>
                            ...
                            <td style="text-align:right;" title="<?php echo $title; ?>"><?php echo str_replace(".00","",number_format_ind($display_available_birds)); ?></td>
                            <td title="Latest Entry Date" style="text-align:left;"><?php echo $latest_entry_date; ?></td>
                            <td title="Gap Days" style="text-align:center;"><?php echo $ldays; ?></td>
                        </tr>
                        <?php
                ...
                <tfoot>
                    <tr>
                        <th colspan="10" style="text-align:center;"><b style="color:red;">Note: </b>Farms that haven't been visited will display if there are more than 50 birds available.</th>
                    </tr>
                </tfoot>
            <?php
                }
        ...
        <script>
            function checkval(){
                ...
                if(days == "" || days == 0 || days == "0"){
                    alert("Please enter appropriate gap Days");
...
<script>
        ...
        <script type="text/javascript">
            function test(table, name, filename, chosen)
            { 
            ...
            //  return function(table, name, filename, chosen) {
                ...
                html += '<th id="order_num">Sl.No</th>';
                        ...
                        html += '<th id="order_num">Available Birds</th>';
                        html += '<th id="order_date">Latest Entry Date</th>';
