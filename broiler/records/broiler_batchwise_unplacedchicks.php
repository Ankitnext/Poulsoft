<?php
//broiler_batchwise_unplacedchicks.php
...
include $num_format_file;
    global $page_title; $page_title = "Feed Transferred & Unplaced Active Batches Report";
        ...
        <table class="tbl" align="center">
            ...
            <thead class="thead1" align="center" style="width:auto;">
                <tr align="center">
                    ...
                    <th colspan="8" align="center" style="border-right:none;"><?php echo $row['cdetails']; ?><h5>Feed Transferred & Unplaced Active Batches Report</h5></th>
                </tr>
            </thead>
            <?php } ?>
            <?php if($db == ''){?>
            <form action="broiler_batchwise_unplacedchicks.php" method="post"  onsubmit="return checkval()">
                 <?php } else { ?>
                <form action="broiler_batchwise_unplacedchicks.php?db=<?php echo $db; ?>" method="post" onsubmit="return checkval()">
                <?php } ?>
                ...
                <thead class="thead3" align="center">
                    <tr align="center">
                        ...
                        <th id="order_date">Feed Sent Date</th>
                        <th id="order">Feed Item</th>
                        <th id="order_num">Available Feed</th>
                        <th id="order_num">No. of Days</th>
                    </tr>
                </thead>
                <?php
                if(isset($_POST['submit_report']) == true){
                    ...
                    $b1 = $b2 = $batch_farm1 = $batch_farm2 = array();
                    //Chick Purchase
                    ...
                    $query = mysqli_query($conn,$sql); while($row = mysqli_fetch_assoc($query)){ if(number_format_ind($row['rcd_qty']) != "0.00"){ $key_code = $row['farm_batch']; $b1[$key_code] = $key_code; } }
                    //Chick Transfer IN
                    ...
                    $query = mysqli_query($conn,$sql_record); while($row = mysqli_fetch_assoc($query)){ if(number_format_ind($row['quantity']) != "0.00"){ $key_code = $row['to_batch']; $b1[$key_code] = $key_code; } }

                    $b1_list = implode("','", $b1);
                    //Chick Purchase
                    $sql = "SELECT SUM(rcd_qty) as rcd_qty,SUM(fre_qty) as fre_qty,MIN(date) as sdate,icode,farm_batch FROM `broiler_purchases` WHERE `icode` IN ('$feed_list') AND `farm_batch` IN ('$batch_list') AND `farm_batch` NOT IN ('$b1_list') AND `active` = '1' AND `dflag` = '0' GROUP BY `farm_batch` ORDER BY `farm_batch` ASC";
                    $query = mysqli_query($conn,$sql);
                    while($row = mysqli_fetch_assoc($query)){
                        if(number_format_ind($row['rcd_qty']) != "0.00"){
                            $key_code = $row['farm_batch'];
                            $b2[$key_code] = $key_code;
                            $feedin_qty[$key_code] += ((float)$row['rcd_qty'] + (float)$row['fre_qty']);
                            $feedin_date[$key_code] = $row['sdate'];
                            $feedin_item[$key_code] = $row['icode'];
                        }
                    }
                    //Chick Transfer IN
                    $sql_record = "SELECT SUM(quantity) as quantity,MIN(date) as sdate,code,to_batch FROM `item_stocktransfers` WHERE `code` IN ('$feed_list') AND `to_batch` IN ('$batch_list') AND `to_batch` NOT IN ('$b1_list') AND `active` = '1' AND `dflag` = '0' GROUP BY `to_batch` ORDER BY `to_batch` ASC";
                    $query = mysqli_query($conn,$sql_record);
                    while($row = mysqli_fetch_assoc($query)){
                        if(number_format_ind($row['quantity']) != "0.00"){
                            $key_code = $row['to_batch'];
                            $b2[$key_code] = $key_code;
                            $feedin_qty[$key_code] += ((float)$row['quantity']);
                            $feedin_date[$key_code] = $row['sdate'];
                            $feedin_item[$key_code] = $row['code'];
                        }
                    }
                    $b2_list = implode("','", $b2);
                    $sql = "SELECT * FROM `broiler_batch` WHERE `code` IN ('$b2_list') AND `dflag` = '0' AND `gc_flag` = '0'"; $query = mysqli_query($conn,$sql);
                ...
                <tbody class="tbody1" id="tbody1">
                    <?php
                        $slno = 0; $today = date("Y-m-d"); $fdate = date("d.m.Y",strtotime($today."-6 days"));
                         foreach($f1 as $fcode){
                            $bcode = ""; $bcode = $batch_farm2[$fcode];
                            $days = (INT)((strtotime($today) - strtotime($feedin_date[$bcode])) / 60 / 60 / 24);
                            if($days >= 1){
                                if($feedin_qty[$bcode] > 0){
                                $slno++;
                                $nof_days = 0;
                                if(date("d.m.Y",strtotime($feedin_date[$bcode])) == "01.01.1970"){ }
                                else{
                                    $nof_days = ((strtotime($today) - strtotime($feedin_date[$bcode])) / 60 / 60 / 24);
                                }
                            ?>
                            <tr>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;"'; } ?> title="Sl. No."><?php echo $slno; ?></td>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;"'; } ?> title="Branch Name"><?php echo $branch_name[$farm_branch[$fcode]]; ?></td>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;"'; } ?> title="Line"><?php echo $line_name[$farm_line[$fcode]]; ?></td>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;"'; } ?> title="Supervisor"><?php echo $supervisor_name[$farm_supervisor[$fcode]]; ?></td>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;"'; } ?> title="Farm"><?php echo $farm_name[$fcode]; ?></td>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;"'; } ?>><?php echo $batch_name[$bcode]; ?></td>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;"'; } ?>><?php if(date("d.m.Y",strtotime($feedin_date[$bcode])) == "01.01.1970"){ echo ""; } else{ echo date("d.m.Y",strtotime($feedin_date[$bcode])); } ?></td>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;text-align:left;"'; } else{ echo 'style="text-align:left;"'; } ?>><?php echo $feed_name[$feedin_item[$bcode]]; ?></td>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;text-align:right;"'; } else{ echo 'style="text-align:right;"'; } ?>><?php echo number_format_ind($feedin_qty[$bcode]); ?></td>
                                <td <?php if((int)$nof_days > 7){ echo 'style="color:red;text-align:right;"'; } else{ echo 'style="text-align:right;"'; } ?>><?php echo str_replace(".00","",number_format_ind($nof_days)); ?></td>
                            </tr>
                        <?php
                            $tot_feeds += (float)$feedin_qty[$bcode];
                                }
                            }
                        }
                    ?>
                ...
                <thead class="thead2">
                    <tr>
                        <td style="text-align:right;" title="total" colspan="8"><b>Total</b></td>
