...
if($db == ''){
    ...
    $form_path = "broiler_salesreturn.php";
}
else{
    $user_code = $_GET['userid'];
    include "APIconfig.php";
    include "header_head.php";
    $form_path = "broiler_salesreturn.php?db=$db&userid=".$user_code;
}

$file_name = "Sales Return";
    ...
    <body align="center">
        ...
        <table id="main_table" class="tbl" align="center">
            ...
            <tbody class="tbody1">
                ...
                $sql = "SELECT * FROM `broiler_itemreturns` WHERE `date` >= '$fdate' AND `date` <= '$tdate'".$vendor_filter."".$item_filter."".$sector_filter." AND `type` = 'Customer' AND `rtype` = 'SalesReturn' AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC"; $query = mysqli_query($conn,$sql);
                while($row = mysqli_fetch_assoc($query)){
                ?>
                <tr>
                    <td><?php echo date("d.m.Y",strtotime($row['date'])); ?></td>
                    <td><?php echo $row['trnum']; ?></td>
                    <td><?php echo $vendor_ccode[$row['vcode']]; ?></td>
