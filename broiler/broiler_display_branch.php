<?php
//broiler_display_branch.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['loc_branch'] = $cid; } else{ $cid = $_SESSION['loc_branch']; }
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                           <div class="float-left"><h3 class="card-title">Branch</h3></div>
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        while($row = mysqli_fetch_assoc($query)){
                                            ...
                                            $nisansync_url = "broiler_nisansync_branch.php?utype=authorize&name=".$row['description']."&code=".$row['code'];
                                            if($row['active'] == 1){
        ...
        <script>
			function checkdelete(x){
                ...
                if(a != ""){
                    ...
                    var method = "GET";
                    var url = "broiler_check_branch.php?id="+a;
                    //window.open(url);
                    ...
                    inv_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(parseFloat(count) > 0){
                                alert("You can't delete the Branch: "+val+", as Branch is already in use!");
                            }
                            else{
                                var b = "<?php echo $delete_url; ?>"+a;
                                var c = confirm("are you sure you want to delete the Branch: "+val+"?");
                                if(c == true){
