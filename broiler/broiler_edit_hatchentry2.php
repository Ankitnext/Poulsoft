...
if($link_active_flag > 0){
    ...
    if($acount == 1){
        ...
        $sql = "SELECT * FROM `item_details` WHERE `category` IN ('$item_cat') AND `active` = '1' AND `dflag` = '0'";
        $query = mysqli_query($conn,$sql); $reject_code = $reject_name = $item_name = array();
        while($row = mysqli_fetch_assoc($query)){ $reject_code[$row['code']] = $row['code']; $reject_name[$row['code']] = $row['description']; $item_name[$row['code']] = $row['description']; }
...
<html lang="en">
    ...
    <body class="m-0 p-0 hold-transition">
        <?php
        $ids = $_GET['trnum'];
        $sql = "SELECT * FROM `broiler_hatchentry` WHERE `trnum` = '$ids' AND `dflag` = '0'";
        $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($query)){
            $link_trnum = $row['link_trnum'];
            $hatchery_code = $row['sector_code'];
            ...
            $nof_egg_set = round($row['nof_egg_set'],5);
            $hatch = round($row['hatch'],5);
            $hatch_per = round($row['hatch_per'],5);
            $deathin_hatch_per = round($row['deathin_hatch_per'],5);
            $deathin_hatch_nos = round($row['deathin_hatch_nos'],5);
            $culls = round($row['culls'],5);
            $culls_per = round($row['culls_per'],5);
            $saleable_chicks = round($row['saleable_chicks'],5);
            $saleable_chicks_per = round($row['saleable_chicks_per'],5);
            $avg_chick_weight = round($row['avg_chick_weight'],5);
            $avg_egg_price = round($row['avg_egg_price'],5);
            $avg_egg_amount = round($row['avg_egg_amount'],5);
            $avg_chick_price = round($row['avg_chick_price'],5);
            $avg_chick_amount = round($row['avg_chick_amount'],5);
            $remarks = $row['remarks'];

            $infertile_nos = round($row['infertile_nos'],5);
            $infertile_per = round($row['infertile_per'],5);

            $eci_nos = round($row['eci_nos'],5);
            $eci_per = round($row['eci_per'],5);

            $blasting_nos = round($row['blasting_nos'],5);
            $blasting_per = round($row['blasting_per'],5);
        }
        ?>
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                            <div class="float-left"><h3 class="card-title"> Edit Hatch Entry</h3></div>
                        </div>
                        <div class="m-0 p-2 card-body">
                            <div class="col-md-18">
                                <form action="broiler_modify_hatchentry2.php" method="post" role="form" onsubmit="return checkval()">
                                    ...
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group" style="width:220px;">
                                            ...
                                            <select name="warehouse" id="warehouse" class="form-control select2" style="width:210px;" onchange="fetch_trayset_trnums();">
                                                ...
                                                <?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($hatchery_code == $scode){ echo "selected"; } ?>><?php echo $sector_name[$scode]; ?></option><?php } ?>
                                            </select>
                                        </div>
                                        <div class="form-group" style="width:220px;">
                                            <label>Transaction No.<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="trnum" id="trnum" class="form-control" value="<?php echo $ids; ?>" style="width:210px;" readonly >
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group" style="width:220px;">
                                            <label>Tray Set No.<b style="color:red;">&nbsp;*</b></label>
                                            <select name="link_trnum" id="link_trnum" class="form-control select2" style="width:210px;" onchange="fetch_tray_setting_details();fetch_hatchery_expenses();">
                                            <option value="<?php echo $link_trnum; ?>" selected><?php echo $link_trnum; ?></option>
                                            </select>
                                        </div>
                                        <div class="form-group" style="width:220px;">
                                            <label>Supplier<b style="color:red;">&nbsp;*</b></label>
                                            <select name="vcode" id="vcode" class="form-control select2" style="width:210px;">
                                    ...
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="hatch_date" id="hatch_date" class="form-control" value="<?php echo date('d.m.Y',strtotime($hatch_date)); ?>" style="width:110px;" readonly >
                                        </div>
                                        <div class="form-group">
                                            <label>Transfer Date<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="transfer_date" id="transfer_date" class="form-control" value="<?php echo date('d.m.Y',strtotime($transfer_date)); ?>" style="width:110px;" readonly >
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            <label>Setting Date<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="setting_date" id="setting_date" class="form-control" value="<?php echo date('d.m.Y',strtotime($setting_date)); ?>" style="width:110px;" readonly >
                                        </div>
                                        <div class="form-group">
                                            <label>No. of Eggs Set<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="nof_egg_set" id="nof_egg_set" class="form-control text-right" value="<?php echo $nof_egg_set; ?>" style="width:110px;" readonly >
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group" style="width:210px;">
                                            <label>Item<b style="color:red;">&nbsp;*</b></label>
                                            <select name="item_code" id="item_code" class="form-control select2" style="width:200px;">
                                                <option value="<?php echo $item_code; ?>" selected><?php echo $item_name[$item_code]; ?></option>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Avg.Prc<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="avg_price" id="avg_price" class="form-control text-right" value="<?php echo $avg_egg_price; ?>" style="width:110px;" readonly >
                                        </div>
                                        <div class="form-group">
                                            <label>Avg.Amt<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="avg_amount" id="avg_amount" class="form-control text-right" value="<?php echo $avg_egg_amount; ?>" style="width:110px;" readonly >
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group bg-primary" style="width:440px; text-align:center;">
                                    ...
                                    <div class="row justify-content-center align-items-center">
                                        <table>
                                            ...
                                            <tbody id="row_body2">
                                                <?php
                                                $sql = "SELECT * FROM `broiler_hatchery_rejectitems` WHERE `link_trnum` = '$ids' AND `dflag` = '0'";
                                                $query = mysqli_query($conn,$sql); $rcount = mysqli_num_rows($query); $c = 0; $i = $rcount - 1;
                                                if($rcount > 0){
                                                    while($row = mysqli_fetch_assoc($query)){
                                                    ?>
                                                    <tr id="row_no2[<?php echo $c; ?>]">
                                                        <td><select name="rejection_type[]" id="rejection_type[<?php echo $c; ?>]" class="form-control select2" style="width:210px;" ><option value="select">select</option><?php foreach($reject_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($row['rejection_type'] == $scode){ echo "selected"; } ?>><?php echo $reject_name[$scode]; ?></option><?php } ?></select></td>
                                                        <td><input type="text" name="reject_egg_nos[]" id="reject_egg_nos[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $row['reject_egg_nos']; ?>" style="width:110px;" onkeyup="calculate_row_total()" /></td>
                                                        <td><input type="text" name="reject_egg_per[]" id="reject_egg_per[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $row['reject_egg_per']; ?>" style="width:110px;" readonly /></td>
                                                        <td><input type="checkbox" name="reject_egg_stk[]" id="reject_egg_stk[<?php echo $c; ?>]" <?php if($row['reject_egg_stk'] == "1"){ echo "checked"; } ?> onchange="update_chkbox_val(this.id);"><input type="text" name="stk_val[]" id="stk_val[<?php echo $c; ?>]" value="<?php if($row['reject_egg_stk'] == "1"){ echo "1"; } else{ echo "0"; } ?>" style="width:10px;visibility:hidden;" readonly /></td>
                                                        <?php
                                                        if($c == $i){ echo '<td id="action2['.$c.']" style="padding-top: 5px;visibility:visible;">'; }
                                    ...
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="hatch_nos" id="hatch_nos" class="form-control text-right" value="<?php echo $hatch; ?>" style="width:110px;" readonly />
                                        </div>
                                        <div class="form-group">
                                            <label>Hatch%<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="hatch_per" id="hatch_per" class="form-control text-right" value="<?php echo $hatch_per; ?>" style="width:110px;" readonly />
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            <label>Death in Hatch<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="deathin_hatch_nos" id="deathin_hatch_nos" class="form-control text-right" value="<?php echo $deathin_hatch_nos; ?>" style="width:110px;" onkeyup="calculate_row_total()" />
                                        </div>
                                        <div class="form-group">
                                            <label>Death in Hatch%<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="deathin_hatch_per" id="deathin_hatch_per" class="form-control text-right" value="<?php echo $deathin_hatch_per; ?>" style="width:110px;" readonly />
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            <label>Culls<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="culls_nos" id="culls_nos" class="form-control text-right" value="<?php echo $culls; ?>" style="width:110px;" onkeyup="calculate_row_total()" />
                                        </div>
                                        <div class="form-group">
                                            <label>Culls%<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="culls_per" id="culls_per" class="form-control text-right" value="<?php echo $culls_per; ?>" style="width:110px;" readonly />
                                        </div>
                                    </div>
                                    ...
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="infertile_nos" id="infertile_nos" class="form-control text-right" style="width:110px; text-right" value="<?php echo $infertile_nos;  ?>" onkeyup="calculate_row_total()" />
                                        </div>
                                        <div class="form-group">
                                            <label>Infertile Egg%</label>
                                            <input type="text" name="infertile_per" id="infertile_per" class="form-control text-right" style="width:110px; text-right" value="<?php echo $infertile_per;  ?>" readonly />
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            ...
                                            <label>E.C.I</label>
                                            <?php } ?>
                                            <input type="text" name="eci_nos" id="eci_nos" class="form-control text-right" style="width:110px; text-right" value="<?php echo $eci_nos;  ?>" onkeyup="calculate_row_total()" />
                                        </div>
                                        <div class="form-group">
                                             <?php if($ecm_flag > 0){ ?>
                                            <label>E.C.M%</label>
                                            ...
                                            <label>E.C.I%</label>
                                            <?php } ?>
                                            <input type="text" name="eci_per" id="eci_per" class="form-control text-right" style="width:110px; text-right" value="<?php echo $eci_per;  ?>" readonly />
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            <label>Blasting Egg</label>
                                            <input type="text" name="blasting_nos" id="blasting_nos" class="form-control text-right" style="width:110px; text-right" value="<?php echo $blasting_nos;  ?>" onkeyup="calculate_row_total()" />
                                        </div>
                                        <div class="form-group">
                                            <label>Blasting Egg%</label>
                                            <input type="text" name="blasting_per" id="blasting_per" class="form-control text-right" style="width:110px; text-right" value="<?php echo $blasting_per;  ?>" readonly />
                                        </div>
                                    </div>
                                    ...
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="salable_chick_nos" id="salable_chick_nos" class="form-control text-right" value="<?php echo $saleable_chicks; ?>" style="width:110px;" readonly />
                                        </div>
                                        <div class="form-group">
                                            <label>salable Chicks%<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="salable_chick_per" id="salable_chick_per" class="form-control text-right" value="<?php echo $saleable_chicks_per; ?>" style="width:110px;" readonly />
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group">
                                            <label>Avg. Chick Weight<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="avg_chick_weight" id="avg_chick_weight" class="form-control text-right" value="<?php echo $avg_chick_weight; ?>" style="width:110px;" />
                                        </div>
                                    </div>
                                           ...
                                           <table class="w-80" style="width:70%;">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                       ...
                                                       <td><input type="text" name="total_chicks" id="total_chicks" class="form-control text-right" style="width:110px;" onkeyup="validatenum(this.id);" readonly /></td>
                                                       <td><input type="text" name="chicks_rate" id="chicks_rate" class="form-control text-right" style="width:110px;" onkeyup="validatenum(this.id);" readonly /></td>
                                                       <td><input type="text" name="chicks_amount" id="chicks_amount" class="form-control text-right" style="width:110px;" onkeyup="validatenum(this.id);" readonly /></td>
                                                    </tr>
                                                </tbody>
                                                ...
                                                <tbody id="row_body">
                                                    ...
                                                    <?php
                                                    $sql = "SELECT * FROM `broiler_hatchery_consumed` WHERE `link_trnum` = '$ids' AND `dflag` = '0'";
                                                    $query = mysqli_query($conn,$sql); $vcount = mysqli_num_rows($query); $c = 0; $i = $vcount - 1;
                                                    if($vcount > 0){
                                                        while($row = mysqli_fetch_assoc($query)){
                                                    ?>
                                                    <tr id="row_no[<?php echo $c; ?>]">
                                                        <td>
                                                            <table>
                                                                <tr>
                                                                    <td>
                                                                        <select name="vaccine_code[]" id="vaccine_code[<?php echo $c; ?>]" class="form-control select2" style="width:300px;border:1px solid #ccc;" onchange="fetch_stock_master(this.id);calculate_vaccine_amount(this.id);">
                                                                            <option value="select">select</option><?php foreach($medvac_code as $mcode){ ?><option value="<?php echo $mcode; ?>" <?php if($row['item_code'] == $mcode){ echo "selected"; } ?>><?php echo $medvac_name[$mcode]; ?></option><?php } ?>
                                                                        </select>
                                                                    </td>
                                                                    <?php
                                                                    if($c == $i){ echo '<td id="action['.$c.']" style="padding-top: 5px;width:70px;visibility:visible;"><br class="labelrow" style="display:none;" />'; }
                                                                    else{ echo '<td id="action['.$c.']" style="padding-top: 5px;width:70px;visibility:hidden;"><br class="labelrow" style="display:none;" />'; }
                                                        ...
                                                        <td><input type="text" name="vaccine_qty[]" id="vaccine_qty[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo round($row['quantity'],5); ?>" style="width:110px;" onkeyup="validatenum(this.id);calculate_vaccine_amount(this.id);" onchange="validateamount(this.id);" /></td>
                                                        <td><input type="text" name="vaccine_rate[]" id="vaccine_rate[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo round($row['rate'],5); ?>" style="width:110px;" readonly /></td>
                                                        <td><input type="text" name="vaccine_amount[]" id="vaccine_amount[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo round($row['amount'],5); ?>" style="width:110px;" readonly /></td>
                                                    </tr>
                                                    <?php
                                                        $c++;
                                                        }
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><input type="text" name="vaccine_qty[]" id="vaccine_qty[0]" class="form-control text-right" style="width:110px;" onkeyup="validatenum(this.id);calculate_vaccine_amount(this.id);" onchange="validateamount(this.id);" /></td>
                                                ...
                                                <tbody>
                                                    <tr>
                                                       ...
                                                       <td></td>
                                                       <td><input type="text" name="avg_chick_price" id="avg_chick_price" class="form-control text-right" style="width:110px;" onkeyup="validatenum(this.id);" readonly /></td>
                                                       <td><input type="text" name="avg_chick_amount" id="avg_chick_amount" class="form-control text-right" style="width:110px;" onkeyup="validatenum(this.id);" readonly /></td>
                                                    </tr>
                                              </tbody>
                                         </table>
                                       </div>
                                    ...
                                    <div class="row" style="margin-bottom:3px;">
                                        ...
                                        <div class="col-md-4 form-group">
                                            <label>Remarks</label>
                                            <textarea name="remarks" id="remarks" class="form-control" style="height:55px;"><?php echo $remarks; ?></textarea>
                                        </div>
                                        <div class="col-md-4 form-group"></div>
                                    </div>
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-1" style="visibility:hidden;">
                                            ...
                                            <input type="text" name="vincr" id="vincr" class="form-control" value="<?php echo $vcount - 1; ?>">
                                        </div>
                                        <div class="form-group col-md-1" style="visibility:hidden;">
                                            <label>R-incr<b style="color:red;">&ensp;*</b></label>
                                            <input type="text" name="rincr" id="rincr" class="form-control" value="<?php echo $rcount - 1; ?>">
                                        </div>
        ...
        <script>
            ...
            function create_row(a){
                ...
                html += '</tr></table></td>';
                html += '<td><input type="text" name="vaccine_qty[]" id="vaccine_qty['+d+']" class="form-control" style="width:110px;" onkeyup="validatenum(this.id);calculate_vaccine_amount(this.id);" onchange="validateamount(this.id);" /></td>';
            ...
            function fetch_hatchery_expenses(){
                ...
                var inv_items = new XMLHttpRequest();
				var method = "GET";
				var url = "broiler_fetch_hatchery_expenses2.php?warehouse="+warehouse+"&trnum="+trnum+"&type=edit";
                //window.open(url);
				var asynchronous = true;
				inv_items.open(method, url, asynchronous);
				inv_items.send();
