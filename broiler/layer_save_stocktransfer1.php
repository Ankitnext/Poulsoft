...
for($i = 0;$i < $dsize;$i++){
    ...
    if(!mysqli_query($conn,$sql)){ die("Error 2:-".mysqli_error($conn)); }
        ...
        VALUES ('CR','$coa_Cr','$date[$i]','$trnum','$code[$i]','$quantity[$i]','$price[$i]','$amount','$fromwarehouse[$i]','$remarks[$i]','0','layer-Stock Transfer Out','$flag','$active','$dflag','$addedemp','$addedtime','$addedtime')";
