...
for($i = 0;$i < $ssize;$i++){
    ...
    $sql = "INSERT INTO `broiler_feed_formula` (incr,prefix,code,formula_item_code,date,mill_code,item_code,unit_code,item_qty,total_qty,rate,total_rate,amount,total_amt,flag,active,dflag,addedemp,addedtime,updatedtime) VALUES ('$incr','$prefix','$code','$formula_item_code','$date','$mill_code','$item_code[$i]','$unit_code[$i]','$item_qty[$i]','$total_qty','$rate[$i]','$total_rate','$amount[$i]','$total_amt','0','1','0','$addedemp','$addedtime','$addedtime')";
    if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); } else { }
}
