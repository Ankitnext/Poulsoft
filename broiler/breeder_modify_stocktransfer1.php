...
if(!mysqli_query($conn,$sql)){ die("Error 2:-".mysqli_error($conn)); }
    ...
    VALUES ('CR','$coa_Cr','$date','$trnum','$code','$quantity','$price','$amount','$fromwarehouse','$remarks','0','Breeder-Stock Transfer Out','$flag','$active','$dflag','$aemp','$atime','$addedemp','$addedtime')";
    if(!mysqli_query($conn,$from_post)){ die("Error 3:-".mysqli_error($conn)); } else{ }
    $from_post = "INSERT INTO `account_summary` (`crdr`,`coa_code`,`date`,`trnum`,`item_code`,`quantity`,`price`,`amount`,`location`,`remarks`,`gc_flag`,`etype`,`flag`,`active`,`dflag`,`addedemp`,`addedtime`,`updatedemp`,`updatedtime`) 
    VALUES ('DR','$coa_Dr','$date','$trnum','$code','$quantity','$price','$amount','$towarehouse','$remarks','0','Breeder-Stock Transfer In','$flag','$active','$dflag','$aemp','$atime','$addedemp','$addedtime')";
    if(!mysqli_query($conn,$from_post)){ die("Error 3:-".mysqli_error($conn)); } else{ }
}
?>
<script>
