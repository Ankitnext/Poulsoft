<?php
//broiler_display_feedsale5.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['feedsale5'] = $cid; } else{ $cid = $_SESSION['feedsale5']; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            if(in_array("nof_bags", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_sales` ADD `nof_bags` decimal(20,2) NOT NULL DEFAULT '0' AFTER `icode`"; mysqli_query($conn,$sql); }
            if(in_array("mrp_prc", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_sales` ADD `mrp_prc` decimal(20,2) NOT NULL DEFAULT '0' AFTER `rcd_qty`"; mysqli_query($conn,$sql); }
            if(in_array("dis_pper", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_sales` ADD `dis_pper` decimal(20,2) NOT NULL DEFAULT '0' AFTER `mrp_prc`"; mysqli_query($conn,$sql); }
            if(in_array("dis_pamt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_sales` ADD `dis_pamt` decimal(20,2) NOT NULL DEFAULT '0' AFTER `dis_pper`"; mysqli_query($conn,$sql); }
            if(in_array("bag_rate", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_sales` ADD `bag_rate` decimal(20,2) NOT NULL DEFAULT '0' AFTER `dis_pamt`"; mysqli_query($conn,$sql); }
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM `broiler_sales` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `dflag` = '0' AND `trlink` LIKE '$href' ORDER BY `date`,`trnum` ASC";
