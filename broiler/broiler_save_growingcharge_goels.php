<?php
//broiler_save_growingcharge_goels.php
session_start(); include "newConfig.php";
$dbname = $_SESSION['dbase'];
$addedemp = $_SESSION['userid'];
date_default_timezone_set("Asia/Kolkata");
$addedtime = date('Y-m-d H:i:s');
$ccid = $_SESSION['growingcharge_goels'];
...
if(in_array("sales_incentive_avgwt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `sales_incentive_avgwt` double(20,2) NOT NULL DEFAULT '0' COMMENT 'Sale Incentive calculation on Body Weight' AFTER `grow_charge_exp_amt`"; mysqli_query($conn,$sql); }
if(in_array("avgwt_incentive_amt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `avgwt_incentive_amt` double(20,2) NOT NULL DEFAULT '0' COMMENT 'Avg. Weight Incentive calculation amount' AFTER `sales_incentive_avgwt`"; mysqli_query($conn,$sql); }
if(in_array("unload_incentive_prc", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `unload_incentive_prc` double(20,2) NOT NULL DEFAULT '0' COMMENT 'Unloading Charge Incentive Price' AFTER `ifft_charges`"; mysqli_query($conn,$sql); }
if(in_array("unload_incentive_amt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `unload_incentive_amt` double(20,2) NOT NULL DEFAULT '0' COMMENT 'Unloading Charge Incentive Amount' AFTER `unload_incentive_prc`"; mysqli_query($conn,$sql); }
...
if(in_array("high_feedin_sup_code", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `high_feedin_sup_code` VARCHAR(200) NULL DEFAULT NULL COMMENT 'Highest Feed Stock-In Supplier Code' AFTER `grade`"; mysqli_query($conn,$sql); }
if(in_array("high_feedin_brand_code", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `high_feedin_brand_code` VARCHAR(200) NULL DEFAULT NULL COMMENT 'Highest Feed Stock-In Brand Code' AFTER `high_feedin_sup_code`"; mysqli_query($conn,$sql); }
if(in_array("high_chickin_secvcode", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `high_chickin_secvcode` VARCHAR(200) NULL DEFAULT NULL COMMENT 'Highest Chick Stock-In Supplier/Sector Code' AFTER `high_feedin_brand_code`"; mysqli_query($conn,$sql); }
...
$sql = "SELECT * FROM `acc_coa` WHERE `description` = 'Farmer TDS A/c'"; $query = mysqli_query($conn,$sql);
while($row = mysqli_fetch_assoc($query)){ $fmrtds_code = $row['code']; }
...
$eef = $_POST['eef'];
$grade = $_POST['grade'];
$high_feedin_sup_code = $_POST['high_feedin_sup_code'];
$high_feedin_brand_code = $_POST['high_feedin_brand_code'];
$high_chickin_secvcode = $_POST['high_chickin_secvcode'];
...
$sales_incentive_avgwt = $_POST['sales_incentive_avgwt'];
$avgwt_incentive_amt = $_POST['avgwt_incentive_amt'];
...
$unload_incentive_prc = $_POST['unload_incentive_prc'];
$unload_incentive_amt = $_POST['unload_incentive_amt'];
...
while($row = mysqli_fetch_assoc($query)){ $gcgenerate = $row['gcgenerate']; } $incr = $gcgenerate + 1;

$sql = "UPDATE `master_generator` SET `gcgenerate` = '$incr' WHERE `fdate` <='$date' AND `tdate` >= '$date' AND `type` = 'transactions'";
if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); } else { }

$sql = "SELECT * FROM `prefix_master` WHERE `transaction_type` LIKE 'gcgenerate' AND `active` = '1'"; $query = mysqli_query($conn,$sql);
...
//if($grade == "" || $grade == NULL || $grade == 0){ $grade = "0.00"; }
...
if($sales_incentive_avgwt == "" || $sales_incentive_avgwt == NULL || $sales_incentive_avgwt == 0){ $sales_incentive_avgwt = "0.00"; }
if($avgwt_incentive_amt == "" || $avgwt_incentive_amt == NULL || $avgwt_incentive_amt == 0){ $avgwt_incentive_amt = "0.00"; }
...
if($unload_incentive_prc == "" || $unload_incentive_prc == NULL || $unload_incentive_prc == 0){ $unload_incentive_prc = "0.00"; }
if($unload_incentive_amt == "" || $unload_incentive_amt == NULL || $unload_incentive_amt == 0){ $unload_incentive_amt = "0.00"; }
...
if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); }
    ...
    $from_post = "INSERT INTO `account_summary` (crdr,coa_code,date,trnum,item_code,quantity,price,amount,location,batch,vehicle_code,driver_code,remarks,gc_flag,flag,active,dflag,etype,addedemp,addedtime,updatedtime) 
    VALUES ('CR','$coa_code','$date','$trnum','$chick_code','$sold_birds','$actual_chick_price','$actual_chick_cost','$farm_code','$batch_code',NULL,NULL,NULL,'1','0','1','0','GC Chick-Bird','$addedemp','$addedtime','$addedtime')";
    ...
    $from_post = "INSERT INTO `account_summary` (crdr,coa_code,date,trnum,vendor,quantity,price,amount,location,batch,vehicle_code,driver_code,remarks,gc_flag,flag,active,dflag,etype,addedemp,addedtime,updatedtime) 
    VALUES ('CR','$coa_code','$date','$trnum','$farmer_code','0','0','$fpay','$farm_code','$batch_code',NULL,NULL,NULL,'1','0','1','0','GC-FarmerPayGeneration','$addedemp','$addedtime','$addedtime')";
    if(!mysqli_query($conn,$from_post)){ die("Error:-".mysqli_error($conn)); } else{ }

    if((float)$tds_amt != 0 || $tds_amt != ""){
        $from_post = "INSERT INTO `account_summary` (crdr,coa_code,date,trnum,vendor,quantity,price,amount,location,batch,vehicle_code,driver_code,remarks,gc_flag,flag,active,dflag,etype,addedemp,addedtime,updatedtime) 
        VALUES ('CR','$fmrtds_code','$date','$trnum','$farmer_code','0','0','$tds_amt','$farm_code','$batch_code',NULL,NULL,NULL,'1','0','1','0','GC-FarmerTDSGeneration','$addedemp','$addedtime','$addedtime')";
        if(!mysqli_query($conn,$from_post)){ die("Error:-".mysqli_error($conn)); } else{ }
        $from_post = "INSERT INTO `account_summary` (crdr,coa_code,date,trnum,vendor,quantity,price,amount,location,batch,vehicle_code,driver_code,remarks,gc_flag,flag,active,dflag,etype,addedemp,addedtime,updatedtime) 
        VALUES ('DR','$coa_code','$date','$trnum','$farmer_code','0','0','$tds_amt','$farm_code','$batch_code',NULL,NULL,NULL,'1','0','1','0','GC-FarmerTDS','$addedemp','$addedtime','$addedtime')";
        if(!mysqli_query($conn,$from_post)){ die("Error:-".mysqli_error($conn)); } else{ }
    }
    ...
    foreach($active_wip_coas as $acoas){
        ...
        $from_post = "INSERT INTO `account_summary` (crdr,coa_code,date,trnum,item_code,quantity,price,amount,location,batch,vehicle_code,driver_code,remarks,gc_flag,flag,active,dflag,addedemp,addedtime,updatedtime) 
        VALUES ('CR','$coa_code','$date','$trnum','$item_code','0','0','$wip_amount','$farm_code','$batch_code',NULL,NULL,NULL,'1','0','1','0','$addedemp','$addedtime','$addedtime')";
        if(!mysqli_query($conn,$from_post)){ die("Error:-".mysqli_error($conn)); } else{ }
    }
    
    //Broiler Bird Production
    $coa_code = ""; $coa_code = $icat_iac[$bird_cat];
    $mgmt_bird_amount = (float)$actual_chick_cost + (float)$actual_feed_cost + (float)$mgmt_admin_amt + (float)$actual_medicine_cost + (float)$farmer_payable + (float)$tds_amt;
    $mgmt_bird_price = (float)$mgmt_bird_amount / (float)$sold_weight;
    $from_post = "INSERT INTO `account_summary` (crdr,coa_code,date,trnum,item_code,quantity,price,amount,location,batch,vehicle_code,driver_code,remarks,gc_flag,flag,active,dflag,etype,addedemp,addedtime,updatedtime) 
    VALUES ('DR','$coa_code','$date','$trnum','$bird_code','$sold_weight','$mgmt_bird_price','$mgmt_bird_amount','$farm_code','$batch_code',NULL,NULL,NULL,'1','0','1','0','GC-BirdProduction','$addedemp','$addedtime','$addedtime')";
    if(!mysqli_query($conn,$from_post)){ die("Error:-".mysqli_error($conn)); } else{ }

    //Update Previous Sale Entries
    $iac_coa = $icat_iac[$bird_cat]; $cogs_coa = $icat_cogsac[$bird_cat];
    $sql = "SELECT * FROM `account_summary` WHERE `coa_code` IN ('$iac_coa','$cogs_coa') AND `item_code` = '$bird_code' AND `location` = '$farm_code' AND `batch` = '$batch_code' AND `active` = '1' AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($query)){
        ...
        if($coa_code == $iac_coa){
            $sql2 = "UPDATE `account_summary` SET `crdr` = 'CR',`price` = '$mgmt_bird_price',`amount` = '$sale_amt' WHERE `id` = '$id' AND `trnum` NOT IN ('$trnum')"; mysqli_query($conn,$sql2);
        }
        else if($coa_code == $cogs_coa){
            $sql2 = "UPDATE `account_summary` SET `crdr` = 'DR',`price` = '$mgmt_bird_price',`amount` = '$sale_amt' WHERE `id` = '$id' AND `trnum` NOT IN ('$trnum')"; mysqli_query($conn,$sql2);
        }
    }
}
header('location:broiler_display_growingcharge_goels.php?ccid='.$ccid);
?>