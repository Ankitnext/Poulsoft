<?php
//layer_display_stocktransfer2.php
include "newConfig.php";
include "number_format_ind.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['stocktransfer2'] = $cid; } else{ $cid = $_SESSION['stocktransfer2']; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            if(in_array("lyr_sflag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `inv_sectors` ADD `lyr_sflag` INT(100) NOT NULL DEFAULT '0' COMMENT 'layer Sectors' AFTER `dflag`"; mysqli_query($conn,$sql); }
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM `item_stocktransfers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `dflag` = '0' AND `trtype` = 'stocktransfer2' AND `trlink` = 'layer_display_stocktransfer2.php' ORDER BY `id` DESC";
        ...
        <script>
			function checkdelete(x){
                ...
                if(trnum != ""){
                    ...
                    var method = "GET";
                    var url = "layer_check_stocktransfer2.php?trnum="+trnum;
                    //window.open(url);
