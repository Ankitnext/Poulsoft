                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM `layer_sheds` WHERE `dflag` = '0' AND `trtype` = 'shed1' AND `trlink` = 'layer_display_shed1.php' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['id'];
        ...
        <script>
			function checkdelete(x){
                ...
                if(id != ""){
                    ...
                    var method = "GET";
                    var url = "layer_check_shed1.php?id="+id+"&code="+code;
                    //window.open(url);
                    ...
                    inv_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(parseFloat(count) > 0){
                                alert("You can't delete the Shed: "+name+", as Shed is already in use!");
                            }
                            else{
                                var b = "<?php echo $delete_url; ?>"+id;
                                var c = confirm("are you sure you want to delete the Shed: "+name+"?");
                                if(c == true){
