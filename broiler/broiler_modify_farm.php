...
$id = $_POST['idvalue'];
$sql = "SELECT code  FROM `broiler_farm` WHERE id = '$id'";
...
$file_detail = "";
if($agreement_copy_path != ""){
    if($file_detail == ""){ $file_detail = ",`agreement_copy_path` = '".$agreement_copy_path."',`agreement_copy_name` = '".$agreement_copy_name."'"; }
    else{ $file_detail = $file_detail.",`agreement_copy_path` = '".$agreement_copy_path."',`agreement_copy_name` = '".$agreement_copy_name."'"; }
}
if($other_doc_path != ""){
    if($file_detail == ""){ $file_detail = ",`other_doc_path` = '".$other_doc_path."',`other_doc_name` = '".$other_doc_name."'"; }
    else{ $file_detail = $file_detail.",`other_doc_path` = '".$other_doc_path."',`other_doc_name` = '".$other_doc_name."'"; }
}
$sql='SHOW COLUMNS FROM `broiler_farm`'; $query=mysqli_query($conn,$sql); $existing_col_names = array(); $i = 0;
while($row = mysqli_fetch_assoc($query)){ $existing_col_names[$i] = $row['Field']; $i++; }
...
$sql = "UPDATE `broiler_farm` SET `farm_pincode`='$farm_pincode' , `bird_type` = '$bird_type'".$file_detail.",`farm_code` = '$farm_code',`description` = '$description',`region_code` = '$region_code',`branch_code` = '$branch_code',`line_code` = '$line_code',`supervisor_code` = '$supervisor_code',`farmer_code` = '$farmer_code',`farm_capacity` = '$farm_capacity',`farm_type` = '$farm_type',`farm_type2` = '$farm_type2',`state_code` = '$state_code',`district_name` = '$district_name',`area_name` = '$area_name',`farm_address` = '$farm_address',`agreement_months` = '$agreement_months',`security_cheque1` = '$security_cheque1',`security_cheque2` = '$security_cheque2',`security_cheque3` = '$security_cheque3',`security_cheque4` = '$security_cheque4',`remarks` = '$remarks',`flag` = '0',`active` = '1',`dflag` = '0',`updatedemp` = '$addedemp',`updatedtime` = '$addedtime' WHERE `id` = '$id'";
if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); }
else {
    mysqli_query($conn,"DELETE FROM `broiler_farm_shed_details` WHERE farm_code = '$code1' ");
    $dsize = sizeof($shed_no);
    for ($ab = 0; $ab < $dsize;$ab++){
        mysqli_query($conn,"INSERT INTO `broiler_farm_shed_details`( `farm_code`, `shed_no`, `shed_dimentions`, `shed_sqft`) VALUES ('$code1','$shed_no[$ab]','$shed_dimentions[$ab]','$shed_sqft[$ab]')");
    }
