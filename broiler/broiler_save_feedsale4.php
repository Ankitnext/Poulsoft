...
$sql = "SELECT *  FROM `extra_access` WHERE `field_name` LIKE 'SaleAutoWapp:broiler_display_feedsale4.php'";
...
$i = 0; foreach($_POST['mrp_amt'] as $mrp_amts){ $mrp_amt[$i]= $mrp_amts; $i++; }
...
$trno_dt1 = generate_transaction_details($date,"sales","SIN","generate",$_SESSION['dbase']);
...
for($i = 0;$i < $dsize;$i++){
    ...
    if($mrp_amt == "" || $mrp_amt == NULL || $mrp_amt == 0 || $mrp_amt == "0.00"){ $mrp_amt = "0.00"; }
    if($dis_per == "" || $dis_per == NULL || $dis_per == 0 || $dis_per == "0.00"){ $dis_per = "0.00"; }
    if($dis_amt == "" || $dis_amt == NULL || $dis_amt == 0 || $dis_amt == "0.00"){ $dis_amt = "0.00"; }
    ...
    $mrp_amt[$i] = floatval($mrp_amt[$i]);
    $dis_per[$i] = floatval($dis_per[$i]);
    $dis_amt[$i] = floatval($dis_amt[$i]);
    $final_total[$i] = floatval($final_total[$i]);
    $final_avg_total[$i] = floatval($final_avg_total[$i]);
...
header('location:broiler_display_feedsale4.php?ccid='.$ccid);
?>