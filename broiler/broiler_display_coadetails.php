<?php
//broiler_display_coadetails.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['coadetails'] = $cid; } else{ $cid = $_SESSION['coadetails']; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            if(in_array("broiler_openings", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.broiler_openings LIKE poulso6_admin_broiler_broilermaster.broiler_openings;"; mysqli_query($conn,$sql1); }
            
            //Fetch Column From CoA Table
            $sql='SHOW COLUMNS FROM `broiler_openings`'; $query=mysqli_query($conn,$sql); $existing_col_names = array(); $i = 0;
            while($row = mysqli_fetch_assoc($query)){ $existing_col_names[$i] = $row['Field']; $i++; }
            if(in_array('opn_type', $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_openings`  ADD `opn_type` VARCHAR(300) NULL DEFAULT NULL COMMENT ''  AFTER `type`;"; mysqli_query($conn,$sql); }
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                           <div class="float-left"><h3 class="card-title">CoA</h3></div>
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                       ...
                                       $sql = "SELECT * FROM `acc_schedules` WHERE `dflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql);
                                       while($row = mysqli_fetch_assoc($query)){
                                           $sname[$row['code']] = $row['description'];
                                       }
                                        
                                       $delete_url = $delete_link."?utype=delete&id=";
                                       $sql = "SELECT * FROM `acc_coa` WHERE `visible_flag` = '1' AND `dflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['id'];
        ...
        <script>
			function checkdelete(x){
                ...
                if(a != ""){
                    ...
                    var method = "GET";
                    var url = "broiler_check_coacodes.php?id="+a;
                    //window.open(url);
                    ...
                    inv_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(parseFloat(count) > 0){
                                alert("You can't delete the CoA: "+val+", as CoA is already in use!");
                            }
                            else{
                                var b = "<?php echo $delete_url; ?>"+a;
                                var c = confirm("are you sure you want to delete the CoA: "+val+"?");
                                if(c == true){
