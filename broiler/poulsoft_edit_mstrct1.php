<?php
//poulsoft_edit_mstrct1.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $ccid = $_SESSION['mstrct1'];
...
if($link_active_flag > 0){
    ...
    if($user_type == "S"){ $acount = 1; } else{ foreach($elink as $edit_access_flag){ if($edit_access_flag == $link_childid){ $acount = 1; } } }
    if($acount == 1){
        ...
        $query = mysqli_query($conn,$sql); $drange_flag = mysqli_num_rows($query); if($drange_flag <= 0){ $s_days = $e_days = 9999; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition">
        <?php
        $ids = $_GET['trnum'];
        $sql = "SELECT * FROM `master_receipts` WHERE `trnum` = '$ids' AND `dflag` = '0' AND `trlink` = 'poulsoft_display_mstrct1.php'";
        ...
        while($row = mysqli_fetch_assoc($query)){
            ...
            $v_type[$c] = $row['v_type'];
            $to_account[$c] = $row['to_account'];
            
            $amount[$c] = round($row['amount'],5);
            $pay_type[$c] = $row['pay_type'];
            $gc_amount[$c] = round($row['gc_amount'],5);
            $cheque_date[$c] = $row['cheque_date'];
            $ref_no[$c] = $row['ref_no'];
            $sector = $row['sector'];
            $remarks[$c] = $row['remarks'];

            //Fetch Farm Batch Info
            $farm_batch[$c] = '<option value="select">-select-</option>';
            if($row['farm_batch'] != "select" && $row['farm_batch'] != ""){
                $sql2 = "SELECT * FROM `broiler_batch` WHERE `farm_code` = '$to_account[$c]' AND `active` = '1' AND `dflag` = '0' ORDER BY `batch_no` ASC";
                $query2 = mysqli_query($conn,$sql2);
                while($row2 = mysqli_fetch_assoc($query2)){
                    $code = $row2['code']; $name = $row2['description'];
                    if($row['farm_batch'] == $code){ $farm_batch[$c] .= '<option value="'.$code.'" selected>'.$name.'</option>'; }
                    else{ $farm_batch[$c] .= '<option value="'.$code.'">'.$name.'</option>'; }
                }
            }
            $c++;
        } $incr = $c - 1;
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <!--<div class="card-header">
                            <div class="float-left"><h3 class="card-title">Edit Receipt</h3></div>
                        </div>-->
                        <div class="pl-2 card-body">
                            <form action="poulsoft_modify_mstrct1.php" method="post" role="form" onsubmit="return checkval()">
                                <div class="row row_body2">
                                    <div class="pl-3 row">
                                        ...
                                        <div class="form-group" style="width:200px;">
                                            ...
                                            <select name="sector" id="sector" class="form-control select2" style="width:190px;" onchange="fetch_balace_amt();"><option value="select">-select-</option><?php foreach($sector_code as $ucode){ ?><option value="<?php echo $ucode; ?>" <?php if($sector == $ucode){ echo "selected"; } ?>><?php echo $sector_name[$ucode]; ?></option><?php } ?></select>
                                        </div>
                                    </div>
                                    <table class="p-1 table1" style="width:auto;">
                                        <thead>
                                            ...
                                            <tr>
                                                <th colspan="7">
                                                    <div class="pt-1 row">
                                                        ...
                                                        <div class="form-group" style="width:300px;"><select name="from_account" id="from_account" class="form-control select2" style="width:290px;" onchange="fetch_balace_amt();"><option value="select">-select-</option><?php foreach($chbk_code as $ucode){ ?><option value="<?php echo $ucode; ?>" <?php if($from_account == $ucode){ echo "selected"; } ?>><?php echo $chbk_name[$ucode]; ?></option><?php } ?></select></div>
                                                    </div>
                                                </th>
                                            </tr>
                                            ...
                                            <tr>
                                                <th colspan="7" style="background-color:#d1ffe4;color:#00722f;text-align:center;">Receipt Details</th>
                                            </tr>
                                        </thead>
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody id="tbody">
                                            <?php for($c = 0;$c <= $incr;$c++){ ?>
                                            <tr style="line-height:30px;" id="row_no[<?php echo $c; ?>]">
                                                <td><select name="to_account[]" id="to_account[<?php echo $c; ?>]" class="form-control select2" style="width:290px;" onchange="fetch_party_type(this.id);"><option value="select">-select-</option><?php foreach($acc_code as $ucode){ ?><option value="<?php echo $ucode; ?>" <?php if($to_account[$c] == $ucode){ echo "selected"; } ?>><?php echo $acc_name[$ucode]; ?></option><?php } ?></select></td>
                                                <td><input type="text" name="amount[]" id="amount[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $amount[$c]; ?>" style="width:90px;" onkeyup="validatenum(this.id);cal_tot_amt(this.id);" onchange="validateamount(this.id);"/></td>
                                                <td><input type="text" name="ref_no[]" id="ref_no[<?php echo $c; ?>]" class="form-control" value="<?php echo $ref_no[$c]; ?>" style="width:90px;" onkeyup="validatename(this.id);" /></td>
                                                <td><textarea name="remarks[]" id="remarks[<?php echo $c; ?>]" class="form-control" style="padding:0;width:150px;height:28px;" onkeyup="validatename(this.id);"><?php echo $remarks[$c]; ?></textarea></td>
                                                <?php
                                                ...
                                                echo '</td>';
                                                ?>
                                                <td style="visibility:hidden;" id="c_date2[<?php echo $c; ?>]"><input type="text" name="cheque_date[]" id="cheque_date[<?php echo $c; ?>]" class="form-control rc_datepicker" value="<?php echo $cheque_date[$c]; ?>" style="width:120px;" /></td>
                                                <td style="visibility:hidden;" id="p_type2[<?php echo $c; ?>]"><select name="pay_type[]" id="pay_type[<?php echo $c; ?>]" class="form-control select2" style="width:110px;"><option value="gc_pay" <?php if($pay_type[$c] == "gc_pay"){ echo "selected"; } ?>>GC Pay</option><option value="adv_pay" <?php if($pay_type[$c] == "adv_pay"){ echo "selected"; } ?>>Advance Pay</option><option value="addition_pay" <?php if($pay_type[$c] == "addition_pay"){ echo "selected"; } ?>>Additional Pay</option></select></td>
                                                <td style="visibility:hidden;" id="b_type2[<?php echo $c; ?>]"><select name="farm_batch[]" id="farm_batch[<?php echo $c; ?>]" class="form-control select2" style="width:160px;" onchange="fetch_gc_amount(this.id);"><?php echo $farm_batch[$c]; ?></select></td>
                                                <td style="visibility:hidden;" id="g_type2[<?php echo $c; ?>]"><input type="text" name="gc_amount[]" id="gc_amount[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $gc_amount[$c]; ?>" style="width:100px;" readonly /></td>
                                                <td style="visibility:hidden;"><input type="text" name="v_type[]" id="v_type[<?php echo $c; ?>]" class="form-control" value="<?php echo $v_type[$c]; ?>" style="width:30px;" readonly /></td>
                                            </tr>
                                            <?php } ?>
                                        </tbody>
                                        <tfoot>
        ...
        <script>
            ...
            function return_back(){
                ...
                window.location.href = 'poulsoft_display_mstrct1.php?ccid='+ccid;
            }
            ...
            function fetch_balace_amt(){
                ...
                if(date == "" || sector == "" || sector == "select" || from_acc == "" || from_acc == "select"){ update_ebtn_status(0); upd_chk_date(); }
                    ...
                    var method = "GET";
                    var url = "poulsoft_fetch_chbk_balamt1.php?date="+date+"&sector="+sector+"&from_acc="+from_acc+"&trnum="+trnum+"&ttype=edit";
                    //window.open(url);
			...
			function fetch_party_type(a){
                ...
                if(from_acc != "select" && to_acc != "select" && from_acc == to_acc){
                    ...
                    var oldqty = new XMLHttpRequest();
                    var method = "GET";
                    var url = "poulsoft_fetch_partytype1.php?to_acc="+to_acc+"&sector="+sector+"&r_cnt="+d+"&ttype=edit";
                    //window.open(url);
                    var asynchronous = true;
                    oldqty.open(method, url, asynchronous);
                    oldqty.send();
            ...
            fetch_balace_amt(); upd_frmr_details(); cal_tot_amt();
