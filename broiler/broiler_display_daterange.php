<?php
//broiler_display_daterange.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['daterange'] = $cid; } else{ $cid = $_SESSION['daterange']; }
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                           <div class="float-left"><h3 class="card-title">Date Range</h3></div>
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM `dataentry_daterange_master` WHERE `dflag` = '0' ORDER BY `id` ASC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['id'];
                                            ...
                                            if($row['active'] == 1){ $update_url = $update_link."?utype=pause&id=".$id; } else{ $update_url = $update_link."?utype=activate&id=".$id; }
                                            */
                                    ?>
                                    <tr>
                                        <td><?php echo $emp_name[$row['user_code']]; ?></td>
                                        <td><?php echo $file_name[$row['file_name']]; ?></td>
