...
if($link_active_flag > 0){
    ...
    if($acount == 1){
		...
		$sql = "SELECT * FROM `acc_coa` WHERE `visible_flag` = '1' AND `active` = '1' AND `dflag` = '0' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($query)){ $coa_code[$row['code']] = $row['code']; $coa_name[$row['code']] = $row['description']; }

        $sql = "SELECT * FROM `main_contactdetails` WHERE `active` = '1' AND `dflag` = '0' ORDER BY `name` ASC"; $query = mysqli_query($conn,$sql);
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        ...
                        <div class="m-0 p-2 card-body">
                            <div class="col-md-18">
                                <form action="broiler_save_contraaccount.php" method="post" role="form" onSubmit="return checkval()">
                                    <div class="row">
                                    <table>
                                        <thead>
