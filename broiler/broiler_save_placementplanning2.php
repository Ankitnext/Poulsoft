<?php
//broiler_save_placementplanning2.php
...
if (in_array('chicks_place', $diff_array)) {
	$q = "ALTER TABLE `broiler_placementplan` ADD `chicks_place` VARCHAR(300) NULL DEFAULT NULL COMMENT 'Chicks Placement' AFTER `supervisor_code`;";
	$qr = mysqli_query($conn, $q);
}
...
// $chicks_place = $_POST['chicks_place'];
$to_date = $from_date = date("Y-m-d",strtotime($_POST['from_date']));
// $to_date = date("Y-m-d",strtotime($_POST['to_date']));
$i = 0; foreach($_POST['farm_code'] as $fcode){ $fmcode[$i] = $fcode; $i++; }
$i = 0; foreach($_POST['remarks'] as $remarkss){ $remarks[$i] = $remarkss; $i++; }
$i = 0; foreach($_POST['chicks_place'] as $chicks_places){ $chicks_place[$i] = $chicks_places; $i++; }
