...
$sql = "INSERT INTO `item_stocktransfers` (`incr`,`prefix`,`trnum`,`date`,`dcno`,`fromwarehouse`,`code`,`sent_qty`,`quantity`,`price`,`amount`,`towarehouse`,`remarks`,`flag`,`active`,`dflag`,`trtype`,`trlink`,`addedemp`,`addedtime`,`updatedemp`,`updatedtime`) 
VALUES('$incr','$prefix','$trnum','$date','$dcno','$fromwarehouse','$code','$quantity','0','$price','$amount','$towarehouse','$remarks','$flag','$active','$dflag','$trtype','$trlink','$aemp','$atime','$addedemp','$addedtime')";
if(!mysqli_query($conn,$sql)){ die("Error 2:-".mysqli_error($conn)); }
    ...
    VALUES ('CR','$coa_Cr','$date','$trnum','$code','$quantity','$price','$amount','$fromwarehouse','$remarks','0','Breeder-Send Stock','$flag','$active','$dflag','$aemp','$atime','$addedemp','$addedtime')";
    if(!mysqli_query($conn,$from_post)){ die("Error 3:-".mysqli_error($conn)); } else{ }
}
?>
<script>
    var a = '<?php echo $ccid; ?>';
    var x = confirm("Would you like send more Stocks?");
    if(x == true){
