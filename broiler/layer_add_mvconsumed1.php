...
if($link_active_flag > 0){
    ...
    if($acount == 1){
        ...
        $sql = "SELECT * FROM `item_category` WHERE `active` = '1' AND `lmv_flag` = '1' AND `dflag` = '0' ORDER BY `description` ASC";
    ...
    <body class="m-0 hold-transition">
            ...
            <section class="m-0 p-0 content">
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        ...
                        <div class="card-body">
                            <form action="layer_save_mvconsumed1.php" method="post" role="form" onsubmit="return checkval()">
                                <div class="row row_body2">
                                    <table class="p-1 table1" style="width:auto;">
                                        ...
                                        <tbody id="tbody">
                                            <tr>
                                                <td><select name="flock_code[]" id="flock_code[0]" class="form-control select2" style="width:190px;" onchange="layer_fetch_item_details(this.id);"><option value="select">-select-</option><?php foreach($bflk_code as $ucode){ ?><option value="<?php echo $ucode; ?>"><?php echo $bflk_name[$ucode]; ?></option><?php } ?></select></td>
                                                <td><select name="item_code[]" id="item_code[0]" class="form-control select2" style="width:190px;" onchange="update_item_details(this.id);fetch_item_uom(this.id);layer_fetch_item_details(this.id);"><option value="select">-select-</option><?php foreach($item_code as $ucode){ ?><option value="<?php echo $ucode; ?>"><?php echo $item_code[$ucode]; ?></option><?php } ?></select></td>
                                                <td><select name="item_code1[]" id="item_code1[0]" class="form-control select2" style="width:190px;" onchange="update_item_details(this.id);fetch_item_uom(this.id);layer_fetch_item_details(this.id);"><option value="select">-select-</option><?php foreach($item_code as $ucode){ ?><option value="<?php echo $ucode; ?>"><?php echo $item_name[$ucode]; ?></option><?php } ?></select></td>
        ...
        <script>
			...
			function create_row(a){
                ...
                html += '<tr id="row_no['+d+']">';
                html += '<td><select name="flock_code[]" id="flock_code['+d+']" class="form-control select2" style="width:190px;" onchange="layer_fetch_item_details(this.id);"><option value="select">-select-</option><?php foreach($bflk_code as $ucode){ ?><option value="<?php echo $ucode; ?>"><?php echo $bflk_name[$ucode]; ?></option><?php } ?></select></td>';
			...
			function layer_fetch_item_details(a){
                update_ebtn_status(1);
                var b = a.split("["); var c = b[1].split("]"); var d = c[0];
				...
				if(item_code != "select" && flock_code != "select"){
					...
					var method = "GET";
                    var url = "layer_fetch_avlstock_quantity.php?date="+date+"&flock_code="+flock_code+"&item_code="+item_code+"&rows="+d+"&itype=medvac&ftype=brd_mventry&ttype=add";
                    //window.open(url);
