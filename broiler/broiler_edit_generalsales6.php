<?php
//broiler_edit_generalsales6.php
...
<html lang="en">
    ...
    <body class="m-0 hold-transition">
        <?php
        $ids = $_GET['trnum'];
        $sql = "SELECT * FROM `broiler_sales` WHERE `trnum` = '$ids' AND (`sale_type` = 'generalsales6' OR `trlink` = 'broiler_display_generalsales6.php')";
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        ...
                        <div class="m-0 p-2 card-body">
                            <div class="col-md-18">
                                <form action="broiler_modify_generalsales6.php" method="post" role="form" enctype="multipart/form-data" onSubmit="return checkval()">
                                    <div class="row">
                                    ...
                                    <table>
                                        ...
                                        <?php $i = $c; for($c = 0;$c <= $i;$c++){ ?>
                                            <tr id="row_no[<?php echo $c; ?>]">
                                                <td><select name="icode[]" id="icode[<?php echo $c; ?>]" class="form-control select2" style="width:180px;" onchange="fetch_stock_master(this.id);fetch_customer_pricemaster(this.id);fetch_itemuom(this.id);"><option value="select">select</option><?php foreach($item_code as $icode){ ?><option value="<?php echo $icode; ?>" <?php if($icodes[$c] == $icode){ echo "selected"; } ?>><?php echo $item_name[$icode]; ?></option><?php } ?></select></td>
                                                <td><input type="text" name="uom[]" id="uom[<?php echo $c; ?>]" class="form-control" value="<?php echo $item_cunit[$icodes[$c]]; ?>" style="width:80px;" readonly /></td>
                                                ...
                                                <td><select name="warehouse[]" id="warehouse[<?php echo $c; ?>]" class="form-control select2" style="width:160px;"><option value="select">select</option><?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($warehouse[$c] == $scode){ echo "selected"; } ?>><?php echo $sector_name[$scode]; ?></option><?php } ?></select></td>
                                                <?php
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <?php 
                                             if(strlen($file_path1)>0){
                                                echo "<b>Existing:</b>".basename($file_path1);
                                             }
                                            ?>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                             ...
                                             if(strlen($file_path2)>0){
                                                echo "<b>Existing:</b>".basename($file_path2);
                                             }
                                            ?>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                             ...
                                             <?php 
                                             if(strlen($file_path3)>0){
                                                echo "<b>Existing:</b>".basename($file_path3);
                                             }
                                            ?>
                                        </div>
        ...
        <script>
            ...
            function create_row(a){
                ...
                html += '<td><select name="warehouse[]" id="warehouse['+d+']" class="form-control select2" style="width:160px;"><option value="select">select</option><?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($warehouse == $scode){ echo "selected"; } ?>><?php echo $sector_name[$scode]; ?></option><?php } ?></select></td>'
            ...
            function calculate_total_amt(a) {
                // Parse the element ID to extract index
                var b = a.split("["); 
                var c = b[1].split("]"); 
                var d = c[0]; // Extract index safely

                // Get field values
                var rcd_qty = document.getElementById("rcd_qty[" + d + "]")?.value || 0;
                var rate = document.getElementById("rate[" + d + "]")?.value || 0;
                ...
                // Apply discount
                var dis_amt = document.getElementById("dis_amt[" + d + "]")?.value || 0;
                var item_tamt = parseFloat(amount1) - parseFloat(dis_amt);

                // Handle GST
                var gst_amt = 0;
                var gst_val = document.getElementById("gst_val[" + d + "]")?.value || "select";
                if (gst_val != "select") {
                    var gst_val2 = gst_val.split("@");
                    var gst_per = parseFloat(gst_val2[1]) || 0;
                    gst_amt = ((gst_per / 100) * item_tamt);
                    document.getElementById("gst_amt[" + d + "]").value = gst_amt.toFixed(2);
                }
                item_tamt += gst_amt;
                ...
                var avg_price = document.getElementById("avg_price[" + d + "]")?.value || 0;
