<?php
//broiler_display_hatchentry2.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['hatchentry2'] = $cid; } else{ $cid = $_SESSION['hatchentry2']; }
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM `broiler_hatchery_hatcher`"; $query = mysqli_query($conn,$sql); $setter_name = array();
                                        while($row = mysqli_fetch_assoc($query)){ $hatcher_name[$row['code']] = $row['hatcher']; }

                                        $sql = "SELECT * FROM `broiler_hatchery_setter`"; $query = mysqli_query($conn,$sql); $setter_name = array();
                                        while($row = mysqli_fetch_assoc($query)){ $setter_name[$row['code']] = $row['setter_no']; }
                                        ...
                                        $sql = "SELECT * FROM `".$table_name."` WHERE `hatch_date` >= '$fdate' AND `hatch_date` <= '$tdate' AND `dflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['trnum'];
                                            ...
                                            $hentry_url = "broiler_add_hatcherychicksale_lsfi.php?utype=egr&trnum=".$id;
                                    ?>
                                    <tr>
										
                                        <td data-sort="<?= strtotime($row['setting_date']) ?>"><?= date('d.m.Y',strtotime($row['setting_date'])) ?></td>
                                        <td data-sort="<?= strtotime($row['hatch_date']) ?>"><?= date('d.m.Y',strtotime($row['hatch_date'])) ?></td>
                                        <td><?php echo $row['link_trnum']; ?></td>
										<td><?php echo $row['trnum']; ?></td>
										<td><?php echo $sector_name[$row['sector_code']]; ?></td>
										<td><?php echo $setter_name[$row['setter_no']]; ?></td>
										<td><?php echo $item_name[$row['item_code']]; ?></td>
										<td><?php echo $row['nof_egg_set']; ?></td>
										<td><?php echo $row['saleable_chicks']; ?></td>
										<td><?php echo round($row['avg_chick_amount'],2); ?></td>
                                        ...
                                        <?php
                                            if($row['flag'] == 1){
                                                ...
                                                echo "<a href='".$hentry_url."' target='_BLANK'><i class='fa fa-arrow-right' style='color:green;' title='Tray Setting'></i></a>&ensp;&ensp;";
                                            }
                                        ?>
                                        </td>
                                    </tr>
