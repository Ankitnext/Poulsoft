<?php
//broiler_display_line.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['loc_line'] = $cid; } else{ $cid = $_SESSION['loc_line']; }
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                           <div class="float-left"><h3 class="card-title">Line</h3></div>
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        $sql = "SELECT * FROM `location_region`"; $query = mysqli_query($conn,$sql);
                                        while($row = mysqli_fetch_assoc($query)){
                                       ...
                                       // $sql = "SELECT * FROM `location_branch` WHERE `dflag` = '0' ".$branch_access_filter1." ORDER BY `description` ASC";
                                        $sql = "SELECT * FROM `location_branch`"; $query = mysqli_query($conn,$sql);
                                        while($row = mysqli_fetch_assoc($query)){
                                        $branch_code[$row['code']] = $row['code']; $branch_name[$row['code']] = $row['description'];
                                        }
                                        // $branch_fltr = implode($branch_code); print_r($branch_fltr);
                                        $delete_url = $delete_link."?utype=delete&id=";
                                        $sql = "SELECT * FROM `location_line` WHERE `dflag` = '0'".$line_access_filter1."".$branch_access_filter2." ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['id'];
        ...
        <script>
			function checkdelete(x){
                ...
                if(a != ""){
                    ...
                    var method = "GET";
                    var url = "broiler_check_line.php?id="+a;
                    //window.open(url);
                    ...
                    inv_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(parseFloat(count) > 0){
                                alert("You can't delete the Line: "+val+", as Line is already in use!");
                            }
                            else{
                                var b = "<?php echo $delete_url; ?>"+a;
                                var c = confirm("are you sure you want to delete the Line: "+val+"?");
                                if(c == true){
