<?php
//layer_display_mvconsumed1.php
include "newConfig.php";
include "number_format_ind.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['mvconsumed1'] = $cid; } else{ $cid = $_SESSION['mvconsumed1']; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            while($row = mysqli_fetch_assoc($query)){ $existing_col_names[$i] = $row['Field']; $i++; }
            if(in_array("flock_code", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `account_summary` ADD `flock_code` VARCHAR(300) NULL DEFAULT NULL AFTER `batch`"; mysqli_query($conn,$sql); }
                    ...
                    <div class="m-0 p-0 card">
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        $sql = "SELECT * FROM `layer_shed_allocation` WHERE `dflag` = '0' AND `cls_flag` = '0' ORDER BY `description` ASC";
                                        ...
                                        $sql = "SELECT * FROM `layer_medicine_consumed` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `dflag` = '0' AND `trtype` = 'mvconsumed1' AND `trlink` = 'layer_display_mvconsumed1.php' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['trnum'];
                                            ...
                                            $val = ""; $val = $row['trnum']."@".$bflk_name[$row['flock_code']]."@".$item_name[$row['item_code']]."@".round($row['quantity'],5);
                                    ?>
                                    <tr>
										...
										<td><?php echo round($row['quantity'],5); ?></td>
        ...
        <script>
			function checkdelete(x){
                ...
                if(trnum != ""){
                    ...
                    var method = "GET";
                    var url = "layer_check_mvconsumed1.php?trnum="+trnum+"&flock="+flock+"&iname="+iname+"&iqty="+iqty;
                    //window.open(url);
                    ...
                    inv_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(parseFloat(count) > 0){
                                alert("You can't delete the Medicine/Vaccine Consumption with Flock: "+flock+", Item: "+iname+", Quantity: "+iqty+", as Medicine/Vaccine is already in use!");
                            }
                            else{
                                var b = "<?php echo $delete_url; ?>"+trnum;
                                var c = confirm("are you sure you want to delete the Medicine/Vaccine Consumption with Flock: "+flock+", Item: "+iname+", Quantity: "+iqty+"?");
                                if(c == true){
