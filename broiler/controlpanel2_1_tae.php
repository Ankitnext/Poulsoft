        ...
        <section class="content">
            <div class="container-fluid">
                <div class="content-header">
                    <div class="container-fluid">
                        <form action="controlpanel2_1_tae.php" method="post">
                            <!--<div class="row">
                                <div class="col-md-2">
                    ...
                    <div class="row">
                        ...
                        foreach($sorts as $sorting){
                            ...
                            if($sorting == "Customer Supplier Balance-List" && (int)$Customer_Supplier_Balance_details == 1){
                                ...
                                echo  $sql = "SELECT * FROM `main_groups` WHERE `gtype` LIKE '%C%' AND `dflag` = '0'".$group_access_filter1." ORDER BY `description` ASC";
                                $query = mysqli_query($conn,$sql);
                                while($row = mysqli_fetch_assoc($query)){
                                ...
                                $glist = implode("','",$vengrp_code);
                               echo $sql = "SELECT * FROM `main_contactdetails` WHERE `contacttype` LIKE 'C' AND `groupcode` IN ('$glist') AND `active` ='1' AND `dflag` = '0' ORDER BY `name` ASC"; $query = mysqli_query($conn,$sql);
                                while($row = mysqli_fetch_assoc($query)){ 
                                    ...
                                    // $jk += $obamt[$row['code']];
                                    $cus_code[$row['code']] = $row['code']; if($row['obtype'] == "Cr" || $row['obtype'] == "CR"){ $cus_obcramt += (float)$row['obamt']; } else{ $cus_obdramt += (float)$row['obamt']; } 
                                }
                            ...
                            //    echo $sql_record = "SELECT * FROM `broiler_sales` WHERE `date` <= '$today' AND `vcode` IN ('$cus_list') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
                            ...
                            //     if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ if($old_inv != $row['trnum']){ $cus_sales += (float)$row['finl_amt']; $old_inv = $row['trnum']; } } }
                             ...
                             $sql_record = "SELECT * FROM `broiler_sales` WHERE `date` <= '$today' AND `vcode` IN ('$cus_list') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
                                $query = mysqli_query($conn,$sql_record); $transaction_count = 0; if(!empty($query)){ $transaction_count = mysqli_num_rows($query); }
                                if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ if($old_inv != $row['trnum']){ $cus_sales += (float)$row['finl_amt']; $old_inv = $row['trnum']; } $cus_sales += (float)$row['customer_freight_amt'];  } }

                             $sql_record = "SELECT * FROM `broiler_sales` WHERE `date` >= '$today' AND `date` <= '$today' AND `vcode` IN ('$cus_list') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
                                $query = mysqli_query($conn,$sql_record); $transaction_count = 0; if(!empty($query)){ $transaction_count = mysqli_num_rows($query); }
                                if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ if($oldd_inv != $row['trnum']){ $bet_sales += (float)$row['finl_amt']; $oldd_inv = $row['trnum']; } $bet_sales += (float)$row['customer_freight_amt'];  } }
                                ...
                                $sql_record = "SELECT SUM(amount) as amount FROM `broiler_receipts` WHERE `date` >= '$today' AND `date` <= '$today' AND `ccode` IN ('$cus_list') AND `vtype` IN ('Customer') AND `active` = '1' AND `dflag` = '0'";
                                $query = mysqli_query($conn,$sql_record); $transaction_count = 0; if(!empty($query)){ $transaction_count = mysqli_num_rows($query); }
                                if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ $bet_receipts += (float)$row['amount']; } }
                                ...
                                $sql_record = "SELECT SUM(amount) as amount FROM `broiler_itemreturns` WHERE `date` >= '$today' AND `date` <= '$today' AND `vcode` IN ('$cus_list') AND `type` IN ('Customer') AND `active` = '1' AND `dflag` = '0'";
                                $query = mysqli_query($conn,$sql_record); $transaction_count = 0; if(!empty($query)){ $transaction_count = mysqli_num_rows($query); }
                                if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ $bet_returns += (float)$row['amount']; } }
                                ...
                                $sql_record = "SELECT SUM(amount) as amount,crdr FROM `broiler_crdrnote` WHERE `date` >= '$today' AND `date` <= '$today' AND `vcode` IN ('$cus_list') AND `type` IN ('Customer') AND `active` = '1' AND `dflag` = '0' GROUP BY `crdr` ORDER BY `crdr` ASC";
                                $query = mysqli_query($conn,$sql_record); $transaction_count = 0; if(!empty($query)){ $transaction_count = mysqli_num_rows($query); }
                                if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ if($row['crdr'] == "Credit"){ $bet_ccn += (float)$row['amount']; } else{ $bet_cdn += (float)$row['amount']; } } }
                                ...
                                $sql_record = "SELECT SUM(amount) as amount FROM `account_contranotes` WHERE `date` >= '$today' AND `date` <= '$today' AND `fcoa` IN ('$cus_list') AND `type` IN ('ContraNote') AND `active` = '1' AND `dflag` = '0'";
                                $query = mysqli_query($conn,$sql_record); $i = 0; $transaction_count = 0; if(!empty($query)){ $transaction_count = mysqli_num_rows($query); }
                                if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ $bet_contra_cr += (float)$row['amount']; } }
                                ...
                                $sql_record = "SELECT * FROM `master_payments` WHERE `date` < '$today' AND `to_account` IN ('$cus_list') AND `t_type` IN ('Customer Payment') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
                                $query = mysqli_query($conn, $sql_record); $transaction_count = 0; 
                                while($row = mysqli_fetch_assoc($query)){
                                     $opening_payments[$row['to_account']] += (float)$row['amount'];
                                     $pay = $opening_payments[$row['to_account']];
                                }
                                $customer_filter = implode("','",$vendor_code);
                                $sql_record = "SELECT * FROM `master_payments` WHERE `date` < '$today' AND `t_type` IN ('Customer Payment') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
                                $query = mysqli_query($conn, $sql_record); $transaction_count = 0; 
                                while($row = mysqli_fetch_assoc($query)){
                                    $bet_payments[$row['to_account']] += (float)$row['amount'];
                                    $bet_pay = $bet_payments[$row['to_account']];
                                }
                                $sql_record = "SELECT SUM(amount) as amount FROM `account_contranotes` WHERE `date` >= '$today' AND `date` <= '$today' AND `tcoa` IN ('$cus_list') AND `type` IN ('ContraNote') AND `active` = '1' AND `dflag` = '0'";
                                $query = mysqli_query($conn,$sql_record); $i = 0; $transaction_count = 0; if(!empty($query)){ $transaction_count = mysqli_num_rows($query); }
                                if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ $bet_contra_dr += (float)$row['amount']; } }

                                $opn_cus_bal = (((float)round($cus_sales,5) + (float)round($cus_cdn,5) + (float)round($cus_contra_dr,5) + (float)round($cus_obdramt,5) + (float)round($pay)) - ((float)round($cus_receipts,5) + (float)round($cus_returns,5) + (float)round($cus_ccn,5) + (float)round($cus_contra_cr,5) + (float)round($cus_obcramt,5)));
                                $bet_cus_bal = (((float)round($bet_sales,5) + (float)round($bet_cdn,5) + (float)round($bet_contra_dr,5) + (float)$bet_pay) - ((float)round($bet_receipts,5) + (float)round($bet_returns,5) + (float)round($bet_ccn,5) + (float)round($bet_contra_cr,5)));
                                $tot_cus_bal = $opn_cus_bal + $bet_cus_bal;
                                

                                echo "All Openings <br>";
                                echo "Customer Sales";
                                echo $cus_sales;
                                echo "<br> Cr Dr - Customer";
                                echo $cus_cdn;
                                echo "<br> account_contranotes";
                                echo $cus_contra_dr;
                                echo "<br> main_contactdetails";
                                echo $cus_obdramt;
                                echo "<br> minus";
                                echo "<br> broiler_receipts";
                                echo $cus_receipts;
