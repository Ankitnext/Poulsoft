<?php
//broiler_add_inventoryissued.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $ccid = $_SESSION['inventoryissued'];
...
if($link_active_flag > 0){
    ...
    if($acount == 1){
        ...
        $sql = "SELECT *  FROM `extra_access` WHERE `field_name` LIKE 'Stock Issue' AND `field_function` LIKE 'Stock Check'"; $query = mysqli_query($conn,$sql); $stockcheck_flag = 0; $sccount = mysqli_num_rows($query);
        ...
        $sql = "SELECT *  FROM `extra_access` WHERE `field_name` LIKE 'Stock Issue' AND `field_function` LIKE 'Auto Avg Price'"; $query = mysqli_query($conn,$sql); $autoavgprice_flag = 0; $aapcount = mysqli_num_rows($query);
    ...
    <body class="m-0 hold-transition">
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                            <div class="float-left"><h3 class="card-title">Add Stock Issue</h3></div>
                        </div>
                        <div class="m-0 p-2 card-body">
                            <div class="col-md-18">
                                <form action="broiler_save_inventoryissued.php" method="post" role="form" onsubmit="return checkval()">
                                    <div class="row">
                                    ...
                                    <table>
                                        ...
                                        <tbody id="row_body">
                                            <tr>
                                                ...
                                                <td><select name="sector[]" id="sector[0]" class="form-control select2" style="width:180px;" onchange="fetch_stock_master(this.id);fetch_active_farmbatch(this.id);"><option value="select">select</option><?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>"><?php echo $sector_name[$scode]; ?></option><?php } ?></select></td>
                                                <td><input type="text" name="farm_batch[]" id="farm_batch[0]" class="form-control" style="width:120px;" readonly ></td>
                                                ...
                                                <td style="visibility:hidden;"><input type="text" name="available_stock[]" id="available_stock[0]" class="form-control" placeholder="0.00" style="width:90px;" readonly ></td>
        ...
        <script>
            function return_back(){
                ...
                window.location.href = 'broiler_display_inventoryissued.php?ccid='+ccid;
            }
            function checkval(){
                    ...
                    for(d = 0;d <= incrs;d++){
                        if(l == true){
                            ...
                            if(icode.match("select")){
                                ...
                                alert("Kindly select Location Type in row: "+c);
            ...
            function create_row(a){
                ...
                html += '<td><select name="sector[]" id="sector['+d+']" class="form-control select2" style="width:180px;" onchange="fetch_stock_master(this.id);fetch_active_farmbatch(this.id);"><option value="select">select</option><?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>"><?php echo $sector_name[$scode]; ?></option><?php } ?></select></td>';
                html += '<td><input type="text" name="farm_batch[]" id="farm_batch['+d+']" class="form-control" style="width:120px;" readonly ></td>';
                ...
                html += '<td style="visibility:hidden;"><input type="text" name="available_stock[]" id="available_stock['+d+']" class="form-control" placeholder="0.00" style="width:90px;" readonly ></td>';
            ...
            function fetch_stock_master(a){
                ...
                var item_code = document.getElementById("icode["+d+"]").value;
                if(sector != "select" && item_code != "select"){
                    var fetch_items = new XMLHttpRequest();
                    var method = "GET";
                    var url = "broiler_fetch_itemstockmaster_lsfi.php?sector="+sector+"&item_code="+item_code+"&date="+date+"&trtype=stkissue&etype=stkissue";
                    //window.open(url);
                    ...
                    fetch_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(item_price.length > 0){
                                ...
                                calculate_total_amt("avg_price["+d+"]");
                            }
                            else{
            ...
            function fetch_active_farmbatch(a){
                ...
                if(sector != "select"){
                    var fetch_items = new XMLHttpRequest();
                    var method = "GET";
                    var url = "broiler_fetch_farmbatchactive.php?sector="+sector;
                    //window.open(url);
                    var asynchronous = true;
                    fetch_items.open(method, url, asynchronous);
                    fetch_items.send();
                    fetch_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            var farm_batch = this.responseText;
                            if(farm_batch.length > 0){
                                document.getElementById("farm_batch["+d+"]").value = farm_batch;
                            }
                            else{
                                document.getElementById("farm_batch["+d+"]").value = "";
                            }
                        }
                    }
                }
