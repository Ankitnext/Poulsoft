<?php
//broiler_fetch_rearingcharge_sspoultry2.php
...
if(!empty($farm_batch)){
    ...
    while($row = mysqli_fetch_assoc($query)){
        ...
        $ficode = $fidate = ""; $farmer_price = 0;
        if($medicine_cost == "M"){
            if(!empty($datewise_fmr_prc[$row['date']."@".$row['item_code']]) && $datewise_fmr_prc[$row['date']."@".$row['item_code']] > 0){
    ...
    while($row = mysqli_fetch_assoc($query)){
        ...
        if(!empty($feed_cat[$so_codes])){
            ...
            $fmr_stkin_medvac_amt[$mv_codes] = (float)$fmr_stkin_medvac_amt[$mv_codes] - ((float)$farmer_price * (float)$so_quant);
    ...
    if($tds_flag == 1 && $tds_fmflag == 1) {
        ...
        $tds_amount = round(((float)$amount_payable * (float)$tds_per));
    }
    else if($tds_flag == 1) { $tds_lbl = 1; $tds_per = 0.01; $tds_amount = round(((float)$amount_payable * (float)$tds_per)); }
