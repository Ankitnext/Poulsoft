<?php
//broiler_modify_feedproduction3.php
session_start(); include "newConfig.php";
$dbname = $_SESSION['dbase'];
$addedemp = $_SESSION['userid'];
date_default_timezone_set("Asia/Kolkata");
$addedtime = date('Y-m-d H:i:s');
$ccid = $_SESSION['feedproduction3'];
...
$prod_path_update = "";
if($prodoc_path1 != ""){ $prod_path_update = ", `prodoc_path1` = '$prodoc_path1'"; }
if($prodoc_path2 != ""){ $prod_path_update = ", `prodoc_path2` = '$prodoc_path2'"; }
if($prodoc_path3 != ""){ $prod_path_update = ", `prodoc_path3` = '$prodoc_path3'"; }
...
$price = array();
$sql = "UPDATE `broiler_feed_production` SET `date` = '$date'".$prod_path_update.",`dcno` = '$dcno',`fbatch_no` = '$fbatch_no',`make_date` = '$make_date',`exp_date` = '$exp_date',`feed_mill` = '$feed_mill',`feed_code` = '$feed_code',`formula_code` = '$formula_code',`total_tons` = '$total_tons',`consumed_quantity` = '$consumed_quantity',`wastage_quantity` = '$wastage_quantity',`wastage_per` = '$wastage_per',`wastage_price` = '$wastage_price',`wastage_amount` = '$wastage_amount',`produced_quantity` = '$produced_quantity',`produced_price` = '$produced_price',`produced_amount` = '$produced_amount',`bag_code_feed` = '$bag_code_feed',`no_of_bags_feed` = '$no_of_bags_feed',`bag_code_empty` = '$bag_code_empty',`no_of_bags_empty` = '$no_of_bags_empty',`item_cost` = '$item_cost',`labour_cost` = '$labour_cost',`packing_cost` = '$packing_cost',`electricity_cost` = '$electricity_cost',`transport_cost` = '$transport_cost',`other_cost` = '$other_cost',`other_cost2` = '$other_cost2',`bag_amount` = '$bag_amount',`bag_price` = '$bag_price',`total_cost` = '$total_cost',`margin_type` = '$margin_type',`margin_value` = '$margin_value',`margin_amount` = '$margin_amount',`feed_margin_type` = '$feed_margin_type',`feed_margin_value` = '$feed_margin_value',`feed_margin_amount` = '$feed_margin_amount',`remarks` = '$remarks',`flag` = '$flag',`active` = '$active',`dflag` = '$dflag',`updatedemp` = '$addedemp',`updatedtime` = '$addedtime' WHERE `code` = '$code'";
if(!mysqli_query($conn,$sql)){ die("Error-1:-".mysqli_error($conn)); }
else {
    $sql = "DELETE FROM `broiler_feed_consumed` WHERE `link_trnum` = '$code'"; mysqli_query($conn,$sql);
    $sql = "DELETE FROM `account_summary` WHERE `trnum` = '$code'"; mysqli_query($conn,$sql);
    $dsize = sizeof($item_code);
    for($i = 0;$i < $dsize;$i++){
        if($item_qty[$i] == "" || $item_qty[$i] == NULL || $item_qty[$i] == 0 || $item_qty[$i] == "0.00"){ $item_qty[$i] = 0; }
        if($item_amount[$i] == "" || $item_amount[$i] == NULL || $item_amount[$i] == 0 || $item_amount[$i] == "0.00"){ $itm_amt[$i] = 0; }
    ...
    if(!mysqli_query($conn,$from_post)){ die("Error-9:-".mysqli_error($conn)); }
    else{ }
}
header('location:broiler_display_feedproduction3.php?ccid='.$ccid);
