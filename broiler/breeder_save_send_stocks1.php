...
$date = $dcno = $fromwarehouse = $code = $quantity = $amount = $price = $towarehouse = $remarks = $item_sqty = $item_sprc = array();
...
$trtype = "send_stocks1";
$trlink = "breeder_display_send_stocks1.php";
...
for($i = 0;$i < $dsize;$i++){
    ...
    $trno_dt1 = generate_transaction_details($date[$i],"send_stocks1","BSS","generate",$_SESSION['dbase']);
    $trno_dt2 = explode("@",$trno_dt1);
    $incr = $trno_dt2[0]; $prefix = $trno_dt2[1]; $trnum = $trno_dt2[2]; $fyear = $trno_dt2[3];
    
    //Feed Consumption in Bags
    if((int)$bfstk_bags == 1){
        $fsql1 = "SELECT * FROM `feed_bagcapacity` WHERE `code` LIKE '$code[$i]' AND `active` = '1' AND `dflag` = '0' AND `dflag` = '0'";
    ...
    $sql = "INSERT INTO `item_stocktransfers` (`incr`,`prefix`,`trnum`,`date`,`dcno`,`fromwarehouse`,`code`,`sent_qty`,`quantity`,`price`,`amount`,`towarehouse`,`remarks`,`flag`,`active`,`dflag`,`trtype`,`trlink`,`addedemp`,`addedtime`,`updatedtime`) 
    VALUES('$incr','$prefix','$trnum','$date[$i]','$dcno[$i]','$fromwarehouse[$i]','$code[$i]','$quantity[$i]','0','$price[$i]','$amount','$towarehouse[$i]','$remarks[$i]','$flag','$active','$dflag','$trtype','$trlink','$addedemp','$addedtime','$addedtime')";
...
<script>
    ...
    if(x == true){
        window.location.href = "breeder_add_send_stocks1.php";
    }
    else if(x == false) {
        window.location.href = "breeder_display_send_stocks1.php?ccid="+a;
    }
    else {
        window.location.href = "breeder_display_send_stocks1.php?ccid="+a;
    }
