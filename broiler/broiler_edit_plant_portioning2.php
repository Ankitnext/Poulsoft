<?php
//broiler_edit_plant_portioning2.php
...
if($link_active_flag > 0){
    ...
    if($acount == 1){
        ...
        $mcat_list = implode("','",$imcat_code);
        $sql = "SELECT * FROM `item_category` WHERE `main_category` IN ('$mcat_list') AND `plant_portioning` LIKE '1' AND `active` = '1' AND `dflag` = '0'";
        $query = mysqli_query($conn,$sql); $icat_alist = array();
        while($row = mysqli_fetch_assoc($query)){ $icat_alist[$row['code']] = $row['code']; $mcat_alist[$row['code']] = $row['main_category']; }
        ...
        $query = mysqli_query($conn,$sql); $item_cat = array();
		while($row = mysqli_fetch_assoc($query)){ $item_cat[$row['code']] = $row['category']; }
...
<html lang="en">
    ...
    <body class="m-0 p-0 hold-transition">
        <?php
        $ids = $_GET['trnum'];
        $sql = "SELECT * FROM `plant_bird_portioning_produced_details` WHERE `trnum` LIKE '$ids' AND `dflag` = '0' ORDER BY `id` ASC";
        $query = mysqli_query($conn,$sql); $port_icat = $port_icode = $port_weight = $port_yield = $item_icats = array(); $c = 0;
        while($row = mysqli_fetch_assoc($query)){
            $date = $row['date'];
            $receive_type = $row['receive_type'];
            $vcode = $row['cus_code'];
            $batch_no = $row['batch_no'];
            $mnubch_no = $row['mnubch_no'];
            $billno = $row['billno'];

            $itm_code[$c] = $row['item_code'];
            $remarks2[$c] = $row['remarks2'];
            $itm_weight[$c] = round($row['weight'],5);
            $itm_yper[$c] = round($row['yield_per'],5);
            $port_icat[$mcat_alist[$item_cat[$row['item_code']]]] = $mcat_alist[$item_cat[$row['item_code']]];
            $item_icats[$item_cat[$row['item_code']]] = $item_cat[$row['item_code']];
            $remarks = $row['remarks'];
            $c++;
        } $c = $c - 1; $iincr = $c;
        $icat_list = implode("','",$item_icats);
		$sql = "SELECT * FROM `item_details` WHERE `category` IN ('$icat_list') AND `active` = '1' AND `dflag` = '0' ORDER BY `description` ASC";
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        ...
                        <div class="p-0 pt-5 card-body">
                            <div class="col-md-12">
                                <form action="broiler_modify_plant_portioning2.php" method="post" role="form" onsubmit="return checkval()">
                                    <div class="row"style="visibility:hidden;">
                                        <div class="col-md-4"></div>
                                        <div class="col-md-4">
                                            <div class="row">
                                                <div class="custom-control custom-radio"><input type="radio" name="receive_type" id="receive_type1" class="custom-control-input custom-control-input-purple" value="plant_jobwork" onclick="update_link_trnums();" <?php if($receive_type == "plant_jobwork"){ echo "checked"; } ?> /><label for="receive_type1" class="custom-control-label">Customer</label></div>&ensp;&ensp;
                                                <div class="custom-control custom-radio"><input type="radio" name="receive_type" id="receive_type2" class="custom-control-input custom-control-input-purple" value="plant_received" onclick="update_link_trnums();" <?php if($receive_type == "plant_received"){ echo "checked"; } ?> /><label for="receive_type2" class="custom-control-label">Received Batch</label></div>
                                            </div>
                                        </div>
                                        <div class="col-md-4"></div>
                                    </div><br/>
                                    <div class="row justify-content-center align-items-center">
                                        <div class="form-group" <?php if($receive_type == "plant_jobwork"){ echo 'style="width:230px;visibility:visible;"'; } else{ echo 'style="width:230px;visibility:hidden;"'; } ?> id="customer_selection">
                                            <label>Customer<b style="color:red;">&nbsp;*</b></label>
							                <select name="cus_code" id="cus_code" class="form-control select2" style="width:220px;">
                                                <?php if($vcode == "" || $vcode == "select"){ echo '<option value="select">-select-</option>'; } else{ echo '<option value="'.$vcode.'">'.$cus_name[$vcode].'</option>'; } ?>
                                            </select>
                                        </div>
                                        <div class="form-group" style="width:230px;">
                                            <label>Grading No.<b style="color:red;">&nbsp;*</b></label>
							                <select name="batch_no" id="batch_no" class="form-control select2" style="width:220px;" data-placeholder="Select Batch No." data-dropdown-css-class="select2-purple">
                                                <?php echo '<option value="'.$batch_no.'">'.$batch_no.'</option>'; ?>
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group" style="width:110px;">
                                            <label>Bill No</label>
                                            <input type="text" name="billno" id="billno" class="form-control" value="<?php echo $billno; ?>" style="width:100px;" />
                                        </div>
                                        <div class="form-group" style="width:140px;">
                                            <label>Batch No.<b style="color:red;">&nbsp;*</b></label>
                                            <input type="text" name="mnubch_no" id="mnubch_no" class="form-control" value="<?php echo $mnubch_no; ?>" style="width:130px;" />
                                        </div>
                                    </div>
                                    <div class="row justify-content-center align-items-center">
                                                ...
                                                <td>
                                                    ...
                                                    <?php
                                                        ...
                                                        foreach($imcat_code as $ccode){
                                                            $c++; $pincr++;
                                                            $checked = "";
                                                            foreach($port_icat as $mcat){ if($mcat == $ccode){ $checked = "checked"; } }
                                                            echo '<td><input type="radio" name="portion_type" id="portion_type['.$c.']" class="form-control" style="transform: scale(.7);" value="'.$ccode.'" onclick="clear_portioning_alist();" '.$checked.' /></td>';
                                                ...
                                                <table>
                                                    ...
                                                    <tbody id="portioning_items">
                                                        ...
                                                        <tr id="row_no[<?php echo $c; ?>]">
                                                            <td><select name="port_icode1[]" id="port_icode1[<?php echo $c; ?>]" class="form-control select2" style="min-width:130px;" onchange="update_row_item(this.id);"><option value="select">-select-</option><?php foreach($item_code as $icode){ ?><option value="<?php echo $icode; ?>" <?php if($itm_code[$c] == $icode){ echo "selected"; } ?>><?php echo $item_code[$icode]; ?></option><?php } ?></select></td>
                                                            <td><select name="port_icode[]" id="port_icode[<?php echo $c; ?>]" class="form-control select2" style="min-width:380px;" onchange="update_row_item(this.id);"><option value="select">-select-</option><?php foreach($item_code as $icode){ ?><option value="<?php echo $icode; ?>" <?php if($itm_code[$c] == $icode){ echo "selected"; } ?>><?php echo $item_name[$icode]; ?></option><?php } ?></select></td>
                                                            <td><input type="text" name="port_weight[]" id="port_weight[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $itm_weight[$c]; ?>" style="width:90px;" onkeyup="validatenum(this.id);cal_tot_iport_details();" onchange="validateamount(this.id);" /></td>
                                                            <td><input type="text" name="port_yield[]" id="port_yield[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $itm_yper[$c]; ?>" style="width:90px;" readonly /></td>
                                                            <td><textarea name="remarks2[]" id="remarks2[<?php echo $c; ?>]" class="form-control" style="width:90px;"><?php echo $remarks2[$c]; ?></textarea></td>
                                                            <?php
                                                            if($c == $iincr){ echo '<td id="action['.$c.']" style="padding-top: 5px;visibility:visible;">'; }
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label>Remarks</label>
                                                <textarea name="remarks" id="remarks" class="form-control" style="width:100%;"><?php echo $remarks; ?></textarea>
                                            </div>
                                        </div>
                                        <div class="col-md-4"></div>
                                    </div>
                                    <div class="row" style="visibility:hidden;">
                                        ...
                                        <div class="form-group" style="width:25px;">
                                            <label>IN</label>
                                            <input type="text" name="iincr" id="iincr" class="form-control" value="<?php echo $iincr; ?>" style="width:20px;" readonly />
                                        </div>
                                        <div class="form-group" style="width:25px;">
                                            <label>EC</label>
        ...
        <script>
            ...
            function fetch_plant_grading_trnum_details(){
                ...
                if(batch_no == "" || batch_no == "select"){
                    ...
                    var prices = new XMLHttpRequest();
                    var method = "GET";
                    var url = "broiler_fetch_plant_grading_trdetails2.php?batch_no="+batch_no+"&trnum="+ids+"&type=edit";
                    //window.open(url);
                    var asynchronous = true;
                    prices.open(method, url, asynchronous);
                    prices.send();
            ...
            /*function fetch_portioning_items(){
                ...
                if(pincr <= 0){
                    ...
                    if(icat_list != ""){
                        ...
                        var prices = new XMLHttpRequest();
                        var method = "GET";
                        var url = "broiler_fetch_plant_portioning_items2.php?icat_list="+icat_list+"&trnum="+ids+"&type=edit";
                        //window.open(url);
                        var asynchronous = true;
                        prices.open(method, url, asynchronous);
                        prices.send();
            ...
            fetch_plant_grading_trnum_details();
            //fetch_portioning_items();
