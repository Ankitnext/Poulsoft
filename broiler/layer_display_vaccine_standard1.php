...
if($cid != ""){ $_SESSION['vaccine_standard1'] = $cid; } else{ $cid = $_SESSION['vaccine_standard1']; }
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        while($row = mysqli_fetch_assoc($query)){
                                            ...
                                            $val = ""; $val = $row['id']."@".$bbreed_name[$row['breed_code']]."@".round($row['age'],5)."@".$item_name[$row['medvac_code']];
                                    ?>
                                    <tr>
										<td><?php echo $bbreed_name[$row['breed_code']]; ?></td>
										<td><?php echo round($row['age'],5); ?></td>
										<td><?php echo $item_name[$row['medvac_code']]; ?></td>
                    ...
                    inv_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(parseFloat(count) > 0){
                                alert("You can't delete the Vaccine Shedule with Breed: "+breed+", Age: "+age+", Vaccine Name: "+vname+", as Vaccine Shedule is already in use!");
                            }
                            else{
                                var b = "<?php echo $delete_url; ?>"+id;
                                var c = confirm("are you sure you want to delete the Vaccine Shedule with Breed: "+breed+", Age: "+age+", Vaccine Name: "+vname+"?");
                                if(c == true){
