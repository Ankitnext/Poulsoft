<?php
//broiler_fetch_rearingcharge_rainbow2.php
session_start(); include "newConfig.php";
$farm_code = $_GET['farm_code'];
$batch_code = $_GET['batch_code'];

$fsql = "SELECT * FROM `broiler_batch` WHERE `farm_code` = '$farm_code' AND `code` = '$batch_code' AND `gc_flag` = '1' AND `active` = '1' AND `dflag` = '0'"; $fquery = mysqli_query($conn,$fsql);
...
if(!empty($farm_batch)){
    ...
    //Daily Entry
    $sql = "SELECT * FROM `broiler_daily_record` WHERE `farm_code` = '$farm_code' AND `batch_code` = '$farm_batch' AND `brood_age` > '0' AND `active` = '1' AND `dflag` = '0' ORDER BY `brood_age` ASC";
    ...
    while($row = mysqli_fetch_assoc($query)){
        ...
        if($medicine_cost == "M"){
            ...
            if($m_cnt > 0){ while($rowf = mysqli_fetch_assoc($queryf)){ $farmer_price = $rowf['rate']; } }
            else if(!empty($datewise_fmr_prc[$row['date']."@".$row['item_code']]) && $datewise_fmr_prc[$row['date']."@".$row['item_code']] > 0){ $farmer_price = $datewise_fmr_prc[$row['date']."@".$row['item_code']]; }
            else{ }
        }
        ...
        $fmr_stkin_medvac_amt[$mv_codes] = (float)$fmr_stkin_medvac_amt[$mv_codes] - ((float)$farmer_price * (float)$mv_quant);
    ...
    $sql = "SELECT * FROM `broiler_sales` WHERE `warehouse` = '$farm_code' AND `farm_batch` = '$farm_batch' AND `active` = '1' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($query)){
    ...
    $farmer_sales_amt = round($farmer_sales_amt);
    //Transfer Out
    $count = 0;
    ...
    while($row = mysqli_fetch_assoc($query)){
      ...
      //Farmer Receipt Amount
      $sql = "SELECT * FROM `broiler_receipts` WHERE `warehouse` = '$farm_code' AND `farm_batch` = '$farm_batch' AND `active` = '1' AND `dflag` = '0'";
      $query = mysqli_query($conn,$sql); $frmr_rct_amt = 0;
      while($row = mysqli_fetch_array($query)){ $frmr_rct_amt += $row['amount']; }
    ...
    if($placed_birds > 0){ $fmr_total_medvac_consumed_prc = round(((float)$fmr_total_medvac_consumed_amt / (float)$placed_birds),2); } else { $fmr_total_medvac_consumed_prc = 0; }
    ...
    if((float)$max_prod_cost > 0 && $fmr_act_prod_prc > $max_prod_cost){ $sale_inc_prc = 0; } else{ }
    if((float)$sales_max_rates > 0 && $sale_inc_prc >= $sales_max_rates){ $sale_inc_prc = $sales_max_rates; } else{$sale_inc_prc = round($sale_inc_prc,5); }
    ...
    /*********** Summer Incentive ***********/
    $summer_inc_prc = $summer_inc_amount = 0;
    
    $sql = "SELECT * FROM `broiler_gc_smr_incentive` WHERE `std_code` = '$gc_code' AND `prod_cost_from` <= '$fmr_act_prod_prc' AND `prod_cost_to` >= '$fmr_act_prod_prc' AND `min_prod_cost` <= '$fmr_act_prod_prc' AND `max_prod_cost` >= '$fmr_act_prod_prc'"; $query = mysqli_query($conn,$sql); $minc_count = mysqli_num_rows($query);
    if($minc_count > 0){
        while($row = mysqli_fetch_assoc($query)){
            $incentive_on = $row['incentive_on'];
            
            if($incentive_on == "placed_birds"){
                $summer_inc_amount = round(($row['incentive_rate'] * $placed_birds),2);
            }
            else if($incentive_on == "sold_birds"){
                $summer_inc_amount = round(($row['incentive_rate'] * $sold_birds),2);
            }
            else if($incentive_on == "sold_weight"){
                $summer_inc_amount = round(($row['incentive_rate'] * $sold_weight),2);
            }
            else{ }
        }
    }
    ...
    */
    ...
    //Advance Payment
    $sql = "SELECT * FROM `broiler_payments` WHERE `ccode` = '$farmer_code' AND `warehouse` = '$farm_code' AND `farm_batch` = '$farm_batch' AND `pay_type` = 'adv_pay' AND `vtype` = 'FarmerPay' AND `active` = '1' AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql); $advance_deduction = 0; while($row = mysqli_fetch_assoc($query)){ $advance_deduction += (((float)$row['amount'])); }
    ...
    $total_amount_payable = round(((float)$total_incentive - (float)$total_decentives - (float)$farmer_sales_amt + (float)$frmr_rct_amt));
    ...
    if($tds_flag == 1 && $tds_fmflag == 1) {
        ...
        $tds_amount = round(((float)$total_incentive * (float)$tds_per));
    }
    else if($tds_flag == 1) { $tds_lbl = 1; $tds_per = 0.01; $tds_amount = round(((float)$total_incentive * (float)$tds_per)); }
    else{ $tds_lbl = 0; $tds_amount = $tds_per = 0; }

    $farmer_payable = round(((float)$total_amount_payable - (float)$tds_amount - (float)$advance_deduction));
