                    ...
                    <div class="m-0 p-0 card">
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php 
                                        $sql = "SELECT * FROM `layer_shed_allocation` WHERE `active` = '1' AND `dflag` = '0' ORDER BY `description` ASC";
                                        $query = mysqli_query($conn,$sql); $lflk_code = $lflk_name = $lflk_alist = array();
                                        while($row = mysqli_fetch_assoc($query)){
                                            $lflk_code[$row['code']] = $row['code']; $lflk_name[$row['code']] = $row['description'];
                                            if((int)$row['cls_flag'] == 0){ $lflk_alist[$row['code']] = $row['code']; }
                                        } 
                                        $lflk_list = implode("','", $lflk_alist);
                                        $sql = "SELECT MAX(breed_age) as breed_age,flock_code FROM `layer_dayentry_consumed` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `flock_code` IN ('$lflk_list') AND `dflag` = '0' GROUP BY `flock_code` ORDER BY `id` DESC";
                                        $query = mysqli_query($conn,$sql);
                                        while($row = mysqli_fetch_assoc($query)){ $flk_ages[$row['flock_code']] = $row['breed_age']; }

                                        $delete_url = $delete_link."?utype=delete&trnum=";
                                        $sql = "SELECT * FROM `layer_dayentry_consumed` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `dflag` = '0' AND `trtype` = 'dailyentry1' AND `trlink` = 'layer_display_dailyentry1.php' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['trnum'];
                                            ...
                                            $val = ""; $val = $row['trnum']."@".$lflk_name[$row['flock_code']]."@".round($row['age'],5);
                                    ?>
                                    <tr>
                                        <td data-sort="<?= strtotime($row['date']) ?>"><?= date('d.m.Y',strtotime($row['date'])) ?></td>
										<td><?php echo $row['trnum']; ?></td>
										<td><?php echo $lflk_name[$row['flock_code']]; ?></td>
										<td style="text-align:right;"><?php echo round($row['breed_wage'],5); ?></td>
										<td style="text-align:right;"><?php echo str_replace(".00","",number_format_ind($row['mort_qty'])); ?></td>
										<td style="text-align:right;"><?php echo number_format_ind($row['feed_qty1']); ?></td>
        ...
        <script>
			function checkdelete(x){
                ...
                if(trnum != ""){
                    ...
                    var method = "GET";
                    var url = "layer_check_dailyentry1.php?trnum="+trnum+"&flock="+flock+"&age="+age+"&vname="+vname;
                    //window.open(url);
