<?php
//broiler_display_feedsale_lsfi2.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['feedsale_lsfi2'] = $cid; } else{ $cid = $_SESSION['feedsale_lsfi2']; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            if(in_array("prt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_sales` ADD `prt` decimal(20,2) NOT NULL DEFAULT '0' AFTER `rate`"; mysqli_query($conn,$sql); }
            if(in_array("prt_amt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_sales` ADD `prt_amt` decimal(20,2) NOT NULL DEFAULT '0' AFTER `prt`"; mysqli_query($conn,$sql); }
            if(in_array("chrg_prc", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_sales` ADD `chrg_prc` decimal(20,2) NOT NULL DEFAULT '0' AFTER `prt_amt`"; mysqli_query($conn,$sql); }
            if(in_array("branch_code", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_sales` ADD `branch_code` VARCHAR(200) NOT NULL DEFAULT '0' AFTER `chrg_prc`"; mysqli_query($conn,$sql); }
            ...
            $sale_print1 = 0;
            $sql = "SELECT * FROM `extra_access` WHERE `field_name` = 'broiler_display_feedsale_lsfi2.php' AND `field_function` = 'Sale Invoice Format-1' AND `user_access` LIKE 'all' AND `flag` = '1'";
