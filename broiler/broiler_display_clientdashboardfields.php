<?php
//broiler_display_clientdashboardfields.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['clientdashboardfields'] = $cid; } else{ $cid = $_SESSION['clientdashboardfields']; }
    ...
    <body class="m-0 hold-transition sidebar-mini">
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                           <div class="float-left"><h3 class="card-title">Dashboard Access</h3></div>
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        while($row = mysqli_fetch_assoc($query)){ $emp_dname[$row['code']] = $row['name']; }

                                        $sql = "SELECT * FROM `main_access` ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql);
                                        while($row = mysqli_fetch_assoc($query)){ $emp_dcode[$row['empcode']] = $row['db_emp_code']; }

                                        $sql = "SELECT * FROM `log_useraccess` WHERE `dblist` = '$db_name' AND `account_access` = 'BTS' ORDER BY `username` ASC"; $query = mysqli_query($conns,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){ $emp_uname[$row['empcode']] = $row['username']; }
                                        
                                        $delete_url = $delete_link."?utype=delete&id=";
                                        $sql = "SELECT * FROM `master_dashboard_links` WHERE `dflag` = '0' ORDER BY `field_name`,`panel_name`,`sort_order` ASC"; $query = mysqli_query($conn,$sql);
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['id'];
                                    ...
                                    ?>
                                    <tr>
										<td><?php echo $emp_dname[$emp_dcode[$row['user_code']]]; ?></td>
										<td><?php echo $emp_uname[$row['user_code']]; ?></td>
        ...
        <script>
			function checkdelete(a){
				...
				var c = confirm("are you sure you want to delete the transaction?");
				if(c == true){
