<?php
//broiler_add_feedsale_lsfi2.php
include "newConfig.php";
include "broiler_generate_trnum_details.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $ccid = $_SESSION['feedsale_lsfi2'];
...
if($link_active_flag > 0){
    ...
    if($acount == 1){
        ...
        $sql = "SELECT * FROM `location_branch` WHERE `active` = '1' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
        ...
        $sql4 = "SELECT *  FROM `extra_access` WHERE `field_name` LIKE 'broiler_display_feedsale_lsfi2.php' AND `field_function` LIKE 'Fetch Customer Item Prices from Price Master'  AND  `user_access` = 'all' AND `flag` = '1'";
        $query4 = mysqli_query($conn, $sql4); $cip_aflag = mysqli_num_rows($query4);

        $sql4 = "SELECT *  FROM `extra_access` WHERE `field_name` LIKE 'broiler_display_feedsale_lsfi2.php' AND `field_function` LIKE 'Display Only Stock Available Items'  AND  `user_access` = 'all' AND `flag` = '1'";
        $query4 = mysqli_query($conn, $sql4); $istk_aflag = mysqli_num_rows($query4);
?>
<html lang="en">
    <head>
    ...
    <body class="m-0 hold-transition">
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        ...
                        <div class="m-0 p-2 card-body">
                            <div class="col-md-18">
                                <form action="broiler_save_feedsale_lsfi2.php" method="post" role="form" onSubmit="return checkval()">
                                    <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="warehouse" id="warehouse" class="form-control select2" style="width:160px;" onchange="fetch_stkavl_items('icode[0]');">
                                                <option value="select">select</option>
                                    ...
                                    <table>
                                        ...
                                        <tbody id="row_body">
                                            <tr>
                                                ...
                                                <td><input type="text" name="prt_per[]" id="prt_per[0]" class="form-control" placeholder="0.00" style="width:90px;" onkeyup="validatenum5(this.id);calculate_total_amt(this.id);" onchange="validateamount5(this.id);" ></td>
                                                <td><input type="text" name="prt_pamt[]" id="prt_pamt[0]" class="form-control" placeholder="0.00" style="width:90px;" onkeyup="validatenum5(this.id);calculate_total_amt(this.id);" onchange="validateamount5(this.id);" readonly></td>
                                                <td><input type="text" name="chrg_prc[]" id="chrg_prc[0]" class="form-control" placeholder="0.00" style="width:90px;" onkeyup="validatenum5(this.id);calculate_total_amt(this.id);" onchange="validateamount5(this.id);" readonly></td>
                                                <td><input type="text" name="item_tamt[]" id="item_tamt[0]" class="form-control" placeholder="0.00" style="width:90px;" readonly ></td>
                                                <td><select name="branch[]" id="branch[0]" class="form-control select2" style="width:180px;" onchange=""><option value="select">select</option><?php foreach($branch_code as $prod_code){ ?><option value="<?php echo $prod_code; ?>"><?php echo $branch_name[$prod_code]; ?></option><?php } ?></select></td>
        ...
        <script>
            function return_back(){
                ...
                window.location.href = 'broiler_display_feedsale_lsfi2.php?ccid='+ccid;
            }
            function checkval(){
            ...
            function create_row(a){
                ...
                html += '<td><input type="text" name="prt_per[]" id="prt_per['+d+']" class="form-control" placeholder="0.00" style="width:90px;" onkeyup="validatenum5(this.id);calculate_total_amt(this.id);" onchange="validateamount5(this.id);" ></td>';
                html += '<td><input type="text" name="prt_pamt[]" id="prt_pamt['+d+']" class="form-control" placeholder="0.00" style="width:90px;" onkeyup="validatenum5(this.id);calculate_total_amt(this.id);" onchange="validateamount5(this.id);" readonly></td>';
                html += '<td><input type="text" name="chrg_prc[]" id="chrg_prc['+d+']" class="form-control" placeholder="0.00" style="width:90px;" onkeyup="validatenum5(this.id);calculate_total_amt(this.id);" onchange="validateamount5(this.id);" readonly></td>';
                html += '<td><input type="text" name="item_tamt[]" id="item_tamt['+d+']" class="form-control" placeholder="0.00" style="width:90px;" readonly ></td>';
                html += '<td><select name="branch[]" id="branch['+d+']" class="form-control select2" style="width:180px;" onchange="fetch_stock_master(this.id);"><option value="select">select</option><?php foreach($branch_code as $prod_code){ ?><option value="<?php echo $prod_code; ?>"><?php echo $branch_name[$prod_code]; ?></option><?php } ?></select></td>';
                ...
                var istk_aflag = '<?php echo $istk_aflag; ?>'; if(parseInt(istk_aflag) > 0){ var prx = "icode["+d+"]"; fetch_stkavl_items(prx); }
            }
            ...
            function calculate_total_amt(a){
                var b = a.split("["); var c = b[1].split("]"); var d = c[0];
                var profit = document.getElementById("prt_per["+d+"]").value;
                ...
                if(qty == "" || qty.length == 0 || qty == "0.00" || qty == "0"){ qty = 0; }
                if(profit == "" || profit.length == 0 || profit == "0.00" || profit == "0"){ profit = 0; }
                if(price == "" || price.length == 0 || price == "0.00" || price == "0"){ price = 0; }
                if(avg_price == "" || avg_price.length == 0 || avg_price == "0.00" || avg_price == "0"){ avg_price = 0; }

                var prt_amt = (parseFloat(profit) / 100) * parseFloat(price);
                document.getElementById("prt_pamt[" + d + "]").value = prt_amt.toFixed(2);

                // Calculate charge price correctly
                var chrg_price = parseFloat(prt_amt) + parseFloat(price);
                document.getElementById("chrg_prc[" + d + "]").value = chrg_price.toFixed(2);

                // Calculate total amount
                var total_amtt = parseFloat(chrg_price) * parseFloat(qty);
               document.getElementById("item_tamt[" + d + "]").value = total_amtt.toFixed(0);
            ...
            function fetch_stock_master(a){
                var cip_aflag = '<?php echo $cip_aflag; ?>';
                var b = a.split("["); var c = b[1].split("]"); var d = c[0];
                var date = document.getElementById("date").value;
				...
				fetch_items.onreadystatechange = function(){
					if(this.readyState == 4 && this.status == 200){
                        ...
                        if(parseInt(cip_aflag) > 0){ fetch_customer_pricemaster(a); }
                    }
                }
            }
            ...
            function fetch_stkavl_items(a){
                var b = a.split("["); var c = b[1].split("]"); var d = c[0];
                var istk_aflag = '<?php echo $istk_aflag; ?>';
                if(parseInt(istk_aflag) == 1){
                    var date = document.getElementById("date").value;
                    var sectors = document.getElementById("warehouse").value;
                    removeAllOptions(document.getElementById("icode["+d+"]"));
                    
                    var fetch_items = new XMLHttpRequest();
                    var method = "GET";
                    var url = "broiler_fetch_stockexist_items.php?date="+date+"&sectors="+sectors+"&row_no="+d+"&trtype=add";
                    //window.open(url);
                    var asynchronous = true;
                    fetch_items.open(method, url, asynchronous);
                    fetch_items.send();
                    fetch_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            var item_opt = item_dt2[0];
                            var rows = item_dt2[1];
                            $('#icode\\['+rows+'\\]').append(item_opt);
                        }
                    }
                }
            }
