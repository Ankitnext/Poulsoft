<?php
//broiler_display_vouchers2.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['vouchers2'] = $cid; } else{ $cid = $_SESSION['vouchers2']; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            $sql1 = "SHOW TABLES WHERE ".$table_head." LIKE 'broiler_voucher_notes';"; $query1 = mysqli_query($conn,$sql1); $tcount = mysqli_num_rows($query1);
            if($tcount > 0){ } else{ $sql1 = "CREATE TABLE $database_name.broiler_voucher_notes LIKE poulso6_admin_broiler_broilermaster.broiler_voucher_notes;"; mysqli_query($conn,$sql1); }
            ...
            $fsdate = $cid."-fdate"; $tsdate = $cid."-tdate"; $tsvou_type = $cid."-vou_type";
            if(isset($_POST['submit']) == true){
                ...
                $_SESSION[$tsvou_type] = $vou_type;
            }
            else {
                $fdate = $tdate = date("Y-m-d"); $vou_type = "all";
                ...
                if(!empty($_SESSION[$tsvou_type])){ $vou_type = $_SESSION[$tsvou_type]; }
            }
        ?>
                    ...
                    <div class="m-0 p-0 card">
                                    ...
                                    <div class="col-md-8">
                                        <div class="row" align="left">
                                            ...
                                            <div class="form-group" style="width:150px;">
                                                <label for="tdate">Voucher Type: </label>
                                                <select name="vou_type" id="vou_type" class="form-control select2" style="width:140px;">
                                                    <option value="all" <?php if($vou_type == "all"){ echo "selected"; } ?>>-All-</option>
                                                    <option value="payment" <?php if($vou_type == "payment"){ echo "selected"; } ?>>PAY Voucher</option>
                                                    <option value="receipt" <?php if($vou_type == "receipt"){ echo "selected"; } ?>>RCT Voucher</option>
                                                </select>
                                            </div>
                                            <div class="form-group" style="width:100px;">
                                                <br/>
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                    $sql = "SELECT * FROM `main_contactdetails` WHERE `dflag` = '0' ORDER BY `name` ASC"; $query = mysqli_query($conn,$sql); $acc_name = array();
                                    while($row = mysqli_fetch_assoc($query)){ $acc_name[$row['code']] = $row['name']; }

                                    $sql = "SELECT * FROM `acc_coa` WHERE `dflag` = '0' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
                                    while($row = mysqli_fetch_assoc($query)){ $acc_name[$row['code']] = $row['description']; }
                                    
                                    $sql = "SELECT * FROM `inv_sectors` WHERE `dflag` = '0' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql); $sector_name = array();
                                    ...
                                    if($vou_type == "all"){ $vou_filter = ""; } else{ $vou_filter = " AND `vtype` = '$vou_type'"; }
                                    $sql = "SELECT * FROM `".$table_name."` WHERE `date` >= '$fdate' AND `date` <= '$tdate'".$vou_filter." AND `trtype` = 'vouchers2' AND `trlink` = 'broiler_display_vouchers2.php' AND `dflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                    while($row = mysqli_fetch_assoc($query)){
                                        $id = $row['trnum'];
                                        $edit_url = $edit_link."?utype=edit&trnum=".$id;
                                        $print_url = "print/Examples/broiler_print_vouchernote.php?utype=print&trnum=".$id;
                                    ...
                                    <td data-sort="<?= strtotime($row['date']) ?>"><?= date('d.m.Y',strtotime($row['date'])) ?></td>
                                        ...
                                        <td><?php echo ucfirst($row['vtype']); ?></td>
                                        <td><?php echo $acc_name[$row['vcode']]; ?></td>
                                        <td><?php echo $row['crdr']; ?></td>
										<td style="text-align:right;"><?php echo round($row['amount']); ?></td>
