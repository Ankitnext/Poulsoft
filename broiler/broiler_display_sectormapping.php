<?php
//broiler_display_sectormapping.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['sectormapping'] = $cid; } else{ $cid = $_SESSION['sectormapping']; }
    ...
    <body class="m-0 hold-transition sidebar-mini">
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                           <div class="float-left"><h3 class="card-title">Sector-Branch Mapping</h3></div>
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $delete_url = $delete_link."?utype=delete&id="; $edit_url = $edit_link."?utype=edit&id=";
                                        $sql = "SELECT * FROM `broiler_secbrch_mapping` WHERE `dflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['id'];
                                            ...
                                            $edit_path = $edit_link."?utype=edit&id=".$id;
                                            $val = ""; $val = $row['id']."@".$sector_name[$row['sector_code']]."@".$branch_name[$row['branch_code']];
                                    ?>
                                    <tr>
                                        ...
                                        <?php
                                            if($row['flag'] == 1){
                                                ...
                                                if($edit_flag == 1){
                                                    ?>
                                                    <a href='javascript:void(0)' id='<?php echo $val; ?>' value='<?php echo $val; ?>' onclick='checkedit(this.id)'>
                                                    <i class='fa fa-pen' style='color:brown;' title='Edit'></i>
                                                    </a>&ensp;&ensp;
                                                <?php
                                                }
                                                if($delete_flag == 1){
        ...
        <script>
			function checkedit(x){
                ...
                var branch = val1[2];
                if(id != ""){
                    var b = "<?php echo $edit_url; ?>"+id;
                    var c = confirm("are you sure you want to edit the sector mapping for the below details\n Sector: "+sector+", Branch: "+branch+"?");
                    if(c == true){
            ...
            function checkdelete(x){
                ...
                if(id != ""){
                    ...
                    var c = confirm("are you sure you want to delete the standard Egg Rate for the below details\n Sector: "+sector+", Branch: "+branch+"?");
                    if(c == true){
                        window.location.href = b;
                    }
                    else{ }
			...
			/*function checkdelete(x){
                ...
                if(a != ""){
                    ...
                    var method = "GET";
                    var url = "broiler_check_farmermedvacprices.php?id="+a;
                    //window.open(url);
                    ...
                    inv_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(parseFloat(count) > 0){
                                alert("You can't delete the Farmer Item Price: "+val+", as Farmer Item Price is already in use!");
                            }
                            else{
                                var b = "<?php //echo $delete_url; ?>"+a;
                                var c = confirm("are you sure you want to delete the Farmer Item Price: "+val+"?");
                                if(c == true){
                                    window.location.href = b;
                                }
                                else{ }
