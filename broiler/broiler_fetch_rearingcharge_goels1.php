<?php
//broiler_fetch_rearingcharge_goels1.php
...
if(!empty($farm_batch)){
    ...
    $trot_qty = $trot_amount = $item_all = $sal_birds = $feed_cat = $medvac_cat = array(); $item_all[$chick_code] = $chick_code;
    ...
    $id = $_GET['schema_id']; $schema_name = $_GET['schema_name'];
    $sql = "SELECT * FROM `broiler_gc_standard` WHERE `schema_name` = '$schema_name' AND `region_code` = '$region_code' AND `branch_code` = '$branch_code' AND `from_date` <= '$sdate' AND `to_date` >= '$edate' AND `active` = '1' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($query)){
        $gc_code = $row['code'];
        ...
        $gcm_unl_charge = $row['gcm_unl_charge'];
    }

    //Farmer and mgmt costing
    //Purchases
    $sql = "SELECT * FROM `broiler_purchases` WHERE `warehouse` = '$farm_code' AND `farm_batch` = '$farm_batch' AND `active` = '1' AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql); $hchkin_vcode_qty = $hstkin_vcode_qty = array();
    while($row = mysqli_fetch_assoc($query)){
        if($row['icode'] == $chick_code){
            ...
            //Highest Chick-In Supplier
            $hchkin_vcode_qty[$row['vcode']] += (((float)$row['rcd_qty']+ (float)$row['fre_qty']));
        }
            ...
            //Highest Stock-In Supplier and Brand
            $hstkin_vcode_qty[$row['vcode']] += (((float)$row['rcd_qty']+ (float)$row['fre_qty']));
        }
    ...
    //Transfer In
    $sql = "SELECT * FROM `item_stocktransfers` WHERE `towarehouse` = '$farm_code' AND `to_batch` = '$farm_batch' AND `active` = '1' AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql); $transport_charges = 0;
    while($row = mysqli_fetch_assoc($query)){
        if($row['code'] == $chick_code){
            ...
            //Highest Chick-In Sector
            $hchkin_vcode_qty[$row['fromwarehouse']] += (float)$row['quantity'];
        }
        ...
        if($row['paid_by'] == "ToFarm"){ $transport_charges += (float)$row['transport_cost']; }

        if($start_date == ""){ $start_date = $row['date']; } else{ if(strtotime($start_date) >= strtotime($row['date'])){ $start_date = $row['date']; } }
        if($end_date == ""){ $end_date = $row['date']; } else{ if(strtotime($end_date) <= strtotime($row['date'])){ $end_date = $row['date']; } }
    }
    //Highest Feed-In Supplier
    $sup_code = $brand_code = $itm_code = $secven_code = "";
    if(sizeof($hstkin_vcode_qty) > 0){
        if(sizeof($hstkin_vcode_qty) > 1){ $maxValue = 0; $maxValue = max($hstkin_vcode_qty); $sup_code = array_keys($hstkin_vcode_qty, $maxValue)[0]; }
        else{ $sup_code = array_keys($hstkin_vcode_qty)[0]; }
    }
    //Highest Feed-In Supplier/Hatchery
    if(sizeof($hchkin_vcode_qty) > 0){
        if(sizeof($hchkin_vcode_qty) > 1){ $maxValue = 0; $maxValue = max($hchkin_vcode_qty); $secven_code = array_keys($hchkin_vcode_qty, $maxValue)[0]; }
        else{ $secven_code = array_keys($hchkin_vcode_qty)[0]; }
    }
    ...
    while($row = mysqli_fetch_assoc($query)){
        ...
        $total_feed_consumed_qty += ((float)$de_fkgs1 + (float)$de_fkgs2);
    ...
    while($row = mysqli_fetch_assoc($query)){
        ...
        $ficode = $fidate = ""; $farmer_price = 0;
        if($medicine_cost == "M"){
            $ficode = $row['item_code']; $fidate = $row['date'];
            $sqlf = "SELECT * FROM `farmer_item_price` WHERE `itemcode` = '$ficode' AND `active` = '1' AND `dflag` = '0' AND `id` IN (SELECT MAX(id) as id FROM `farmer_item_price` WHERE `itemcode` = '$ficode' AND `date` <= '$fidate' AND `active` = '1' AND `dflag` = '0')";
            $queryf = mysqli_query($conn,$sqlf); while($rowf = mysqli_fetch_assoc($queryf)){ $farmer_price = $rowf['rate']; }
        }
    ...
    //Sales
    $count = $cus_sale_birdno = $cus_sale_birdwt = $cus_sale_amount = $farmer_sales_amt = 0;
    $sql = "SELECT * FROM `broiler_sales` WHERE `farm_batch` = '$farm_batch' AND `active` = '1' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($query)){
        if($row['icode'] == $bird_code){
            ...
            if($row['sale_type'] == "FormMBSale" || $row['sale_type'] == "FarmerSale"){
    ...
    //Bird Sending Details
    $sql = "SELECT * FROM `broiler_bird_transferout` WHERE `fromwarehouse` = '$farm_code' AND `from_batch` = '$farm_batch' AND `active` = '1' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($query)){
        if($row['item_code'] == $bird_code){
            $cus_sale_birdno += (((float)$row['birds']));
            $cus_sale_birdwt += (((float)$row['weight']));
            $cus_sale_amount += ((float)$row['avg_amount']);
            //$farmer_sales_amt += ((float)$row['weight'] * (float)$row['avg_price']);
    ...
    while($row = mysqli_fetch_assoc($query)){
        ...
        //Transport Cost Calculations
        if($row['paid_by'] == "FromFarm"){ $transport_charges += (float)$row['transport_cost']; }
        
        if($start_date == ""){ $start_date = $row['date']; } else{ if(strtotime($start_date) >= strtotime($row['date'])){ $start_date = $row['date']; } }
        if($end_date == ""){ $end_date = $row['date']; } else{ if(strtotime($end_date) <= strtotime($row['date'])){ $end_date = $row['date']; } }
    }
    $count = 0;
    $sql = "SELECT * FROM `broiler_itemreturns` WHERE `warehouse` = '$farm_code' AND `farm_batch` = '$farm_batch' AND `active` = '1' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($query)){
        $so_id = $row['id']; $so_trnum = $row['trnum']; $so_dates = $row['date']; $so_codes = $row['itemcode']; $so_quant = $row['quantity']; $so_price = $row['price'];
         $so_amont = $row['amount'];
        $so_frsec = $row['warehouse']; $so_frbch = $row['farm_batch']; $so_tosec = $row['warehouse']; $so_tobch = $row['farm_batch']; $so_remrk = $row['remarks'];
        $so_ademp = $row['addedemp']; $so_adtme = $row['addedtime']; $so_udemp = $row['updatedemp']; $so_udtme = $row['updatedtime'];
        ...
        if(!empty($feed_cat[$so_codes])){
            ...
            $sql2 = "UPDATE `broiler_itemreturns` SET `price` = '$destk_prc1',`amount` = '$destk_amt1' WHERE `id` = '$so_id'";
            if(!mysqli_query($conn,$sql2)){ die("Error:-".mysqli_error($conn)); } else {
                $sql2 = "DELETE FROM `account_summary` WHERE `date` = '$so_dates' AND `trnum` = '$so_trnum' AND `item_code` IN ('$so_codes') AND `quantity` IN ('$so_quant') AND `location` IN ('$so_frsec','$so_tosec') AND `batch` IN ('$so_frbch','$so_tobch')";
                if(!mysqli_query($conn,$sql2)){ die("Error:-".mysqli_error($conn)); } else{
                    $il1 = ""; $il1 = $icat_code[$so_codes];
            ...
            $sql2 = "UPDATE `broiler_itemreturns` SET `price` = '$destk_prc1',`farmer_price` = '$farmer_price',`amount` = '$destk_amt1' WHERE `id` = '$so_id'";
            if(!mysqli_query($conn,$sql2)){ die("Error:-".mysqli_error($conn)); } else {
                $sql2 = "DELETE FROM `account_summary` WHERE `date` = '$so_dates' AND `trnum` = '$so_trnum' AND `item_code` IN ('$so_codes') AND `quantity` IN ('$so_quant') AND `location` IN ('$so_frsec','$so_tosec') AND `batch` IN ('$so_frbch','$so_tobch')";
                if(!mysqli_query($conn,$sql2)){ die("Error:-".mysqli_error($conn)); } else{
                    $il1 = ""; $il1 = $icat_code[$so_codes];
    ...
    //Farmer Receipt
    $sql = "SELECT * FROM `broiler_receipts` WHERE `ccode` = '$farmer_code' AND `farm_batch` = '$farm_batch' AND `vtype` LIKE '%farmer%' AND `active` = '1' AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql); $frmr_rct_amt = 0; while($row = mysqli_fetch_assoc($query)){ $frmr_rct_amt += (((float)$row['amount'])); }
    $frmr_rct_amt = round($frmr_rct_amt,2);
    $farmer_sales_amt = (float)$farmer_sales_amt - (float)$frmr_rct_amt;
    ...
    if($sold_birds > 0){ $avg_weight = round(((float)$sold_weight / (float)$sold_birds),3); } else { $avg_weight = 0; }
    if($sold_weight > 0){ $fcr = round(((float)$total_feed_consumed_qty / (float)$sold_weight),3); } else { $fcr = 0; }
    ...
    $fmr_act_prod_amt = round(((float)$fmr_stkin_chick_amt + (float)$fmr_total_feed_consumed_amt + (float)$fmr_admin_amt + (float)$fmr_total_medvac_consumed_amt),2);
    if($sold_weight > 0){ $fmr_act_prod_prc = round(((float)$fmr_act_prod_amt / (float)$sold_weight),2); } else { $fmr_act_prod_prc = 0; }
    
    //Std. Production Cost calculations
    $sql = "SELECT * FROM `broiler_gc_cop_standards` WHERE `std_code` = '$gc_code' AND `copabw_from_val` <= '$avg_weight' AND `copabw_to_val` >= '$avg_weight'"; // AND `copfcr_from_val` <= '$fcr' AND `copfcr_to_val` >= '$fcr'
    $query = mysqli_query($conn,$sql); $std_count = mysqli_num_rows($query);
    if($std_count > 0){ while($row = mysqli_fetch_assoc($query)){ $standard_prod_cost = (float)$row['copfcr_std_val']; } }
    ...
    if(round($fmr_act_prod_prc,2) <= round($standard_prod_cost,2)){        
        $sql = "SELECT * FROM `broiler_gc_pc_incentive` WHERE `std_code` = '$gc_code' AND `prod_from_inc` <= '$fmr_act_prod_prc' AND `prod_to_inc` >= '$fmr_act_prod_prc' AND `std_prod_cost` = '$standard_prod_cost'"; $query = mysqli_query($conn,$sql); $i = 0;
        while($row = mysqli_fetch_assoc($query)){
            $i++;
        ...
        $act_gc_prc = (float)$prod_inc_rate; $act_gc_amt = round(((float)$act_gc_prc * (float)$sold_weight),2);
        $actual_gc_per_kg = (float)$standard_cost + (float)$prod_inc_rate; $actual_gc_amount = round(((float)$actual_gc_per_kg * (float)$sold_weight),2);
    }
    else{
        $sql = "SELECT * FROM `broiler_gc_pc_decentive` WHERE `std_code` = '$gc_code' AND `prod_from_dec` <= '$fmr_act_prod_prc' AND `prod_to_dec` >= '$fmr_act_prod_prc' AND `std_prod_cost` = '$standard_prod_cost'"; $query = mysqli_query($conn,$sql); $i = 0;
        while($row = mysqli_fetch_assoc($query)){
            $i++;
        ...
        if($actual_gc_per_kg <= $minimum_cost){ $actual_gc_per_kg = (float)$minimum_cost; }
    ...
    /*********** Production Cost Incentive **********
    if(round($fmr_act_prod_prc,2) <= round($standard_prod_cost,2)){
        $sql = "SELECT * FROM `broiler_gc_pc_incentive` WHERE `std_code` = '$gc_code' AND `prod_from_inc` <= '$fmr_act_prod_prc' AND `prod_to_inc` >= '$fmr_act_prod_prc' AND `std_prod_cost` = '$standard_prod_cost'";
        $query = mysqli_query($conn,$sql); $rate_incs = $prod_inc_rate = 0;
        while($row = mysqli_fetch_assoc($query)){
            $prod_to_inc = $row['prod_to_inc'];
            $rate_incs = (float)$row['rate_inc'] / 100;
        }
        $inc_val1 = (float)$prod_to_inc - (float)$fmr_act_prod_prc;
        $prod_inc_rate = (float)$inc_val1 * (float)$rate_incs;
        ...
        $sql = "SELECT * FROM `broiler_gc_pc_decentive` WHERE `std_code` = '$gc_code' AND `prod_from_dec` <= '$fmr_act_prod_prc' AND `prod_to_dec` >= '$fmr_act_prod_prc' AND `std_prod_cost` = '$standard_prod_cost'"; $query = mysqli_query($conn,$sql); $i = 0;
        $query = mysqli_query($conn,$sql); $rate_decs = $prod_dec_rate = 0;
        while($row = mysqli_fetch_assoc($query)){
            $prod_from_dec = $row['prod_from_dec'];
            $rate_decs = (float)$row['prod_rate_dec'] / 100;
        }
        $dec_val1 = (float)$fmr_act_prod_prc - (float)$prod_from_dec;
        $prod_dec_rate = (float)$dec_val1 * (float)$rate_decs;

        $prod_dec_rate = round($prod_dec_rate,3);
        $act_gc_prc = -1 * (float)$prod_dec_rate; $act_gc_amt = round(((float)$act_gc_prc * (float)$sold_weight),2);
        $actual_gc_per_kg = (float)$standard_cost - (float)$prod_dec_rate; $actual_gc_amount = round(((float)$actual_gc_per_kg * (float)$sold_weight),2);
    }
    */
    /*********** Sales Incentive ***********/
    ...
    if($sale_inc_prc >= $sales_max_rates){ $sale_inc_prc = $sales_max_rates; } else{$sale_inc_prc = round($sale_inc_prc,4); }
    
    $sale_inc_amount = round(((float)$sale_inc_prc * (float)$sold_weight),3);

    /*********** Sales Incentive: Avg Body Weight ***********/
    if($sold_birds > 0){ $avgbd_size = round(((float)$sold_weight / (float)$sold_birds),3); } else { $avgbd_size = 0; }
    if((float)$avgbd_size > 0){
        $sql = "SELECT * FROM `broiler_si_avgwts` WHERE `std_code` = '$gc_code' AND `si_from_avgwt` <= '$avgbd_size' AND `si_to_avgwt` >= '$avgbd_size' AND `awti_max_prod_cost` >= '$fmr_act_prod_prc'";
        $query = mysqli_query($conn,$sql); $abd_count = mysqli_num_rows($query);
        if($abd_count > 0){
            while($row = mysqli_fetch_assoc($query)){ $siavgbg_weight = (float)$row['avgwt_value']; }
            if(((float)$sold_birds * (float)$siavgbg_weight * (float)$standard_cost) > 0){
                $avgwt_amount = round((((float)$sold_birds * (float)$siavgbg_weight * (float)$standard_cost) - (float)$standard_amount),2);
            }
            else{
                $avgwt_amount = 0;
            }
    ...
    /*********** Mortality Incentive ***********/
    $mort_inc_prc = $mort_inc_amount = $sm_c = $dm_c = 0;
    $sm_c = (((float)$placed_birds / 100) * (float)$standard_mortality);
    if((float)$total_mort_count < (float)$sm_c){
       $sql = "SELECT * FROM `broiler_gc_mi_incentive` WHERE `std_code` = '$gc_code' AND `mort_from_inc` <= '$total_mort_per' AND `mort_to_inc` >= '$total_mort_per'"; $query = mysqli_query($conn,$sql); $minc_count = mysqli_num_rows($query);
        if($minc_count > 0){
            while($row = mysqli_fetch_assoc($query)){
                $mt_cnt = $row['mort_to_inc']; if($mt_cnt == ""){ $mt_cnt = 0; }
                $sm_c = 0; $sm_c = (((float)$placed_birds / 100) * (float)$mt_cnt);
                
                $dm_c = (float)$sm_c - (float)$total_mort_count;
                $mort_inc_amount += ((float)$dm_c * $row['mort_rate_inc']);
    ...
    if($minc_count > 0){
        while($row = mysqli_fetch_assoc($query)){
            $mort_inc_amount += round(((float)$row['mort_rate_inc'] * (float)$sold_birds),2);
        }
    }
    else{ }
    if((float)$sold_birds != 0){ $mort_inc_prc = round(((float)$mort_inc_amount * (float)$sold_birds),2); }*/
    
    /*********** Unloading Charge Incentive ***********/
    $unloading_prc = round($gcm_unl_charge,5);
    $unloading_amt = round((($total_feed_consumed_qty / 1000) * $unloading_prc),0);

    $summer_inc_prc = $summer_inc_amount = $other_inc_amount = $ifft_charges = 0;

    $total_incentive =  round(((float)$actual_gc_amount + (float)$sale_inc_amount + (float)$avgwt_amount + (float)$mort_inc_amount + (float)$summer_inc_amount + (float)$unloading_amt + (float)$other_inc_amount + (float)$ifft_charges + (float)$fcr_inc_amt + (float)$transport_charges));
    ...
    $sm_c = (((float)$placed_birds / 100) * (float)$standard_mortality);
    if((float)$total_mort_count > (float)$sm_c){
       $sql = "SELECT * FROM `broiler_gc_mi_decentive` WHERE `std_code` = '$gc_code' AND `weeks` = 'GDO' AND `mort_from_dec` <= '$total_mort_per' AND `mort_to_dec` >= '$total_mort_per'"; $query = mysqli_query($conn,$sql); $mdcount = mysqli_num_rows($query);
        if($mdcount > 0){
            while($row = mysqli_fetch_assoc($query)){
                $dm_c = (float)$total_mort_count - (float)$sm_c;
                $mort_deduct_amount += ((float)$dm_c * $row['mort_rate_dec']);
            }
        }
    }
        ...
        if($mdcount > 0){
            while($row = mysqli_fetch_assoc($query)){
                if($row['mort_to_dec'] < (float)$total_mort_per){
                    ...
                    $mort_deduct_amount += (((((float)$amd1 * (float)$placed_birds) / 100)) * $row['mort_rate_dec']); // * $avg_weight
                }
                else if((float)$total_mort_per > (float)$row['mort_from_dec']){
                    $amd1 =  (float)$total_mort_per - (float)$row['mort_from_dec'];
                    $mort_deduct_amount += (((((float)$amd1 * (float)$placed_birds) / 100)) * $row['mort_rate_dec']); // * $avg_weight
                }
            }
        }
    ...
    $total_decentives = round(((float)$bird_shortage_amount + (float)$fcr_deduct_amount + (float)$mort_deduct_amount));
    ...
    if((float)$total_amount_payable <= 0){ $tds_lbl = 0; $tds_amount = $tds_per = 0; }
    
    $farmer_payable = round(((float)$total_amount_payable - (float)$tds_amount));
    if((float)$sold_birds != 0){ $per_bird_price = round(((float)$total_amount_payable / (float)$sold_birds),2); } else{ $per_bird_price = 0; }
