    ...
    <body class="m-0 hold-transition">
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                            <div class="float-left"><h3 class="card-title">Add Sector-Unit Mapping</h3></div>
                        </div>
                        <div class="card-body">
                            <div class="col-md-12">
                                <form action="breeder_save_unitmap1.php" method="post" role="form" onsubmit="return checkval()">
                                    <div class="row">
                                            ...
                                            <table style="width:auto;">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th style="text-align:center;"><label>Units<b style="color:red;">&nbsp;*</b></label></th>
                                                ...
                                                <tbody id="tbody">
                                                    <tr>
                                                        ...
                                                        <td><select name="unit_code[]" id="unit_code[0]" class="form-control select2" style="width:190px;" onchange="check_duplicate(this.id);"><option value="select">select</option><?php foreach($unit_code as $bcode){ ?><option value="<?php echo $bcode; ?>"><?php echo $unit_name[$bcode]; ?></option><?php } ?></select></td>
                                                        ...
                                                        <td style="visibility:visible;"><input type="text" name="dupflag[]" id="dupflag[0]" class="form-control text-right" value="0" style="width:20px;" readonly /></td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
        ...
        <script>
			function checkval(){
				...
				var sector_code = unit_code = scode = bcode = ""; var dupflag = a = e = 0;
				var l = true;
				for(var d = 0;d <= incr;d++){
					if(l == true){
						a = d + 1;
						sector_code = document.getElementById("sector_code["+d+"]").value;
						unit_code = document.getElementById("unit_code["+d+"]").value;
						...
						if(sector_code == "select"){
							...
							alert("Please select Unit in row: "+a);
							document.getElementById("unit_code["+d+"]").focus();
							l = false;
						}
                            ...
                            for(var c = 0;c <= incr;c++){
                                if(l == true){
                                    if(c == d){ }
                                        ...
                                        bcode = document.getElementById("unit_code["+c+"]").value;
                                        if(sector_code == scode || unit_code == bcode){
                                            e = c + 1;
                                            alert("Same Sector (or) Unit mappings are already available in row: "+e);
                                            document.getElementById("sector_code["+d+"]").focus();
                                            l = false;
                                        }
                                    }
			...
			function create_row(a){
                ...
                html += '<td><select name="unit_code[]" id="unit_code['+d+']" class="form-control select2" style="width:190px;" onchange="check_duplicate(this.id);"><option value="select">select</option><?php foreach($unit_code as $bcode){ ?><option value="<?php echo $bcode; ?>"><?php echo $unit_name[$bcode]; ?></option><?php } ?></select></td>';
                ...
                html += '<td style="visibility:visible;"><input type="text" name="dupflag[]" id="dupflag['+d+']" class="form-control text-right" value="0" style="width:20px;" /></td>';
				html += '</tr>';
				$('#tbody').append(html);
				$('.select2').select2();
            }
            ...
            function check_duplicate(a){
                ...
                if(sector_code != "select" && unit_code != "select"){
                    ...
                    var method = "GET";
                    var url = "breeder_check_unitmap1_duplicate.php?sector_code="+sector_code+"&unit_code="+unit_code+"&row_count="+d;
                    //window.open(url);
