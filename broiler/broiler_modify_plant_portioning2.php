<?php
//broiler_modify_plant_portioning2.php
...
$ids = $_POST['idvalue']; $incr = $prefix = $trnum = $ltrnum = $bchno = $aemp = $atime = "";
$sql = "SELECT * FROM `plant_bird_portioning_produced_details` WHERE `trnum` = '$ids' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql);
while($row = mysqli_fetch_assoc($query)){
    if($trnum == ""){ $incr = $row['incr']; $prefix = $row['prefix']; $trnum = $row['trnum']; $ltrnum = $row['link_trnum']; $bchno = $row['batch_no']; $aemp = $row['addedemp']; $atime = $row['addedtime']; }
}
if($trnum != ""){
    $sql1 = "SELECT * FROM `plant_bird_portioning_consumed_details` WHERE `trnum` = '$trnum' AND `link_trnum` = '$ltrnum' AND `batch_no` = '$bchno' AND `dflag` = '0'";
    $query1 = mysqli_query($conn,$sql1);
    while($row1 = mysqli_fetch_assoc($query1)){
        $icode = $isize = ""; $ibirds = $iweight = 0;
        $icode = $row1['item_code'];
        $isize = $row1['isize_code'];
        $ibirds = $row1['birds'];
        $iweight = $row1['weight'];

        $sql2 = "SELECT * FROM `plant_bird_grading_item_stocks` WHERE `trnum` = '$ltrnum' AND `batch_no` = '$bchno' AND `item_code` = '$icode' AND `isize_code` = '$isize' AND `active` = '1' AND `dflag` = '0'";
        $query2 = mysqli_query($conn,$sql2);
        while($row2 = mysqli_fetch_assoc($query2)){
            $gbirds = $gweight = $abirds = $aweight = 0; $portioning_flag = 1;
            $gbirds = (float)$row2['birds'];
            $gweight = (float)$row2['weight'];
            $abirds = (float)$row2['avl_birds'];
            $aweight = (float)$row2['avl_weight'];
            $abirds = (float)$abirds + (float)$ibirds;
            $aweight = (float)$aweight + (float)$iweight;

            if($abirds == $gbirds && $aweight == $gweight){ $portioning_flag = 0; }
            
            $sql3 = "UPDATE `plant_bird_grading_item_stocks` SET `avl_birds` = '$abirds',`avl_weight` = '$aweight',`portioning_flag` = '$portioning_flag',`updatedemp` = '$addedemp',`updatedtime` = '$addedtime' WHERE `trnum` = '$ltrnum' AND `batch_no` = '$bchno' AND `item_code` = '$icode' AND `isize_code` = '$isize' AND `active` = '1' AND `dflag` = '0'";
            if(!mysqli_query($conn,$sql3)){ die("Error:-".mysqli_error($conn)); } else{ }
        }
    }

    $sql = "DELETE FROM `plant_bird_portioning_produced_details` WHERE `trnum` = '$trnum' AND `dflag` = '0'";
    if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); } else{ }
    $sql = "DELETE FROM `plant_bird_portioning_consumed_details` WHERE `trnum` = '$trnum' AND `dflag` = '0'";
    if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); } else{ }
    $sql = "DELETE FROM `account_summary` WHERE `trnum` = '$trnum' AND `dflag` = '0'";
    if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); } else{ }

    $sql = "SELECT * FROM `plant_bird_grading_item_stocks` WHERE `trnum` = '$ltrnum' AND `batch_no` = '$bchno' AND `portioning_flag` = '1' AND `active` = '1' AND `dflag` = '0'";
    $query = mysqli_query($conn,$sql); $p_cnt = mysqli_num_rows($query);
    if((int)$p_cnt > 0){ }
    else{
        $sql = "UPDATE `plant_bird_grading_details` SET `portioning_flag` = '0' WHERE `trnum` = '$ltrnum' AND `batch_no` = '$bchno' AND `active` = '1' AND `dflag` = '0'";
        if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); } else{ }
    }
}
...
for($i = 0; $i < $dsize;$i++){
    ...
    if((float)$isize_weight[$i] != 0){
        ...
        $sql = "INSERT INTO `plant_bird_portioning_consumed_details` (`trnum`,`link_trnum`,`date`,`receive_type`,`cus_code`,`batch_no`,`mnubch_no`,`billno`,`item_category`,`item_subcategory`,`item_code`,`isize_code`,`birds`,`weight`,`warehouse`,`remarks`,`remarks2`,`flag`,`active`,`dflag`,`trtype`,`trlink`,`addedemp`,`addedtime`,`updatedemp`,`updatedtime`) 
        VALUES ('$trnum','$link_trnum','$date','$receive_type','$cus_code','$batch_no','$mnubch_no','$billno','$item_category','$item_subcategory','$item_code[$i]','$isize_code[$i]','$isize_birds[$i]','$isize_weight[$i]','$warehouse','$remarks','$remarks2[$i]','$flag','$active','$dflag','$trtype','$trlink','$aemp','$atime','$addedemp','$addedtime')";
        if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); }
            ...
            $price = $amount = 0;
            $from_post = "INSERT INTO `account_summary` (crdr,coa_code,date,dc_no,trnum,item_code,isize_code,plant_batch,quantity,price,amount,location,batch,vehicle_code,driver_code,remarks,gc_flag,flag,active,dflag,etype,addedemp,addedtime,updatedemp,updatedtime) 
            VALUES ('CR','$coa_code','$date','$billno','$trnum','$item_code[$i]','$isize_code[$i]','$batch_no','$isize_weight[$i]','$price','$amount','$warehouse','','','','$remarks','0','$flag','$active','$dflag','Plant Portioning: Item Consumed','$aemp','$atime','$addedemp','$addedtime')";
...
for($i = 0; $i < $dsize;$i++){
    ...
    if((float)$port_weight[$i] != 0){
        ...
        $sql = "INSERT INTO `plant_bird_portioning_produced_details` (`incr`,`prefix`,`trnum`,`date`,`receive_type`,`cus_code`,`batch_no`,`mnubch_no`,`link_trnum`,`billno`,`item_code`,`weight`,`yield_per`,`warehouse`,`remarks`,`remarks2`,`total_weight`,`process_loss_weight`,`flag`,`active`,`dflag`,`trtype`,`trlink`,`addedemp`,`addedtime`,`updatedemp`,`updatedtime`) 
        VALUES ('$incr','$prefix','$trnum','$date','$receive_type','$cus_code','$batch_no','$mnubch_no','$link_trnum','$billno','$port_icode[$i]','$port_weight[$i]','$port_yield[$i]','$warehouse','$remarks','$remarks2[$i]','$tot_port_weight','$tot_pl_weight','$flag','$active','$dflag','$trtype','$trlink','$aemp','$atime','$addedemp','$addedtime')";
        if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); }
        else{
            $coa_code = $icat_iac[$icat_code[$port_icode[$i]]];
            $price = $amount = 0;
            $from_post = "INSERT INTO `account_summary` (crdr,coa_code,date,dc_no,trnum,item_code,plant_batch,mnubch_no,quantity,price,amount,location,batch,vehicle_code,driver_code,remarks,gc_flag,flag,active,dflag,etype,addedemp,addedtime,updatedemp,updatedtime) 
            VALUES ('DR','$coa_code','$date','$billno','$trnum','$port_icode[$i]','$batch_no','$mnubch_no','$port_weight[$i]','$price','$amount','$warehouse','','','','$remarks','0','$flag','$active','$dflag','Plant Portioning: Item Consumed','$aemp','$atime','$addedemp','$addedtime')";
            if(!mysqli_query($conn,$from_post)){ die("Error:-".mysqli_error($conn)); } else{ }
