...
$sql ="SELECT * FROM `item_details` WHERE `description` = '$cdesc' AND `dflag` = '0'"; $query = mysqli_query($conn,$sql); $ccount = mysqli_num_rows($query);
if($ccount > 0){ ?>
	<script>
        var ccid = '<?php echo $ccid; ?>';
		var x = alert("The Description is already available \n Please check and try again ..!");
		if(x == true){ window.location.href = "broiler_add_item.php?ccid="+ccid; } else if(x == false) { window.location.href = "broiler_add_item.php?ccid="+ccid; } else { window.location.href = "broiler_add_item.php?ccid="+ccid; }
	</script>
<?php
}
else {
	...
	$sql = "INSERT INTO `item_details` (incr,prefix,category,sub_category,hsn_code,code,description,sunits,cunits,einv_units,item_size,offals_flag,bfamf_flag,sector_access,ob_stock,price,amount,ob_date,gst_code,lsflag,lsqty,addedemp,addedtime) VALUES 
	('$incrs','$prefix','$ctype','$sub_category','$hsn_code','$code','$cdesc','$sunits','$cunits','$einv_units','$item_size','$offals_flag','$bfamf_flag','$sector_access','$ob_stock','$price','$amount','$ob_date','$gst_code','$lsflag','$lsqty','$addedemp','$addedtime')";
	if(!mysqli_query($conn,$sql)){ die("Error:-".mysqli_error($conn)); } else { header('location:broiler_display_item.php?ccid='.$ccid); }
}