        ...
        <section class="content">
            <div class="container-fluid">
                <div class="content-header">
                    <div class="container-fluid">
                        <form action="controlpanel2_1_ta.php" method="post">
                            <!--<div class="row">
                                <div class="col-md-2">
                    ...
                    <div class="row">
                        ...
                        foreach($sorts as $sorting){
                            ...
                            if($sorting == "Customer Supplier Balance-List" && (int)$Customer_Supplier_Balance_details == 1){
                                ...
                                while($row = mysqli_fetch_assoc($query)){ $sup_code[$row['code']] = $row['code']; if($row['obtype'] == "Cr" || $row['obtype'] == "CR"){ $cus_obcramt += (float)$row['obamt']; } else{ $cus_obdramt += (float)$row['obamt']; } }
                                ...
                                echo $sql_record = "SELECT SUM(amount) as amount FROM `broiler_receipts` WHERE `date` <= '$today' AND `ccode` IN ('$cus_list') AND `vtype` IN ('Customer') AND `active` = '1' AND `dflag` = '0'";
                                ...
                                $sql_record = "SELECT * FROM `master_receipts` WHERE `date` < '$today' AND `to_account` IN ('$cus_list') AND `t_type` IN ('Customer Receipt') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
                                $query = mysqli_query($conn,$sql_record); $transaction_count = 0; if(!empty($query)){ $transaction_count = mysqli_num_rows($query); }
                                if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ $cus_receipts += (float)$row['amount']; } }
                                ...
                                if($transaction_count > 0){ while($row = mysqli_fetch_assoc($query)){ $opening_vcr += (float)$row['cr_amt']; $opening_vdr[$row['vcode']] += (float)$row['dr_amt']; } }
                                ...
                                $query = mysqli_query($conn, $sql_record); $i = 0; $transaction_count = 0; if (!empty($query)) { $transaction_count = mysqli_num_rows($query); }
                                if($transaction_count > 0){ while ($row = mysqli_fetch_assoc($query)) { $opening_tloss += (float)$row['amount']; } }
                                ...
                                $tot_cus_bal = (($cus_sales + $cus_cdn + $cus_contra_dr + $cus_obdramt + $opening_vdr + $opening_payments) - ($cus_receipts + $cus_returns + $cus_ccn + $cus_contra_cr + $cus_obcramt + $opening_vcr + $opening_tloss));
