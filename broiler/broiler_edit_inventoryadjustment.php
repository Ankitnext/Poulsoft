<?php
//broiler_edit_inventoryadjustment.php
...
if($link_active_flag > 0){
    ...
    if($acount == 1){
        ...
        $sql = "SELECT * FROM `extra_access` WHERE `field_name` LIKE 'Stock Adjustment' AND `field_function` LIKE 'Bags' AND `flag` = '1'"; $query = mysqli_query($conn,$sql); $bag_access_flag = mysqli_num_rows($query);
...
<html lang="en">
    ...
    <body class="m-0 hold-transition">
        <?php
        $trnum = $_GET['trnum'];
        $sql = "SELECT * FROM `broiler_inv_adjustment` WHERE `trnum` = '$trnum'"; $query = mysqli_query($conn,$sql); $i = 0;
        while($row = mysqli_fetch_assoc($query)){
            $date = $row['date'];
            $doc_no = $row['doc_no'];
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                            <div class="float-left"><h3 class="card-title">Edit Inventory Adjustment</h3></div>
                        </div>
                        <div class="m-0 p-2 card-body">
                            <div class="col-md-18">
                                <form action="broiler_modify_inventoryadjustment.php" method="post" role="form" onsubmit="return checkval()">
                                    <div class="row">
                                        ...
                                        <div class="form-group">
                                            <label>Bill No.</label>
                                            <input type="text" name="billno" id="billno" class="form-control" value="<?php echo $doc_no; ?>" style="width:60px;" >
                                        </div>
                                        <div class="form-group">
                                    ...
                                    <table>
                                        ...
                                        <tbody id="row_body">
                                            <tr>
                                                ...
                                                <td style="visibility:visible;"><input type="text" name="available_stock" id="available_stock" class="form-control" placeholder="0.00" style="width:90px;" readonly ></td>
        ...
        <script>
            ...
            function checkval(){
                ...
                if(date == ""){
                    ...
                    alert("Kindly selecy Add/Deduct Type ");
            ...
            function fetch_stock_master(){
				...
				var method = "GET";
				var url = "broiler_fetch_itemstockmaster_lsfi.php?sector="+sector+"&item_code="+item_code+"&date="+date+"&id="+idvalue+"&trtype=stkadjustment&etype=stkadjustment";
                //window.open(url);
            ...
            function fetch_stock_master2(){
                ...
                var fetch_items = new XMLHttpRequest();
				var method = "GET";
				var url = "broiler_fetch_itemstockmaster_lsfi.php?sector="+sector+"&item_code="+item_code+"&date="+date+"&id="+idvalue+"&trtype=stkadjustment";
                //window.open(url);
				var asynchronous = true;
				fetch_items.open(method, url, asynchronous);
				fetch_items.send();
