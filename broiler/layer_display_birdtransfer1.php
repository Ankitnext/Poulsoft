<?php
//layer_display_birdtransfer1.php
include "newConfig.php";
include "number_format_ind.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['birdtransfer1'] = $cid; } else{ $cid = $_SESSION['birdtransfer1']; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            if(in_array("layer_bird_transfer", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.layer_bird_transfer LIKE poulso6_admin_broiler_broilermaster.layer_bird_transfer;"; mysqli_query($conn,$sql1); }
            ...
            if(in_array("bfeed_flag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `item_category` ADD `bfeed_flag` INT(100) NOT NULL DEFAULT '0' COMMENT 'layer Feed' AFTER `main_category`"; mysqli_query($conn,$sql); }
            if(in_array("begg_flag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `item_category` ADD `begg_flag` INT(100) NOT NULL DEFAULT '0' COMMENT 'layer Eggs' AFTER `bfeed_flag`"; mysqli_query($conn,$sql); }
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        $sector_name = array();
                                        $sql = "SELECT * FROM `layer_shed_allocation` WHERE `dflag` = '0' ORDER BY `description` ASC";
                                        $query = mysqli_query($conn,$sql); $bflk_name = array();
                                        while($row = mysqli_fetch_assoc($query)){ $bflk_name[$row['code']] = $row['description']; }

                                        $delete_url = $delete_link."?utype=delete&trnum=";
                                        $sql = "SELECT * FROM `layer_bird_transfer` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `dflag` = '0' AND `trlink` = '$href' ORDER BY `date`,`trnum` ASC";
                                    ...
                                    <tr>
										...
										<td><?php echo $bflk_name[$row['from_flock']]; ?></td>
										<td style="text-align:right;"><?php echo round($row['lyr_bqty'],5); ?></td>
										<td><?php echo $bflk_name[$row['to_flock']]; ?></td>
        ...
        <script>
			function checkdelete(x){
                ...
                if(trnum != ""){
                    ...
                    var method = "GET";
                    var url = "layer_check_birdtransfer1.php?trnum="+trnum;
                    //window.open(url);
