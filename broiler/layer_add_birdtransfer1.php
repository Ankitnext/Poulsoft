    ...
    <body class="m-0 hold-transition">
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        <div class="card-header">
                            <div class="float-left"><h3 class="card-title">Add Bird Transfer</h3></div>
                        </div>
                        <div class="pl-2 card-body">
                            <form action="layer_save_birdtransfer1.php" method="post" role="form" onsubmit="return checkval()">
                                <div class="row row_body2">
                                    <table class="p-1 table1" style="width:auto;" align="center">
                                        ...
                                        <tbody id="tbody">
                                            <tr>
                                                <td><select name="to_flock[]" id="to_flock[0]" class="form-control select2" style="width:190px;"><option value="select">-select-</option><?php foreach($bflk_code as $ucode){ ?><option value="<?php echo $ucode; ?>"><?php echo $bflk_name[$ucode]; ?></option><?php } ?></select></td>
                                                <td><input type="text" name="lyr_bqty[]" id="lyr_bqty[0]" class="form-control text-right" style="width:90px;" onkeyup="validate_count(this.id);cal_tot_qty();" /></td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <th><input type="text" name="tot_qty" id="tot_qty" class="form-control text-right" style="width:90px;" readonly /></th>
                                                <th></th>
                                                <th></th>
                                            </tr>
                                        </tfoot>
        ...
        <script>
			function checkval(){
                ...
                if(date == ""){
                    ...
                    for(var d = 0;d <= incr;d++){
                        if(l == true){
                            e = d + 1;
                            to_flock = document.getElementById("to_flock["+d+"]").value;
                            lyr_bqty = document.getElementById("lyr_bqty["+d+"]").value; if(lyr_bqty == ""){ lyr_bqty = 0; }
                            
                            if(to_flock == "" || to_flock == "select"){
                                alert("Please select To Flock in row: "+e);
                                document.getElementById("to_flock["+d+"]").focus();
                                l = false;
                            }
                            else if(from_flock == to_flock){
                                alert("From and To Flocks are same in row: "+e);
                                document.getElementById("to_flock["+d+"]").focus();
                                l = false;
                            }
                            else if(parseInt(lyr_bqty) == 0 ){
                                alert("Please enter Bird quantity in row: "+e);
                                document.getElementById("lyr_bqty["+d+"]").focus();
                                l = false;
                            }
                            else{ }
                        }
			...
			function create_row(a){
                ...
                html += '<tr id="row_no['+d+']">';
                html += '<td><select name="to_flock[]" id="to_flock['+d+']" class="form-control select2" style="width:190px;"><option value="select">-select-</option><?php foreach($bflk_code as $ucode){ ?><option value="<?php echo $ucode; ?>"><?php echo $bflk_name[$ucode]; ?></option><?php } ?></select></td>';
                html += '<td><input type="text" name="lyr_bqty[]" id="lyr_bqty['+d+']" class="form-control text-right" style="width:90px;" onkeyup="validate_count(this.id);cal_tot_qty();" /></td>';
            ...
            function fetch_bird_details(){
                ...
                if(date == "" || from_flock == "" || from_flock == "select"){ update_ebtn_status(0); }
                    ...
                    var method = "GET";
                    var url = "layer_fetch_avlstock_quantity.php?date="+date+"&fflock="+from_flock+"&rows=0&ftype=bird_transfer&ttype=add";
                    //window.open(url);
            ...
            function cal_tot_qty(){
                var incr = document.getElementById("incr").value;
                var tot_qty = lyr_bqty = 0;
                for(var d = 0;d <= incr;d++){
                    lyr_bqty = 0;
                    lyr_bqty = document.getElementById("lyr_bqty["+d+"]").value; if(lyr_bqty == ""){ lyr_bqty = 0; }
                    tot_qty = parseFloat(tot_qty) + parseFloat(lyr_bqty);
                }
                document.getElementById("tot_qty").value = parseFloat(tot_qty).toFixed(0);
            }
