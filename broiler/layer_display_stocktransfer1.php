<?php
//layer_display_stocktransfer1.php
include "newConfig.php";
include "number_format_ind.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['stocktransfer1'] = $cid; } else{ $cid = $_SESSION['stocktransfer1']; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            if(in_array("layer_dayentry_consumed", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.layer_dayentry_consumed LIKE poulso6_admin_broiler_broilermaster.layer_dayentry_consumed;"; mysqli_query($conn,$sql1); }
            if(in_array("layer_dayentry_produced", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.layer_dayentry_produced LIKE poulso6_admin_broiler_broilermaster.layer_dayentry_produced;"; mysqli_query($conn,$sql1); }
            ...
            if(in_array("brd_sflag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `inv_sectors` ADD `brd_sflag` INT(100) NOT NULL DEFAULT '0' COMMENT 'layer Sectors' AFTER `dflag`"; mysqli_query($conn,$sql); }
            ...
            if(in_array("bffeed_flag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `item_category` ADD `bffeed_flag` INT(100) NOT NULL DEFAULT '0' COMMENT 'layer Feed' AFTER `main_category`"; mysqli_query($conn,$sql); }
            if(in_array("bmfeed_flag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `item_category` ADD `bmfeed_flag` INT(100) NOT NULL DEFAULT '0' COMMENT 'layer Feed' AFTER `bffeed_flag`"; mysqli_query($conn,$sql); }
            if(in_array("begg_flag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `item_category` ADD `begg_flag` INT(100) NOT NULL DEFAULT '0' COMMENT 'layer Eggs' AFTER `bmfeed_flag`"; mysqli_query($conn,$sql); }
            if(in_array("bmv_flag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `item_category` ADD `bmv_flag` INT(100) NOT NULL DEFAULT '0' COMMENT 'layer MedVac' AFTER `begg_flag`"; mysqli_query($conn,$sql); }
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        $sector_name = array();
                                        $sql = "SELECT * FROM `layer_farms` WHERE `active` = '1' AND `dflag` = '0' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
                                        while($row = mysqli_fetch_assoc($query)){ $sector_name[$row['code']] = $row['description']; }
                                        $sql = "SELECT * FROM `layer_units` WHERE `active` = '1' AND `dflag` = '0' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
                                        while($row = mysqli_fetch_assoc($query)){ $sector_name[$row['code']] = $row['description']; }
                                        $sql = "SELECT * FROM `layer_sheds` WHERE `active` = '1' AND `dflag` = '0' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
                                        while($row = mysqli_fetch_assoc($query)){ $sector_name[$row['code']] = $row['description']; }
                                        ...
                                        $sql = "SELECT * FROM `item_stocktransfers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `dflag` = '0' AND `trtype` = 'stocktransfer1' AND `trlink` = 'layer_display_stocktransfer1.php' ORDER BY `id` DESC";
                                    ...
                                    <tr>
										...
										<td style="text-align:right;"><?php echo round($row['quantity'],5); ?></td>
        ...
        <script>
			function checkdelete(x){
                ...
                if(trnum != ""){
                    ...
                    var method = "GET";
                    var url = "layer_check_stocktransfer1.php?trnum="+trnum;
                    //window.open(url);
