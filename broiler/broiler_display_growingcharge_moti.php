<?php
//broiler_display_growingcharge_moti.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['growingcharge_moti'] = $cid; } else{ $cid = $_SESSION['growingcharge_moti']; }
...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            if(in_array("ifft_charges", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `ifft_charges` double(20,2) NULL DEFAULT NULL COMMENT 'IFFT Charges' AFTER `sales_incentive_amt`"; mysqli_query($conn,$sql); }
            if(in_array("fcr_incentive_prc", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `fcr_incentive_prc` double(20,2) NULL DEFAULT NULL COMMENT 'Equipment Charges' AFTER `tds_amt`"; mysqli_query($conn,$sql); }
            if(in_array("equipment_charges", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `equipment_charges` double(20,2) NULL DEFAULT NULL COMMENT 'Equipment Charges' AFTER `tds_amt`"; mysqli_query($conn,$sql); }
            if(in_array("actual_charge_exp_prc", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `actual_charge_exp_prc` double(20,2) NULL DEFAULT NULL COMMENT 'Equipment Charges' AFTER `grow_charge_exp_prc`"; mysqli_query($conn,$sql); }
            if(in_array("actual_charge_exp_amt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `broiler_rearingcharge` ADD `actual_charge_exp_amt` double(20,2) NULL DEFAULT NULL COMMENT 'Equipment Charges' AFTER `actual_charge_exp_prc`"; mysqli_query($conn,$sql); }
            ...
            $sql = "SELECT * FROM `extra_access` WHERE `field_name` IN ('Pause/Active','Authorize','FarmerGCSingleWapp:broiler_display_page') AND `user_access` LIKE '%$user_code%' OR `field_name` IN ('Pause/Active','Authorize','FarmerGCSingleWapp:broiler_display_page') AND `user_access` LIKE 'all'"; $query = mysqli_query($conn,$sql);
            while($row = mysqli_fetch_assoc($query)){ if($row['field_name'] == "Pause/Active"){ $paflag = $row['flag']; } else if($row['field_name'] == "Authorize"){ $autflag = $row['flag']; } else if($row['field_name'] == "FarmerGCSingleWapp:broiler_display_page"){ $fgc_wapp = $row['flag']; } else{ } }
            if($paflag == "" || $paflag == NULL || $paflag == 0){ $paflag = 0; }
            if($autflag == "" || $autflag == NULL || $autflag == 0){ $autflag = 0; }
            if($fgc_wapp == "" || $fgc_wapp == NULL || $fgc_wapp == 0){ $fgc_wapp = 0; }
                    ...
                    <div class="m-0 p-0 card">
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        $sql = "SELECT * FROM `broiler_farm` WHERE `dflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $item_name = array();
                                        while($row = mysqli_fetch_assoc($query)){ $farm_name[$row['code']] = $row['description']; }
                                        $sql = "SELECT * FROM `broiler_batch` WHERE `dflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $item_name = array();
                                        ...
                                        while($row = mysqli_fetch_assoc($query)){
                                            ...
                                            $print_url1 = "print/Examples/farmerrearingchargeprint_moti.php?trnum=".$id;
                                            $print_url2 = "print/Examples/mgmtrearingchargeprint_moti.php?trnum=".$id;
                                            $print_url3 = "print/Examples/farmerrearingchargeprint_moti_ty_new1.php?trnum=".$id;
                                            $print_url4 = "print/Examples/farmerrearingchargeprint_moti_ty_newm.php?trnum=".$id;
                                            $authorize_url = $update_link."?utype=authorize&id=".$id;
                                            $fgc_wapp_url = "broiler_sendsinglefarmergcwapp.php?trnum=".$id."&url=".$file_name[0]."?ccid=".$cid;
                                            if($row['active'] == 1){
                                    ...
                                    <tr>
                                        ...
                                        <?php
                                            if($row['flag'] == 1){
                                                ...
                                                if($fgc_wapp == 1){
                                                    echo "<a href='".$fgc_wapp_url."'><i class='fa-brands fa-whatsapp' style='color:#3AC418;' title='Send Farmer-GC WhatsApp'></i></a>&ensp;&ensp;";
                                                }
                                                if($print_flag == 1){
                                                    //echo "<a href='".$print_url1."' target='_BLANK'><i class='fa fa-print' style='color:black;' title='Farmer Report'></i></a>&ensp;&ensp;";
                                                    echo "<a href='".$print_url1."' target='_BLANK'><i class='fa fa-print' style='color:brown;' title='Farmer Report'></i></a>&ensp;&ensp;";
                                                    echo "<a href='".$print_url2."' target='_BLANK'><i class='fa fa-print' style='color:green;' title='Management Report'></i></a>&ensp;&ensp;";
                                                    echo "<a href='".$print_url3."' target='_BLANK'><i class='fa fa-print' style='color:orange;' title='Management Report'></i></a>&ensp;&ensp;";
                                                    echo "<a href='".$print_url4."' target='_BLANK'><i class='fa fa-print' style='color:blue;' title='Management Report'></i></a>&ensp;&ensp;";
                                                }
                                            }
                                        ?>
                                        </td>
