<?php
//broiler_display_purchase_lsfi.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $cid = $_GET['ccid'];
if($cid != ""){ $_SESSION['purchase_lsfi'] = $cid; } else{ $cid = $_SESSION['purchase_lsfi']; }
...
if($link_active_flag > 0){
    ...
    $colarr1=array('receive_date');

    $q='show columns from '.$table_name;
    $qr=mysqli_query($conn,$q) or die(mysql_error());
    $i=0;
    $columns=array();
    ...
    $diff_array1=array_diff($colarr1,$columns);

    if(in_array('receive_date',$diff_array1))
    {
        $q="ALTER TABLE `".$table_name."`  ADD `receive_date` DATE NULL DEFAULT NULL";
        $qr=mysqli_query($conn,$q) or die(mysql_error());
    }
?>
<html lang="en">
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM `".$table_name."` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `dflag` = '0' $cond_assigned ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['trnum'];
                                            ...
                                            $lqt_url = "broiler_add_inventorylabtest.php?utype=lqt&trnum=".$id;
                                            $lqte_url = "broiler_edit_inventorylabtest.php?utype=edit&link_trnum=".$id;
                                            $id2 = $row['trnum'];
                                    ...
                                    <tr>
										...
										<td><?php if(!empty($vehicle_name[$row['vehicle_code']])){ echo $vehicle_name[$row['vehicle_code']]; } else if($row['vehicle_code'] == "select"){ echo ""; } else{ echo $row['vehicle_code']; } ?></td>
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-min" role="document">
                                <div class="modal-content">
                                    <div class="modal-body text-left">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <form action="broiler_upload_purchase_lsfi.php" method="post" onsubmit="return add_newcomment()" enctype="multipart/form-data">
