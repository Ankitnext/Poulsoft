...
<html lang="en">
    ...
    <body class="m-0 hold-transition sidebar-mini">
        ...
        if($acount == 1){
            ...
            if(in_array("layer_breed_details", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.layer_breed_details LIKE poulso6_admin_broiler_broilermaster.layer_breed_details;"; mysqli_query($conn,$sql1); }
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM `layer_breed_details` WHERE `dflag` = '0' AND `trtype` = 'breedname1' AND `trlink` = 'layer_display_breedname1.php' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql); $c = 0;
                                        while($row = mysqli_fetch_assoc($query)){
                                            $id = $row['id'];
        ...
        <script>
			function checkdelete(x){
                ...
                if(id != ""){
                    ...
                    var method = "GET";
                    var url = "layer_check_breedname1.php?id="+id+"&code="+code;
                    //window.open(url);
                    ...
                    inv_items.onreadystatechange = function(){
                        if(this.readyState == 4 && this.status == 200){
                            ...
                            if(parseFloat(count) > 0){
                                alert("You can't delete the Layer Name: "+name+", as Layer Name is already in use!");
                            }
                            else{
                                var b = "<?php echo $delete_url; ?>"+id;
                                var c = confirm("are you sure you want to delete the Layer Name: "+name+"?");
                                if(c == true){
