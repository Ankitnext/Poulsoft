<?php
//breeder_add_send_stocks1.php
include "newConfig.php";
$user_name = $_SESSION['users']; $user_code = $_SESSION['userid']; $ccid = $_SESSION['send_stocks1'];
    ...
    <body class="m-0 hold-transition">
                ...
                <div class="m-0 p-0 container-fluid">
                    <div class="m-0 p-0 card">
                        ...
                        <div class="pl-2 card-body">
                            <form action="breeder_save_send_stocks1.php" method="post" role="form" onsubmit="return checkval()">
                                    ...
                                    <table class="p-1 table1" style="width:auto;">
                                        ...
                                        <tbody id="tbody">
                                            <tr>
                                                ...
                                                <td><input type="text" name="quantity[]" id="quantity[0]" class="form-control text-right" style="width:90px;" onkeyup="validatenum(this.id);cal_stk_qty(this.id); calculate_amount(this.id);" onchange="validateamount(this.id);" /></td>
                                                <td><input type="text" name="price[]" id="price[0]" class="form-control text-right" style="width:90px;"  onkeyup="validatenum(this.id); calculate_amount(this.id)"/></td>
                                                <td><input type="text" name="amount[]" id="amount[0]" class="form-control text-right" style="width:90px;" onkeyup="validatenum(this.id); calculate_amount(this.id)" /></td>
        ...
        <script>
            ...
            function calculate_amount(a) {
                var b = a.split("["); var c = b[1].split("]"); var d = c[0];  // This will be your row ID or unique identifier
                let quantity = parseFloat(document.getElementById('quantity[' + d + ']').value); if (isNaN(quantity)) { quantity = 0; }  // Handle case when input is not a number
                let price = parseFloat(document.getElementById('price[' + d + ']').value);
                if (isNaN(price)) { price = 0; }  // Handle case when input is not a number
            ...
            function return_back(){
                ...
                window.location.href = 'breeder_display_send_stocks1.php?ccid='+ccid;
            }
			function create_row(a){
                ...
                html += '<td><input type="text" name="amount[]" id="amount['+d+']" class="form-control text-right" style="width:90px;" onkeyup="validatenum(this.id); calculate_amount(this.id)" onchange="validateamount(this.id);" /></td>';
            ...
            function fetch_stock_qty(a){
                ...
                if(date == "" || fsector == "" || fsector == "select" || code == "" || code == "select"){ update_ebtn_status(0); }
                    ...
                    var method = "GET";
                    var url = "breeder_fetch_avlstock_quantity.php?date="+date+"&fsector="+fsector+"&item_code="+code+"&rows="+d+"&ftype=stk_transfer&ttype=add";
                    //window.open(url);
