...
if($access_error_flag == 0){
    ...
    if($acslist_error_flag == 0){
        ...
        //Check Import Flag
        $sql = "SELECT * FROM `extra_access` WHERE `field_name` LIKE '$href' AND `field_function` LIKE 'Import Purchase-Sale Transactions' AND `user_access` LIKE 'all' AND `flag` = '1'";
    ...
    <html>
		...
		<section class="content-header">
			<ol class="breadcrumb">
				...
				<li class="active">Display Purchase-sale</li>
			</ol>
			<h1>Purchase-sale</h1>
		</section><br/>
            <div class="row" style="margin: 10px 10px 0 10px;">
                ...
                <div align="right">
                    <?php if((int)$iport_cflag == 1){ ?><button type="button" class="btn btn-success" id="import_page" value="chicken_import_pursale7.php" onclick="add_page(this.id)" ><i class="fa-solid fa-circle-down"></i> Import</button><?php } ?>
			        <?php /*if($acs_edit_flag == 1){ ?><button type="button" class="btn btn-info" id="editpage" value="chicken_edit_pursale7m.php" onclick="add_page(this.id)" ><i class="fa fa-align-left"></i> Edit Multiple</button><?php }*/ ?>
                        ...
                        <div class="box-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <?php
                                    ...
                                    $sql = "SELECT * FROM `customer_sales` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `tdflag` = '0' AND `pdflag` = '0' AND `trlink` LIKE '$href' ORDER BY `date` ASC";
                                    $query = mysqli_query($conn,$sql);
                                    while($row = mysqli_fetch_assoc($query)){
                                        $id = $row['invoice'];
                                        // echo $ides = $row['link_trnum'];
                                        // echo $ide = $row['bookinvoice'];
                                        ...
                                        $vendorcode = '';
                                        $sup = 'N/A';
                                        $pquery = mysqli_query($conn, "SELECT vendorcode FROM pur_purchase WHERE link_trnum = '$id' AND trlink = '{$row['trlink']}' LIMIT 1");
                                        if ($prow = mysqli_fetch_assoc($pquery)) {
                                            $vendorcode = $prow['vendorcode'];
                                             $purchase_trnum = $prow['bookinvoice'];
                                            $sup = isset($sup_name[$vendorcode]) ? $sup_name[$vendorcode] : 'Unknown';
                                        }
                                    ?>
                                    <tr>
