<?php
//chicken_display_supplierpayment1.php
...
if($access_error_flag == 0){
    ...
    if($acslist_error_flag == 0){
        ...
        if($_SESSION['usr_atype'] = "S" || $_SESSION['usr_atype'] = "A"){
            ...
            $sql = "SELECT * FROM `dataentry_daterange` WHERE `type` = 'Payments' OR `type` = 'all' AND `active` = '1' ORDER BY `type` ASC";
         ...
         //Check Import Flag
         $sql = "SELECT * FROM `extra_access` WHERE `field_name` LIKE 'Supplier Payment' AND `field_function` LIKE 'Import Supplier Payment Transactions' AND `user_access` LIKE 'all' AND `flag` = '1'";
    ...
    <html>
		...
		<section class="content-header">
			<ol class="breadcrumb">
				...
				<li class="active">Display Payments</li>
			</ol>
			<h1>Payments</h1>
		</section><br/>
            <div class="row" style="margin: 10px 10px 0 10px;">
                ...
                <div align="right">
                    <?php if((int)$iport_cflag == 1){ ?><button type="button" class="btn btn-success" id="import_page" value="chicken_import_supplierpayment1.php" onclick="add_page(this.id)" ><i class="fa-solid fa-circle-down"></i> Import</button><?php } ?>
                        ...
                        <div class="box-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <?php
                                    ...
                                    $sql='SHOW COLUMNS FROM `pur_payments`'; $query=mysqli_query($conn,$sql); $existing_col_names = array(); $i = 0;
                                    while($row = mysqli_fetch_assoc($query)){ $existing_col_names[$i] = $row['Field']; $i++; }
                                    if(in_array("amount1", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `pur_payments` ADD `amount1` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `method`"; mysqli_query($conn,$sql); }
                                    if(in_array("tcds_per", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `pur_payments` ADD `tcds_per` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `amount1`"; mysqli_query($conn,$sql); }
                                    if(in_array("tcds_amt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `pur_payments` ADD `tcds_amt` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `tcds_per`"; mysqli_query($conn,$sql); }
                                    if(in_array("trtype", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `pur_payments` ADD `trtype` VARCHAR(300) NULL DEFAULT NULL AFTER `pdflag`"; mysqli_query($conn,$sql); }
                                    if(in_array("trlink", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `pur_payments` ADD `trlink` VARCHAR(300) NULL DEFAULT NULL AFTER `trtype`"; mysqli_query($conn,$sql); }
                                    
                                    /*Check for Table Availability*/
                                    $database_name = $_SESSION['dbase']; $table_head = "Tables_in_".$database_name; $exist_tbl_names = array(); $i = 0;
                                    $sql1 = "SHOW TABLES;"; $query1 = mysqli_query($conn,$sql1); while($row1 = mysqli_fetch_assoc($query1)){ $exist_tbl_names[$i] = $row1[$table_head]; $i++; }
                                    if(in_array("pur_payments", $exist_tbl_names, TRUE) == ""){ $sql1 = "CREATE TABLE $database_name.pur_payments LIKE poulso6_admin_chickenmaster.pur_payments;"; mysqli_query($conn,$sql1); }
                                    ...
                                    /*Check for Extra Access Flag*/
                                    $sql = "SELECT * FROM `extra_access` WHERE `field_name` = 'Supplier Payment' AND `field_function` = 'Display TCDS Calculations' AND `user_access` = 'all'"; $query = mysqli_query($conn,$sql); $count = mysqli_num_rows($query);
                                    if((int)$count > 0){ } else{ $sql = "INSERT INTO `extra_access` (`field_name`,`field_function`,`field_value`,`user_access`,`flag`) VALUES ('Supplier Payment','Display TCDS Calculations',NULL,'all','0');"; mysqli_query($conn,$sql); }
                                    ...
                                    $sql = "SELECT * FROM `pur_payments` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `vtype` LIKE '%S%' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `id` DESC"; // AND `trlink` LIKE '$href'
