<?php
	...
	include "xendorheadlink.php";
			...
			if($_GET['cid'] == "" && $_SESSION['dispstktr'] != ""){ $cid = $_SESSION['dispstktr']; } else if($_GET['cid'] != "" && $_SESSION['dispstktr'] == ""){ $cid = $_GET['cid']; $_SESSION['dispstktr'] = $cid; } else { $cid = $_GET['cid']; $_SESSION['dispstktr'] = $cid; }
			...
			if($utype = "S" || $utype = "A"){
				...
				$sql = "SELECT * FROM `dataentry_daterange` WHERE `type` = 'Stock Transfer' OR `type` = 'all' AND `active` = '1'";
	...
	<body class="hold-transition skin-blue sidebar-mini">
		<section class="content-header">
			<h1>Inventory - Stock Transfer</h1>
			<ol class="breadcrumb">
				<li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
				<li><a href="#">Purchases</a></li>
				<li class="active">Display Stock Transfer</li>
			</ol>
		</section><br/>
		<div class="row" style="margin: 10px 10px 0 10px;">
		<form action="main_displaystocktransfer.php?cid=<?php echo $cid; ?>" method="post">
			...
			<div class="row">
				<div class="col-lg-19">
					...
					<?php
						...
						while($row = mysqli_fetch_assoc($query)){
						$cdesc[$row['code']] = $row['description'];
						}
						$sql = "SELECT * FROM `item_stocktransfers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `id` DESC"; $query = mysqli_query($conn,$sql);
					?>
						<div class="box-body">
							<table id="example1" class="table table-bordered table-striped">
								...
								<?php $c = 0; while($row = mysqli_fetch_assoc($query)){
									 ...
									 $path = "\printformatlibrary\Examples\stockTransferPDF.php?towarehouse=$towarehouse"."&date=".$row['date'];
									?>
										<tr>
