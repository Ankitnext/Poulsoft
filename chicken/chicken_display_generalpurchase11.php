<?php
//chicken_display_generalpurchase11.php
    ...
    <html>
		...
		<section class="content-header">
            ...
            <div class="row" style="margin: 10px 10px 0 10px;">
                ...
                <div align="right">
                    <?php if($acs_edit_flag == 1){ ?><button type="button" class="btn btn-info" id="editpage" value="chicken_edit_generalpurchase11m.php" onclick="add_page(this.id)" ><i class="fa fa-align-left"></i> Edit Multiple</button><?php } ?>
                        ...
                        <div class="box-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <?php
                                    ...
                                    if(in_array("vtype", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `vtype` VARCHAR(300) NULL DEFAULT NULL AFTER `link_trnum`"; mysqli_query($conn,$sql); }
                                    if(in_array("group_code", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `group_code` VARCHAR(300) NULL DEFAULT NULL AFTER `dcno`"; mysqli_query($conn,$sql); }
                                    if(in_array("advance_amt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `advance_amt` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `group_code`"; mysqli_query($conn,$sql); }
                                    if(in_array("balance_amt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `balance_amt` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `amount`"; mysqli_query($conn,$sql); }
                                    if(in_array("from_location", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `from_location` VARCHAR(300) NULL DEFAULT NULL AFTER `remarks`"; mysqli_query($conn,$sql); }
                                    if(in_array("to_location", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `to_location` VARCHAR(300) NULL DEFAULT NULL AFTER `from_location`"; mysqli_query($conn,$sql); }
                                    if(in_array("from_kms", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `from_kms` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `to_location`"; mysqli_query($conn,$sql); }
                                    if(in_array("to_kms", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `to_kms` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `from_kms`"; mysqli_query($conn,$sql); }
                                    if(in_array("total_kms", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `total_kms` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `to_kms`"; mysqli_query($conn,$sql); }
                                    if(in_array("trtype", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `trtype` VARCHAR(300) NULL DEFAULT NULL DEFAULT '0' AFTER `pdflag`"; mysqli_query($conn,$sql); }
                                    if(in_array("trlink", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `trlink` VARCHAR(300) NULL DEFAULT NULL DEFAULT '0' AFTER `trtype`"; mysqli_query($conn,$sql); }
                                    if(in_array("emp_ctype", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `emp_ctype` VARCHAR(300) NULL DEFAULT NULL AFTER `tcoa`"; mysqli_query($conn,$sql); }
                                    if(in_array("emp_cvalue", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `emp_cvalue` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `emp_ctype`"; mysqli_query($conn,$sql); }
                                    if(in_array("supr_flag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `supr_flag` INT(100) NOT NULL DEFAULT '0' AFTER `advance_amt`"; mysqli_query($conn,$sql); }
                                    if(in_array("sup_eamt", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `acc_vouchers` ADD `sup_eamt` DECIMAL(20,5) NOT NULL DEFAULT '0' AFTER `supr_flag`"; mysqli_query($conn,$sql); }
                                    ...
                                    $sql = "SELECT * FROM `pur_purchase` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `trlink` LIKE '$href' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date` ASC";
