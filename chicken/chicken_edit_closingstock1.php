<?php
//chicken_edit_closingstock1.php
...
?>
    <html>
        ...
        <body>
            <?php
            $ids = $_GET['trnum'];
            $sql = "SELECT * FROM `item_closingstock` WHERE `trnum` = '$ids' AND `trlink` = 'chicken_display_closingstock1.php' AND `tdflag` = '0' AND `pdflag` = '0'";
            $query = mysqli_query($conn,$sql);
            while($row = mysqli_fetch_assoc($query)){
                $date = date("d.m.Y",strtotime($row['date']));
                $warehouse = $row['warehouse'];
                $code = $row['code'];
                $jals = round($row['closedjals'],2);
                $birds = round($row['closedbirds'],2);
                $quantity = round($row['closedquantity'],2);
            ...
            <div class="card border-secondary mb-3">
                ...
                <form action="chicken_modify_closingstock1.php" method="post" onsubmit="return checkval();">
                    <div class="card-body">
                        <div class="row">
                            <table align="center">
                                <thead>
                                ...
                                <tbody id="row_body">
                                    <tr style="margin:5px 0px 5px 0px;">
                                        ...
                                        <td><select name="code" id="code" class="form-control select2" style="width:180px;"><option value="select">-select-</option><?php foreach($item_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($scode == $code){ echo "selected"; } ?>><?php echo $item_name[$scode]; ?></option><?php } ?></select></td>
            ...
            <script>
                function return_back(){
                    window.location.href = "chicken_display_closingstock1.php";
                }
                function checkval(){
