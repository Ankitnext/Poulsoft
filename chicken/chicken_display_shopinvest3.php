<?php
	//chicken_display_shopinvest1.php
    ...
    if(!empty($_SESSION['shopinvest1'])){ $gp_id = $cid = $_SESSION['shopinvest1']; }
    else{
        $sql = "SELECT *  FROM `main_linkdetails` WHERE `href` LIKE '%$href%' AND `activate` = '1' ORDER BY `sortorder` ASC"; $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($query)){ $gp_id = $cid = $_SESSION['shopinvest1'] = $row['childid']; }
    }
			...
			<div class="row">
				<div class="col-lg-19">
					...
					<?php
						...
						$sql = "SELECT * FROM `shop_machine_investment` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `active` = '1' AND `dflag` = '0' ORDER BY `id` DESC";
						$query = mysqli_query($conn,$sql);
					?>
						<div class="box-body">
							<table id="example1" class="table table-bordered table-striped">
								...
								<tbody>
								<?php $c = 0; $sl = 1; while($row = mysqli_fetch_assoc($query)){ $c = $c + 1; ?>
										<tr>
											<td><?php echo $sl++; ?></td>
											<td><?php echo date("d.m.Y",strtotime($row['date'])); ?></td>
											<td><?php echo $ven_name[$row['vcode']]; ?></td>
