<?php
//chicken_add_multiplesale6.php
//if Layer Birds, price calculations on birds instead of weight added
include "newConfig.php";
include "chicken_generate_trnum_details.php";
$path = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH); $href = basename($path);
global $ufile_name; $ufile_name = 'chicken_add_multiplesale6.php'; include "chicken_check_accessmaster.php";

if($access_error_flag == 0){
    $date = date("Y-m-d");
    //Generate Transaction No.
    $incr = 0; $prefix = $trnum = "";
    $trno_dt1 = generate_transaction_details($date,"multiplesale6","STI","display",$_SESSION['dbase']);
    ...
    $sql = "SELECT * FROM `extra_access` WHERE `field_name` LIKE 'chicken_display_multiplesale6.php' AND `field_function` LIKE 'Display Groupwise customer filter' AND `flag` = '1'";
    $query = mysqli_query($conn,$sql); $vgcf_dflag = mysqli_num_rows($query);

    $sql = "SELECT * FROM `extra_access` WHERE `field_name` LIKE 'chicken_display_multiplesale6_t.php' AND `field_function` LIKE 'Display Customer Balance' AND `user_access` LIKE 'all' AND `flag` = '1'";
    ...
    <html>
        ...
        <body>
            <div class="card border-secondary mb-3">
                ...
                <form action="chicken_save_multiplesale6.php" method="post" onsubmit="return checkval();">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <?php if((int)$vgcf_dflag > 0){  ?>
                            <div class="form-group" style="width:190px;">
                                <label for="groups">Group<b style="color:red;">&nbsp;*</b></label>
                                <select name="groups" id="groups" class="form-control select2" style="width:180px;" onchange="filter_group_customers(this.id);">
                                    <option value="all">-all-</option>
                                    <?php foreach($cgrp_code as $scode){ ?><option value="<?php echo $scode; ?>"><?php echo $cgrp_name[$scode]; ?></option><?php } ?>
                                </select>
                            </div>
                            <?php } ?>
                            ...
                            <table>
                                ...
                                <tbody id="row_body">
                                    <tr style="margin:5px 0px 5px 0px;">
                                        <td><select name="vcode[]" id="vcode[0]" class="form-control select2" style="width:250px;" onchange="fetch_paperrate_master(this.id);fetchbalance(this.id);"><option value="select">-select-</option><?php foreach($cus_code as $scode){ ?><option value="<?php echo $scode; ?>"><?php echo $cus_name[$scode]; ?></option><?php } ?></select></td>
            ...
            <script>
                ...
                function create_row(a){
                    ...
                    html += '<tr id="row_no['+d+']">';
                    html += '<td><select name="vcode[]" id="vcode['+d+']" class="form-control select2" style="width:250px;" onchange="fetch_paperrate_master(this.id);fetchbalance(this.id);"><option value="select">-select-</option><?php foreach($cus_code as $scode){ ?><option value="<?php echo $scode; ?>"><?php echo $cus_name[$scode]; ?></option><?php } ?></select></td>';
                    ...
                    update_row_fields();
                    var x = "action["+d+"]";
                    var vgcf_dflag = '<?php echo (int)$vgcf_dflag ?>';
                    if(parseInt(vgcf_dflag) > 0){ filter_group_customers(x); }
