<?php
//chicken_edit_multiplesale3.php
...
?>
    <html>
        ...
        <body>
            <?php
            $ids = $_GET['trnum'];
            $sql = "SELECT * FROM `customer_sales` WHERE `invoice` = '$ids' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`invoice`,`id` ASC";
            $query = mysqli_query($conn,$sql); $cmn_trnum = "";
            while($row = mysqli_fetch_assoc($query)){ $cmn_trnum = $row['cmn_trnum']; }

            if($cmn_trnum != ""){
                $sql = "SELECT * FROM `customer_sales` WHERE `cmn_trnum` = '$cmn_trnum' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`invoice`,`id` ASC";
                $query = mysqli_query($conn,$sql); $c = 0; $invoice = array();
                while($row = mysqli_fetch_assoc($query)){
                    $date[$c] = $row['date'];
                    $invoice[$c] = $row['invoice'];
                    $bookinvoice[$c] = $row['bookinvoice'];
                    $customercode[$c] = $row['customercode'];
                    ...
                    $itemprice[$c] = round($row['itemprice'],5);
                    $totalamt[$c] = round($row['totalamt'],5);
                    $tcdsamt[$c] = round($row['tcdsamt'],5);
                    $roundoff[$c] = round($row['roundoff'],5);
                    $finaltotal[$c] = round($row['finaltotal'],5);
                    $warehouse[$c] = $row['warehouse'];
                    $remarks[$c] = $row['remarks'];
                    $drivercode[$c] = $row['drivercode'];
                    $vehiclecode[$c] = $row['vehiclecode'];
    
                    if((float)$birds[$c] != 0){ $avg_wt[$c] = round(((float)$netweight[$c] / (float)$birds[$c]),3); } else{ $avg_wt[$c] = 0; }
            ...
            $sql = "SELECT * FROM `main_tcds` WHERE `fdate` <= '$date[$c]' AND `tdate` >= '$date[$c]' AND `type` = 'TDS' AND `active` = '1' AND `dflag` = '0'";
            $query = mysqli_query($conn,$sql); $tcds_per = 0;
            while($row = mysqli_fetch_assoc($query)){ $tcds_per = $row['tcds']; }
            
            $inv_list = implode("','",$invoice);
            $sql = "SELECT * FROM `customer_receipts` WHERE `link_trnum` IN ('$inv_list') AND `mode` = '$cash_mode' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`trnum`,`id` ASC";
            $query = mysqli_query($conn,$sql); $rct_amt1 = array();
            while($row = mysqli_fetch_assoc($query)){ $rct_amt1[$row['link_trnum']] = round($row['amount'],5); }

            $sql = "SELECT * FROM `customer_receipts` WHERE `link_trnum` IN ('$inv_list') AND `mode` = '$bank_mode' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`trnum`,`id` ASC";
            $query = mysqli_query($conn,$sql); $rct_amt2 = $bank_method = array();
            while($row = mysqli_fetch_assoc($query)){ $rct_amt2[$row['link_trnum']] = round($row['amount'],5); $bank_method[$row['link_trnum']] = $row['method']; }
            ...
            <div class="card border-secondary mb-3">
                ...
                <form action="chicken_modify_multiplesale3.php" method="post" onsubmit="return checkval();">
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group" style="width:110px;">
                                <label for="date">Date<b style="color:red;">&nbsp;*</b></label>
                                <input type="text" name="date" id="date" class="form-control range_picker" value="<?php echo date("d.m.Y",strtotime($date[$c])); ?>" style="width:100px;" onchange="fetch_tcds_per(this.id);fetch_stockwise_sectors();" readonly />
                            </div>
                            <!--<div class="form-group" style="width:190px;">
                                ...
                                <select name="itemcode" id="itemcode" class="form-control select2" style="width:180px;" onchange="update_row_fields();">
                                    ...
                                    <?php /*foreach($item_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($itemcode[$c] == $scode){ echo "selected"; } ?>><?php echo $item_name[$scode]; ?></option><?php }*/ ?>
                                </select>
                            </div>-->
                            <div class="form-group" style="width:190px;">
                                ...
                                <select name="warehouse" id="warehouse" class="form-control select2" style="width:180px;" onchange="fetch_pur_stock();">
                                    <option value="select">select</option>
                                    <?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($warehouse[$c] == $scode){ echo "selected"; } ?>><?php echo $sector_name[$scode]; ?></option><?php } ?>
                                </select>
                            </div>
                            <div class="form-group" style="width:30px;visibility:hidden;">
                                <label for="vehicle">VH</label>
                                <input type="text" name="vehicle" id="vehicle" class="form-control" value="<?php echo $vehiclecode[$c]; ?>" style="width:20px;" />
                            </div>
                            <div class="form-group" style="width:30px;visibility:hidden;">
                                <label for="driver">DR</label>
                                <input type="text" name="driver" id="driver" class="form-control" value="<?php echo $drivercode[$c]; ?>" style="width:20px;" />
                            </div>
                            <div class="form-group" style="width:30px;visibility:hidden;">
                            ...
                            <table class="p-1 table1">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width:20px;visibility:hidden;">IV</th>
                                        <th style="width:20px;visibility:hidden;">OP</th>
                                ...
                                <?php $incr = $c; for($c = 0;$c <= $incr;$c++){ ?>
                                    <tr id="row_no[<?php echo $c; ?>]">
                                        <td><select name="vcode[]" id="vcode[<?php echo $c; ?>]" class="form-control select2" style="width:350px;" onchange="fetch_customer_balance_master(this.id);fetch_paperrate_master(this.id);"><option value="select">-select-</option><?php foreach($cus_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($customercode[$c] == $scode){ echo "selected"; } ?>><?php echo $cus_name[$scode]; ?></option><?php } ?></select></td>
                                        <td><select name="itemcode[]" id="itemcode[<?php echo $c; ?>]" class="form-control select2" style="width:180px;" onchange="update_row_fields();fetch_paperrate_master(this.id);"><option value="select">select</option><?php foreach($item_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($itemcode[$c] == $scode){ echo "selected"; } ?>><?php echo $item_name[$scode]; ?></option><?php } ?></select></td>
                                        <?php
                                        ...
                                        ?>
                                        <td><input type="text" name="nweight[]" id="nweight[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $netweight[$c]; ?>" style="width:90px;" onkeyup="validate_num(this.id);calculate_total_amt();" onchange="validate_amount(this.id);" /></td>
                                        <?php
                                        if((int)$birds_flag == 1){ ?><td><input type="text" name="birds[]" id="birds[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $birds[$c]; ?>" style="width:90px;" onkeyup="validate_count(this.id);calculate_total_amt();" /></td><?php }
                                        ?>
                                        <td><input type="text" name="price[]" id="price[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $itemprice[$c]; ?>" style="width:90px;" onkeyup="validate_num(this.id);calculate_total_amt();" onchange="validate_amount(this.id);" /></td>
                                        ...
                                        <td><input type="text" name="avg_wt[]" id="avg_wt[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $avg_wt[$c]; ?>" style="width:90px;" readonly /></td>
                                        <?php
                                        ...
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="finaltotal[]" id="finaltotal[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $finaltotal[$c]; ?>" style="width:20px;" onkeyup="validate_num(this.id);calculate_total_amt();" onchange="validate_amount(this.id);" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="trnums[]" id="trnums[<?php echo $c; ?>]" class="form-control" value="<?php echo $invoice[$c]; ?>" style="width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="opn_cbamt[]" id="opn_cbamt[<?php echo $c; ?>]" class="form-control text-right" style="width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="checkbox" name="tcds_chk[]" id="tcds_chk[<?php echo $c; ?>]" onchange="calculate_total_amt();" <?php if((float)$tcdsamt[$c] > 0){ echo "checked"; } ?> /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="tcds_amt[]" id="tcds_amt[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $tcdsamt[$c]; ?>" style="width:20px;" onkeyup="validate_num(this.id);calculate_total_amt();" onchange="validate_amount(this.id);" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="rct_amt1[]" id="rct_amt1[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $rct_amt1[$invoice[$c]]; ?>" style="width:20px;" onkeyup="validate_num(this.id);calculate_total_amt();" onchange="validate_amount(this.id);" /></td>
                                        <td style="width:20px;visibility:hidden;"><select name="bank_method1[]" id="bank_method1[<?php echo $c; ?>]" class="form-control select2" style="width:20px;"><option value="select">-select-</option><?php foreach($bank_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($bank_method == $scode){ echo "selected"; } ?>><?php echo $bank_name[$scode]; ?></option><?php } ?></select></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="rct_amt2[]" id="rct_amt2[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $rct_amt2[$invoice[$c]]; ?>" style="width:20px;" onkeyup="validate_num(this.id);calculate_total_amt();" onchange="validate_amount(this.id);" /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="cls_cbamt[]" id="cls_cbamt[<?php echo $c; ?>]" class="form-control text-right" style="width:20px;" onkeyup="validate_num(this.id);calculate_total_amt();" onchange="validate_amount(this.id);" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="bookinvoice[]" id="bookinvoice[<?php echo $c; ?>]" class="form-control" value="<?php echo $bookinvoice[$c]; ?>" style="width:20px;" /></td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
                                <tfoot>
                            ...
                            <table>
                                <thead>
                                    <tr>
                                        ...
                                        <th>Remarks</th>
                                        <th>+/-</th>
                                        <th style="visibility:hidden;">VT</th>
                                    </tr>
                                </thead>
                                <tbody id="row_body1">
                        ...
                        */ ?>
                        <div class="row" style="visibility:hidden;">
                            ...
                            <div class="form-group" style="width:30px;">
                                <label>ID</label>
                                <input type="text" name="cmn_trnum" id="cmn_trnum" class="form-control" value="<?php echo $cmn_trnum; ?>" style="width:20px;" readonly />
                            </div>
                            <div class="form-group" style="width:30px;">
                                <label>IN</label>
                            ...
                            <div class="form-group" style="width:30px;">
                                <label>DI</label>
                                <input type="text" name="dincr" id="dincr" class="form-control" value="<?php echo $dincr; ?>" style="width:20px;" readonly />
                            </div>
                            <div class="form-group" style="width:30px;">
                                <label>DV</label>
            ...
            <script>
                ...
                function fetch_pur_stock(){
                    ...
                    if(date == ""){
                        ...
                        var dc_nos = new XMLHttpRequest();
                        var method = "GET";
                        var url = "chicken_fetch_sector_stock1.php?date="+date+"&warehouse="+warehouse+"&trnum="+trnum;
                        //window.open(url);
                        var asynchronous = true;
                        dc_nos.open(method, url, asynchronous);
                        dc_nos.send();
