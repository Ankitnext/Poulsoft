	...
	<body class="hold-transition skin-blue sidebar-mini">
		...
		<section class="content">
			...
			<?php
				...
				$fdate = date("Y-m-d");
				$item_ccode = $item_cname = array();
				$sql = "SELECT * FROM `item_category` WHERE `active` = '1' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
				while($row = mysqli_fetch_assoc($query)){ $item_ccode[$row['code']] = $row['code']; $item_cname[$row['code']] = $row['description']; }

				$item_code = $item_name = $item_cat = array();
				$sql = "SELECT * FROM `item_details` WHERE `active` = '1' ORDER BY `description` ASC"; $query = mysqli_query($conn,$sql);
				while($row = mysqli_fetch_assoc($query)){ $item_code[$row['code']] = $row['code']; $item_cat[$row['code']] = $row['category']; $item_name[$row['code']] = $row['description']; }
				...
				// $sector_code = $sector_name = array();
				$sql = "SELECT * FROM `extra_access` WHERE `field_name` = 'pur_addpurchases.php' AND `field_function` = 'Category Birds' AND `flag` = '1'  ORDER BY `id` ASC"; $query = mysqli_query($conn,$sql);
				$cat_flag = mysqli_num_rows($query);
				...
				$idisplay = 'style="width: 90px;padding-right:20px;"'; $ndisplay = 'style="display:none;';
				...
				$view = $_GET['view'];
                $sql = "SELECT * FROM `pur_purchase` WHERE `invoice` LIKE '$ids' AND `tdflag` = '0' AND `pdflag` = '0'";
                $query = mysqli_query($conn,$sql); $i = 0; $avg_wt = array();
                while($row = mysqli_fetch_assoc($query)){
                    $date[$i] = $row['date'];
                    $trnum[$i] = $row['invoice'];
                    $billno[$i] = $row['bookinvoice'];
                    $vcode[$i] = $row['vendorcode'];
                    $icode[$i] = $row['itemcode'];
                    $jals[$i] = $row['jals'];
                    $birds[$i] = $row['birds'];
                    $total_wt[$i] = $row['totalweight'];
                    $empty_wt[$i] = $row['emptyweight'];
                    $net_wt[$i] = $row['netweight'];
					if((float)$birds[$i] != 0){ $avg_wt[$i] = round((float)$net_wt[$i] / (float)$birds[$i],3); } else{ $avg_wt[$i] = 0; }
                    $price[$i] = $row['itemprice'];
                    $itm_amt[$i] = $row['totalamt'];
                    $tcdsamt[$i] = $row['tcdsamt'];
                    $finv_amt[$i] = $row['finaltotal'];
                    $sectors[$i] = $row['warehouse'];
                    $drivercode[$i] = $row['drivercode'];
                    $vehiclecode[$i] = $row['vehiclecode'];
                    $amtinwords[$i] = $row['amtinwords'];
                    $remarks[$i] = $row['remarks'];
                    $addedempdt = $row['addedemp']."@".$row['addedtime'];
                    $tporter_code = $row['transporter_code'];
			...
			?>
				...
				<div class="box-body">
					<div class="row">
						<div class="col-md-12">
							<div class="col-md-18">
								<form action="pur_updatepurchaseinvoice.php" method="post" role="form" onsubmit="return checkval()" name="form_name" id = "form_id" >
									<div class="form-group col-md-2" style="width: 100px; text-align:Left;">
										<label>Date<b style="color:red;">&nbsp;*</b></label>
										<input type="text" style="width: 90px; text-align:Left;" class="form-control" name="pdate" value="<?php echo date("d.m.Y",strtotime($date[0])); ?>" id="pur_datepickers" onchange="fetchtds()" readonly >
									</div>
									<div class="form-group col-md-1" style="width: 120px;text-align:Left;">
										<label style="padding-left:10px;">Invoice</label>
									<input type="text" name="inv" id="inv" style="width:100px;background:none;border:none;text-decoration:none;" class="form-control" value="<?php echo $trnum[0]; ?>" placeholder="Enter Location..." readonly>
									</div>
									<div class="form-group col-md-3" style="width: 250px;text-align:Left;">
										...
										<select name="pname" id="pname" class="form-control select2"  style="width: 230px;text-align:Left; "> <!-- onchange="fetch_supplier_outstanding();"-->
											...
											<?php
												...
												while($row = mysqli_fetch_assoc($query)){
											?>
													<option value="<?php echo $row['code']; ?>" <?php if($vcode[0] == $row['code']){ echo "selected"; } ?>><?php echo $row['name']; ?></option>
											<?php
												}
											?>
										</select>
									...
									<div class="form-group col-md-2"  style="width: 170px;">
									    ...
									    <input type="text" name="binv" id="binv" style="width: 150px;text-align:left;" class="form-control" value="<?php echo $billno[0]; ?>" placeholder="Enter Book Invoice...">
									</div>
									<div class="form-group col-md-1" style="visibility:hidden;">
										<label>incr<b style="color:red;">&nbsp;*</b></label>
										<input type="text" style="width:auto;" class="form-control" name="incr" id="incr" value="<?php echo $c; ?>">
									</div>
									<div class="col-md-12">
										<table style="line-height:30px;" id="tab3">
                                            ...
                                            <tbody id="row_body">
												<?php for($i = 0;$i <= $c;$i++){ ?>
                                                <tr id="row_no[<?php echo $i; ?>]" style="margin:5px 0px 5px 0px;">
													<td style="width: 180px;padding-right:20px;"><select name="scat[]" id="scat[<?php echo $i; ?>]" class="form-control select2" style="width: 100%;" onchange="chk_itemtype();"><?php foreach($item_code as $ic){ ?><option value="<?php echo $ic."@".$item_name[$ic]."@".$item_cname[$item_cat[$ic]]; ?>" <?php if($ic == $icode[$i]){ echo "selected"; } ?>><?php echo $item_name[$ic]; ?></option><?php } ?></select></td>
													<td <?php if($ifjbwen == 1 || $ifjbw == 1){ echo $idisplay; } else { echo $ndisplay; } ?>><input type="text" style="width: 90px;" name="jval[]" id="jval[<?php echo $i; ?>]" class="form-control text-right" value="<?php echo round($jals[$i],5); ?>" onkeyup="validatebirds(this.id);calculatetotal(this.id);" onchange="validatebirds(this.id);" /></td>
													<td <?php if($ifjbwen == 1 || $ifjbw == 1 || $ifbw == 1){ echo $idisplay; } else { echo $ndisplay; } ?>><input type="text" style="width: 90px;" name="bval[]" id="bval[<?php echo $i; ?>]" class="form-control text-right" value="<?php echo round($birds[$i],5); ?>" onkeyup="validatebirds(this.id);calculatetotal(this.id);" onchange="validatebirds(this.id);" /></td>
													<td <?php if($ifjbwen == 1){ echo $idisplay; } else { echo $ndisplay; } ?>><input  type="text" style="width: 90px;" name="wval[]" id="wval[<?php echo $i; ?>]" class="form-control text-right" value="<?php echo $total_wt[$i]; ?>" onkeyup="validatenum(this.id);calculatetotal(this.id);" onchange="validateamount(this.id);" /></td>
													<td <?php if($ifjbwen == 1){ echo $idisplay; } else { echo $ndisplay; } ?>><input type="text" style="width: 90px;" name="ewval[]" id="ewval[<?php echo $i; ?>]" class="form-control text-right" value="<?php echo $empty_wt[$i]; ?>" onkeyup="validatenum(this.id);calculatetotal(this.id);" onchange="validateamount(this.id);" /></td>

													<td style="width: 90px;padding-right:20px;"><input type="text" style="width: 90px;" name="nwval[]" id="nwval[<?php echo $i; ?>]" class="form-control text-right" value="<?php echo $net_wt[$i]; ?>" onkeyup="validatenum(this.id);calculatetotal(this.id);" onchange="validateamount(this.id);" /></td>
													<?php if($dap_flag == 1){ ?>
														<td style='width: 90px;padding-right:20px;'><input type='text' name='avg_wt[]' id='avg_wt[<?php echo $i; ?>]' class='form-control text-right' value="<?php echo $avg_wt[$i]; ?>" style='width: 90px;' readonly /></td>
													<?php } ?>
													<td style="width: 90px;padding-right:20px;"><input type="text" style="width: 90px;" name="iprice[]" id="iprice[<?php echo $i; ?>]" class="form-control text-right" value="<?php echo $price[$i]; ?>" onkeyup="validatenum(this.id);calculatetotal(this.id);" onchange="validateamount2(this.id);"></td>
													<td style="width: 110px;padding-right:20px;"><input type="text" style="width: 110px;" name="tamt[]" id="tamt[<?php echo $i; ?>]" value="<?php echo $itm_amt[$i]; ?>" class="form-control text-right" readonly></td>
													<td style="width: 150px;padding-right:30px;"><select name="wcodes[]" id="wcodes[<?php echo $i; ?>]" class="form-control select2" style="width: 150px;"><?php foreach($sector_code as $it){ ?><option value="<?php echo $it; ?>" <?php if($it == $sectors[$i]){ echo "selected"; } ?>><?php echo $sector_name[$it]; ?></option><?php } ?></select></td>
                                                    <td id="action[<?php echo $i; ?>]" <?php if($c == $i){ echo 'style="padding-top: 5px;"'; } else{ echo 'style="padding-top: 5px;visibility:hidden;"'; } ?> >
                                                        <a href="javascript:void(0);" id="addrow[<?php echo $i; ?>]" onclick="create_row(this.id)" >
                                                            <i class="fa fa-plus"></i>
                                                        </a>
                                                            &ensp;
                                                        <a href="javascript:void(0);" id="deductrow[<?php echo $i; ?>]" onclick="destroy_row(this.id);" <?php if($c == $i && $i != 0){ } else{ echo 'style="display:none;"'; } ?> >
									   ...
									   <div class="col-md-12" align="left" style="padding-left:457px">
										...
										<?php } ?>
											...
											<div class="col-md-1" style="width: 90px; text-align:Left;">
												<input type="checkbox" name="tds" id="tds" onchange="caltds()" <?php if((float)$tcdsamt[0] > 0){ echo "checked"; } ?>>
												<label>TCS</label>
												<input type="text" name="tdsamt" id="tdsamt" class="form-control" value="<?php if((float)$tcdsamt[0] > 0){ echo $tcdsamt[0]; } ?>" style="width: 90px; text-align:right;"  readonly >
											</div>
											...
											<div class="col-md-1" style="width: 150px; text-align:Left;  ">
												...
												<input type="text" name="gtamt" id="gtamt" class="form-control text-right" value="<?php echo $finv_amt[0]; ?>" style="width: 110px; text-align:Right;" readonly>
											</div>
										</div>
										<div class="col-md-12" align="left" style="padding-top:30px;">
											<div class="col-md-4" style="width: 120px; text-align:Left;">
												<label>Vehicle No.</label>
												<input type="text" name="vno" id="vno" class="form-control" value="<?php echo $vehiclecode[0]; ?>" style="width: 100px; text-align:Left;">
											</div>
											<div class="col-md-4" style="width: 120px; text-align:Left;">
												<label>Driver</label>
												<input type="text" name="dname" id="dname" class="form-control" value="<?php echo $drivercode[0]; ?>" style="width: 100px; text-align:Left;" >
											</div>
											
										
											<div class="col-md-4"  style="width: 150px; text-align:Left;">
												<label>Transporter</label>
												<select name="transporter_code" id="transporter_code" class="form-control select2" style="width: 130px;"  >
													<option value="select">-select-</option>
													<?php foreach($transporter_code as $tcode){ ?><option value="<?php echo $tcode;?>" <?php if($tcode == $tporter_code){ echo "selected"; } ?>><?php echo $transporter_name[$tcode];?></option><?php } ?>
												</select>
											</div>
											<div class="col-md-4"  style="width: 150px; text-align:Left;">
												<label>Freight Amount</label>
												<input type="text" style="width: 120px;"   name="freight_amount" id="freight_amount" class="form-control text-right" value="<?php echo $freight_amount; ?>"  onkeyup="validatenum(this.id);" onchange="validateamount(this.id);" />
											</div>
										</div>
										<div class="col-md-12" align="left">
											<div class="col-md-1" style="width:auto;visibility:hidden;">
												<label>Amount in words</label>
												<textarea type="text" name="gtamtinwords" id="gtamtinwords" class="form-control" style="width:100%;" readonly><?php echo $amtinwords[0]; ?></textarea>
											</div>
											<div class="col-md-1" style="width:auto;visibility:hidden;">
											...
											<div class="col-md-1" style="width:auto;visibility:hidden;">
												<label>Ainfo<b style="color:red;">&nbsp;*</b></label>
												<input type="text" style="width:auto;" class="form-control" name="addedempdetails" id="addedempdetails" value="<?php echo $addedempdt; ?>">
											</div>
										</div>
										<div class="col-md-16" align="center">
											<label>Remarks</label>
											<textarea name="narr" id="narr" class="form-control" style="width:210px;" ><?php echo $remarks[0]; ?></textarea>
										</div><br/>
		...
		<script>
			...
			function create_row(a){
				...
				html += '<td style="width: 180px;padding-right:20px;"><select name="scat[]" id="scat['+d+']" class="form-control select2"  style="width: 100%;"  onchange="chk_itemtype();"><?php foreach($item_code as $ic){ ?><option value="<?php echo $ic."@".$item_name[$ic]."@".$item_cname[$item_cat[$ic]]; ?>"><?php echo $item_name[$ic]; ?></option><?php } ?></select></td>';
				html += '<td <?php if($ifjbwen == 1 || $ifjbw == 1){ echo $idisplay; } else { echo $ndisplay; } ?>><input type="text"  style="width: 90px;" name="jval[]" id="jval['+d+']" class="form-control text-right" onkeyup="validatebirds(this.id);calculatetotal(this.id);" onchange="validatebirds(this.id);" /></td>';
				html += '<td <?php if($ifjbwen == 1 || $ifjbw == 1 || $ifbw == 1){ echo $idisplay; } else { echo $ndisplay; } ?>><input type="text"  style="width: 90px;" name="bval[]" id="bval['+d+']" class="form-control text-right" onkeyup="validatebirds(this.id);calculatetotal(this.id);" onchange="validatebirds(this.id);" /></td>';
				html += '<td <?php if($ifjbwen == 1){ echo $idisplay; } else { echo $ndisplay; } ?>><input type="text"  style="width: 90px;" name="wval[]" id="wval['+d+']" class="form-control text-right" onkeyup="validatenum(this.id);calculatetotal(this.id);" onchange="validateamount(this.id);" /></td>';
				html += '<td <?php if($ifjbwen == 1){ echo $idisplay; } else { echo $ndisplay; } ?>><input type="text"  style="width: 90px;" name="ewval[]" id="ewval['+d+']" class="form-control text-right" onchange="validateamount(this.id);calculatetotal(this.id);" /></td>';
				html += '<td  style="width: 90px;padding-right:20px;"><input type="text"  style="width: 90px;" name="nwval[]" id="nwval['+d+']" class="form-control text-right" onkeyup="validatenum(this.id);calculatetotal(this.id);" onchange="validateamount(this.id);" /></td>';
				...
				html += '<td  style="width: 90px;padding-right:20px;"><input type="text"  style="width: 90px;" name="iprice[]" id="iprice['+d+']" class="form-control text-right" onkeyup="validatenum(this.id);calculatetotal(this.id);" onchange="validateamount2(this.id);"></td>';
				html += '<td  style="width: 110px;padding-right:20px;"><input type="text"  style="width: 110px;" name="tamt[]" id="tamt['+d+']" class="form-control text-right" readonly></td>';
				html += '<td style="width: 150px;padding-right:20px;"><select name="wcodes[]" id="wcodes['+d+']" class="form-control select2"  style="width: 150px;"><?php foreach($sector_code as $it){ ?><option value="<?php echo $it; ?>"><?php echo $sector_name[$it]; ?></option><?php } ?></select></td>';
			...
			function calculatetotal(a){
				...
				if(cat_flag > 0){
					var bird_cflag = scat1[2].search(/Birds/i);
				 } else {
					var bird_cflag = 0;
				  }
				...
				if(parseInt(bird_flag) > 0 || parseInt(bird_cflag) > 0){
					if(r.match("WT")){ }
					else if(r.match("BAW")){
				...
				for(var i = 0;i <= incr;i++){
					...
					if(parseInt(bird_flag) > 0 && parseInt(birds_flag) > 0 && parseInt(dap_flag) > 0 && parseInt(bird_cflag) > 0){
            ...
            function chk_itemtype(){
				var incr = document.getElementById("incr").value;
                var scat = r = ""; var bird_flag = 0; var scat1 = [];
                r = document.getElementById("itemfields").value;

				for(var d = 0;d <= incr;d++){
                    scat1 = []; bird_flag = 0;
                    scat = document.getElementById("scat["+d+"]").value;
                    scat1 = scat.split("@");
                    bird_flag = scat1[1].search(/Birds/i);
					var cat_flag = "<?php echo $cat_flag; ?>";
			...
			function validateamount2(x) { expr = /^[0-9.]*$/; var a = document.getElementById(x).value; if(a.length > 50){ a = a.substr(0,a.length - 1); } while(!a.match(expr)){ a = a.replace(/[^0-9.]/g, ''); } if(a == ""){ a = 0; } else { } var b = parseFloat(a).toFixed(5); document.getElementById(x).value = b; }
		</script>
