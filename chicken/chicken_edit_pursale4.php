...
?>
    <html>
        ...
        <body>
            <?php
            $ids = $_GET['trnum']; $sdate = $ltrnum = $remarks = ""; $sincr = $pincr = 0;
            ...
            if($ids != ""){
                $sql = "SELECT * FROM `customer_sales` WHERE `flag` = '1' AND `tdflag` = '0' AND `pdflag` = '0' AND `link_trnum` IN (SELECT `link_trnum` FROM `customer_sales` WHERE `invoice` = '$ids' AND `flag` = '1' AND `tdflag` = '0' AND `pdflag` = '0') ORDER BY `date`,`id` ASC";
                $query = mysqli_query($conn,$sql); $c = 0;
                while($row = mysqli_fetch_assoc($query)){
                    $sdate = $row['date'];
                    $ltrnum = $row['link_trnum'];
                    $strnum[$c] = $row['invoice'];
                    $cus_ccode[$c] = $row['customercode'];
                    $cus_icode[$c] = $row['itemcode'];
                    $cus_jals[$c] = round($row['jals'],2);
                    $cus_birds[$c] = round($row['birds'],2);
                    $cus_tweight[$c] = round($row['totalweight'],2);
                    $cus_eweight[$c] = round($row['emptyweight'],2);
                    $cus_nweight[$c] = round($row['netweight'],2);
                    $cus_prc[$c] = round($row['itemprice'],2);
                    $cus_amt[$c] = round($row['totalamt'],2);
                    $tcs_per[$c] = round($row['tcdsper'],2);
                    $tcs_amt[$c] = round($row['tcdsamt'],2);
                    $roff_camt[$c] = round($row['roundoff'],2);
                    $cus_famt[$c] = round($row['finaltotal'],2);
                    $remarks = $row['remarks'];
                    $c++;
                } if($c > 0){ $sincr = $c - 1; }
            }
            $date = $warehouse = $vendorcode = ""; $sup_icode = $sup_jals = $sup_birds = $sup_tweight = $sup_eweight = $sup_nweight = $sup_prc = $sup_amt = array(); $tds_per = $tds_amt = $roff_samt = $sup_famt = $line_sexp = 0;
            if($ltrnum != ""){
                $sql = "SELECT * FROM `pur_purchase` WHERE `invoice` = '$ltrnum' AND `flag` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`id` ASC";
                $query = mysqli_query($conn,$sql); $c = 0;
                while($row = mysqli_fetch_assoc($query)){
                    $date = $row['date'];
                    ...
                    $sup_icode[$c] = $row['itemcode'];
                    $sup_jals[$c] = round($row['jals'],2);
                    $sup_birds[$c] = round($row['birds'],2);
                    $sup_tweight[$c] = round($row['totalweight'],2);
                    $sup_eweight[$c] = round($row['emptyweight'],2);
                    $sup_nweight[$c] = round($row['netweight'],2);
                    $sup_prc[$c] = round($row['itemprice'],2);
                    $sup_amt[$c] = round($row['totalamt'],2);
                    $tds_per = round($row['tcdsper'],2);
                    $tds_amt = round($row['tcdsamt'],2);
                    $roff_samt = round($row['roundoff'],2);
                    $sup_famt = round($row['finaltotal'],2);
                    $line_sexp = round($row['line_sexp'],2);
                    $c++;
                } if($c > 0){ $pincr = $c - 1; }
            }
            $sql = "SELECT * FROM `chicken_supplier_branch` WHERE `sup_code` LIKE '$vendorcode' AND `active` = '1' AND `dflag` = '0' ORDER BY `description` ASC";
            ...
            <div class="card border-secondary mb-3">
                <div class="card-header">Edit Purchase-sale</div>
                <form action="chicken_modify_pursale4.php" method="post" onsubmit="return checkval();">
                    <div class="ml-5 card-body">
                        <div class="row justify-content-center align-items-center">
                            ...
                            <div class="form-group" style="width:210px;">
                                ...
                                <select name="vendorcode" id="vendorcode" class="form-control select2" style="width:180px;" onchange="fetch_sup_branches2(this.id);"><option value="select">-select-</option><?php foreach($sup_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($scode == $vendorcode){ echo "selected"; } ?>><?php echo $sup_name[$scode]; ?></option><?php } ?></select>
                            </div>
                            <div class="form-group" style="width:210px;">
                                <label for="supbrh_code">Branch<b style="color:red;">&nbsp;*</b></label>
                                <select name="supbrh_code" id="supbrh_code" class="form-control select2" style="width:180px;" ><option value="select">-select-</option><?php foreach($branch_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($scode == $supbrh_code){ echo "selected"; } ?>><?php echo $branch_name[$scode]; ?></option><?php } ?></select>
                            </div>
                            <div class="form-group" style="width:20px;visibility:hidden;">
                                <label for="warehouse">Vehicle<b style="color:red;">&nbsp;*</b></label>
                                <select name="warehouse" id="warehouse" class="form-control select2" style="width:80px;"><?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($scode == $warehouse){ echo "selected"; } ?>><?php echo $sector_name[$scode]; ?></option><?php } ?></select>
                            </div>
                        </div>
                        <div class="row">
                            <table align="center">
                                ...
                                <tbody id="row_body1">
                                    <?php for($c = 0;$c <= $pincr;$c++){ ?>
                                        <tr id="row_no1[<?php echo $c; ?>]">
                                        <td><select name="sup_icode[]" id="sup_icode[<?php echo $c; ?>]" class="form-control select2" style="width:200px;" onchange="update_srow_fields(this.id);"><option value="select">-select-</option><?php foreach($item_code as $icode){ ?><option value="<?php echo $icode; ?>" <?php if($icode == $sup_icode[$c]){ echo "selected"; } ?>><?php echo $item_name[$icode]; ?></option><?php } ?></select></td>
                                        <?php
                                        if((int)$jals_flag == 1){ echo '<td><input type="text" name="sup_jals[]" id="sup_jals['.$c.']" class="form-control text-right" value="'.$sup_jals[$c].'" style="width:50px;" onkeyup="validate_count(this.id);calculate_tot_samt();" /></td>'; }
                                        if((int)$birds_flag == 1){ echo '<td><input type="text" name="sup_birds[]" id="sup_birds['.$c.']" class="form-control text-right" value="'.$sup_birds[$c].'" style="width:50px;" onkeyup="validate_count(this.id);calculate_tot_samt();" /></td>'; }
                                        if((int)$tweight_flag == 1){ echo '<td><input type="text" name="sup_tweight[]" id="sup_tweight['.$c.']" class="form-control text-right" value="'.$sup_tweight[$c].'" style="width:90px;" onkeyup="validate_num(this.id);calculate_tot_samt();" onchange="validate_amount(this.id);" /></td>'; }
                                        if((int)$eweight_flag == 1){ echo '<td><input type="text" name="sup_eweight[]" id="sup_eweight['.$c.']" class="form-control text-right" value="'.$sup_eweight[$c].'" style="width:90px;" onkeyup="validate_num(this.id);calculate_tot_samt();" onchange="validate_amount(this.id);" /></td>'; }
                                        ?>
                                        <td><input type="text" name="sup_nweight[]" id="sup_nweight[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $sup_nweight[$c]; ?>" style="width:90px;" onkeyup="validate_num(this.id);calculate_tot_samt();" onchange="validate_amount(this.id);" /></td>
                                        <td><input type="text" name="sup_prc[]" id="sup_prc[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $sup_prc[$c]; ?>" style="width:60px;" onkeyup="validate_num(this.id);check_snrow(this.id);calculate_tot_samt();" onchange="validate_amount(this.id);" /></td>
                                        <td><input type="text" name="sup_amt[]" id="sup_amt[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $sup_amt[$c]; ?>" style="width:90px;" readonly /></td>
                                        <?php
                                        if($c == $pincr){ echo '<td id="action1['.$c.']" style="padding-top: 5px;visibility:visible;">'; }
                                        ...
                                        <th colspan="<?php echo $tcds_cnt; ?>" style="text-align:right;">
                                            <table align="right">
                                                <tr style="text-align:right;">
                                                    <th><input type="text" name="tds_per" id="tds_per" class="form-control text-right" value="<?php echo $tds_per; ?>" style="padding:0;width:50px;" readonly /></th>
                                                    <th>TDS:&ensp;<input type="checkbox" name="tds_chk" id="tds_chk" onchange="fetch_tds_per(this.id);"<?php if($tds_amt > 0){ echo "checked"; } ?> /></th>
                                                </tr>
                                            </table>
                                        </th>
                                        <th><input type="text" name="tds_amt" id="tds_amt" class="form-control text-right" value="<?php echo $tds_amt; ?>" style="width:90px;" readonly /></td>
                                    </tr>
                                    <tr>
                                        <th colspan="<?php echo $tcds_cnt; ?>" style="text-align:right;">Round-Off: </th>
                                        <th><input type="text" name="roff_samt" id="roff_samt" class="form-control text-right" value="<?php echo $roff_samt; ?>" style="width:90px;" readonly /></td>
                                    </tr>
                                    <tr>
                                        <th colspan="<?php echo $tcds_cnt; ?>" style="text-align:right;">Billing Amount: </th>
                                        <th><input type="text" name="sup_famt" id="sup_famt" class="form-control text-right" value="<?php echo $sup_famt; ?>" style="width:90px;" readonly /></td>
                                    </tr>
                                    <tr>
                                        <th colspan="<?php echo $tcds_cnt; ?>" style="text-align:right;">Line Expense: </th>
                                        <th><input type="text" name="line_sexp" id="line_sexp" class="form-control text-right" value="<?php echo $line_sexp; ?>" style="width:90px;" onkeyup="validate_num(this.id);calculate_wlpl_qtyamt();" onchange="validate_amount(this.id);" /></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div><br/><br/>
                        <div class="row">
                            <table align="center">
                                    ...
                                    <tr>
                                        <th colspan="<?php echo $cus_ccnt; ?>">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="sdate" id="sdate" class="form-control range_picker" value="<?php echo date('d.m.Y',strtotime($sdate)); ?>" style="width:100px;" readonly />
                                            </div>
                                        </th>
                                    </tr>
                                    <tr>
                                ...
                                <tbody id="row_body2">
                                    <?php for($c = 0;$c <= $sincr;$c++){ ?>
                                    <tr id="row_no2[<?php echo $c; ?>]">
                                        <td><select name="cus_ccode[]" id="cus_ccode[<?php echo $c; ?>]" class="form-control select2" style="width:200px;" onchange="fetch_latest_customer_paperrate(this.id);"><option value="select">-select-</option><?php foreach($cus_code as $icode){ ?><option value="<?php echo $icode; ?>" <?php if($icode == $cus_ccode[$c]){ echo "selected"; } ?>><?php echo $cus_name[$icode]; ?></option><?php } ?></select></td>
                                        <td><select name="cus_icode[]" id="cus_icode[<?php echo $c; ?>]" class="form-control select2" style="width:200px;" onchange="update_crow_fields(this.id);fetch_latest_customer_paperrate(this.id);"><option value="select">-select-</option><?php foreach($item_code as $icode){ ?><option value="<?php echo $icode; ?>" <?php if($icode == $cus_icode[$c]){ echo "selected"; } ?>><?php echo $item_name[$icode]; ?></option><?php } ?></select></td>
                                        <?php
                                        if((int)$jals_flag == 1){ echo '<td><input type="text" name="cus_jals[]" id="cus_jals['.$c.']" class="form-control text-right" value="'.$cus_jals[$c].'" style="width:50px;" onkeyup="validate_count(this.id);calculate_tot_camt();" /></td>'; }
                                        if((int)$birds_flag == 1){ echo '<td><input type="text" name="cus_birds[]" id="cus_birds['.$c.']" class="form-control text-right" value="'.$cus_birds[$c].'" style="width:50px;" onkeyup="validate_count(this.id);calculate_tot_camt();" /></td>'; }
                                        if((int)$tweight_flag == 1){ echo '<td><input type="text" name="cus_tweight[]" id="cus_tweight['.$c.']" class="form-control text-right" value="'.$cus_tweight[$c].'" style="width:90px;" onkeyup="validate_num(this.id);calculate_tot_camt();" onchange="validate_amount(this.id);" /></td>'; }
                                        if((int)$eweight_flag == 1){ echo '<td><input type="text" name="cus_eweight[]" id="cus_eweight['.$c.']" class="form-control text-right" value="'.$cus_eweight[$c].'" style="width:90px;" onkeyup="validate_num(this.id);calculate_tot_camt();" onchange="validate_amount(this.id);" /></td>'; }
                                        ?>
                                        <td><input type="text" name="cus_nweight[]" id="cus_nweight[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $cus_nweight[$c]; ?>" style="width:90px;" onkeyup="validate_num(this.id);calculate_tot_camt();" onchange="validate_amount(this.id);" /></td>
                                        <td><input type="text" name="cus_prc[]" id="cus_prc[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $cus_prc[$c]; ?>" style="width:60px;" onkeyup="validate_num(this.id);check_cnrow(this.id);calculate_tot_camt();" onchange="validate_amount(this.id);" /></td>
                                        <td style="text-align:center;"><input type="checkbox" name="tcs_chk[]" id="tcs_chk[<?php echo $c; ?>]" onchange="fetch_tcs_per(this.id);" <?php if($tcs_amt[$c] > 0){ echo "checked"; } ?> /></td>
                                        <td><input type="text" name="cus_famt[]" id="cus_famt[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $cus_famt[$c]; ?>" style="width:90px;" readonly /></td>
                                        <?php
                                        if($c == $sincr){ echo '<td id="action2['.$c.']" style="padding-top: 5px;visibility:visible;">'; }
                                        else{ echo '<td id="action2['.$c.']" style="padding-top: 5px;visibility:hidden;">'; }
                                        echo '<!--<a href="javascript:void(0);" id="addrow2['.$c.']" onclick="create_row2(this.id)"><i class="fa fa-plus"></i></a>&ensp;-->';
                                        if($c > 0){ echo '<a href="javascript:void(0);" id="deductrow2['.$c.']" onclick="destroy_row2(this.id)"><i class="fa fa-minus" style="color:red;"></i></a>'; }
                                        echo '</td>';
                                        ?>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="roff_camt[]" id="roff_camt[<?php echo $c; ?>]" value="<?php echo $roff_camt[$c]; ?>" class="form-control text-right" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="cus_amt[]" id="cus_amt[<?php echo $c; ?>]" value="<?php echo $cus_amt[$c]; ?>" class="form-control text-right" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="tcs_per[]" id="tcs_per[<?php echo $c; ?>]" value="<?php echo $tcs_per[$c]; ?>" class="form-control text-right" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="tcs_amt[]" id="tcs_amt[<?php echo $c; ?>]" value="<?php echo $tcs_amt[$c]; ?>" class="form-control text-right" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="sid[]" id="sid[<?php echo $c; ?>]" value="<?php echo $strnum[$c]; ?>" class="form-control" style="padding:0;width:20px;" readonly /></td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
                        ...
                        <div class="row" style="visibility:hidden;">
                            <div class="form-group" style="width:30px;">
                                <label>PID</label>
                                <input type="text" name="pid" id="pid" class="form-control" value="<?php echo $ltrnum; ?>" style="width:20px;" readonly />
                            </div>
                            <div class="form-group" style="width:30px;">
                                <label>IN</label>
                                <input type="text" name="pincr" id="pincr" class="form-control" value="<?php echo $pincr; ?>" style="width:20px;" readonly />
                            </div>
                            <div class="form-group" style="width:30px;">
                                <label>IN</label>
                                <input type="text" name="sincr" id="sincr" class="form-control" value="<?php echo $sincr; ?>" style="width:20px;" readonly />
                            </div>
                            <div class="form-group" style="width:30px;">
                                <label>EB</label>
            ...
            <script>
                ...
                function update_all_srow_fields(){
                    var pincr = document.getElementById("pincr").value; if(pincr == ""){ pincr = 0; }
                    for(var d = 0; d <= pincr;d++){
                        update_srow_fields("sup_icode["+d+"]");
                    }
                }
                function update_all_crow_fields(){
                    var pincr = document.getElementById("pincr").value; if(pincr == ""){ pincr = 0; }
                    for(var d = 0; d <= pincr;d++){
                        update_crow_fields("cus_icode["+d+"]");
                    }
                ...
                update_all_srow_fields();
                update_all_crow_fields();
            </script>
