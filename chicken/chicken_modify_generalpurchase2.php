<?php
//chicken_modify_generalpurchase2.php
...
if((float)$tamount > 0){
    $incr = 0; $prefix = $trnum = $aemp = $atime = "";
    if($trnum2 != ""){
        $sql = "SELECT * FROM `acc_vouchers` WHERE `trnum` = '$trnum2' AND `link_trnum` = '$trip_trnum' AND `tdflag` = '0' AND `pdflag` = '0'"; $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($query)){
            if($trnum == ""){ $incr = $row['incr']; $prefix = $row['prefix']; $trnum = $row['trnum']; $aemp = $row['addedemp']; $atime = $row['addedtime']; }
        }
        if($trnum != ""){
            $sql3 = "DELETE FROM `acc_vouchers` WHERE `trnum` = '$trnum2' AND `link_trnum` = '$trip_trnum' AND `tdflag` = '0' AND `pdflag` = '0'";
            if(!mysqli_query($conn,$sql3)){ die("Error: 1".mysqli_error($conn)); } else{ }
        }
    }
    else{
    ...
    for($i = 0;$i < $dsize;$i++){
        ...
        if($tcoa_amt[$i] > 0){
            $sql = "INSERT INTO `acc_vouchers` (`incr`,`prefix`,`trnum`,`link_trnum`,`vtype`,`date`,`dcno`,`group_code`,`advance_amt`,`fcoa`,`tcoa`,`amount`,`balance_amt`,`warehouse`,`remarks`,`from_location`,`to_location`,`from_kms`,`to_kms`,`total_kms`,`flag`,`active`,`tdflag`,`pdflag`,`trtype`,`trlink`,`addedemp`,`addedtime`,`updatedemp`,`updatedtime`) 
            VALUES('$incr','$prefix','$trnum','$trip_trnum','$vtype','$date','$billno','$group_code','$fcoa_amt','$fcoa','$tcoa[$i]','$tcoa_amt[$i]','$balance_amt','$warehouse','$remarks2[$i]','$from_location','$to_location','$from_kms','$to_kms','$total_kms','$flag','$active','$tdflag','$pdflag','$trtype','$trlink','$aemp','$atime','$addedemp','$addedtime')";
...
if((float)$tot_empsal_amt > 0){
    $incr = 0; $prefix = $trnum = $aemp = $atime = "";
    if($trnum3 != ""){
        $sql = "SELECT * FROM `acc_vouchers` WHERE `trnum` = '$trnum3' AND `link_trnum` = '$trip_trnum' AND `tdflag` = '0' AND `pdflag` = '0'"; $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($query)){
            if($trnum == ""){ $incr = $row['incr']; $prefix = $row['prefix']; $trnum = $row['trnum']; $aemp = $row['addedemp']; $atime = $row['addedtime']; }
        }
        if($trnum != ""){
            $sql3 = "DELETE FROM `acc_vouchers` WHERE `trnum` = '$trnum3' AND `link_trnum` = '$trip_trnum' AND `tdflag` = '0' AND `pdflag` = '0'";
            if(!mysqli_query($conn,$sql3)){ die("Error: 1".mysqli_error($conn)); } else{ }
        }
    }
    else{
    ...
    for($i = 0;$i < $dsize;$i++){
        ...
        if($advance_amt[$i] > 0){
            ...
            $sql = "INSERT INTO `acc_vouchers` (`incr`,`prefix`,`trnum`,`link_trnum`,`vtype`,`date`,`dcno`,`group_code`,`fcoa`,`tcoa`,`advance_amt`,`supr_flag`,`sup_eamt`,`amount`,`balance_amt`,`warehouse`,`remarks`,`from_location`,`to_location`,`from_kms`,`to_kms`,`total_kms`,`flag`,`active`,`tdflag`,`pdflag`,`trtype`,`trlink`,`addedemp`,`addedtime`,`updatedemp`,`updatedtime`) 
            VALUES('$incr','$prefix','$trnum','$trip_trnum','$vtype','$date','$billno',NULL,'$labour_fcoa','$emp_scode[$i]','$advance_amt[$i]','$supr_flag','$sup_eamt[$i]','$advance_amt[$i]','0','$warehouse','$remarks3[$i]','$from_location','$to_location','$from_kms','$to_kms','$total_kms','$flag','$active','$tdflag','$pdflag','$trtype','$trlink','$aemp','$atime','$addedemp','$addedtime')";
            if(!mysqli_query($conn,$sql)){ die("Error 1:-".mysqli_error($conn)); } else{ }
        }
    }
}
