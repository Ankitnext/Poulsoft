<?php 
	...
	$cname = $_POST['cname']; $wname = $_POST['wname']; $gname = $_POST['gname'];

	$icategories = $iname = "all";
	if(isset($_POST['submit']) == true){
		$icategories = $_POST['icategories'];
		$iname = $_POST['iname'];
	}
	if($iname != "all"){
		$inames = " AND `itemcode` = '$iname'";
	}
	else if($icategories != "all"){
		$item_list = "";
		foreach($item_code as $icode){
			$ic1 = $item_cats[$icode];
			if($item_cats[$icode] == $icategories){
				if($item_list == ""){
					$item_list = $icode;
				}
				else{
		...
		$inames = " AND `itemcode` IN ('$item_list')";
	}
	else{
		$inames = "";
	}
	...
	if($prate_flag == 1 || $prate_flag == "1"){
		...
		while($row = mysqli_fetch_assoc($query)){ $prates[$row['date']."@".$row['code']."@".$row['cgroup']] = $row['new_price']; }
	}
 ...
 background-color: #98fb98;
	...
	<?php } ?>
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
				<form action="cus_salesreport.php" method="post" >
					<?php } else { ?>
					<form action="cus_salesreport.php?db=<?php echo $db; ?>" method="post" >
					<?php } ?>
							...
							<thead class="thead1" style="background-color: #98fb98;">
								<tr>
									...
									&ensp;&ensp;
										...
										<select name="icategories" id="icategories" class="form-control select2">
											<option value="all">-All-</option>
											<?php
												foreach($icat_code as $icode){
											?>
													<option <?php if($icategories == $icode) { echo 'selected'; } ?> value="<?php echo $icode; ?>"><?php echo $icat_name[$icode]; ?></option>
											<?php
												}
											?>
										</select>
							...
							<?php
								if($_POST['export'] != "exportexcel"){
									...
									while($row = mysqli_fetch_assoc($query)){
										...
										if($prate_flag == 1 || $prate_flag == "1"){
											$prate_index = $row['date']."@".$row['itemcode']."@".$cus_group[$row['customercode']];
