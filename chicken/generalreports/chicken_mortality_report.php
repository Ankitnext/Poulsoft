				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            $nhtml .= '<th style="text-align:center;" >Mortality From</th>'; $fhtml .= '<th style="text-align:center;" id="order_num">Mortality From</th>';
                            ...
                            $mtype_fltr = ""; if($types != "all"){ $mtype_fltr = " AND `mtype` = '$types'"; }
                           // $area_fltr = ""; if($areas != "all"){ $area_fltr = " AND `area_code` = '$areas'"; }".$item_fltr."
                            ...
                            while($row = mysqli_fetch_assoc($query)){ $cus_alist[$row['code']] = $row['code']; $ven_name[$row['code']] = $row['name']; }

                             //Sector Details
                             $sql = "SELECT * FROM `inv_sectors` WHERE `active` = '1' ORDER BY `id` ASC";
                             $query = mysqli_query($conn,$sql); $sec_alist = array();
                             while($row = mysqli_fetch_assoc($query)){ $sec_alist[$row['code']] = $row['code']; }
                            
                            $cus_list = implode("','",$cus_alist);
                            $sec_list = implode("','",$sec_alist);

                            //$mtype_fltr = ""; if($types != "all"){ $mtype_fltr = " AND `mtype` = '$types'"; }
                            $cstype_fltr = "";  if($types == "customer" || $types == "supplier") { $cstype_fltr = " AND `ccode` IN ('$cus_list')"; }
                            $wtype_fltr = "";  if($types == "sector") { $wtype_fltr = " AND `ccode` IN ('$sec_list')"; }
                            //Week Wise Sales
                            $sql = "SELECT * FROM `main_mortality` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `active` = '1'".$cstype_fltr."".$wtype_fltr."".$item_fltr."".$sec_fltr."".$mtype_fltr." AND `dflag` = '0' ORDER BY `date`,`code` DESC";
                            ...
                            while($row = mysqli_fetch_assoc($query)){
                                ...
                                $html .= '<td>'.$row['code'].'</td>';
                                $html .= '<td>'.$row['mtype'].'</td>';
                                $html .= '<td>'.$ven_name[$row['ccode']].'</td>';
                               // $html .= '<td>'.$cus_name[$row['ccode']].'</td>';
                                ...
                                $html .= '<td>'.$sector_name[$row['ccode']].'</td>';
