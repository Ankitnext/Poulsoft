							...
							if(isset($_POST['submit']) == true){
								if($_POST['export'] != "exportexcel"){
									...
									<?php
										...
										$i = 0;
										$sql = "SELECT SUM(amount) as receipt_amount,ccode,`date` FROM `customer_receipts` WHERE date BETWEEN '$fdate' AND '$tdate' $customer_filter1 $user_filter3 $dcno_fltr3 GROUP By date,ccode";
										$query = mysqli_query($conn,$sql);
										while($row = mysqli_fetch_assoc($query)){
										...
										$sql = "SELECT SUM(amount) as amount,ccode,`date` FROM `main_mortality` WHERE date BETWEEN '$fdate' AND '$tdate' $customer_filter1 $user_filter3 $dcno_fltr4 GROUP By date,ccode";
										$query = mysqli_query($conn,$sql);
										while($row = mysqli_fetch_assoc($query)){
											$key = $row['date']."@".$row['ccode'];
										...
										if(sizeof($all_cust_codes) > 0){
											if((int)$cob_flag == 1 || (int)$ctb_flag == 1 || (int)$ccb_flag == 1){
												...
												$sql = "SELECT invoice,finaltotal,vendorcode FROM `pur_purchase` WHERE `date` < '$fdate' AND `vendorcode` IN ('$cus_list') AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`invoice` ASC";
												$query = mysqli_query($conn,$sql);
												while($row = mysqli_fetch_assoc($query)){
													$key = $row['vendorcode'];
												...
												$sql = "SELECT SUM(amount) as tamt,ccode FROM `pur_payments` WHERE  `date` < '$fdate' AND `ccode` IN ('$cus_list') AND `active` = '1' GROUP BY `ccode` ORDER BY `ccode` ASC";
												$query = mysqli_query($conn,$sql);
												while($row = mysqli_fetch_assoc($query)){
													$key = $row['ccode'];
													$ppay[$key] += (float)$row['tamt'];
												}
												$sql = "SELECT SUM(amount) as tamt,mode,ccode FROM `main_crdrnote` WHERE  `date` < '$fdate' AND `ccode` IN ('$cus_list') AND `mode` IN ('SCN','SDN') AND `active` = '1' GROUP BY `mode`,`ccode` ORDER BY `mode`,`ccode` ASC";
												$query = mysqli_query($conn,$sql);
												while($row = mysqli_fetch_assoc($query)){
													$key = $row['ccode'];
												...
												$sql = "SELECT * FROM `main_itemreturns` WHERE `date` < '$fdate' AND `vcode` IN ('$cus_list') AND `mode` = 'supplier' AND `active` = '1' AND `dflag` = '0'";
												$query = mysqli_query($conn,$sql);
												while($row = mysqli_fetch_assoc($query)){
													$key = $row['vcode'];
												...
												$old_inv = "";
												$sql = "SELECT invoice,finaltotal,customercode FROM `customer_sales` WHERE `date` < '$fdate' AND `customercode` IN ('$cus_list') AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `invoice` ASC";
												$query = mysqli_query($conn,$sql);
												while($row = mysqli_fetch_assoc($query)){
													if($old_inv != $row['invoice']){
												...
												$sql = "SELECT SUM(amount) as tamt,ccode FROM `customer_receipts` WHERE  `date` < '$fdate' AND `ccode` IN ('$cus_list') AND `active` = '1' GROUP BY `ccode` ORDER BY `ccode` ASC";
												$query = mysqli_query($conn,$sql);
												while($row = mysqli_fetch_assoc($query)){
													$key = $row['ccode'];
													$orct[$key] += (float)$row['tamt'];
												}
												$sql = "SELECT SUM(amount) as tamt,mode,ccode FROM `main_crdrnote` WHERE  `date` < '$fdate' AND `ccode` IN ('$cus_list') AND `mode` IN ('CCN','CDN') AND `active` = '1' GROUP BY `mode`,`ccode` ORDER BY `mode`,`ccode` ASC";
												$query = mysqli_query($conn,$sql);
												while($row = mysqli_fetch_assoc($query)){
													$key = $row['ccode'];
