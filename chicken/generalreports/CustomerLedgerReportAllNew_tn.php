<?php
//CustomerLedgerReportAllNew_tn.php
...
if($db == ''){
    ...
    $form_reload_page = "CustomerLedgerReportAllNew_tn.php";
}
else{
    include "APIconfig.php";
    $dbname = $db;
    $users_code = $_GET['emp_code'];
    $form_reload_page = "CustomerLedgerReportAllNew_tn.php?db=".$db;
}
...
//Font Size Flag
$sql = "SELECT * FROM `extra_access` WHERE `field_name` LIKE 'CustomerLedgerReportAllNew_tn.php' AND `field_function` LIKE 'Font Size' AND `user_access` LIKE 'all' AND `flag` = '1'";
$query = mysqli_query($conn,$sql); $dfont_flag = mysqli_num_rows($query);
if($dfont_flag > 0) { $ft_size = "font-size:x-large;font-weight: bold;"; } else { $ft_size = "";}
				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            $nhtml .= '<tr>'; $fhtml .= '<tr>';
                            $nhtml .= '<th style="'.$ft_size.'">Sl.No.</th>'; $fhtml .= '<th style="'.$ft_size.'" id="order_num">Sl.No.</th>';
                            $nhtml .= '<th style="'.$ft_size.'">Name</th>'; $fhtml .= '<th style="'.$ft_size.'" id="order">Name</th>';
                            // $nhtml .= '<th>Mobile No</th>'; $fhtml .= '<th id="order">Mobile No</th>';
                          
                            $nhtml .= '<th style="'.$ft_size.'">Sales Qty</th>'; $fhtml .= '<th style="'.$ft_size.'" id="order_num">Sales Qty</th>';
                            $nhtml .= '<th style="'.$ft_size.'">Sales</th>'; $fhtml .= '<th style="'.$ft_size.'" id="order_num">Sales</th>';
                            $nhtml .= '<th style="'.$ft_size.'">Receipt</th>'; $fhtml .= '<th style="'.$ft_size.'" id="order_num">Receipt</th>';
                            $nhtml .= '<th style="'.$ft_size.'">B/w days balance</th>'; $fhtml .= '<th style="'.$ft_size.'" id="order_num">B/w days balance</th>';
                            $nhtml .= '<th style="'.$ft_size.'">Opening Balance</th>'; $fhtml .= '<th style="'.$ft_size.'" id="order_num">Opening Balance</th>';
                            $nhtml .= '<th style="'.$ft_size.'">Balance</th>'; $fhtml .= '<th style="'.$ft_size.'" id="order_num">Balance</th>';
                            $nhtml .= '</tr>'; $fhtml .= '</tr>';
                            ...
                            foreach($cus_code as $ccode){
                                ...
                                $t_bamt = (float)$t_ramt - (float)$t_samt;
                                ...
                                if((int)$bwd_aflag == 0 && (float)$c_bamt != 0 || (int)$bwd_aflag == 1 && (float)$t_samt != 0 && (float)$c_bamt != 0 || (int)$bwd_aflag == 1 && (float)$t_ramt != 0 && (float)$c_bamt != 0){
                                    ...
                                    $html .= '<tr style="line-height: 2.1;font-size:19px;">';
                                    $html .= '<td style="text-align:center; '.$ft_size.'">'.$slno.'</td>';
                                    $html .= '<td style="font-family:Palatino, URW Palladio L, serif;'.$ft_size.'">'.$cname.'</td>';
                                    // $html .= '<td>'.$cmobl.'</td>';
                                    
                                    $html .= '<td style="text-align:right;'.$ft_size.'">'.number_format_ind($b_sqty).'</td>';
                                    $html .= '<td style="text-align:right;'.$ft_size.'">'.number_format_ind($t_samt).'</td>';
                                    $html .= '<td style="text-align:right;'.$ft_size.'">'.number_format_ind($t_ramt).'</td>';
                                    $html .= '<td style="text-align:right;'.$ft_size.'">'.number_format_ind($t_bamt).'</td>';
                                    $html .= '<td style="text-align:right;'.$ft_size.'">'.number_format_ind($o_bal).'</td>';
                                    $html .= '<td style="text-align:right;'.$ft_size.'">'.number_format_ind($c_bamt).'</td>';
                                    $html .= '</tr>';
                            ...
                            $html .= '<th colspan="2" style="'.$ft_size.'">Total</th>';
                           
                            $html .= '<th style="text-align:right;'.$ft_size.'">'.number_format_ind($ft_sqty).'</th>';
                            $html .= '<th style="text-align:right;'.$ft_size.'">'.number_format_ind($ft_samt).'</th>';
                            $html .= '<th style="text-align:right;'.$ft_size.'">'.number_format_ind($ft_ramt).'</th>';
                            $html .= '<th style="text-align:right;'.$ft_size.'">'.number_format_ind($ft_bamt).'</th>';
                            $html .= '<th style="text-align:right;'.$ft_size.'">'.number_format_ind($ft_obal).'</th>';
                            $html .= '<th style="text-align:right;'.$ft_size.'">'.number_format_ind($ft_camt).'</th>';
                            $html .= '</tr>';
                            $html .= '</thead>';
