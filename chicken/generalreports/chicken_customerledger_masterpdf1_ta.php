<?php
//chicken_customerledger_masterpdf1_ta.php
...
if($db == ''){
    ...
    $form_reload_page = "chicken_customerledger_masterpdf1_ta.php";
}
else{
    include "APIconfig.php";
    $_SESSION['dbase'] = $dbname = $db;
    $users_code = $_GET['emp_code'];
    $form_reload_page = "chicken_customerledger_masterpdf1_ta.php?db=".$db."&emp_code=".$users_code;
}
...
$fdate = $tdate = $today = date("Y-m-d"); $supervisors = "all"; $today_sale = 0;  $areas = array(); $fl_aflag = 0;
if(isset($_POST['submit_report']) == true){
    $supervisors = $_POST['supervisors'];
    //$areas = $_POST['areas'];
    foreach($_POST['areas'] as $t1){ $areas[$t1] = $t1; }           foreach($areas as $t1){ if($t1 == "all" || $t1 == ""){ $f_aflag = 1; } }
    if($_POST['today_sale'] == true || $_POST['today_sale'] == "on" || $_POST['today_sale'] == 1){ $today_sale = 1; } else{ $today_sale = 0; }
}
// $area_fltr = "";
//if($f_aflag == 0){ $area_list = implode("','",$areas); $area_fltr = " AND `area_code` IN ('$area_list')"; }

$sup_fltr = ""; if($supervisors != "all"){ $sup_fltr = " AND `supr_code` IN ('$supervisors')"; }
$area_fltr = ""; if($f_aflag != 1){ $areas_list = implode("','",$areas); $area_fltr = " AND `area_code` IN ('$areas_list')"; }
?>
<html>
	<head>
	...
	<body>
		<section class="content" align="center">
			<div class="col-md-12" align="center">
                            ...
                            <tr>
                                <th colspan="16">
                                    <div class="m-1 p-1 row">
                                        ...
                                        <div class="m-2 form-group">
                                            <label>Area</label>
                                            <select name="areas[]" id="areas" class="form-control select2" multiple>
                ...
                <form action="..\printformatlibrary\Examples\chicken_customerledger_masterpdf1_ta.php" method="post" onsubmit="return checkval()">
				<table class="table-sm table-hover main-table2">
					<thead class="thead1">
						<tr>
							<td colspan="19" class="p-1">
                                <div class="m-1 p-1 row">
                                   ...
                                   <div class="form-group">
                                            <br/><button type="button" class="btn btn-warning btn-sm" onclick="data_fetch(this.id)" name="download_pdf" id="download_pdf">Download Pdf</button>
                                    </div> &nbsp;&nbsp;
                                    <div class="form-group">
                                        <br/><button type="button" class="btn btn-warning btn-sm" onclick="data_fetch(this.id)" name="view_pdf_print" id="view_pdf_print">Print</button>&nbsp;&nbsp;
                                    </div>
                                    <div class="ml-2 mr-2 form-group" style="width:210px; visibility:hidden;">
                                        <label>Type</label>
    ...
    // Create a form
    const form = $('<form>', {
        action: '../printformatlibrary/Examples/chicken_customerledger_masterpdf1_ta.php',
        method: 'POST',
        target: '_blank'
    });

    // Add data as hidden inputs
    form.append($('<input>', { type: 'hidden', name: 'fdate', value: fdate }));
    form.append($('<input>', { type: 'hidden', name: 'tdate', value: tdate }));
    form.append($('<input>', { type: 'hidden', name: 'inc_sac', value: inc_sac }));
    form.append($('<input>', { type: 'hidden', name: 'send_type', value: send_type }));

    ccode.forEach((val, idx) => {
        form.append($('<input>', {
            type: 'hidden',
            name: `ccode[${idx}]`,
            value: val
