...
<html>
	...
	<?php } ?>
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
				<form action="pur_purchasereport_nb.php" method="post">
					<?php } else { ?>
					<form action="pur_purchasereport_nb.php?db=<?php echo $db; ?>" method="post">
					<?php } ?>
						...
						<?php } ?>
							...
							<?php
								...
								$tbirds = $tjals = $ttotalweight = $temptyweight = $tnetweight = $tdiscountamt = $ttaxamount = $ttcdsamt = $ttotalamt = $finalroundtot = 0; $old_inv = "";
								...
								while($row = mysqli_fetch_assoc($query)){
									...
									if($old_inv != $row['invoice']){
										...
										echo "<td>".number_format_ind($row['finaltotal'])."</td>";
										$finalroundtot += $row['finaltotal'];
							...
							?>
								<tr class="foottr" style="background-color: #98fb98;">
									...
									<td><?php echo number_format_ind($finalroundtot); ?></td>
