	...
	<head><link rel="stylesheet" type="text/css"href="reportstyle.css">
		<?php
			if($exoption == "exportexcel") {
				...
				echo header("Content-Disposition: attachment; filename=itm_weightlossreport2_ta($fromdate-$todate).xls");
		...
		<?php } ?>
			<section class="content" align="center">
				<div class="col-md-18" align="center">
					<form action="itm_weightlossreport2_ta.php" method="post">
							...
							<tbody>
								<?php
									if(isset($_POST['submit']) == true){
										...
										$pdate = date("Y-m-d",strtotime("-1 days".$fromdate));
										...
										$sql = $seq."".$wnames."".$grpstcode; $query = mysqli_query($conn,$sql); $addeditemcode = array();
										while($row = mysqli_fetch_assoc($query)){
											...
											(float)$addeditembds[$row['code']] = $row['closedbirds'];
											(float)$addeditemqty[$row['code']] = $row['closedquantity'];
											(float)$addeditemamt[$row['code']] = $row['closedquantity'] * $row['price'];
										}
										$grpcode = " ORDER BY `itemcode` ASC";
										$pur_date = date("Y-m-d",strtotime($fromdate."-1 days"));
										$seq = "SELECT * FROM `pur_purchase` WHERE `date` = '$pur_date' AND `itemcode` IN ($icode) AND `active` = '1'"; $sql = $seq."".$wnames."".$grpcode;
										$query = mysqli_query($conn,$sql);
										while($row = mysqli_fetch_assoc($query)){
											//$pqty[$row1['itemcode']] = $row1['netweight']; $picode[$row['itemcode']] = $row['itemcode']; $pinos[$row['itemcode']] = $row['birds'];
											echo "<tr>";
												echo "<td colspan='$dc'>".date("d.m.Y",strtotime($pur_date))."</td>";
												echo "<td colspan='$sc'>".$vnames[$row['vendorcode']]."</td>";
												echo "<td colspan='$ic'>".$itemnames[$row['itemcode']]."</td>";
												if($ifjbwen == 1 || $ifjbw == 1){ echo "<td colspan='$jc' id='ac'>".number_format_ind($row['jals'])."</td>"; }
												if($ifjbwen == 1 || $ifjbw == 1 || $ifbw == 1){ echo "<td colspan='$bc' id='ac'>".number_format_ind($row['birds'])."</td>"; }
											...
											(float)$addeditembds[$row['itemcode']] = (float)$addeditembds[$row['itemcode']] + $row['birds'];
											(float)$addeditemqty[$row['itemcode']] = (float)$addeditemqty[$row['itemcode']] + $row['netweight'];
											(float)$addeditemamt[$row['itemcode']] = (float)$addeditemamt[$row['itemcode']] + $row['totalamt'];
										...
										foreach($addeditemcode as $icodes){
											echo "<tr style='font-weight:bold;'>";
												...
												if($ifjbwen == 1 || $ifjbw == 1){ echo "<td colspan='$jc' id='ac'></td>"; }
												if($ifjbwen == 1 || $ifjbw == 1 || $ifbw == 1){ echo "<td colspan='$bc' id='ac'>".number_format_ind($addeditembds[$icodes])."</td>"; }
												if($ifjbwen == 1){ echo "<td colspan='$tc' id='ac'></td>"; }
												if($ifjbwen == 1){ echo "<td colspan='$ec' id='ac'></td>"; }
										...
										while($row = mysqli_fetch_assoc($query)){
											...
											$addeditemcode[$row['code']] = $row['code'];
											(float)$addeditembds[$row['code']] = $addeditembds[$row['code']] + $row['birds'];
											(float)$addeditemqty[$row['code']] = $addeditemqty[$row['code']] + $row['quantity'];
											(float)$addeditemamt[$row['code']] = $addeditemamt[$row['code']] + $amt;
										}
										if($ccount > 0){
											foreach($addeditemcode as $icodes){
												echo "<tr style='font-weight:bold;'>";
										...
										while($row = mysqli_fetch_assoc($query)){
											...
											$addeditemcode[$row['itemcode']] = $row['itemcode'];
											(float)$rmditembds[$row['itemcode']] = $rmditembds[$row['itemcode']] + $row['birds'];
											(float)$rmditemqty[$row['itemcode']] = $rmditemqty[$row['itemcode']] + $row['netweight'];
											(float)$rmditemamt[$row['itemcode']] = $rmditemamt[$row['itemcode']] + $row['totalamt'];
										}
										if($ccount > 0){
											foreach($addeditemcode as $icodes){
												echo "<tr style='font-weight:bold;'>";
													...
													echo "<td colspan='$ic'>".$itemnames[$icodes]."</td>";
													if($ifjbwen == 1 || $ifjbw == 1){ echo "<td colspan='$jc' id='ac'></td>"; }
													if($ifjbwen == 1 || $ifjbw == 1 || $ifbw == 1){ echo "<td colspan='$bc' id='ac'>".number_format_ind($rmditembds[$icodes])."</td>"; }
													if($ifjbwen == 1){ echo "<td colspan='$tc' id='ac'></td>"; }
													if($ifjbwen == 1){ echo "<td colspan='$ec' id='ac'></td>"; }
										...
										while($row = mysqli_fetch_assoc($query)){
											...
											$amt = $row['quantity'] * $row['price'];
											$addeditemcode[$row['code']] = $row['code'];
											(float)$rmditembds[$row['code']] = $rmditembds[$row['code']] + $row['birds'];
											(float)$rmditemqty[$row['code']] = $rmditemqty[$row['code']] + $row['quantity'];
											(float)$rmditemamt[$row['code']] = $rmditemamt[$row['code']] + $amt;
										}
										if($ccount > 0){
											foreach($addeditemcode as $icodes){
												echo "<tr style='font-weight:bold;'>";
										...
										if($qsize == 1) {
											...
											foreach($addeditemcode as $icodes){
												$closingQty = (float)$addeditemqty[$icodes] - (float)$rmditemqty[$icodes];
												if (abs($closingQty) < 0.01) {
													$closingQty = 0;
												}
												echo "<tr>";
													...
													echo "<td colspan='$ic'>".$itemnames[$icodes]."</td>";
													if($ifjbwen == 1 || $ifjbw == 1){ echo "<td colspan='$jc'></td>"; }
													if($ifjbwen == 1 || $ifjbw == 1 || $ifbw == 1){ echo "<td colspan='$bc' id='ac'>".number_format_ind($addeditembds[$icodes] - $rmditembds[$icodes])."</td>"; }
													if($ifjbwen == 1){ echo "<td colspan='$tc' id='ac'></td>"; }
													if($ifjbwen == 1){ echo "<td colspan='$ec' id='ac'></td>"; }
													echo "<td colspan='$nc' id='ac'>".number_format_ind($closingQty)."</td>";
													echo "<td colspan='$rc' id='ac'>".number_format_ind(round($prc[$icodes],2))."</td>";
											...
											foreach($addeditemcode as $icodes){
												echo "<tr>";
													...
													echo "<td colspan='$ic'>".$itemnames[$icodes]."</td>";
													if($ifjbwen == 1 || $ifjbw == 1){ echo "<td colspan='$jc' id='ac'></td>"; }
													if($ifjbwen == 1 || $ifjbw == 1 || $ifbw == 1){ echo "<td colspan='$bc' id='ac'>".number_format_ind($actitembds[$icodes] - ($addeditembds[$icodes] - $rmditembds[$icodes]))."</td>"; }
													if($ifjbwen == 1){ echo "<td colspan='$tc' id='ac'></td>"; }
													if($ifjbwen == 1){ echo "<td colspan='$ec' id='ac'></td>"; }
													echo "<td colspan='$nc' id='ac'>".number_format_ind((float)$actitemqty[$icodes] - ($closingQty))."</td>";
													echo "<td colspan='$rc' id='ac'>".$actitemprc[$icodes]."</td>";
													echo "<td colspan='$ac' id='ac'>".number_format_ind($actitemprc[$icodes] * ($actitemqty[$icodes] - ($addeditemqty[$icodes] - $rmditemqty[$icodes])))."&ensp;</td>";
												echo "</tr>";
											}
											foreach($addeditemcode as $icodes){
												echo "<tr>";
													...
													if((float)$addeditemqty[$icodes] > 0){
														echo "<td colspan='$nc' id='ac' title='($addeditemqty[$icodes] - $rmditemqty[$icodes]) / $addeditemqty[$icodes]'>".number_format_ind((($closingQty) / (float)$addeditemqty[$icodes]) * 100)."</td>";
													}
													else{
														echo "<td colspan='$nc' id='ac' title='($addeditemqty[$icodes] - $rmditemqty[$icodes]) / $addeditemqty[$icodes]'>".number_format_ind(0)."</td>";
													}
											...
											echo "<tr>";
												echo "<td colspan='$sch' id='ac' style='background-color: #98fb98;'><b>Total Purchase/T. IN</b>&ensp;</td>";
												foreach($addeditemcode as $icodes){
													echo "<td colspan='$qc' id='ac'>".number_format_ind($addeditembds[$icodes])."</td>";
											...
											echo "<tr>";
												echo "<td colspan='$sch' id='ac' style='background-color: #98fb98;'><b>Total Sold/T. OUT</b>&ensp;</td>";
												foreach($addeditemcode as $icodes){
													echo "<td colspan='$qc' id='ac'>".number_format_ind($rmditembds[$icodes])."</td>";
											...
											echo "<tr>";
												echo "<td colspan='$sch' id='ac' style='background-color: #98fb98;'><b>Closing</b>&ensp;</td>";
												foreach($addeditemcode as $icodes){
													echo "<td colspan='$qc' id='ac'>".number_format_ind($addeditembds[$icodes] - $rmditembds[$icodes])."</td>";
											...
											echo "<tr>";
												echo "<td colspan='$sch' id='ac' style='background-color: #98fb98;'><b>Weight Loss</b>&ensp;</td>";
												foreach($addeditemcode as $icodes){
													echo "<td colspan='$qc' id='ac'>".number_format_ind($actitembds[$icodes] - ($addeditembds[$icodes] - $rmditembds[$icodes]))."</td>";
											...
											echo "<tr>";
												...
												foreach($addeditemcode as $icodes){
													echo "<td colspan='$qc' id='ac'>".number_format_ind($actitembds[$icodes] - ($addeditembds[$icodes] - $rmditembds[$icodes]))."</td>";
													if((float)$addeditemqty[$icodes] > 0){
														echo "<td colspan='$wc' id='ac'>".number_format_ind($actitemqty[$icodes] - ($addeditemqty[$icodes] - $rmditemqty[$icodes]))." (<b>".number_format_ind((($addeditemqty[$icodes] - $rmditemqty[$icodes]) / $addeditemqty[$icodes]) * 100)."%</b>)</td>";
													}
													else{
										...
										echo "</tr>";
										$ttcoa = 0;
										$seq = "SELECT * FROM `acc_vouchers` WHERE `date` LIKE '$fromdate' AND `prefix` LIKE 'PV' AND `active` = '1'"; $sql = $seq."".$wnames; $query = mysqli_query($conn,$sql);
										while($row = mysqli_fetch_assoc($query)){
											echo "<tr>";
