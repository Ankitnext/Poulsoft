	...
	<?php } ?>
		<section class="content" align="center">
				<div class="col-md-12" align="center">
					<form action="cus_tcsdetailed.php" method="post">
						...
						<?php } ?>
							...
							<?php
								if($_POST['export'] != "exportexcel"){
                                    ...
                                    $sql = "SELECT SUM(tcdsamt) as amount,customercode as ccode,invoice,date FROM `customer_sales` WHERE `date` >= '$fromdate' AND `date` <= '$todate' AND `tcdsamt` != '0' AND `customercode` = '$cname' AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' GROUP BY `date`,`invoice`,`customercode` ORDER BY `date`,`invoice`,`customercode` ASC";
