...
if($db == ''){
    ...
    $form_reload_page = "chicken_labour_ledger_ty.php";
}
else{
				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            $html .= '<th style="text-align:center;">KG</th>';
                           ...
                           $sql = "SELECT * FROM `acc_coa` WHERE `description` LIKE 'Labour Advance' AND `vouexp_flag` = '1' AND `active` = '1' ORDER BY `description` ASC";
                            ...
                            $sql = "SELECT * FROM `acc_vouchers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `fcoa` = '$labour_fcoa' AND `active` = '1' ".$sec_fltr."";
                           ...
                           // echo $sql;
                            ...
                            while($row = mysqli_fetch_assoc($query)){
                                  ...
                                  if($slno == 1){
                                     ...
                                     //  echo "<br/>".$opening_bal."@".$esw_amount."@".$adv[$key];
                                        $html .= '<tr>';
                                        $html .= '<td></td>';
                                        $html .= '<td colspan="5" style="text-align:center;"><b>Opening Balance</b></td>';
                            ...
                            // $sql = "SELECT * FROM `acc_vouchers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND  `active` = '1'".$sec_fltr." AND `vtype` = 'Pay Voucher' AND `fcoa` = '$labour_fcoa'";
                            // $query = mysqli_query($conn, $sql); $total_kms =  $amount = $advance = $final_amt = $arr = array(); $slno = 0; $totalkms = $tamount = $tadvance = $tfinal_amt = 0;
                            // while($row = mysqli_fetch_assoc($query)){
                            //          $key = $row['date'];

                            //          $adv_amt[$key] += $row['advance_amt'];
                            // }
                    ...
                    //                 $html .= '<tr>';
                    //                 $html .= '<td style="text-align:center">'.$slno.'</td>';
                    //                 $html .= '<td style="text-align:right">'.$arr[$a].'</td>';
