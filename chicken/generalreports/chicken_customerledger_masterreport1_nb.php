...
if($db == ''){
    ...
    $form_reload_page = "chicken_customerledger_masterreport1_nb.php";
}
else{
    include "APIconfig.php";
    $dbname = $db;
    $users_code = $_GET['emp_code'];
    $form_reload_page = "chicken_customerledger_masterreport1_nb.php?db=".$db;
}
...
if(isset($_POST['submit']) == true){
    ...
    $fsizes = $_POST['fsizes'];
    $currentIndex = array_search($vendors, $cus_code);
    ...
    while($row = mysqli_fetch_assoc($query)){
        if(strtotime($row['date']) < strtotime($fdate)){
            ...
            if(strtotime($old_date) != strtotime($row['date'])){ $old_date = $row['date']; $i = 0; } $i++; $key = $row['date']."@".$i;
            $cus_srtn[$key] = $row['trnum']."@".$row['date']."@".$row['inv_trnum']."@".$row['vcode']."@".$row['itemcode']."@".$row['jals']."@".$row['birds']."@".$row['quantity']."@".$row['price']."@".$row['amount']."@".$row['rtype']."@".$row['warehouse']."@".$row['addedemp']."@".$row['addedtime']."@".$row['updatedemp']."@".$row['updatedtime']."@".$row['remarks'];
...
if(isset($_POST['next']) == true){
    ...
    $currentIndex = array_search($vendors, $keys);
     if ($currentIndex !== false && $currentIndex < count($keys) - 1) {
            $vendors = $keys[$currentIndex + 1];
        }
    $exports = $_POST['exports'];
    //Sales
    $sql = "SELECT * FROM `customer_sales` WHERE `date` <= '$today' AND `customercode` = '$vendors' AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`invoice`,`id` ASC";
			...
			<div class="col-md-12" align="center">
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                        ...
                        <thead class="thead1">
                            <tr>
                                ...
                                <td colspan="15" align="center">
                                    ...
                                    <?php if((isset($_POST['submit']) == true || isset($_POST['next']) == true || isset($_POST['pre']) == true)  && (int)$cus_outbal_flag == 1){ echo "<h3 style='color:red;'>BALANCE: ".number_format_ind($otb_amt)."</h3>"; } ?>
                                    <?php if((isset($_POST['submit']) == true || isset($_POST['next']) == true || isset($_POST['pre']) == true)  && (int)$cus_cdays_flag == 1){ echo "<h5 style='color:green;'>Credit Days: ".str_replace(".00","",number_format_ind($credit_days[$vendors]))."</h5>"; } ?>
                                    <?php
                                    }
                                    else{
                                    ?>
								...
								<td colspan="19" class="p-1">
                                    <div class="m-1 p-1 row">
                                        ...
                                        <div class="form-group" style="width:300px;">
                                            ...
                                            <select name="vendors" id="vendors" class="form-control select2" style="width:290px;">
                                    ...
                                    <div class="m-1 p-1 row">-->
                                        ...
                                        <div class="form-group" style="width:10px;visibility: hidden;">
                                            <label>Export</label>
                                            <select name="exports" id="exports" class="form-control select2" style="width:5px;" onchange="tableToExcel('main_table', '<?php echo $file_name; ?>','<?php echo $file_name; ?>', this.options[this.selectedIndex].value)">
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true || isset($_POST['next']) == true || isset($_POST['pre']) == true){
                            ...
                            if(isset($_POST['submit']) == true || isset($_POST['next']) == true || isset($_POST['pre']) == true){ $hhtml .= '<h3 style="color:red;">BALANCE: '.number_format_ind($otb_amt).'</h3>'; }
                            $hhtml .= '</td>';
                            $hhtml .= '</tr>';
                                            ...
                                            for($j = 1;$j <= $ccount;$j++){
                                                ...
                                                if(empty($acname[$key1]) || $acname[$key1] == ""){ }
                                                    ...
                                                    if(empty($tbl_col_name[$cname]) || $tbl_col_name[$cname] == ""){ } else{
                                                        ...
                                                        else if($tcname == "remarks"){ $html .= '<td '.$rpt_txt_align[$cname].' title="'.$rpt_col_name[$cname].'">'.$tr_info[16].'</td>'; }
                                                        else if($tcname == "drivercode"){ $html .= '<td '.$rpt_txt_align[$cname].' title="'.$rpt_col_name[$cname].'"></td>'; }
                                                        else if($tcname == "vehiclecode"){ $html .= '<td '.$rpt_txt_align[$cname].' title="'.$rpt_col_name[$cname].'"></td>'; }
                                                        else if($tcname == "addedemp"){ $html .= '<td '.$rpt_txt_align[$cname].' title="'.$rpt_col_name[$cname].'">'.$user_name[$tr_info[12]].'</td>'; }
                                                        else if($tcname == "addedtime"){ $html .= '<td '.$rpt_txt_align[$cname].' title="'.$rpt_col_name[$cname].'">'.date("d.m.Y h:i:s A",strtotime($tr_info[13])).'</td>'; }
