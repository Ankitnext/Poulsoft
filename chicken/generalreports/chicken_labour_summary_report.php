...
if($db == ''){
    ...
    $form_reload_page = "chicken_labour_summary_report.php";
}
else{
...
$file_name = "Labour Summary Report";
				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            $html .= '<th style="text-align:center;">LABOUR NAME.</th>';
                            $html .= '<th style="text-align:center;">TOTAL KGS</th>';
                            $html .= '<th style="text-align:center;">AMOUNT</th>';
                            $html .= '<th style="text-align:center;">ADVANCE</th>';
                            $html .= '<th style="text-align:center;">FINAL BALANCE</th>';
                            $html .= '</tr>';
                           ...
                           $sql = "SELECT * FROM `acc_coa` WHERE `description` LIKE 'Labour Advance' AND `vouexp_flag` = '1' AND `active` = '1' ORDER BY `description` ASC";
                            ...
                            // $sql = "SELECT * FROM `acc_vouchers` WHERE `fcoa` = '$labour_fcoa' AND `active` = '1'".$sec_fltr."";
                            // $query = mysqli_query($conn,$sql); $op_adv = array();
                            // while($row = mysqli_fetch_assoc($query)){
                            //     $key = $row['tcoa'];
                            //     $op_adv[$key] += (float)$row['advance_amt'];
                            // }

                            // $sql = "SELECT * FROM `employee_sale_wages` WHERE `to_coa` = '$ewage_code' AND `active` = '1'".$sec_fltr1." ";
                            // $query = mysqli_query($conn,$sql); $opamount = array();
                            // while($row = mysqli_fetch_assoc($query)){
                            //     $key = $row['from_coa'];
                            //     $opamount[$key] += (float)$row['amount'];
                            // }


                            $sql = "SELECT * FROM `acc_vouchers` WHERE `fcoa` = '$labour_fcoa' AND `active` = '1' ".$sec_fltr." AND `date`>='$fdate' AND `date`<='$tdate'";
                            $query = mysqli_query($conn,$sql); $advn = array();
                            while($row = mysqli_fetch_assoc($query)){
                                $key = $row['tcoa']."@".$row['date'];
                           ...
                           // $opening_bal = $opamount - $op_adv;
                            ...
                            $sql = "SELECT * FROM `employee_sale_wages` WHERE  `to_coa` = '$ewage_code' AND `active` = '1' AND `dflag` = '0'".$sec_fltr1." AND `date`>='$fdate' AND `date`<='$tdate'";
                           // echo $sql;
                            $query = mysqli_query($conn,$sql); $slno = 0;
                            while($row = mysqli_fetch_assoc($query)){
                                  $key = $row['from_coa'];
                                  $labour = $row['from_coa']."@".$row['date'];
                                  $esw_sale_qty[$key] += (float)$row['sale_qty'];
                                  $esw_rate[$key] += (float)$row['rate'];
                                  $esw_amount[$key] += (float)$row['amount'];
                                  $final_amt[$key] += (float)$row['amount'] - (float)$advn[$labour];
                                  $fadv[$key] += $advn[$labour];
                                  $arr[$key] = $key;
                            ...
                            foreach($arr as $r){
                               // echo "<br/>".$r."@".$esw_sale_qty[$r]."@".$esw_amount[$r]."@".$final_amt[$r]."@".$fadv[$r];
                                   $slno++;
                                  $html .= '<tr>';
                                    $html .= '<td style="text-align:center">'.$slno.'</td>';
                                    $html .= '<td style="text-align:left">'.$driver_name[$r].'</td>';
                                    $html .= '<td style="text-align:right">'.$esw_sale_qty[$r].'</td>';
                                    $html .= '<td style="text-align:right">'.$esw_amount[$r].'</td>';
                                    $html .= '<td style="text-align:right">'.$fadv[$r].'</td>';
                                    $html .= '<td style="text-align:right">'.$final_amt[$r].'</td>';
