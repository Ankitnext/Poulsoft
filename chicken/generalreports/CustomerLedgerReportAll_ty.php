<?php 
	...
	if(isset($_POST['submit']) == true){
		if($_POST['ctype'] == "on" || $_POST['ctype'] == true){
			$con_type = " AND`contacttype` LIKE '%C%'";
			$con_code = "S&C";
		}
		else{
			$con_type = " AND`contacttype` LIKE 'C'";
	...
	else{ $con_type = " AND`contacttype` LIKE '%C%'"; $con_code = "S&C"; }
...
<html>
	...
	<?php } ?>
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
				<form action="CustomerLedgerReportAll_ty.php" method="post"  onsubmit="return checkval()">
					<?php } else { ?>
					<form action="CustomerLedgerReportAll.php?db=<?php echo $db; ?>" method="post"  onsubmit="return checkval()">
					<?php } ?>
								...
								if(isset($_POST['submit']) == true){
									if($_POST['export'] != "exportexcel"){
										if($cname == "" || $cname == "all" || $cname == "select"){
											...
											//sales invoice
											$sql = "SELECT * FROM `customer_sales` WHERE `date` < '$fromdate' AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`invoice`,`customercode` ASC";
											...
											//Customer Receipt
											$sql = "SELECT * FROM `customer_receipts` WHERE `date` < '$fromdate' AND `active` = '1' ORDER BY `ccode` ASC";
											$query = mysqli_query($conn,$sql); $ob_receipts = array();
											while($row = mysqli_fetch_assoc($query)){
												$ob_receipts[$row['ccode']] = $ob_receipts[$row['ccode']] + $row['amount'];
											}
											//Customer CrDr Note
											$sql = "SELECT * FROM `main_crdrnote` WHERE `date` < '$fromdate' AND `mode` IN ('CCN','CDN') AND `active` = '1' ORDER BY `ccode` ASC";
											...
											if($count40 > 0){
												...
												$obquery = mysqli_query($conn,$obsql); while($obrow = mysqli_fetch_assoc($obquery)){ $ob_returns[$obrow['vcode']] = (float)$ob_returns[$obrow['vcode']] + (float)$obrow['amount']; }
											}
											if($count44 > 0){
												$obsql = "SELECT * FROM `main_mortality` WHERE `date` < '$fromdate' AND `mtype` = 'customer' AND `active` = '1' AND `dflag` = '0'";
												$obquery = mysqli_query($conn,$obsql); while($obrow = mysqli_fetch_assoc($obquery)){ $ob_mortality[$obrow['ccode']] += (float)$obrow['amount']; }
											}

											$old_inv = "";
											if($count57 > 0){
												$obsql = "SELECT * FROM `pur_purchase` WHERE `date` < '$fromdate'  AND `active` = '1' ORDER BY `date`,`invoice` ASC";
											...
											if($count56 > 0){
											$obsql = "SELECT * FROM `pur_payments` WHERE `date` < '$fromdate'  AND `active` = '1'";
											$obquery = mysqli_query($conn,$obsql); while($obrow = mysqli_fetch_assoc($obquery)){ $ob_payments[$obrow['ccode']] = $ob_payments[$obrow['ccode']] + $obrow['amount']; }
											}
											if($count40 > 0){
												$obsql = "SELECT * FROM `main_itemreturns` WHERE `date` < '$fromdate' AND `mode` = 'supplier' AND `active` = '1' AND `dflag` = '0'";
												$obquery = mysqli_query($conn,$obsql); while($obrow = mysqli_fetch_assoc($obquery)){ $ob_sreturns[$obrow['vcode']] += (float)$obrow['amount']; }
											}
											if($count44 > 0){
											...
											if($count32 > 0){
											$obsql = "SELECT * FROM `main_crdrnote` WHERE `date` < '$fromdate'  AND `mode` IN ('SCN','SDN') AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0'";
											$obquery = mysqli_query($conn,$obsql);
											while($obrow = mysqli_fetch_assoc($obquery)){ if($obrow['mode'] == "SCN"){ $ob_sup_ccn[$obrow['ccode']] = $ob_sup_ccn[$obrow['ccode']] + $obrow['amount']; } else { $ob_sup_cdn[$obrow['ccode']] = $ob_sup_cdn[$obrow['ccode']] + $obrow['amount']; } }
											}
											
											//sales invoice
											$sql = "SELECT * FROM `customer_sales` WHERE `date` >= '$fromdate' AND `date` <= '$todate' AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`invoice` ASC";
											...
											if($count44 > 0){
												...
												$obquery = mysqli_query($conn,$obsql); while($obrow = mysqli_fetch_assoc($obquery)){ $bt_mortality[$obrow['ccode']] += (float)$obrow['amount']; }
											}

											$old_inv = "";
											if($count57 > 0){
												$obsql = "SELECT * FROM `pur_purchase` WHERE `date` >= '$fromdate' AND `date` <= '$todate'  AND `active` = '1' ORDER BY `date`,`invoice` ASC";
												$obquery = mysqli_query($conn,$obsql);
												while($obrow = mysqli_fetch_assoc($obquery)){
													if($old_inv != $obrow['invoice']){
											...
											if($count56 > 0){
											$obsql = "SELECT * FROM `pur_payments` WHERE `date` >= '$fromdate' AND `date` <= '$todate'  AND `active` = '1'";
											$obquery = mysqli_query($conn,$obsql); while($obrow = mysqli_fetch_assoc($obquery)){ $bt_payments[$obrow['ccode']] = $bt_payments[$obrow['ccode']] + $obrow['amount']; }
											}
											if($count40 > 0){
												$obsql = "SELECT * FROM `main_itemreturns` WHERE `date` >= '$fromdate' AND `date` <= '$todate' AND `mode` = 'supplier' AND `active` = '1' AND `dflag` = '0'";
												$obquery = mysqli_query($conn,$obsql); while($obrow = mysqli_fetch_assoc($obquery)){ $bt_sreturns[$obrow['vcode']] += (float)$obrow['amount']; }
											}
											if($count44 > 0){
											...
											if($count32 > 0){
											$obsql = "SELECT * FROM `main_crdrnote` WHERE `date` >= '$fromdate' AND `date` <= '$todate'  AND `mode` IN ('SCN','SDN') AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0'";
											$obquery = mysqli_query($conn,$obsql);
											...
											$ftotal = $ft_ob =  $ft_sq =  $ft_sa =  $ft_rt =  $ft_bb = $total_sales_amt = $total_rct_amt = $total_cdn_amt = $total_ccn_amt = $total_cr_amt = $total_dr_amt = $slno = 0;
											foreach($pcode as $pcodes){
                                                $slno++;
												echo "<tr>";
                                                echo "<td style='text-align:left;'>".$slno."</td>";
