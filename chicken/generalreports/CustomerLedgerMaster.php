<?php
	...
	if($cid == ""){
		...
		$sql = "SELECT * FROM `main_linkdetails` WHERE `href` LIKE '%CustomerLedgerMaster.php%' AND `activate` = '1'";
	...
	$sql = "SELECT *  FROM `extra_access` WHERE `field_name` LIKE 'CustomerLedgerMaster.php' AND `field_function` LIKE 'Display Final total Avg. Price' AND `user_access` LIKE 'all'";
	$query = mysqli_query($conn,$sql); $cnt = mysqli_num_rows($query); if($cnt > 0){ while($row = mysqli_fetch_assoc($query)){ $dtap_flag = $row['flag']; } } else{ $sql = "INSERT INTO `extra_access` (`field_name`,`field_function`,`field_value`,`user_access`,`flag`) VALUES ('CustomerLedgerMaster.php','Display Final total Avg. Price',NULL,'all','1');"; mysqli_query($conn,$sql); }
...
<html>
	...
	<?php } ?>
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
				<form action="CustomerLedgerMaster.php?cid=<?php echo $cid; ?>" method="post" onSubmit="return checkval()">
					<?php } else { ?>
					<form action="CustomerLedgerMaster.php?cid=<?php echo $cid; ?>&db=<?php echo $db; ?>&emp_code=<?php echo $users_code; ?>" method="post" onSubmit="return checkval()">
					<?php } ?>
												...
												if($sales[$date_asc."@".$j] != ""){
													...
													if($exi_inv != $sales_details[1]){
														...
														for($i = 1;$i <= $col_count;$i++){
															...
															else if($field_details[$i.":".$active_flag] == "salesup_flag"){ echo "<td style='text-align:left;'>".$cname."</td>"; }
											...
											for($j = 0;$j <=$ccount;$j++){
												if($ccns[$date_asc."@".$j] != ""){
													...
													for($i = 1;$i <= $col_count;$i++){
														...
														else if($field_details[$i.":".$active_flag] == "item_flag"){ echo "<td style='text-align:left;'>".$coaname[$ccns_details[5]]."</td>"; }
														else if($field_details[$i.":".$active_flag] == "jals_flag"){ echo "<td></td>"; }
														else if($field_details[$i.":".$active_flag] == "birds_flag"){ echo "<td></td>"; }
														else if($field_details[$i.":".$active_flag] == "tweight_flag"){ echo "<td></td>"; }
														else if($field_details[$i.":".$active_flag] == "eweight_flag"){ echo "<td></td>"; }
