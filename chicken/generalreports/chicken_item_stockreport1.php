<?php
	...
	while($row = mysqli_fetch_assoc($query)){ $item_code[$row['code']] = $row['code']; $item_name[$row['code']] = $row['description']; $icat_cat[$row['code']] = $icat_name[$row['category']]; }

	$fdate = $tdate = date("Y-m-d"); $sectors = array(); $item_cat["all"] = $items["all"] = $sectors["all"] = "all"; $sec_all_flag = 0;
    $exports = "display";
	if(isset($_POST['submit']) == true){
        ...
        // 1.1 Categories
        $item_cats        = []; $itemcat_all_flag = 0;
        foreach ($_POST['item_cat'] as $cat) { $item_cats[$cat] = $cat; if ($cat === 'all') { $itemcat_all_flag = 1; }  }
        $icats_list = implode("','", array_map('addslashes', array_keys($item_cats)));

        // 1.2 Items
        $items         = []; $item_all_flag = 0;
        foreach ($_POST['items'] as $it) { $items[$it] = $it; if ($it === 'all') { $item_all_flag = 1; } }
        $its_list   = implode("','", array_map('addslashes', array_keys($items)));

        $item_filter  = '';
        $item_filter1 = '';

        // A) Specific items chosen → filter by those
        if ($item_all_flag === 0) {
            $item_filter  = " AND `itemcode` IN ('$its_list')";
            $item_filter1 = " AND `code`     IN ('$its_list')";

        // B) items=all & categories=all → no filter
        } elseif ($itemcat_all_flag === 1) {
            // both filters remain empty

        // C) items=all but specific categories → filter by codes in those categories
        } else {
            $icat_codes = [];
            foreach ($item_code as $icode) {
                $cat = $item_category[$icode] ?? '';
                if ($cat !== '' && isset($item_cats[$cat])) {
                    $icat_codes[$icode] = $icode;
                }
            }
            if (!empty($icat_codes)) {
                $list = implode("','", array_map('addslashes', array_keys($icat_codes)));
                $item_filter  = " AND `itemcode` IN ('$list')";
                $item_filter1 = " AND `code`     IN ('$list')";
            }
        }
				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                                    ...
                                    <div class="m-1 p-1 row">
                                        ...
                                        <div class="form-group" style="width:170px;">
                                            ...
                                            <select name="item_cat[]" id="item_cat[0]" class="form-control select2" style="width:160px;" onchange="fetch_item_list();" multiple>
                                                ...
                                                <option value="<?php echo $icats; ?>" <?php if(in_array($icats, $item_cat)){ echo "selected"; } ?>><?php echo $icat_name[$icats]; ?></option>
                                                <?php } } ?>
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group" style="width:150px;">
                                            <label>Item</label>
                                            <?php $isPost = $_SERVER['REQUEST_METHOD'] === 'POST'; ?>
                                            <select name="items[]" id="items" class="form-control select2" style="width:140px;" multiple >
                                                <option value="all" <?php if (!$isPost || $item_all_flag) echo 'selected'; ?>>-All-</option>
                                                <?php
                                                // If categories=all, list every item
                                                if ($itemcat_all_flag): foreach ($item_code as $icode): if ($item_name[$icode] === '') continue; $sel = isset($items[$icode]) ? 'selected' : '';
                                                ?>
                                                    <option value="<?php echo $icode; ?>" <?php echo $sel; ?>><?php echo $item_name[$icode]; ?></option>
                                                <?php
                                                endforeach;
                                                // Else, show only items in chosen categories
                                                else:
                                                foreach ($item_code as $icode):
                                                    $cat = $item_category[$icode] ?? '';
                                                    if (!isset($item_cats[$cat])) continue;
                                                    $sel = isset($items[$icode]) ? 'selected' : '';
                                                ?>
                                                    <option value="<?php echo $icode; ?>" <?php echo $sel; ?>><?php echo $item_name[$icode]; ?></option>
                                                <?php
                                                endforeach;
                    ...
                    <table class="main-table table-sm table-hover" id="main_table">
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            // Closing Table 
                            $sql = "SELECT * FROM `item_closingstock` WHERE `date` <= '$tdate'".$item_filter1."".$sector_filter." AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `id` DESC";
                            ...
                            while($row = mysqli_fetch_assoc($query)){ 
                                ...
                                // Opening Transfer
                                if(strtotime($row['date']) < strtotime($fdate)){ 
                                    $im_sale_qnt_opn[$key] += $row['netweight'];
                                    $im_sale_prc_opn[$key] += $row['itemprice'];
                                    $im_sale_amt_opn[$key] += $row['totalamt'];
                                } else {
                                    $im_sale_qnt_btw[$key] += $row['netweight'];
                                    $im_sale_prc_btw[$key] += $row['itemprice'];
                                    $im_sale_amt_btw[$key] += $row['totalamt'];
                                }
                            }
                            // Mortality Table 
                            ...
                            while($row = mysqli_fetch_assoc($query)){ 
                                ...
                                // Opening Transfer
                                if(strtotime($row['date']) < strtotime($fdate)){ 
                                    $im_mort_qnt_opn[$key] += $row['quantity'];
                                    $im_mort_prc_opn[$key] += $row['price'];
                                    $im_mort_amt_opn[$key] += $row['amount'];
                                } else {
                                    $im_mort_qnt_btw[$key] += $row['quantity'];
                                    $im_mort_prc_btw[$key] += $row['price'];
                                    $im_mort_amt_btw[$key] += $row['amount'];
                                }
                            }
                            ...
                            foreach($item_code as $icode){
                                ...
                                $html .= '<td style="text-align:left;">'.$icat_cat[$icode].'</td>';
                                ...
                                $tot_st_in_qty += (float)$bet_in_qty;
                                $tot_in_price += (float)$bet_in_prc;
                                $tot_in_amt += (float)$bet_in_amt;
                                
                                $tot_st_out_qty += (float)$bet_out_qty;
                                $tot_out_price += (float)$bet_out_prc;
                                $tot_out_amt += (float)$bet_out_amt; 

                                $tot_clos_qty += (float)$closing_qty;
                                $tot_clos_price += (float)$closing_prc;
                                $tot_clos_amt += (float)$closing_amt;
                            }
                            ...
                            $html .= '<th style="text-align:right;">'.number_format_ind($tot_clos_qty).'</th>';
                            $html .= '<th style="text-align:right;">'.number_format_ind($tot_clos_price).'</th>';
                            $html .= '<th style="text-align:right;">'.number_format_ind($tot_clos_amt).'</th>';
                            $html .= '</tr>';
                            $html .= '</tfoot>';
