<?php
	...
	if($cid == ""){
		...
		$sql = "SELECT * FROM `main_linkdetails` WHERE `href` LIKE '%CustomerLedgerMaster_tn2.php%' AND `activate` = '1'";
	...
	$sql = "SELECT * FROM `extra_access` WHERE `field_name` LIKE '%CustomerLedgerMaster_tn2.php%' AND `field_function` LIKE 'Display Avg Price in Customer Ledger'";
	...
	if($count > 0){
		...
		$sql = "INSERT INTO `extra_access`(field_name,field_function,user_access,flag) VALUES('CustomerLedgerMaster_tn2.php','Display Avg Price in Customer Ledger','all','1')";
		$query = mysqli_query($conn,$sql);
		$avgprc_flag = 1;
	}
	...
	$sql = "SELECT *  FROM `extra_access` WHERE `field_name` LIKE 'CustomerLedgerMaster_tn2.php' AND `field_function` LIKE 'Display Final total Avg. Price' AND `user_access` LIKE 'all'";
	$query = mysqli_query($conn,$sql); $cnt = mysqli_num_rows($query); if($cnt > 0){ while($row = mysqli_fetch_assoc($query)){ $dtap_flag = $row['flag']; } } else{ $sql = "INSERT INTO `extra_access` (`field_name`,`field_function`,`field_value`,`user_access`,`flag`) VALUES ('CustomerLedgerMaster_tn2.php','Display Final total Avg. Price',NULL,'all','1');"; mysqli_query($conn,$sql); }
...
<html>
	...
	<?php } ?>
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
				<form action="CustomerLedgerMaster_tn2.php?cid=<?php echo $cid; ?>" method="post" onSubmit="return checkval()">
					<?php } else { ?>
					<form action="CustomerLedgerMaster_tn2.php?cid=<?php echo $cid; ?>&db=<?php echo $db; ?>&emp_code=<?php echo $users_code; ?>" method="post" onSubmit="return checkval()">
					<?php } ?>
