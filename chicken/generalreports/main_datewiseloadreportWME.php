<?php 
	...
	$item_codes = "";
	$sql = "SELECT * FROM `item_details` WHERE `description` LIKE '%Birds' AND `active` = '1'"; $query = mysqli_query($conn,$sql);
	while($row = mysqli_fetch_assoc($query)){ $item_name[$row['code']] = $row['description']; if($item_codes == ""){ $item_codes = $row['code']; } else{ $item_codes = $item_codes."','".$row['code']; } }
...
?>
<?php
$file_name = "Load wise Ledger With Manual Expense";
$sectors = $vendors = "all"; $expoption = "displaypage";
...
$url = "../PHPExcel/Examples/main_datewiseloadreportWME-Excel.php?fromdate=".$fromdate."&todate=".$todate."&sectors=".$sectors."&vendors=".$vendors;
?>
<html>
	<head>
	...
	<body class="hold-transition skin-blue sidebar-mini">
		...
		<header align="center">
			<table align="center" class="reportheadermenu">
				...
				<?php
					...
					<td align="center">
						<h3>Load wise Ledger With Manual Expense</h3>
						<?php
		...
		<section class="content" align="center">
				<div class="col-md-12" align="center">
					<form action="main_datewiseloadreportWME.php" method="post" onsubmit="return checkval()">
							...
							<?php
								...
								if(isset($_POST['submit']) == true){
									...
									foreach($index_sale as $isal){
										$avgWt = 0; 
										if($pur_totwt[$isal] > 0 && $pur_birds[$isal] > 0){
									...
									foreach($index_sale as $isal){
										$avgWt = 0;
										if($sale_totwt[$isal] > 0 && $sale_birds[$isal] > 0){
									...
									foreach($pur_docno as $pdno){
										...
										if($exoption == "printerfriendly"){
											...
											echo "<td style='text-align:left;'><a href='main_loadwisestockledgerreportWME.php?id=$pur_val[1]&fromdate=$pur_val[0]' target='_BLANK'>".$pur_val[1]."</a></td>";
										}
										echo "<td style='text-align:left;'>".$sector_name[$pur_sector[$pdno]]."</td>";
										// echo "<td>".$usern[$pdno]."</td>";
										...
										if(number_format_ind($pur_val[4]) == "" || number_format_ind($pur_val[4]) == ".00"){ $pv4 = "0.00"; } else{ $pv4 = number_format_ind($pur_val[4]); }
										echo "<td><input type='text' name='pv4$values' id='pv4$values' value='$pv4' style='width:70px;text-align:right;border:none;background:inherit;' readonly /></td>";
											$tot_pur_qty = (float)$tot_pur_qty + (float)$pur_val[4];
											if(number_format_ind($pur_val[5]) == "INF.00" || number_format_ind($pur_val[5]) == ".00"){ $avg_pur_wt = 0; } else { $avg_pur_wt = number_format_ind($pur_val[5]); }
										echo "<td>".number_format_ind($avg_pur_wt)."</td>";
										echo "<td>".number_format_ind($pur_val[6])."</td>";
										if(number_format_ind($pur_val[7]) == "" || number_format_ind($pur_val[7]) == ".00"){ $pv7 = "0.00"; } else{ $pv7 = number_format_ind($pur_val[7]); }
										echo "<td><input type='text' name='pv7$values' id='pv7$values' value='$pv7' style='width:70px;text-align:right;border:none;background:inherit;' readonly /></td>";
										...
										if(number_format_ind($sal_val[4]) == "" || number_format_ind($sal_val[4]) == ".00"){ $sv4 = "0.00"; } else{ $sv4 = number_format_ind($sal_val[4]); }
										echo "<td><input type='text' name='sv4$values' id='sv4$values' value='$sv4' style='width:70px;text-align:right;border:none;background:inherit;' readonly /></td>";
										...
										echo "<td>".number_format_ind((float)$pur_val[4] - (float)$sal_val[1])."</td>";
											...
											$pfm = 0; $pfm = ((float)$sal_val[4] - (float)$pur_val[7]) - (float)$vou_val;
											if((float)$pur_val[4] != 0){
												$ppk = ((float)$pfm / (float)$pur_val[4]);
											}
											else{
												$ppk = 0;
											}
										if(number_format_ind($vou_val) == "" || number_format_ind($vou_val) == ".00"){ $vou_val = "0.00"; } else{ $vou_val = number_format_ind($vou_val); }
										
										echo "<td><input type='text' name='$values' id='$values' value='$vou_val' style='width:70px;text-align:right;border:none;background:inherit;' onkeyup='calculateprofitamt(this.id)'/></td>";
										//if(number_format_ind(((float)$sal_val[4] - (float)$pur_val[7]) - (float)$vou_val) == "" || number_format_ind(((float)$sal_val[4] - (float)$pur_val[7]) - (float)$vou_val) == ".00") { $pfm = "0.00"; } else{ $pfm = number_format_ind(((float)$sal_val[4] - (float)$pur_val[7]) - (float)$vou_val); }
										$pfm = number_format_ind($pfm);
										echo "<td><input type='text' name='pfm$values' id='pfm$values' value='$pfm'  style='width:70px;text-align:right;border:none;background:inherit;' readonly /></td>";
		...
		<script type="text/javascript" lahguage="javascript">
			...
			function calculateprofitamt(a){
				var b = document.getElementById(a).value;
				b = b.replace(/[^0-9.]/g, '');
				if(b.length == 0){ b = 0; }
				var c = document.getElementById("pv7"+a).value;
				c = c.replace(/[^0-9.]/g, '');
				var d = document.getElementById("sv4"+a).value;
				d = d.replace(/[^0-9.]/g, '');
				var e = document.getElementById("pv4"+a).value;
				e = e.replace(/[^0-9.]/g, '');
			
				var f = ((d - c) - b);
				var g = (((d - c) - b) / e);
				//alert("b="+b+"c="+c+"d="+d+"e="+e+"f="+f+"g="+g);
				document.getElementById("pfm"+a).value = f.toLocaleString('en-IN', { currency: 'INR',minimumFractionDigits: 2,maximumFractionDigits: 2 });
				document.getElementById("ppk"+a).value = g.toLocaleString('en-IN', { currency: 'INR',minimumFractionDigits: 2,maximumFractionDigits: 2 });
				var h = i = j = k = l = 0;
				<?php $a = sizeof($str_values); foreach($str_values as $st_val){ ?>
				h = '<?php echo $st_val; ?>';
				i = document.getElementById("pfm"+h).value;
				i = i.replace(/[^0-9.\-]/g, '');
				k = parseFloat(k) + parseFloat(i);
				
				j = document.getElementById(h).value;
				j = j.replace(/[^0-9.\-]/g, '');
				l = parseFloat(l) + parseFloat(j);
				<?php } ?>
				document.getElementById("tproamt").value = k.toLocaleString('en-IN', { currency: 'INR',minimumFractionDigits: 2,maximumFractionDigits: 2 });
				document.getElementById("tvouamt").value = l.toLocaleString('en-IN', { currency: 'INR',minimumFractionDigits: 2,maximumFractionDigits: 2 });
				var m = document.getElementById("tpurqty").value;
				m = m.replace(/[^0-9.\-]/g, '');
				var n = parseFloat(k) / parseFloat(m);
				document.getElementById("tpropkgamt").value = n.toLocaleString('en-IN', { currency: 'INR',minimumFractionDigits: 2,maximumFractionDigits: 2 });
			}
		</script>
