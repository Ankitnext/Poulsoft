<?php
//chicken_linewise_weightloss3_ta.php
...
if($db == ''){
    ...
    $form_reload_page = "chicken_linewise_weightloss3_ta.php";
}
else{
    include "APIconfig.php";
    $database_name = $db;
    $users_code = $_GET['emp_code'];
    $form_reload_page = "chicken_linewise_weightloss3_ta.php?db=".$db;
}
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            foreach($item_alist as $icode){
                                ...
                                echo $op_qty."<br> Purchase";
                                echo $oa_qty."<br> add";
                                echo $os_qty."<br> sale";
                                echo $od_qty."<br> deduct";
                                echo $om_qty."<br> mortality";
                            ...
                            while($row1 = mysqli_fetch_assoc($query1)){
                             ...
                             //Shortage Screen
                             $sql1 = "SELECT * FROM `item_shortage_screen` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `itemcode` IN ('$item_list') AND `warehouse` IN ('$sec_list') AND `active` = '1' AND `dflag` = '0' ORDER BY `date`,`itemcode`,`trnum` ASC";
                             $query1 = mysqli_query($conn, $sql1); $iwsa_abbds = $iwsa_dbbds = $iwsa_abqty = $iwsa_dbqty = $iwsa_abamt = $iwsa_dbamt = array();
                             while($row1 = mysqli_fetch_assoc($query1)){
                                 $key1 = $row1['itemcode'];
                                 if(strtotime($row1['date']) < strtotime($fdate)){ }
