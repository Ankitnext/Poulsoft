 ...
 background-color: #98fb98;
	...
	<body class="hold-transition skin-blue sidebar-mini">
		...
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
				<form action="profitandlossreport_pop.php" method="post" onsubmit="return checkval()">
					<?php } else { ?>
					<form action="profitandlossreport_pop.php?db=<?php echo $db; ?>" method="post" onsubmit="return checkval()">
					<?php } ?>
						...
						<?php } ?>
							...
							<?php
								if(isset($_POST['submit']) == true){
									...
									$sql = $seq."".$whname."".$grp; //echo $seq;
									$query = mysqli_query($conn,$sql);
									while($row = mysqli_fetch_assoc($query)){
										$exp_amt[$row['fcoa']] = $exp_amt[$row['fcoa']] + $row['amount'];
                                        $hexp_amt[$row['fcoa']] = $hexp_amt[$row['fcoa']] + $row['amount'];
                                  ...
                                  //  print_r($hexp_amt);
									...
									echo "</tr>";
									foreach($catcode as $cat_code){
										if($pur_cat_amt[$cat_code] == "" && $cb_cat_amt[$cat_code] == ""){
									...
									foreach($coa as $coa_codes){
										if($rec_amt[$coa_codes] == "" || $rec_amt[$coa_codes] == "0.00" || $rec_amt[$coa_codes] == ".00"){
                                            ...
                                            if($coaname[$coa_codes] == "Bhada"){
                                                echo "<tr>";
												//echo "<td></td>";
												echo "<td style='text-align:left;'>".$coaname[$coa_codes]."</td>";
												echo "<td>".number_format_ind($rec_amt[$coa_codes] + $exp_amt[$coa_codes] )."</td>";
												//echo "<td></td>";
												echo "<td></td>";
												echo "<td></td>";
											echo "</tr>";
                                            }else if($coaname[$coa_codes] == "Discount"){
                                                echo "<tr>";
												//echo "<td></td>";
												echo "<td style='text-align:left;'>".$coaname[$coa_codes]."</td>";
												echo "<td>".number_format_ind($rec_amt[$coa_codes] - $exp_amt[$coa_codes] )."</td>";
												...
												echo "<td></td>";
											echo "</tr>";
                                            }else if($coaname[$coa_codes] == "Home Expenses"){
                                                echo "<tr>";
												//echo "<td></td>";
												echo "<td style='text-align:left;'>".$coaname[$coa_codes]."</td>";
												echo "<td>".number_format_ind($rec_amt[$coa_codes] + $exp_amt[$coa_codes] - $hexp_amt[$coa_codes])."</td>";
												//echo "<td></td>";
												echo "<td></td>";
												echo "<td></td>";
											echo "</tr>";
									...
									echo "<tr style='background-color: #98fb98;'>";
										...
										echo "<td></td>";
										echo "<td style='font-weight:bold;'>".number_format_ind($t_sl_amt + $t_tout_amt + $t_cl_amt + $net_ls_amt )."</td>";
									echo "</tr>";
								}
							?>
							</tbody>
