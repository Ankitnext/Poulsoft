<?php
//chicken_retail_daywise_salesrpt.php
...
if($db == ''){
    ...
    $form_reload_page = "chicken_retail_daywise_salesrpt.php";
}
else{
    include "APIconfig.php";
    $dbname = $db;
    $users_code = $_GET['emp_code'];
    $form_reload_page = "chicken_retail_daywise_salesrpt.php?db=".$db;
}
...
$file_name = "Day Wise Sales Report";
...
$query = mysqli_query($conn,$sql); $item_code = $item_name = $item_cunits = $item_category = array();
while($row = mysqli_fetch_assoc($query)){ $item_code[$row['code']] = $row['code']; $item_name[$row['code']] = $row['description']; $item_cunits[$row['code']] = $row['cunits']; $item_category[$row['code']] = $row['category'];}
				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
								...
								<td colspan="19" class="p-1">
                                    <div class="m-1 p-1 row">
                                        ...
                                        <div class="m-1 form-group" style="width:150px;">
                                            <label for="items">Item Code</label>
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            $html .= '<th style="text-align:center;">Outlet</th>';
                            ...
                            //$item_fltr = ""; if($items != "all"){ $item_fltr = " AND `icode` = '$items'"; }

                            if($items != "all"){ $item_fltr = " AND `icode` IN ('$items')"; }
                                ...
                                $item_fltr = " AND `icode` IN ('$icat_list')";
                            }

                            $sql = "SELECT * FROM `retail_sales` WHERE `date` >= '$fdate' AND `date` <= '$tdate'".$usr_fltr."".$sec_fltr."".$item_fltr." AND `active` = '1' AND `dflag` = '0' ORDER BY `trnum` ASC";
                            $query = mysqli_query($conn, $sql); $dc_nos = $uname = $isale_qty = $isale_amt = array();
                            while($row = mysqli_fetch_assoc($query)){
                                $key = $row['date']."@".$row['warehouse']."@".$row['icode'];
                                
                                $dc_nos[$key] = $row['billno'];
                                $uname[$key] = $usr_name[$row['addedemp']];
                                ...
                                foreach($sector_code as $wcode){
                                    foreach($item_code as $icode){
                                        ...
                                        if(empty($isale_qty[$key]) || $isale_qty[$key] == "" || (float)$isale_qty[$key] == 0){ }
                                            ...
                                            $html .= '<td>'.$sector_name[$wcode].'</td>';
                                            $html .= '<td>'.$dc_nos[$key].'</td>';
                            ...
                            $price = 0; if((float)$tsale_qty != 0){ $price = (float)$tsale_amt / (float)$tsale_qty; }
                            $html .= '<tr class="thead2">';
                            $html .= '<th colspan="4">Total</th>';
                            $html .= '<th style="text-align:right;">'.number_format_ind(round($tsale_qty,2)).'</th>';
                            $html .= '<th style="text-align:right;">'.number_format_ind(round($price,2)).'</th>';
