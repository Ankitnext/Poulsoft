	...
	<?php } ?>
		<section class="content" align="center">
				<div class="col-md-12" align="center">
					<form action="sup_tdssummary.php" method="post">
						...
						<?php } ?>
							...
							<?php
								if($_POST['export'] != "exportexcel"){
                                    ...
                                    $sql = "SELECT SUM(tcdsamt) as amount,vendorcode as ccode FROM `pur_purchase` WHERE `date` >= '$fromdate' AND `date` <= '$todate' AND `tcdsamt` != '0' AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' GROUP BY `vendorcode` ORDER BY `vendorcode` ASC";
                                    ...
                                    foreach($cus_code as $ccode){
                                        if(!empty($cus_amt[$ccode])){
                                            ...
                                            echo "<td style='text-align:right;'><a href='sup_tdsdetailed.php".$vals."' target='_BLANK'>".number_format_ind($cus_amt[$ccode])."</a></td>";
                                            echo "</tr>";
