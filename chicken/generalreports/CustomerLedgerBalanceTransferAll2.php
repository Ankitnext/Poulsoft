<?php 
    ...
    if(isset($_POST['submit']) == true){
        ...
        $status = $_POST['status'];
        $cname = $_POST['cname'];
        $cus_list = ""; $cus_array_list = array();
        foreach($_POST['c_det'] as $clist){ $cus_array_list[$clist] = $clist; }
        $cus_list = implode("','",$cus_array_list);
    }
    else{
        $todate = $today;
        $status = "display";
...
<html>
	...
	<?php } ?>
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
				<form action="CustomerLedgerBalanceTransferAll2.php" method="post"  onsubmit="return checkval()">
					<?php } else { ?>
					<form action="CustomerLedgerBalanceTransferAll2.php?db=<?php echo $db; ?>" method="post"  onsubmit="return checkval()">
					<?php } ?>
						...
						<?php } ?>
							<thead class="thead2" style="background-color: #98fb98;">
								<tr>
									<th>Sl No.</th>
									<th>Select All<br/><input type="checkbox" name="checkall" id="checkall" onchange="checkedall()" /></th>
									<th>Name</th>
									<th>Balance</th>
								</tr>
							</thead>
                            ...
                            //Returns
                            $rtnsql = "SELECT * FROM `main_itemreturns` WHERE `date` <= '$todate' AND `mode` = 'customer' AND `active` = '1' AND `dflag` = '0'";
                            $rtnquery = mysqli_query($conn,$rtnsql); $returns = array();
                            ...
                            foreach($pcode as $pcodes){
                                ...
                                echo "<td style='text-align:center;'><input type='checkbox' name='c_det[]' id='c_det[$c]' value='$pcodes' /></td>";
                                ...
                                if(!empty($cus_array_list[$pcodes]) && $status == "modify"){
                            ...
                            if($status == "modify" && $cus_list != ""){
                                ...
                                if($count1 > 0){
                                    ...
                                    $sql2 = "INSERT INTO customer_sales_bkpc SELECT * FROM customer_sales WHERE `date` <= '$todate' AND `customercode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                else{
                                    $sql2 = "CREATE TABLE customer_sales_bkpc as SELECT * FROM customer_sales WHERE `date` <= '$todate' AND `customercode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                $sql3 = "DELETE FROM `customer_sales` WHERE `date` <= '$todate' AND `customercode` IN ('$cus_list');"; mysqli_query($conn,$sql3);
                                ...
                                if($count1 > 0){
                                    ...
                                    $sql2 = "INSERT INTO `customer_receipts_bkpc` SELECT * FROM `customer_receipts` WHERE `date` <= '$todate' AND `ccode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                else{
                                    $sql2 = "CREATE TABLE `customer_receipts_bkpc` as SELECT * FROM `customer_receipts` WHERE `date` <= '$todate' AND `ccode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                $sql3 = "DELETE FROM `customer_receipts` WHERE `date` <= '$todate' AND `ccode` IN ('$cus_list')"; mysqli_query($conn,$sql3);
                                ...
                                if($count1 > 0){
                                    ...
                                    $sql2 = "INSERT INTO `main_crdrnote_bkpc` SELECT * FROM `main_crdrnote` WHERE `date` <= '$todate' AND `mode` IN ('CCN','CDN') AND `ccode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                else{
                                    $sql2 = "CREATE TABLE `main_crdrnote_bkpc` as SELECT * FROM `main_crdrnote` WHERE `date` <= '$todate' AND `mode` IN ('CCN','CDN') AND `ccode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                $sql3 = "DELETE FROM `main_crdrnote` WHERE `date` <= '$todate' AND `mode` IN ('CCN','CDN') AND `ccode` IN ('$cus_list');"; mysqli_query($conn,$sql3);
                                ...
                                if($count1 > 0){
                                    ...
                                    $sql2 = "INSERT INTO `main_mortality_bkpc` SELECT * FROM `main_mortality` WHERE `date` <= '$todate' AND `mtype` = 'customer' AND `ccode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                else{
                                    $sql2 = "CREATE TABLE `main_mortality_bkpc` as SELECT * FROM `main_mortality` WHERE `date` <= '$todate' AND `mtype` = 'customer' AND `ccode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                $sql3 = "DELETE FROM `main_mortality` WHERE `date` <= '$todate' AND `mtype` = 'customer' AND `ccode` IN ('$cus_list');"; mysqli_query($conn,$sql3);
                                ...
                                if($count1 > 0){
                                    ...
                                    $sql2 = "INSERT INTO `main_itemreturns_bkpc` SELECT * FROM `main_itemreturns` WHERE `date` <= '$todate' AND `mode` = 'customer' AND `vcode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                else{
                                    $sql2 = "CREATE TABLE `main_itemreturns_bkpc` AS SELECT * FROM `main_itemreturns` WHERE `date` <= '$todate' AND `mode` = 'customer' AND `vcode` IN ('$cus_list');";
                                    if(!mysqli_query($conn,$sql2)){ echo "<br/>".mysqli_error($conn).""; } else{ }
                                }
                                $sql3 = "DELETE FROM `main_itemreturns` WHERE `date` <= '$todate' AND `mode` = 'customer' AND `vcode` IN ('$cus_list');"; mysqli_query($conn,$sql3);
                                echo "<tr><td colspan='3' style='font-weight:bold;color:green;'>Customer Closure completed successfully...!</td></tr>";
                            }
							?>
                            <tr>
                                <th style="padding-left:5px;text-align:left;" colspan="3">Total Balance</th>
			...
			function checkval(){
				...
				if(b == "display"){
                    ...
                    if(checkboxes.length == 0){
                        alert("Please select atleast one checkbox to proceed book Closure ..!");
                        l = false;
                    }
                    else {
                        l = true;
