	...
	<link rel="stylesheet" type="text/css"href="reportstyle.css">
		<?php
			if($exoption == "exportexcel") {
				...
				echo header("Content-Disposition: attachment; filename=profitandlossreport3($fromdate-$todate).xls");
 ...
 background-color: #98fb98;
	...
	<body class="hold-transition skin-blue sidebar-mini">
		...
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
				<form action="profitandlossreport3.php" method="post" onsubmit="return checkval()">
					<?php } else { ?>
					<form action="profitandlossreport3.php?db=<?php echo $db; ?>" method="post" onsubmit="return checkval()">
					<?php } ?>
								...
								if(isset($_POST['submit']) == true){
									...
									foreach($catcode as $cat_code){
										foreach($itemdetail as $itm_code){
											...
											if($cat_code == $spl_code[0]){
												$ob_cat_qty[$cat_code] = $ob_cat_qty[$cat_code] + $ob_qty[$spl_code[1]];
												$ob_cat_amt[$cat_code] = $ob_cat_amt[$cat_code] + $ob_amt[$spl_code[1]];
												$cb_cat_qty[$cat_code] = $cb_cat_qty[$cat_code] + $cb_qty[$spl_code[1]];
												$cb_cat_amt[$cat_code] = $cb_cat_amt[$cat_code] + $cb_amt[$spl_code[1]];
												$pur_cat_qty[$cat_code] = $pur_cat_qty[$cat_code] + $pur_qty[$spl_code[1]];
												$pur_cat_amt[$cat_code] = $pur_cat_amt[$cat_code] + $pur_amt[$spl_code[1]];
												$sal_cat_qty[$cat_code] = $sal_cat_qty[$cat_code] + $sal_qty[$spl_code[1]];
												$sal_cat_amt[$cat_code] = $sal_cat_amt[$cat_code] + $sal_amt[$spl_code[1]];
												$tin_cat_qty[$cat_code] = $tin_cat_qty[$cat_code] + $tin_qty[$spl_code[1]];
												$tin_cat_amt[$cat_code] = $tin_cat_amt[$cat_code] + $tin_amt[$spl_code[1]];
												$tout_cat_qty[$cat_code] = $tout_cat_qty[$cat_code] + $tout_qty[$spl_code[1]];
												$tout_cat_amt[$cat_code] = $tout_cat_amt[$cat_code] + $tout_amt[$spl_code[1]];
												$mort_cat_qty[$cat_code] = $mort_cat_qty[$cat_code] + $mort_qty[$spl_code[1]];
												$mort_cat_amt[$cat_code] = $mort_cat_amt[$cat_code] + $mort_amt[$spl_code[1]];
											} else { }
										}
									}
									echo "<tr>";
									echo "<td colspan='3' style='text-align:center;font-size:15px;font-weight:bold;'>Opening</td>";
									echo "<td colspan='3' style='text-align:center;font-size:15px;font-weight:bold;'>Sales</td>";
									echo "</tr>";
									foreach($catcode as $cat_code){
										if(number_format_ind($ob_cat_amt[$cat_code]) == "0.00" && number_format_ind($sal_cat_amt[$cat_code]) == "0.00"){
											...
											echo "<td>".number_format_ind($ob_cat_qty[$cat_code])."</td>";
											echo "<td>".number_format_ind($ob_cat_amt[$cat_code])."</td>";
											//echo "<td></td>";
											echo "<td style='text-align:left;'>".$catname[$cat_code]."</td>";
											echo "<td>".number_format_ind($sal_cat_qty[$cat_code])."</td>";
											echo "<td>".number_format_ind($sal_cat_amt[$cat_code])."</td>";
											echo "</tr>";
											$tdr_amt = $tdr_amt + $ob_cat_amt[$cat_code] + $pur_cat_amt[$cat_code];
											$t_ob_qty = $t_ob_qty + $ob_cat_qty[$cat_code];
											$t_ob_amt = $t_ob_amt + $ob_cat_amt[$cat_code];
											$t_sl_qty = $t_sl_qty + $sal_cat_qty[$cat_code];
									...
									if($sec_codes[1] != "all"){
										...
										foreach($catcode as $cat_code){
											...
											if(number_format_ind($tout_cat_amt[$cat_code]) == "0.00" && number_format_ind($tin_cat_amt[$cat_code]) == "0.00"){
												...
												//echo "<td></td>";
												echo "<td style='text-align:left;'>".$catname[$cat_code]."</td>";
												echo "<td>".number_format_ind($tin_cat_qty[$cat_code])."</td>";
												echo "<td>".number_format_ind($tin_cat_amt[$cat_code])."</td>";
												//echo "<td></td>";
												echo "<td style='text-align:left;'>".$catname[$cat_code]."</td>";
												echo "<td>".number_format_ind($tout_cat_qty[$cat_code])."</td>";
												echo "<td>".number_format_ind($tout_cat_amt[$cat_code])."</td>";
												echo "</tr>";
												$t_tin_qty = $t_tin_qty + $tin_cat_qty[$cat_code];
												$t_tin_amt = $t_tin_amt + $tin_cat_amt[$cat_code];
												$t_tout_qty = $t_tout_qty + $tout_cat_qty[$cat_code];
									...
									foreach($catcode as $cat_code){
										if($cb_cat_amt[$cat_code] == ""){
											...
											echo "<td></td>";
											echo "<td style='text-align:left;'>".$catname[$cat_code]."</td>";
											echo "<td>".number_format_ind($cb_cat_qty[$cat_code])."</td>";
											echo "<td>".number_format_ind($cb_cat_amt[$cat_code])."</td>";
											echo "</tr>";
											$tcr_amt = $tcr_amt + $sal_cat_amt[$cat_code] + $cb_cat_amt[$cat_code];
											//$t_pur_amt = $t_pur_amt + $pur_cat_amt[$cat_code];
											$t_cl_qty = $t_cl_qty + $cb_cat_qty[$cat_code];
									...
									echo "<td>".number_format_ind($t_cl_qty)."</td>";
