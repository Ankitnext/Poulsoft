<?php
	...
	while($row = mysqli_fetch_assoc($query)){
		...
		$field_details[$row['prate_flag']] = "prate_flag"; if($row['prate_flag'] == 1 || $row['prate_flag'] == "1"){ $prate_flag = 1; }
	...
	$url = "../PHPExcel/Examples/CustomerLedgerMasterReport-Excel3.php?fromdate=".$exl_fdate."&todate=".$exl_tdate."&cname=".$exl_cname."&cid=".$cid;
...
<html>
		...
		<header align="center">
			<table align="center" class="reportheadermenu">
				...
				<?php
					...
					while($row = mysqli_fetch_assoc($query)){ $company_name = $row['cname']; if(!empty($row['qr_img_path'])){ $qr_img_path = $row['qr_img_path']; } ?>
	...
	<?php } ?>
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
				<form action="CustomerLedgerMaster3.php?cid=<?php echo $cid; ?>" method="post"  onsubmit="return checkval()">
					<?php } else { ?>
					<form action="CustomerLedgerMaster3.php?db=<?php echo $db; ?>&cid=<?php echo $cid; ?>" method="post"  onsubmit="return checkval()">
					<?php } ?>
							...
							if(isset($_POST['submit']) == true){
								...
								<?php
									...
									if($count32 > 0){
										...
										if($obtype[$cus_names] == "Cr"){
											$ob_cramt = (float)$obamt[$cus_names];
										}
										else{
											$ob_dramt = (float)$obamt[$cus_names];
										}
									}
									$ob_rcv = (float)$ob_sales + (float)$ob_cdn + (float)$ob_dramt;
									$ob_pid = (float)$ob_receipt + (float)$ob_mortality + (float)$ob_returns + (float)$ob_ccn + (float)$ob_cramt;
									...
									if($ob_rcv >= $ob_pid){
										...
										echo "</tr>";
										$rb_amt = (float)$rb_amt + ((float)$ob_rcv - (float)$ob_pid);
										$ob_rev_amt = (float)$ob_rcv - (float)$ob_pid;
										$ob_pid_amt = 0;
									}
									else{
										echo "<tr>";
										...
										echo "</tr>";
										$rb_amt = (float)$rb_amt + ((float)$ob_rcv - (float)$ob_pid);
										$ob_pid_amt = (float)$ob_pid - (float)$ob_rcv;
										$ob_rev_amt = 0;
									}
									
									//Sales
									$sii_count = $slc_tcdsamt = $slc_roundoff = $slc_finaltotal = $sales = $receipts = $mortality = $returns = $ccns = $cdns = array();
									if($count14 > 0){
										...
										while($row = mysqli_fetch_assoc($query)){
											if(!empty($row['freight_amount_jal'])){ $freight_amount_jal = $row['freight_amount_jal']; } else{ $freight_amount_jal = 0; }
											$i = $i + 1; $sales[$row['date']."@".$i] = $row['date']."@".$row['invoice']."@".$row['bookinvoice']."@".$row['customercode']."@".$row['jals']."@".$row['totalweight']."@".$row['emptyweight']."@".$row['itemcode']."@".$row['birds']."@".$row['netweight']."@".$row['itemprice']."@".$row['totalamt']."@".$row['tcdsper']."@".$row['tcdsamt']."@".$row['roundoff']."@".$row['finaltotal']."@".$row['warehouse']."@".$row['narration']."@".$row['discountamt']."@".$row['taxamount']."@".$row['remarks']."@".$row['vehiclecode']."@".$row['drivercode']."@".$freight_amount_jal;
											
											if(empty($sii_count[$row['invoice']])){
												$sii_count[$row['invoice']] = 1;
												if($row['tcdsamt'] == "" || $row['tcdsamt'] == NULL){ $slc_tcdsamt[$row['invoice']] = 0.00; }
												else{ $slc_tcdsamt[$row['invoice']] = $row['tcdsamt']; }

												if(empty($row['roundoff'])){ $slc_roundoff[$row['invoice']] = 0.00; }
												else{
													if(!empty($row['itotal'])){ $itotal = 0; $itotal = (float)$row['itotal']; } else{ $itotal = 0; }
													if(!empty($row['tcdsamt'])){ $tcdsamt = 0; $tcdsamt = (float)$row['tcdsamt']; } else{ $tcdsamt = 0; }
													if(((float)$itotal + (float)$tcdsamt) <= $row['finaltotal']){
														$slc_roundoff[$row['invoice']] = (float)$row['roundoff'];
													}
													else{ $slc_roundoff[$row['invoice']] = -1 *((float)$row['roundoff']); }
												}
									...
									$fdate = strtotime($_POST['fromdate']); $tdate = strtotime($_POST['todate']); $i = $tacount = $fst_famt = $ft_roundoff = $ft_tcds = 0; $exi_inv = "";
									$frt_famt = $fct_famt = $fdt_famt = 0;
												...
												if($exi_inv != $sales_details[1]){
													...
													for($i = 1;$i <= $col_count;$i++){
														if($sales_details[9] > 0 && $sales_details[8] > 0){
															$avg_wt = (float)$sales_details[9] / (float)$sales_details[8];
														}else{
															$avg_wt = 0;
														}
														if(!empty($field_details[$i.":".$active_flag])){
													...
													$tbcount = (float)$tbcount + (float)$sales_details[8];
													$tjcount = (float)$tjcount + (float)$sales_details[4];
													$tncount = (float)$tncount + (float)$sales_details[9];
													$twcount = (float)$twcount + (float)$sales_details[5];
													$tecount = (float)$tecount + (float)$sales_details[6];
											...
											if(!empty($receipts[$date_asc."@".$j])){
												...
												for($i = 1;$i <= $col_count;$i++){
													if(!empty($field_details[$i.":".$active_flag])){
														...
														else if($field_details[$i.":".$active_flag] == "dr_flag"){ echo "<td>".number_format_ind($receipts_details[10])."</td>"; }
											...
											if(!empty($mortality[$date_asc."@".$j])){
												...
												for($i = 1;$i <= $col_count;$i++){
													if(!empty($field_details[$i.":".$active_flag])){
														...
														else if($field_details[$i.":".$active_flag] == "dr_flag"){ echo "<td>".number_format_ind($mortality_details[8])."</td>"; }
														else if($field_details[$i.":".$active_flag] == "rb_flag"){ echo "<td>".number_format_ind($rb_amt)."</td>"; }
														else{ }
													}
												}
											...
											if(!empty($ccns[$date_asc."@".$j])){
												...
												for($i = 1;$i <= $col_count;$i++){
													if(!empty($field_details[$i.":".$active_flag])){
														...
														else if($field_details[$i.":".$active_flag] == "dr_flag"){ echo "<td>".number_format_ind($ccns_details[7])."</td>"; }
														else if($field_details[$i.":".$active_flag] == "rb_flag"){ echo "<td>".number_format_ind($rb_amt)."</td>"; }
														else{ }
													}
												}
											...
											if(!empty($cdns[$date_asc."@".$j])){
												...
												for($i = 1;$i <= $col_count;$i++){
													if(!empty($field_details[$i.":".$active_flag])){
														...
														else if($field_details[$i.":".$active_flag] == "cr_flag"){ echo "<td>".number_format_ind($cdns_details[7])."</td>"; }
														else if($field_details[$i.":".$active_flag] == "dr_flag"){ echo "<td></td>"; }
														else if($field_details[$i.":".$active_flag] == "rb_flag"){ echo "<td>".number_format_ind($rb_amt)."</td>"; }
														else{ }
													}
								...
								<thead>
									<tr class="foottr" style="background-color: #98fb98;">
										...
										for($i = 1;$i <= $col_count;$i++){
											...
											if(!empty($field_details[$i.":".$active_flag])){
												...
												else if($field_details[$i.":".$active_flag] == "tamt_flag"){ echo "<td style='padding: 0 5px;text-align:right;'>".number_format_ind((float)$tacount + (float)$fdt_famt)."</td>"; }
												...
												else if($field_details[$i.":".$active_flag] == "cr_flag"){ echo "<td style='padding: 0 5px;text-align:right;'>".number_format_ind((float)$fst_famt + (float)$fdt_famt)."</td>"; }
												else if($field_details[$i.":".$active_flag] == "dr_flag"){ echo "<td style='padding: 0 5px;text-align:right;'>".number_format_ind((float)$frt_famt + (float)$fct_famt)."</td>"; }
									...
									<tr class="foottr" style="background-color: #98fb98;">
										...
										for($i = 1;$i <= $col_count;$i++){
											if(!empty($field_details[$i.":".$active_flag])){
												if($field_details[$i.":".$active_flag] == "cr_flag"){ echo "<td style='padding: 0 5px;text-align:right;'>".number_format_ind(((float)$fst_famt + (float)$fdt_famt) + (float)$ob_rev_amt)."</td>"; }
												else if($field_details[$i.":".$active_flag] == "dr_flag"){ echo "<td style='padding: 0 5px;text-align:right;'>".number_format_ind(((float)$frt_famt + (float)$fct_famt) + (float)$ob_pid_amt)."</td>"; }
												else if($field_details[$i.":".$active_flag] == "rb_flag"){ echo "<td></td>"; }
												else{ }
											}
										}
									...
									<tr class="foottr" style="background-color: #98fb98;">
										...
										<?php
											if(number_format_ind(((float)$fst_famt + (float)$fdt_famt) + $ob_rev_amt) == number_format_ind(((float)$frt_famt + (float)$fct_famt) + (float)$ob_pid_amt)){
												for($i = 1;$i <= $col_count;$i++){
													if(!empty($field_details[$i.":".$active_flag])){
														if($field_details[$i.":".$active_flag] == "cr_flag"){ echo "<td></td>"; }
