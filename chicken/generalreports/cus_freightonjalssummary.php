<?php
	...
	$url = "../PHPExcel/Examples/FreightonJalsSummary-Excel.php?fromdate=".$fromdate."&todate=".$todate;
	...
	<?php } ?>
		<section class="content" align="center">
				<div class="col-md-12" align="center">
					<form action="cus_freightonjalssummary.php" method="post">
						...
						<?php } ?>
							...
							<?php
								if($_POST['export'] != "exportexcel"){
                                    ...
                                    $sql = "SELECT SUM(freight_amount_jal) as amount,customercode as ccode FROM `customer_sales` WHERE `date` >= '$fromdate' AND `date` <= '$todate' AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' GROUP BY `customercode` ORDER BY `customercode` ASC";
                                    ...
                                    foreach($cus_code as $ccode){
                                        if(!empty($cus_amt[$ccode])){
                                            ...
                                            echo "<td style='text-align:right;'><a href='cus_freightonjalsdetailed.php".$vals."' target='_BLANK'>".number_format_ind($cus_amt[$ccode])."</a></td>";
                                            echo "</tr>";
