...
<html>
	...
	<?php } ?>
		<section class="content" align="center">
				...
				<?php if($db == ''){?>
					<form action="chicken_check_trnodetails1.php?cid=<?php echo $cid; ?>" method="post" onsubmit="return checkval()">
					<?php } else { ?>
						<form action="chicken_check_trnodetails1.php?cid=<?php echo $cid; ?>&db=<?php echo $db; ?>" method="post" onsubmit="return checkval()">
					<?php } ?>
						...
						<?php if($exoption == "displaypage" || $exoption == "exportpdf") { ?>
								...
								<tr>
									<td colspan="25">
                                        ...
                                        <div class="form-group" style="width: 210px;">
                                            <label for="tr_code">Search</label>
                                            <input type="text" name="tr_code" id="tr_code" class="form-control" style="padding:0;padding-left:2px;width:200px;" />
                                        </div>
                            ...
                            foreach($usr_arr_list as $ecode){
                                $i = 0;
                                if($count57 > 0){
                                    $sql = "SELECT * FROM `pur_purchase` WHERE `addedemp` IN ('$ecode') AND `addedtime` >= '$time1' AND `addedtime` <= '$time2' OR `updatedemp` IN ('$ecode') AND `updated` >= '$time1' AND `updated` <= '$time2' AND `invoice` = '$tr_code'";
                                    ...
                                    if($count > 0){
                                        while($row = mysqli_fetch_assoc($query)){
                                            ...
                                            if($row['addedemp'] != "" && strtotime($adtime1) == strtotime($tdate)){
                                                ...
                                                $html .= "<tr><td style='text-align:center;'>".$i."</td><td>".$edate."</td><td><a href='javascript:void(0);' onclick=".$path.">".$row['invoice']."</a></td>
                                                <td>Purchase</td><td>".$quantity."</td><td>".$amount."</td>
                                                <td>".$user_name[$aemp]."</td>
                                ...
                                if($count56 > 0){
                                    $sql = "SELECT * FROM `pur_payments` WHERE `addedemp` IN ('$ecode') AND `addedtime` >= '$time1' AND `addedtime` <= '$time2' OR `updatedemp` IN ('$ecode') AND `updatedtime` >= '$time1' AND `updatedtime` <= '$time2' AND `trnum` = '$tr_code'";
                                    $query = mysqli_query($conn,$sql); $count = mysqli_num_rows($query);
                                    if($count > 0){
                                        while($row = mysqli_fetch_assoc($query)){
                                            ...
                                            if($row['addedemp'] != "" && strtotime($adtime1) == strtotime($tdate)){
                                                ...
                                                <td>Payment</td><td></td><td>".$amount."</td>
                                                <td style='color:green;'>Entry Created</td><td>".$adtime2."</td></tr>";
                                            }
                                ...
                                if($count14 > 0){
                                    $sql = "SELECT * FROM `customer_sales` WHERE `addedemp` IN ('$ecode') AND `addedtime` >= '$time1' AND `addedtime` <= '$time2' OR `updatedemp` IN ('$ecode') AND `updated` >= '$time1' AND `updated` <= '$time2' AND `invoice` = '$tr_code'";
                                    $query = mysqli_query($conn,$sql); $count = mysqli_num_rows($query);
                                    if($count > 0){
                                        while($row = mysqli_fetch_assoc($query)){
                                            $aemp = $row['addedemp']; $eemp = $row['updatedemp'];$quantity = $row['netweight'];$amount = $row['totalamt']; $edate = date("d.m.Y",strtotime($row['date']));
                                ...
                                if($count13 > 0){
                                    $sql = "SELECT * FROM `customer_receipts` WHERE `addedemp` IN ('$ecode') AND `addedtime` >= '$time1' AND `addedtime` <= '$time2' OR `updatedemp` IN ('$ecode') AND `updatedtime` >= '$time1' AND `updatedtime` <= '$time2' AND `trnum` = '$tr_code'";
                                    $query = mysqli_query($conn,$sql); $count = mysqli_num_rows($query);
                                    if($count > 0){
                                        while($row = mysqli_fetch_assoc($query)){
                                            $aemp = $row['addedemp']; $eemp = $row['updatedemp']; $amount = $row['amount']; $edate = date("d.m.Y",strtotime($row['date']));
                                ...
                                if($count7 > 0){
                                    $sql = "SELECT * FROM `acc_vouchers` WHERE `addedemp` IN ('$ecode') AND `addedtime` >= '$time1' AND `addedtime` <= '$time2' OR `updatedemp` IN ('$ecode') AND `updatedtime` >= '$time1' AND `updatedtime` <= '$time2' AND `trnum` = '$tr_code'";
                                    $query = mysqli_query($conn,$sql); $count = mysqli_num_rows($query);
                                    if($count > 0){
                                        while($row = mysqli_fetch_assoc($query)){
                                            ...
                                            if($row['addedemp'] != "" && strtotime($adtime1) == strtotime($tdate)){
                                                ...
                                                $html .= "<tr><td style='text-align:center;'>".$i."</td><td>".$edate."</td>
                                                <td><a href='javascript:void(0);' onclick=".$path.">".$row['trnum']."</a></td>
                                                <td>Voucher</td><td>".$quantity."</td><td>".$amount."</td>
                                                <td>".$user_name[$aemp]."</td>
                                                <td style='color:green;'>Entry Created</td><td>".$adtime2."</td></tr>";
                                            }
                                            $adtime1 = date("Y-m-d",strtotime($row['updatedtime'])); $adtime2 = date("d.m.Y h:i:sA",strtotime($row['updatedtime']));
                                ...
                                if($count35 > 0){
                                    ...
                                    if($count > 0){
                                        while($row = mysqli_fetch_assoc($query)){
                                            ...
                                            $path = "openPopup('../view_deletion_details.php?id=$id')";
