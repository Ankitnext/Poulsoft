<?php
//chicken_vehiclewise_summary2.php
...
if($db == ''){
    ...
    $form_reload_page = "chicken_vehiclewise_summary2.php";
}
else{
...
$file_name = "Vehicle Summary Report";
				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            $html .= '<th style="text-align:center;">VEHICLE No.</th>';
                            ...
                            $html .= '<th style="text-align:center;">FINAL AMOUNT</th>';
                            $html .= '</tr>';

                          //  $usr_fltr = ""; if($users != "all"){ $usr_fltr = " AND `addedemp` = '$users'"; }
                            $sec_fltr = ""; if($sectors != "all"){ $sec_fltr = " AND `warehouse` = '$sectors'"; }
                            $sec_fltr1 = ""; if($sectors != "all"){ $sec_fltr1 = " AND `ccode` = '$sectors'"; }

                            $sql = "SELECT * FROM `acc_vouchers` WHERE  `active` = '1'".$sec_fltr." AND `vtype` = 'Pay Voucher' AND `date`>='$fdate' AND `date`<='$tdate'";
                            $query = mysqli_query($conn, $sql); $total_kms =  $amount = $advance = $final_amt = $arr = array(); $slno = 0; $totalkms = $tamount = $tadvance = $tfinal_amt = 0;
                            while($row = mysqli_fetch_assoc($query)){
                                     $key = $row['warehouse'];
                                     $total_kms[$key] += (int)$row['total_kms'];
                                     $amount[$key] += (float)$row['veh_amt'];
                                     $advance[$key] += (float)$row['advance_amt'];
                                     $final_amt[$key] +=  (float)$row['veh_amt'] - (float)$row['advance_amt'];
                                     $arr[$key] = $key;
                            }

                            $sql = "SELECT * FROM main_crdrnote WHERE `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0'".$sec_fltr1." AND `trlink` = 'chicken_display_crdrnote3.php' AND `date`>='$fdate' AND `date`<='$tdate'";
                            $query = mysqli_query($conn,$sql); $veh_dn = $veh_cn = array();
                            while($row = mysqli_fetch_assoc($query)){
                                $key = $row['ccode'];
                                if($row['mode'] == 'VDN'){
                                    $veh_dn[$key] += $row['amount'];
                                }else if($row['mode'] == 'VCN'){
                                    $veh_cn[$key] += $row['amount'];
                                }
                            }

                      foreach($arr as $a){
                                    $slno++;
                                    $totalkms += $total_kms[$a];
                                    $tamount += (float)$amount[$a];
                                    $tadvance += (float)$advance[$a];
                                    $tfinal_amt += (float)$final_amt[$a];
                                    $fvcn += $veh_cn[$a];
                                    $fvdn += $veh_dn[$a];
                                    $html .= '<tr>';
                                    $html .= '<td style="text-align:center">'.$slno.'</td>';
                                    $html .= '<td style="text-align:left">'.$sector_name[$arr[$a]].'</td>';
                                    $html .= '<td style="text-align:right">'.$total_kms[$a].'</td>';
                                    $html .= '<td style="text-align:right">'.number_format_ind($amount[$a]).'</td>';
                                    $html .= '<td style="text-align:right">'.number_format_ind($advance[$a]).'</td>';

                                    $html .= '<td style="text-align:right">'.number_format_ind($veh_cn[$a]).'</td>';
                                    $html .= '<td style="text-align:right">'.number_format_ind($veh_dn[$a]).'</td>';

                                    $html .= '<td style="text-align:right">'.number_format_ind($final_amt[$a]).'</td>';
                                    $html .= '</tr>';

                       }
                       $html .= '<tr class="thead2">';
                            $html .= '<th colspan="2">Total</th>';
                            $html .= '<th style="text-align:right;">'.$totalkms.'</th>';
                            $html .= '<th style="text-align:right;">'.number_format_ind($tamount).'</th>';
                            $html .= '<th style="text-align:right;">'.number_format_ind($tadvance).'</th>';

                            $html .= '<th style="text-align:right;">'.number_format_ind($fvcn).'</th>';
                            $html .= '<th style="text-align:right;">'.number_format_ind($fvdn).'</th>';

                            $html .= '<th style="text-align:right;">'.number_format_ind($tfinal_amt).'</th>';
