...
if($db == ''){
    ...
    $form_reload_page = "chicken_shop_day_book_report.php";
}
else{
    include "APIconfig.php";
    $dbname = $db;
    $users_code = $_GET['emp_code'];
    $form_reload_page = "chicken_linewise_weightloss1.php?db=".$db;
}
...
$file_name = "Shop day book report";
...
$sql = "SELECT * FROM `main_officetypes` WHERE `description` LIKE 'Shop'"; $query = mysqli_query($conn,$sql);
				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            $html .= '<th style="text-align:center;"></th>';
                            $html .= '<th style="text-align:center;text-align:right;"><span id="opening_birds"></span></th>';
                            $html .= '<th style="text-align:center;"></th>';
                            $html .= '<th style="text-align:center;"></th>';
                            $html .= '<th style="text-align:center;text-align:right;"><span id="nweight"></span></th>';
                            $html .= '<th style="text-align:center;"></th>';
                            $html .= '<th style="text-align:center;"></th>';
                            $html .= '<th style="text-align:center;text-align:right;"><span id="amount"></span></th>';
                            $html .= '</tr>';
                            ...
                            $sql = "SELECT * FROM `item_closingstock` WHERE `date` = '$pdate' AND `code` IN ('$item_list') AND `warehouse` IN ('$sec_list') AND `active` = '1'"; $query = mysqli_query($conn,$sql);
                            // echo $sql;
                             while($row = mysqli_fetch_assoc($query)){
                                 $key1 = $row['code'];
                               //  if(strtotime($row1['date']) < strtotime($fdate)){
                                     $clstk_obds[$key1] += (float)$row['closedbirds'];
                                     $clstk_oqty[$key1] += (float)$row['closedquantity'];
                                     $clstk_oamt[$key1] += (float)$row['amount'];
                                 // }
                                 // else{ }
                            ...
                            //Transfer-In
                            $sql1 = "SELECT * FROM `item_stocktransfers` WHERE `date` = '$fdate' AND `code` IN ('$item_list') AND `towarehouse` IN ('$sec_list') AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`code`,`trnum` ASC";
                          ...
                          //  print_r($iwso_bbds);
                            //Transfer-Out
                            $sql1 = "SELECT * FROM `item_stocktransfers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `code` IN ('$item_list') AND `fromwarehouse` IN ('$sec_list') AND `active` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`code`,`trnum` ASC"; //echo $sql1;
                            $query1 = mysqli_query($conn,$sql1);
                            while($row1 = mysqli_fetch_assoc($query1)){
                                $j++;
                           ...
                           //  print_r($iwso_bqty);
                            ...
                            $html .= '<tr style="text-decoration: none;background-color: white;">';
                                $html .= '<th colspan="'.$c_cnt.'">';
								if($items == "all"){ $html .= '<table style="width:100%">'; } else{ $html .= '<table>'; }
                                    
                                        $html .= '<tr style="background-color: #d8ccf9;">';
                                        ...
                                        $html .= '<tr>';
                                        $html .= '<th style="background-color: #d8ccf9;">Total Openings</th>';
                                        ...
                                        foreach($item_alist as $icode){
                                            ...
                                            $price = $iwp_opprc[$icode]; if($price == ""){ $price = 0; }
                                              $amount = ((float)$price * (float)$nweight); if($amount == ""){ $amount = 0; }
                                               ...
                                               $total_opening_bird = $birds + $clstk_obds[$icode];
                                               $total_opening_wt = $nweight + $clstk_oqty[$icode];
                                               $total_opening_amt = $amount + $clstk_oamt[$icode];
                                            $html .= '<td style="text-align:right;">'.str_replace( ".00","",number_format_ind($birds + $clstk_obds[$icode])).'</td>';
                                            $html .= '<td style="text-align:right;">'.number_format_ind($nweight + $clstk_oqty[$icode]).'</td>';
                                            $html .= '<td style="text-align:right;">'.number_format_ind($amount + $clstk_oamt[$icode]).'</td>';
                                        }
                                        $html .= '</tr>';
                                        
                                        $html .= '<tr>';
                                        $html .= '<th style="background-color: #d8ccf9;">Total Purchase/T. IN</th>';
                                        // foreach($item_alist as $icode){
                                       ...
                                       // print_r($iwsi_bqty);
                                        ...
                                        $tot_sqty = 0;
                                        $html .= '<tr>';
                                        $html .= '<th style="background-color: #d8ccf9;">Total Sold/T. OUT</th>';
                                        foreach($item_alist as $icode){
                                        ...
                                        $html .= '<tr>';
                                        $html .= '<th style="background-color: #d8ccf9;">Total Mortality</th>';
                                        foreach($item_alist as $icode){
                                        ...
                                        $html .= '<tr>';
                                        $html .= '<th style="background-color: #d8ccf9;">Stock Adjustment</th>';
                                        foreach($item_alist as $icode){
                                        ...
                                        $cls_amt = array();
                                        $html .= '<tr>';
                                        $html .= '<th style="background-color: #d8ccf9;">Total Closing</th>';
                                        foreach($item_alist as $icode){
                                            $birds = round((((float)$opn_bds[$icode] + (float)$iwsi_bbds[$icode] + (float)$iwsa_abbds[$icode]) - ((float)$iwm_bbds[$icode] + (float)$iwso_bbds[$icode] + (float)$iwsa_dbbds[$icode])),2); if($birds == ""){ $birds = 0; }
                                            $nweight = round((((float)$opn_qty[$icode] + (float)$iwsi_bqty[$icode] + (float)$iwsa_abqty[$icode]) - ((float)$iwm_bqty[$icode] + (float)$iwso_bqty[$icode] + (float)$iwsa_dbqty[$icode])),2); if($nweight == ""){ $nweight = 0; }
                                            $price = $iwp_oprc[$icode]; if($price == ""){ $price = 0; }
                                            ...
                                            // calculate weight
                                            $totalclosingwt = ((float)$total_opening_wt + (float)$total_pur_nwt) - ((float)$tsoldtoutwt + (float)$tmortwt);
                                            $totalclosingbrds = $birds;
                                            $html .= '<td style="text-align:right;">'.str_replace( ".00","",number_format_ind($birds)).'</td>';
                                            $html .= '<td style="text-align:right;">'.number_format_ind($totalclosingwt).'</td>';
                                            $html .= '<td style="text-align:right;">'.number_format_ind($cls).'</td>';
                                        }
                                        $html .= '</tr>';


                                        $seq = "SELECT code,closedquantity,closedbirds,price FROM `item_closingstock` WHERE `date` LIKE '$fdate' AND `code` IN ('$icode') AND `warehouse` IN ('$sec_list')"; $sql = $seq; $query = mysqli_query($conn,$sql);
											while($row = mysqli_fetch_assoc($query)){
												$actitemcode[$row['code']] = $row['code'];
                                            ...
                                            $html .= '<th style="background-color: #d8ccf9;">Actual Closing</th>';
                                            foreach($item_alist as $icode){
                                                $finactclamt = $actitemamt[$icode];
                                                $html .= '<td style="text-align:right;">'.str_replace( ".00","",number_format_ind($actitembds[$icode])).'</td>';
                                                $html .= '<td style="text-align:right;">'.number_format_ind($actitemqty[$icode]).'</td>';
                                                $html .= '<td style="text-align:right;">'.number_format_ind($actitemamt[$icode]).'</td>';
                                            }
                                            $html .= '</tr>';
                                        ...
                                        $html .= '<tr>';
                                        $html .= '<th style="background-color: #d8ccf9;">Weight Loss</th>';
                                       // print_r($addeditemcode);
                                        foreach($addeditemcode as $icodes){
                                           

                                            $html .= '<td style="text-align:right;">'.number_format_ind($totalclosingbrds - $actitembds[$icode]).'</td>';
                                            $html .= '<td style="text-align:right;">'.number_format_ind($totalclosingwt - $actitemqty[$icode]).'</td>';
                                       ...
                                       // print_r($addeditemcode);
                                        foreach($addeditemcode as $icodes){
                                            ...
                                            $wlosswt = $totalclosingwt - $actitemqty[$icode];
                                            $totptrin = ((float)$total_opening_wt + (float)$total_pur_nwt);
                                            $html .= '<td style="text-align:right;"></td>';

                                            if((float)$addeditemqty[$icodes] > 0){
                                                $html .= "<td style='text-align:right;'>".number_format_ind(($wlosswt/$totptrin) * 100)."</td>";
                                                        }
                                                        else{
                                        ...
                                        $tot_gamt = 0;
                                        $html .= '<tr>';
                                        $html .= '<th style="background-color: #d8ccf9;">Gross Profit</th>';
                                        foreach($item_alist as $icode){
                                            //$price = $iwp_oprc[$icode]; if($price == ""){ $price = 0; }
                                            $amount = round((((float)$iwso_bamt[$icode] + (float)$cls) - ((float)$opn_amt[$icode] + (float)$iwsi_bamt[$icode])),2); if($amount == ""){ $amount = 0; }
                                          //   echo $ftotalsaleamt."@".$finactclamt."@".$total_opening_amt."@".$total_pur_amt;
                                            $html .= '<td style="text-align:right;"></td>';
                                            $html .= '<td style="text-align:right;"></td>';
                                            $html .= '<th style="text-align:right;">'.number_format_ind($ftotalsaleamt + $finactclamt - $total_opening_amt - $total_pur_amt).'</th>';

                                            $tot_gamt += $ftotalsaleamt + $finactclamt - $total_opening_amt - $total_pur_amt;
                                        }
                                        $html .= '</tr>';
                            ...
                            while($row = mysqli_fetch_assoc($query)){
                                $modename[$row['code']] = $row['description'];
                            }
                            ...
                            $groupcode = "";
                            if($sectors != 'all') { $groupcode = ' WHERE description = "'.$sector_name[$sectors].'"'; }

                            $sql = "SELECT * FROM main_groups $groupcode";  $query = mysqli_query($conn,$sql); $grcode = array();
                            while($row = mysqli_fetch_assoc($query)){
                                $grcode[$row['code']] = $row['code'];
                            }

                            $grplist = implode("','",$grcode);
                            
                          //  echo $grplist;
                            $sql = "SELECT * FROM `main_contactdetails` WHERE groupcode IN ('$grplist') ORDER BY `name` ASC"; // echo $sql;
                            ...
                            // }
                            $clist = implode("','",$cus_code);

                            //opening balance

                            $sql = "SELECT * FROM `acc_coa` WHERE description = '$sector_name[$sectors]'"; $query = mysqli_query($conn,$sql);
                            while($row = mysqli_fetch_assoc($query)){
                                $coa = $row['code'];
                            }

                            $sql = "SELECT SUM(amount) as tamt FROM `pur_payments` WHERE `date` < '$fdate' AND `active` = '1' AND `method` = '$coa' AND `warehouse` = '$sectors' ORDER BY `ccode` ASC"; $query = mysqli_query($conn,$sql);
                            while($row = mysqli_fetch_assoc($query)){ $pre_pur_pay_amt = $row['tamt']; }
                            $sql = "SELECT SUM(amount) as tamt FROM `customer_receipts` WHERE `date` < '$fdate' AND `active` = '1' AND `method` = '$coa' AND `warehouse` = '$sectors' ORDER BY `ccode` ASC"; $query = mysqli_query($conn,$sql);
                            while($row = mysqli_fetch_assoc($query)){ $pre_cus_rct_amt = $row['tamt']; }
                            $sql = "SELECT SUM(amount) as tamt,mode FROM `main_crdrnote` WHERE `date` < '$fdate' AND `active` = '1' AND `coa` = '$coa' AND `warehouse` = '$sectors' GROUP BY `mode` ORDER BY `ccode` ASC"; $query = mysqli_query($conn,$sql);
                            while($row = mysqli_fetch_assoc($query)){
                            ...
                            $sql = "SELECT SUM(amount) as tamt,prefix FROM `acc_vouchers` WHERE `date` < '$fdate' AND `active` = '1' AND `fcoa` = '$coa' AND `warehouse` = '$sectors' GROUP BY `prefix` ORDER BY `fcoa` ASC"; $query = mysqli_query($conn,$sql);
                            while($row = mysqli_fetch_assoc($query)){
                            ...
                            $sql = "SELECT SUM(amount) as tamt,prefix FROM `acc_vouchers` WHERE `date` < '$fdate' AND `active` = '1' AND `tcoa` = '$coa' AND `warehouse` = '$sectors' GROUP BY `prefix` ORDER BY `tcoa` ASC"; $query = mysqli_query($conn,$sql);
                            while($row = mysqli_fetch_assoc($query)){
                            ...
                            $html .= '<th colspan="'.$c_cnt.'" style="text-align:center;background-color: #d8ccf9;">Opening Balance</th>';
                            $html .='</tr>';
                            ...
                            $html .='<th colspan="6" style="text-align:center;width:400px;font-size: 20px;font-weight: 800;">Balance</th>';
                            $html .='<th colspan="" style="text-align:right;font-size: 20px;font-weight: 800;">'.number_format_ind($closing_amt).'('.(($closing_amt > 0) ? 'Dr' : 'Cr').')</th>';
                            ...
                            $html .= '<th colspan="'.$c_cnt.'" style="text-align:center;background-color: #d8ccf9;">Shop Collection</th>';
                            $html .='</tr>';
                            ...
                            $html .= '<table style="width:100%;">';
                            $html .='<tr style="background-color: #98fb98;" align="center">';
                            $html .='<th colspan="" style="text-align:center;">Transaction No</th>';
                            $html .='<th colspan="" style="text-align:center;">Customer</th>';
                            $html .='<th colspan="" style="text-align:center;">Payment Mode</th>';
                            $html .='<th colspan="" style="text-align:center;">Payment Type</th>';
                            $html .='<th colspan="" style="text-align:center;">Remarks</th>';
                            $html .='<th colspan="" style="text-align:center;">Debit</th>';
                            $html .='<th colspan="" style="text-align:center;">Credit</th>';
                            $html .='</tr>';

                            $sql = "SELECT sum(amount) as tamt FROM customer_receipts WHERE active = '1' AND `ccode` IN ('$clist') AND `date` = '$fdate' AND `mode` = 'MOD-001'"; $query = mysqli_query($conn,$sql);
                            while($row = mysqli_fetch_assoc($query)){
                                $total_collection = $row['tamt'];
                            }
                           // echo $total_collection;

                            $sql = "SELECT * FROM customer_receipts WHERE active = '1' AND `ccode` IN ('$clist') AND `date` = '$fdate'"; $query = mysqli_query($conn,$sql);
                            while($row = mysqli_fetch_assoc($query)){
                                $cusreptotal += $row["amount"];
                                $html .='<tr>';
                                        $html .='<td colspan="" style="text-align:left;">'.$row["trnum"].'</td>';
                                        $html .='<td colspan="" style="text-align:left;">'.$ven_name[$row["ccode"]].'</td>';
                                        $html .='<td colspan="" style="text-align:left;">'.$modename[$row["mode"]].'</td>';
                                        $html .='<td colspan="" style="text-align:left;">'.$coaname[$row["method"]].'</td>';
                                        $html .='<td colspan="" style="text-align:left;">'.$row1["remarks"].'</td>';
                                        $html .='<td colspan="" style="text-align:right;">'.$row["amount"].'</td>';
                            ...
                            $html .='<tr>';
                                        $html .='<td colspan="5" style="text-align:center;font-weight:bold;">Total Collection</td>';
                                      
                                        $html .='<td colspan="" style="text-align:right;"><b>'.number_format_ind(round($cusreptotal,2)).'</b></td>';
                                        $html .='<td colspan="" style="text-align:left;"></td>';
                            ...
                            $html .= '<th colspan="'.$c_cnt.'" style="text-align:center;background-color: #d8ccf9;">Payments/Deposits</th>';
                            $html .='</tr>';
                            ...
                            $html .='<tr style="background-color: #98fb98;" align="center">';
                            $html .='<th colspan="" style="text-align:center;">Transaction No.</th>';
                            $html .='<th colspan="" style="text-align:center;">Paid/Received</th>';
                            $html .='<th colspan="" style="text-align:center;">Remarks</th>';
                            $html .='<th colspan="" style="text-align:center;">Debit</th>';
                            $html .='<th colspan="" style="text-align:center;">Credit</th>';
                            ...
                            $sql = "SELECT * FROM `acc_schedules` WHERE `subtype` NOT LIKE 'COA-0003'"; $query = mysqli_query($conn,$sql);
                            ...
                            if($items == "all"){
                                $sql1 = "SELECT * FROM `acc_vouchers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `prefix` LIKE 'PV' AND `tcoa` IN ('$exp_coa_code') AND `warehouse` IN ('$sec_list') AND `active` = '1'"; //echo $sql1;
                                $query1 = mysqli_query($conn,$sql1);
                                while($row1 = mysqli_fetch_assoc($query1)){
                                    $html .='<tr>';
                                        ...
                                        $html .='<td colspan="" style="text-align:left;">'.$row1["remarks"].'</td>';
                                        $html .='<td colspan="" style="text-align:left;"></td>';
                                        $html .='<td colspan="" style="text-align:right;">'.$row1["amount"].'</td>';
                                        
                                       
                                    $html .='</tr>';
                                    $tot_eamt += (float)$row1['amount'];
                                }
                                $total_Payments = $tot_eamt;
                            }
                            // $sql1 = "SELECT * FROM `pur_purchase` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `itemcode` IN ('$item_list') AND `warehouse` IN ('$sec_list') AND `active` = '1'"; // echo $sql1;
                            // $query1 = mysqli_query($conn,$sql1);
                            // while($row1 = mysqli_fetch_assoc($query1)){
                            //     $html .='<tr>';
                            //         $html .='<td colspan="" style="text-align:left;">'.$row1["invoice"].'</td>';
                            //         $html .='<td colspan="" style="text-align:left;">Received</td>';
                            //         $html .='<td colspan="" style="text-align:left;">'.$row1['totalamt'].'</td>';
                            //         $html .='<td colspan="" style="text-align:left;"></td>';
                            //         $html .='<td colspan="" style="text-align:left;">'.$row1["remarks"].'</td>';
                            ...
                            $html .='<tr>';
                                    $html .='<td colspan="3" style="text-align:center;font-weight:bold;">Total Payments</td>';
                                    
                                    $html .='<td colspan="" style="text-align:left;"></td>';
                                    $html .='<td colspan="" style="text-align:right;"><b>'.number_format_ind($tot_eamt).'</b></td>';
                            ...
                            $html .= '<th colspan="'.$c_cnt.'" style="text-align:center;background-color: #d8ccf9;">Expenses</th>';
                            $html .='</tr>';
                            ...
                            $html .='<th colspan="" style="text-align:center;">Cash/Bank</th>';
                            $html .='<th colspan="" style="text-align:center;">Paid/Received</th>';
                            $html .='<th colspan="" style="text-align:center;">Remarks</th>';
                            $html .='<th colspan="" style="text-align:center;">Debit</th>';
                            $html .='<th colspan="" style="text-align:center;">Credit</th>';
                            ...
                            $html .='<tr>';
                            $html .='<td colspan="4" style="text-align:center;font-weight:bold;">Total Expenses</td>';
                          
                            $html .='<td colspan="" style="text-align:left;"></td>';
                            $html .='<td colspan="" style="text-align:right;"><b>'.number_format_ind(round($tot_eamt,2)).'</b></td>';
                            ...
                            $html .= '<th colspan="'.$c_cnt.'" style="text-align:center;background-color: #d8ccf9;">Closing Balance</th>';
                            $html .='</tr>';

                            $final_closing_bal = ($closing_amt + $total_collection) - ($total_Payments + $total_expenses);
                           // echo $final_closing_bal;
                            $html .= '<tr>';
                            $html .= '<th colspan="'.$c_cnt.'">';
                            $html .= '<table style="width:100%;">';
                            $html .='<tr style="background-color: #98fb98;" align="center">';
                            $html .='<th colspan="6" style="text-align:center;width:400px;font-size: 20px;font-weight: 800;">Balance</th>';
                            $html .='<th colspan="" style="text-align:right;font-size: 20px;font-weight: 800;">'.$final_closing_bal.'('.(($final_closing_bal > 0) ? 'Dr' : 'Cr').')</th>';
