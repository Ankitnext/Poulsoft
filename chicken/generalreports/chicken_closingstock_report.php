<?php
//chicken_closingstock_report.php
...
if($db == ''){
    ...
    $form_reload_page = "chicken_closingstock_report.php";
}
else{
    include "APIconfig.php";
    $dbname = $db;
    $users_code = $_GET['emp_code'];
    $form_reload_page = "chicken_closingstock_report.php?db=".$db;
}
...
$fdate = $tdate = date("Y-m-d"); $sectors = "all"; $fstyles = $fsizes = "default"; $exports = "display"; $area_acode = $item_acode = array(); $area_acode["all"] = "all"; $item_acode["all"] = "all"; $area_fltr = $item_fltr = ""; $aa_flag = $it_flag = 0;
if(isset($_POST['submit']) == true){
    ...
    if($it_flag == 0){ $item_list = implode("','",$item_acode); $item_fltr = " AND `code` IN ('$item_list')"; }
}
?>
<html>
	<head>
				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                    ...
                    <?php } ?>
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            $sql = "SELECT * FROM `item_closingstock` WHERE `date` >= '$fdate' AND `date` <= '$tdate'".$item_fltr."".$sec_fltr." AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`trnum` ASC";
