<?php
	...
	if($db == ''){
        ...
        $form_reload_page = "chicken_shrtscs_report.php";
	}
	else{
        ...
        $form_reload_page = "chicken_shrtscs_report.php?db=".$db;
	}
    $file_name = "Shortage Report";
				...
				<form action="<?php echo $form_reload_page; ?>" method="post" onsubmit="return checkval()">
                    ...
                    <table class="main-table table-sm table-hover" id="main_table">
                        ...
                        if(isset($_POST['submit']) == true){
                            ...
                            $item_fltr = ""; if($items != "all"){ $item_fltr = " AND `itemcode` = '$items'"; }
                            ...
                            $fhead_html .= '<tr>';
                            $fhead_html .= '<th id="order_date">Sl No.</th>';
                            ...
                            $fhead_html .= '<th id="order">Items</th>';
                            $fhead_html .= '<th id="order">Quantity</th>';
                            $fhead_html .= '<th id="order">Price</th>';
                            ...
                            $sql = "SELECT * FROM `item_shortage_screen` WHERE `date` >= '$fdate' AND `date` <= '$tdate'".$item_fltr."".$sec_fltr." AND `dflag` = '0'  ORDER BY `date`,`trnum` ASC";
                            $query = mysqli_query($conn,$sql); $tot_qty = $tot_amt = 0; $Sl = 1;
                            while($row = mysqli_fetch_assoc($query)){
                                $date = date("d.m.Y",strtotime($row['date']));
                                $trnum = $row['trnum'];
                                $a_type = ucfirst($row['a_type']);
                                $mode = $adesc[$row['mode']];
                                $descs_name = $desc_name[$row['tcoa']];
                                $sec_name = $sector_name[$row['warehouse']];
                                $it_name = $item_name[$row['itemcode']];
                                $quantity = $row['nweight'];
                                $price = $row['price'];
                                $amount = $row['amount'];
                                $remarks = $row['remarks'];
                                ...
                                $html .= '<td style="text-align:left;">'.$a_type.'</td>';
                            ...
                            $html .= '<th style="text-align:right;">'.number_format_ind($tot_prc).'</th>';
