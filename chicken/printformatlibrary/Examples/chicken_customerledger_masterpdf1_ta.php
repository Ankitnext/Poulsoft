...
foreach($_POST['ccode'] as $ccode){
    ...
    if($dbname == "poulso6_chicken_tn_nataraj_broilers"){
        ...
        $html .= '<td style="width:'.$sh_cnt.'px;border:1px solid black;text-align:center;">NATARAJ BROILER</td>';
        $html .= '<td style="width:'.$sh_cnt.'px;border:1px solid black;text-align:center;">'.$cus_name[$vendors].'</td>';
        $html .= '</tr>';
        $html .= '<tr class="tfoot1" style="line-height:1.7;">';
        $html .= '<td style="width:'.$sh_cnt.'px;border:1px solid black;text-align:center;">SEENAPURAM, PERUNDURAI</td>';
        $html .= '<td style="width:'.$sh_cnt.'px;border:1px solid black;text-align:center;">PH: '.$cus_mobile[$vendors].'</td>';
        $html .= '</tr>';
        $html .= '<tr class="tfoot1" style="line-height:1.7;">';
        $html .= '<td style="width:'.$sh_cnt.'px;border:1px solid black;text-align:center;">GPAY - 9566501234, 9952333880</td>';
        $html .= '<td style="width:'.$sh_cnt.'px;border:1px solid black;text-align:center;">Statement '.date("d.m.Y",strtotime($fdate)).' - '.date("d.m.Y",strtotime($tdate)).'</td>';
        $html .= '</tr>';
        $html .= '<tr class="tfoot1" style="line-height:1.7;">';
        ...
        if(empty($acname[$key1]) && $acname[$key1] == ""){ }
            ...
            if($cname != ""){
                if(empty($tbl_col_name[$cname]) || $tbl_col_name[$cname] == ""){ }
                    ...
                    else if($tcname == "cr_amt"){ $html .= '<th title="'.$rpt_col_name[$cname].'" style="text-align:right;color:green;border:1px solid black;">'.number_format_ind($bcr_amt).'</th>'; }
                    else if($tcname == "dr_amt"){ $html .= '<th title="'.$rpt_col_name[$cname].'" style="text-align:right;color:blue;border:1px solid black;">'.number_format_ind($bdr_amt).'</th>'; }
                    else if($tcname == "rb_amt"){ $html .= '<th title="'.$rpt_col_name[$cname].'" style="text-align:right;color:red;border:1px solid black;"></th>'; }
                    else if($tcname == "odue_days"){ $html .= '<th title="'.$rpt_col_name[$cname].'" style="text-align:right;color:red;border:1px solid black;"></th>'; }
                    else if($tcname == "trns_type"){ $html .= '<td title="'.$rpt_col_name[$cname].'" style="text-align:left;border:1px solid black;"></td>'; }
...
if($send_type == "download_pdf"){
    ...
    header('location: ../../generalreports/chicken_customerledger_masterpdf1_ta.php');
}
?>