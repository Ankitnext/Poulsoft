...
foreach($inv_nos as $inv_no){
	...
	if($inv_no != ""){
		...
		$html .= '<tr>';
		$html .= '<th colspan="3" style="text-align:center;"><br/><br/><br/>';
		...
		$html .= '</tr>';
		$html .= '<tr>';
		$html .= '<th colspan="5" style="padding-left: 10px;text-align:left;">';
		...
		$html .= '<b style="padding-left: 10px;text-align:right;">Vehicle:</b>'.$vehiclecode.'<br/>';
		$html .= '</th>';
		$html .= '</tr>';
		$html .= '<tr>';
		$html .= '<th colspan="8" style="padding-left: 10px;text-align:left;">';
		...
		$html .= '</tr>';
		$html .= '<tr>';
		$html .= '<th style="width:40px;border: 1px solid black;">Sl.No.</th>';
		$html .= '<th style="width:90px;border: 1px solid black;">Particulars</th>';
		$html .= '<th style="width:45px;border: 1px solid black;">Birds</th>';
		$html .= '<th style="width:60px;border: 1px solid black;">Quantity</th>';
		$html .= '<th style="width:60px;border: 1px solid black;">Rate</th>';
		$html .= '<th style="width:90px;border: 1px solid black;">Amount</th>';
		$html .= '</tr>';
		$html .= '</table>';
		$slnos = $inames = $ibird = $qtys = $prices = $amts = "<br/><br/>";
		for($i = 1;$i <= $c;$i++){
			if($i <$c){
				$slnos = $slnos."".$slno[$i]."<br/><br/>";
				$inames = $inames."".$iname[$i]."<br/><br/>";
				$ibird = $ibird."".str_replace(".00","",number_format_ind($birds[$i]))."<br/><br/>";
				...
				$ibird = $ibird."".str_replace(".00","",number_format_ind($birds[$i]))." ".$br;
		...
		$html .= '<tr>';
			$html .= '<td style="width:40px;">'.$slnos.'</td>';
			$html .= '<td style="width:90px;">'.$inames.'</td>';
			$html .= '<td style="width:45px;padding:5px;text-align:right;">'.$ibird.'</td>';
			$html .= '<td style="width:60px;padding:5px;text-align:right;">'.$qtys.'</td>';
			$html .= '<td style="width:60px;padding:5px;text-align:right;">'.$prices.'</td>';
			$html .= '<td style="width:90px;padding:5px;text-align:right;">'.$amts.'</td>';
		$html .= '</tr>';
		$html .= '<tr>';
			$html .= '<th style="width:295px;">TCS @ '.$tcdsper.' %</th>';
			$html .= '<td style="width:90px;padding:5px;text-align:right;">'.number_format_ind($tcdsamt).'</td>';
		$html .= '</tr>';
		$html .= '<tr>';
			$html .= '<th style="width:295px;">Round Off</th>';
			$html .= '<td style="width:90px;padding:5px;text-align:right;">'.number_format_ind($roundoff).'</td>';
		$html .= '</tr>';
		$html .= '<tr>';
			$html .= '<th style="width:295px;"><br/>Invoice Total</th>';
			$html .= '<td style="width:90px;padding:5px;text-align:right;"><br/>'.number_format_ind($famt).'</td>';
		$html .= '</tr>';
		$html .= '<tr>';
			//$html .= '<td colspan="4"><br/><br/><b>Amount in words:</b> '.$amtwds.'.<br/></td>';
			$html .= '<td style="width:175px;"><br/><br/><b>Amount in words:</b> '.ucfirst(strtolower(convert_number_to_words($famt))).' Rupees Only.<br/></td>';
			//$html .= '<th colspan="3"><br/><br/>Previous Balance : '.number_format_ind($post).'<br/>Closing Balance : '.number_format_ind($camt).'</th>';
		$html .= '</tr>';
		
		if($bank_flag == 1){
			$html .= '<tr>';
				$html .= '<td style="width:190px;text-align:left;font-size:8px;">';
					$html .= '&nbsp;<b>Bank Name : </b>'.$bank_name.'<br/>';
					$html .= '&nbsp;<b>Branch : </b>'.$bank_branch.'<br/>';
					$html .= '&nbsp;<b>IFSC Code: </b>'.$bank_ifsc.'<br/>';
				$html .= '</td>';
				$html .= '<td style="width:195px;text-align:left;font-size:8px;">';
		...
		$html .= '<tr>';
			$html .= '<td style="width:385px;">Declaration: This is a computer generated invoice,<br/>Seal (or) Signature is not required.</td>';
		$html .= '</tr>';
		$html .= '</table>';
		if($icount != $isize){
