...
if ($_GET['coa'] == "all") {
	...
	$sql = "SELECT * FROM `acc_vouchers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `active` = '1' AND `fcoa` IN ('$coa')" . $wname . " ORDER BY `fcoa` ASC";
	$query = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($query)) {
		if ($row['prefix'] == "PV") {
			$c = $c + 1;
	...
	$c = $d = $e = $f = $g = 0;
	$sql = "SELECT * FROM `acc_vouchers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `active` = '1' AND `tcoa` IN ('$coa')" . $wname . " ORDER BY `tcoa` ASC";
	$query = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($query)) {
		if ($row['prefix'] == "PV") {
			$c = $c + 1;
...
} else if ($_GET['coa'] != "all" && $_GET['whname'] == "all") {
	$fdate = date("Y-m-d", strtotime($_GET['fromdate']));
	$tdate = date("Y-m-d", strtotime($_GET['todate']));
	if ($_GET['coa'] == "all") {
		$coa = $allCoA;
...
$objPHPExcel->getActiveSheet()->getStyle("$from:$to")->getFont()->setBold(true);
	...
	$to = "J" . $rows; // or any value
	$objPHPExcel->getActiveSheet()->getStyle("$from:$to")->getFont()->setBold(true);
} else if ($_GET['coa'] != "all" and $_GET['whname'] != "all") {
	$fdate = date("Y-m-d", strtotime($_GET['fromdate']));
	$tdate = date("Y-m-d", strtotime($_GET['todate']));
	if ($_GET['coa'] == "all") {
		$coa = $allCoA;
	...
	$sql = "SELECT SUM(amount) as tamt,prefix FROM `acc_vouchers` WHERE `date` < '$fdate' AND `active` = '1' AND `fcoa` IN ('$coa') AND `warehouse` = '$wname' GROUP BY `prefix` ORDER BY `fcoa` ASC";
	$query = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($query)) {
		if ($row['prefix'] == "PV") {
			$pre_fpv_amt = $row['tamt'];
	...
	$sql = "SELECT SUM(amount) as tamt,prefix FROM `acc_vouchers` WHERE `date` < '$fdate' AND `active` = '1' AND `tcoa` IN ('$coa') AND `warehouse` = '$wname' GROUP BY `prefix` ORDER BY `tcoa` ASC";
	$query = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($query)) {
		if ($row['prefix'] == "PV") {
			$pre_tpv_amt = $row['tamt'];
	...
	$c = $d = $e = $f = $g = 0;
	$bt_fpv_amt = $bt_frv_amt = $bt_fjv_amt = $bt_foth_vou_amt = $bt_tpv_amt = $bt_trv_amt = $bt_tjv_amt = $bt_toth_vou_amt = array();
	$sql = "SELECT * FROM `acc_vouchers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `active` = '1' AND `fcoa` IN ('$coa') AND `warehouse` = '$wname' ORDER BY `fcoa` ASC";
	$query = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($query)) {
		if ($row['prefix'] == "PV") {
			$c = $c + 1;
	...
	$c = $d = $e = $f = $g = 0;
	$sql = "SELECT * FROM `acc_vouchers` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `active` = '1' AND `tcoa` IN ('$coa') AND `warehouse` = '$wname' ORDER BY `tcoa` ASC";
	$query = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($query)) {
		if ($row['prefix'] == "PV") {
			$c = $c + 1;
