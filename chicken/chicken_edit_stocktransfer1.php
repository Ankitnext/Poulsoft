<?php
//chicken_edit_stocktransfer1.php
...
?>
    <html>
        ...
        <body>
            ...
            if($ids != ""){
              echo  $sql = "SELECT * FROM `item_stocktransfers` WHERE `trnum` = '$ids' AND `flag` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`trnum` ASC";
                $query = mysqli_query($conn,$sql); $tcdsamt = array(); $c = 0;
                while($row = mysqli_fetch_assoc($query)){
                    $date = $row['date'];
                  echo  $invoice = $row['trnum'];
                   echo $dcno = $row['dcno'];
            ...
            <div class="card border-secondary mb-3">
                <div class="card-header">Edit Stock Transfer</div>
                <form action="chicken_modify_stocktransfer1.php" method="post" onsubmit="return checkval();">
                            ...
                            <table>
                                ...
                                <?php  ?>
                                    <tr id="">
                                        ...
                                        <td><select name="fsector" id="fsector" class="form-control select2" style="width:180px;"><option value="select">-select-</option><?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($fromwarehouse == $scode) { echo "selected";} ?>><?php echo $sector_name[$scode]; ?></option><?php } ?></select></td>
                                        <td><select name="icode" id="icode" class="form-control select2" style="width:180px;" onchange=""><option value="select">-select-</option><?php foreach($item_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($code == $scode) { echo "selected";} ?>><?php echo $item_name[$scode]; ?></option><?php } ?></select></td>
                                        <?php
                                        if((int)$jals_flag == 1){ echo '<td><input type="text" name="jalqty" id="jalqty" class="form-control text-right" style="width:90px;" value="'.$jals[$c].'" onkeyup="validate_count(this.id);" /></td>'; }
                                        if((int)$birds_flag == 1){ echo '<td><input type="text" name="birdqty" id="birdqty" class="form-control text-right" style="width:50px;" value="'.$birds[$c].'" onkeyup="validate_count(this.id);" /></td>'; }
                                        ?>
                                        <td><input type="text" name="qty" id="qty" class="form-control text-right" style="width:60px;" value="<?php echo $quantity ?>" onkeyup="validate_num(this.id);" onchange="validate_amount(this.id);" /></td>
                                        <td><input type="text" name="price" id="price" class="form-control text-right" style="width:60px;" value="<?php echo $price ?>" onkeyup="validate_num(this.id);" onchange="validate_amount(this.id);" /></td>
                                        <td><select name="tsector" id="tsector" class="form-control select2" style="width:180px;"><option value="select">-select-</option><?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($towarehouse == $scode) { echo "selected";} ?>><?php echo $sector_name[$scode]; ?></option><?php } ?></select></td>
