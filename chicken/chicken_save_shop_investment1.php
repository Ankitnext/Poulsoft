<?php
//chicken_save_shop_investment1.php
...
//Payment Information
$date = $customer = $a_type = $itemcode = $jals = $birds = $quantity = $price = $amount = $remarks = array();
$i = 0; foreach($_POST['date'] as $dates){ $date[$i] = date("Y-m-d", strtotime($dates)); $i++; }
$i = 0; foreach($_POST['customer'] as $customers){ $customer[$i] = $customers; $i++; }
...
for($i = 0;$i < $dsize;$i++){
    ...
    $trno_dt1 = generate_transaction_details($date[$i],"shop_investment1","VSI","generate",$_SESSION['dbase']);
    ...
    $sql = "INSERT INTO `vendor_shop_investment` (`incr`,`prefix`,`trnum`,`date`,`vcode`,`icode`,`amount`,`remarks`,`flag`,`active`,`dflag`,`trtype`,`trlink`,`addedemp`,`addedtime`,`updatedtime`) 
    VALUES ('$incr','$prefix','$trnum','$date[$i]','$customer[$i]','$itemcode[$i]','$amount[$i]','$remarks[$i]','$flag','$active','$dflag','$trtype','$trlink','$addedemp','$addedtime','$addedtime')";
