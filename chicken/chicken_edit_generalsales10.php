<?php
//chicken_edit_generalsales10.php
...
?>
    <html>
        ...
        <body>
            <?php
            $ids = $_GET['trnum'];
            $sql = "SELECT * FROM `customer_sales` WHERE `invoice` = '$ids' AND `trlink` = 'chicken_display_generalsales10.php' ORDER BY `id` ASC";
            $query = mysqli_query($conn,$sql); $c = 0; $totalamt = array();
            while($row = mysqli_fetch_assoc($query)){
                $date = $row['date'];
                $invoice = $row['invoice'];
                $bookinvoice = $row['bookinvoice'];
                $vendorcode = $row['sup_code'];
            ...
            <div class="card border-secondary mb-3">
                <div class="card-header">Edit Feed Sale</div>
                <form action="chicken_modify_generalsales10.php" method="post" onsubmit="return checkval();">
                    <div class="card-body">
