        ...
        <body>
                            ...
                            <table>
                                ...
                                <tbody id="row_body">
                                    <tr style="margin:5px 0px 5px 0px;">
                                        ...
                                        <td style="width: 80px;"><input type="text" name="amount[]" id="amount[0]" class="form-control amount-format" style="width: 80px;" onkeyup="validatenum(this.id);check_nrow(this.id);" onchange="validateamount(this.id)" readonly></td>
                                        <td id="action[0]"><a href="javascript:void(0);" name="addrow[]" id="addrow[0]" onclick="create_row(this.id)" class="form-control" style="width:15px; height:15px;border:none;"><i class="fa fa-plus" style="color:green;"></i></a></td>
            ...
            <script>
                ...
                function check_nrow(a){
                    ...
                    if(d == incr){
                        var price = document.getElementById("amount["+d+"]").value;
                      console.log(price);
               ...
               function checkval(){
                    ...
                    var company = pdate = itemcode = ""; var quantity = price = amount = 0;
                    var incr = parseInt(document.getElementById("incr").value);
                    for(var d = 0;d <= incr;d++){
                        if(l == true){
                            c = d + 1;
                            pdate = document.getElementById("pdate["+d+"]").value;
                            company = document.getElementById("company["+d+"]").value;
                           ...
                           if(d > 0 && parseFloat(amount) == 0){ destroy_row("amount["+d+"]"); }
                           else {
                                if(pdate == ""){
                                    ...
                                    alert("Please select Company names in row: "+c);
                                    document.getElementById("company["+d+"]").focus();
                                    l = false;
                                    break;
                                }
                ...
                function calculate_amt(a){
                    ...
                    document.getElementById("amount["+d+"]").value = amt.toFixed(0);
                }
                function create_row(a){
                    ...
                    html += '<tr id="row_no['+d+']">';
                    html+= '<td style="width: 100px;"><input type="text" name="pdate[]" id="pdate['+d+']" style="width:100px;" class="form-control range_picker"  value="<?php echo $date; ?>" readonly></td>';
                    html+= '<td style="width: 250px;padding-right:5px;"><select name="company[]" id="company['+d+']" class="form-control select2" style="width: 250px;"><option value="select">-select-</option><?php foreach($sup_code as $cc){ ?><option value="<?php echo $sup_code[$cc]; ?>"><?php echo $sup_name[$cc]; ?></option><?php } ?></select></td>';
                    ...
                    html+= '<td style="width: 80px;"><input type="text" name="quantity[]" id="quantity['+d+']" style="width: 80px;" onkeyup="validatenum(this.id);calculate_amt(this.id);" onchange="validateamount(this.id)" class="form-control amount-format"></td>';
                    html+= '<td style="width: 80px;"><input type="text" name="price[]" id="price['+d+']" style="width: 80px;" onkeyup="validatenum(this.id);calculate_amt(this.id);" onchange="validateamount(this.id)" class="form-control amount-format"></td>';
                    html+= '<td style="width: 80px;"><input type="text" name="amount[]" id="amount['+d+']" style="width: 80px;" onkeyup="validatenum(this.id);check_nrow(this.id);" onchange="validateamount(this.id)" class="form-control amount-format" readonly></td>';
            ...
            <script>
                   document.addEventListener("DOMContentLoaded", function () {
                    ...
                    const interval = setInterval(() => {
                        const company = document.getElementById('company[0]');
                        if(company && $(company).hasClass('select2-hidden-accessible')){
