<?php
//chicken_add_supplierpayment2.php
...
if($access_error_flag == 0){ 
    ...
    global $trns_dtype; $trns_dtype = "Payment"; include "chicken_fetch_daterangemaster.php"; if($rng_mdate == ""){ $rng_mdate = $today; }
    ...
    <html>
        ...
        <body>
            <div class="card border-secondary mb-3">
                ...
                <form action="chicken_save_supplierpayment2.php" method="post" onsubmit="return checkval();">
                    <div class="card-body">
                        <div class="row">
                            <table align="center">
                                <thead>
                                ...
                                <tbody id="row_body">
                                    <tr style="margin:5px 0px 5px 0px;">
                                        <td><input type="text" name="date[]" id="date[0]" class="form-control pay_datepickers" value="<?php echo date("d.m.Y",strtotime($date)); ?>" style="width:100px;" onchange="fetch_tcds_per(this.id);" readonly /></td>
                                        <td><select name="ccode[]" id="ccode[0]" class="form-control select2" style="width:180px;"><option value="select">-select-</option><?php foreach($sup_code as $scode){ ?><option value="<?php echo $scode; ?>"><?php echo $sup_name[$scode]; ?></option><?php } ?></select></td>
                                        ...
                                        <td><input type="text" name="amount1[]" id="amount1[0]" class="form-control text-right" style="width:90px;" onkeyup="validate_num(this.id);calculate_row_amt(this.id);" onchange="validate_amount(this.id);" /></td>
                                        <td><input type="checkbox" name="tcds_chk[]" id="tcds_chk[0]" onchange="calculate_row_amt(this.id);" /></td>

                                        <td><input type="text" name="tcds_chk_val" id="tcds_chk_val[0]" class="form-control text-right" style="width:90px;" readonly/></td>
            ...
            <script>
                ...
                function create_row(a){
                    ...
                    var hdcsec     = '<?php echo (int)$hdcsec_flag; ?>';
                    html += '<tr id="row_no['+d+']">';
                    html += '<td><input type="text" name="date[]" id="date['+d+']" class="form-control pay_datepickers" value="'+date+'" style="width:100px;" onchange="fetch_tcds_per(this.id);" readonly /></td>';
                    html += '<td><select name="ccode[]" id="ccode['+d+']" class="form-control select2" style="width:180px;"><option value="select">-select-</option><?php foreach($sup_code as $scode){ ?><option value="<?php echo $scode; ?>"><?php echo $sup_name[$scode]; ?></option><?php } ?></select></td>';
                    ...
                    html += '<td><input type="text" name="amount1[]" id="amount1['+d+']" class="form-control text-right" style="width:90px;" onkeyup="validate_num(this.id);calculate_row_amt(this.id);" onchange="validate_amount(this.id);" /></td>';

                     html += '<td><input type="checkbox" name="tcds_chk[]" id="tcds_chk['+d+']" onchange="calculate_row_amt(this.id);" /></td>'; 
                  
               
                    html += '<td><input type="text" name="tcds_chk_val[]" class="form-control text-right" style="width:90px;" id="tcds_chk_val['+d+']" readonly /></td>';
                    ...
                    $('.pay_datepickers').datepicker({ dateFormat:'dd.mm.yy',changeMonth:true,changeYear:true,minDate: rng_mdate,maxDate: today,autoclose: true });
                ...
                function calculate_row_amt(a){
                    ...
                    document.getElementById("tcds_chk_val["+d+"]").value =  tcds_amt;
