<?php
	//chicken_display_area1.php
	session_start(); include "newConfig.php";
	include "xendorheadlink.php";
	$time = microtime(); $time = explode(' ', $time); $time = $time[1] + $time[0]; $start = $time;
	$user_name = $_SESSION['users']; $user_code = $_SESSION['userid'];
	if($_GET['ccid'] == "" && $_SESSION['area1'] != ""){ $cid = $_SESSION['area1']; } else if($_GET['ccid'] != "" && $_SESSION['area1'] == ""){ $cid = $_GET['ccid']; $_SESSION['area1'] = $cid; } else { $cid = $_GET['ccid']; $_SESSION['area1'] = $cid; }
	...
	while($row = mysqli_fetch_assoc($query)){
        if($row['name'] == "display" || $row['name'] == "Display"){
            $gp_id = $row['childid'];
        }
					...
					<div class="box">
						<div class="box-body">
							<table id="example1" class="table table-bordered table-striped">
                                ...
                                $sql = "SELECT * FROM `main_areas` WHERE `dflag` = '0' ORDER BY `description` ASC";
                                $query = mysqli_query($conn,$sql);
                                while($row = mysqli_fetch_assoc($query)){
                                    $id = $row['code'];
                                    $id1 = $row['code']."@".$row['description'];
