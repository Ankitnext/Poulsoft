<?php
//chicken_add_generalsales10.php
...
if($access_error_flag == 0){
    ...
    $incr = 0; $prefix = $trnum = "";
    $trno_dt1 = generate_transaction_details($date,"generalsales10","FDS","display",$_SESSION['dbase']);
    ...
    $sql = "SELECT * FROM `extra_access` WHERE `field_name` = 'chicken_display_generalsales10.php' AND `field_function` = 'Amount' AND `flag` = '1' ORDER BY `id` ASC";
    ...
    <html>
        ...
        <body>
            <div class="card border-secondary mb-3">
                <div class="card-header">Create Feed Sale</div>
                <form action="chicken_save_generalsales10.php" method="post" onsubmit="return checkval();">
                    <div class="card-body">
                            ...
                            <table align="center">
                                ...
                                <tbody id="row_body">
                                    <tr style="margin:5px 0px 5px 0px;">
                                        ...
                                        <td><input type="text" name="nof_bags[]" id="nof_bags[0]" class="form-control text-right" style="width:90px;" onkeyup="validate_num(this.id);calkgs(this.id);calafterdiscamt(this.id);" onchange="validate_amount(this.id);" /></td>
            ...
            <script>
                function return_back(){
                    window.location.href = "chicken_display_generalsales10.php";
                }

                function calkgs(a){
                    var b = a.split("["); var c = b[1].split("]"); var d = c[0];
                    var bpkg = document.getElementById("bperkg["+d+"]").value; if(bpkg == ""){ bpkg = 0; }
             ...
             function calafterdiscamt(a){
                ...
                // var d4 = d3 - (d3 * 0.02);
                // console.log(d4);
                var noofbgs = document .getElementById("nof_bags["+d+"]").value;  if(noofbgs == ""){ noofbgs = 0; }
                var tamt = (parseFloat(rate) * parseFloat(noofbgs)).toFixed(2);
               ...
               calculate_final_totalamt();
                ...
                function update_row_fields(a) {
                    ...
                    var bag_size = bag_asize[icode];

                    // ✅ Use 0 if bag_size is undefined, null, empty, or not a number
                    if (!bag_size || isNaN(bag_size)) { bag_size = 0; }
                    document.getElementById("bperkg[" + d + "]").value = bag_size;

                    // Optional: auto-calculate kgs = nof_bags * bperkg
                    var bags = parseFloat(document.getElementById("nof_bags[" + d + "]").value) || 0;
                    var kgs = bags * bag_size;

                    // ✅ Always set kgs field with a valid number
                    document.getElementById("kgs[" + d + "]").value = kgs.toFixed(2);
                }
