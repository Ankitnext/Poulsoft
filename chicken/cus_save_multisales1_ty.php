...
//COA check
$sql = "SELECT * FROM `acc_coa` WHERE `active` = '1' AND `description` = 'Transportation Charges'"; $query = mysqli_query($conn,$sql);
...
if($_POST['submittrans'] == "addpage"){
	...
	for($j = 1;$j <= $i;$j++){
		if($warehouse == "select" || $itemcode[$j] == "select") {
            ...
            if($tcost[$j] > 0){
                $sql2 = "INSERT INTO `main_crdrnote`(`mode`,`trnum`,`date`,`ccode`,`coa`,`crdr`,`amount`,`flag`,`active`,`addedemp`,`addedtime`) VALUES('CCN','$invoice','$date','$vendorcode[$j]','$coacode','Dr','$tcost[$j]','0','1','$addedemp','$addedtime')";
