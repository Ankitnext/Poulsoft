...
?>
    <html>
        ...
        <body>
            ...
            $ids = $_GET['trnum'];
            if($ids != ""){
                $sql = "SELECT * FROM `item_stock_opening` WHERE `trnum` = '$ids' AND `dflag` = '0' ORDER BY `date`,`trnum` ASC";
            ...
            <div class="card border-secondary mb-3">
                <div class="card-header">Edit Stock opening</div>
                <form action="chicken_modify_stockopen1.php" method="post" onsubmit="return checkval();">
                    <div class="ml-5 card-body">
                        <div class="row">
                            <div class="form-group col-md-1">
                                <label>Date<b style="color:red;">&nbsp;*</b></label>
                                <input type="text" style="width:100px;" class="form-control range_picker" name="pdate" value="<?php echo $fdate; ?>" id="pdate" readonly>
                            </div>
                        </div>
                        <div class="row">
                            <table>
                                ...
                                <?php  ?>
                                    <tr id="">
                                        <td style="width: 250px;"><select name="itemcode" id="itemcode" class="form-control select2" style="width: 250px;" onchange=""><option value="select">-select-</option><?php foreach($item_code as $cc){ ?><option value="<?php echo $item_code[$cc]; ?>" <?php if($itemcode == $cc){ echo "selected"; } ?>><?php echo $item_name[$cc]; ?></option><?php } ?></select></td>
                                        <td style="width: 80px;"><input type="text" name="quantity" id="quantity" value="<?php echo $quantity; ?>" style="text-align:right;width: 80px;" class="form-control amount-format" onkeyup="validatenum(this.id);calculate_amount(this.id);" onchange="validateamount(this.id)" ></td>
                                        <td style="width: 80px;"><input type="text" name="price" id="price" value="<?php echo $price; ?>" style="text-align:right;width: 80px;" class="form-control amount-format" onkeyup="validatenum(this.id);calculate_amount(this.id);" onchange="validateamount(this.id)" ></td>
                                        <td style="width: 80px;"><input type="text" name="amount" id="amount" value="<?php echo $amount; ?>" style="text-align:right;width: 80px;" class="form-control amount-format" onkeyup="validatenum(this.id);" onchange="validateamount(this.id)"  readonly></td>
                                        <td style="width: 200px;"><select name="warehouse" id="warehouse" class="form-control select2" style="width: 200px;" onchange=""><option value="select">-select-</option><?php foreach($sector_code as $cc){ ?><option value="<?php echo $sector_code[$cc]; ?>" <?php if($warehouse == $cc){ echo "selected"; } ?>><?php echo $sector_name[$cc]; ?></option><?php } ?></select></td>
            ...
            <script>
                ...
                function checkval(){
                    ...
                    if(itemcode == "select"){
						...
						alert("Please select warehouse names in row: ");
                ...
                function calculate_amount(a) {
                    ...
                    var prc = parseFloat(document.getElementById("price").value); 

                    // If the parsed values are NaN, default to 0
                    qty = isNaN(qty) ? 0 : qty;
                    prc = isNaN(prc) ? 0 : prc;
                    var amt = qty * prc;

                    document.getElementById("amount").value = amt.toFixed(2);// Ensuring consistency in decimal formatting
                }
