<?php
    ...
    if(!empty($_SESSION['pursale5'])){ $gp_id = $cid = $_SESSION['pursale5']; }
    else{
        $sql = "SELECT *  FROM `main_linkdetails` WHERE `href` LIKE '%$href%' AND `activate` = '1' ORDER BY `sortorder` ASC"; $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($query)){ $gp_id = $cid = $_SESSION['pursale5'] = $row['childid']; }
    }
