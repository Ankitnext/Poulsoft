<?php
//chicken_edit_multiplesale1_ta.php
...
?>
    <html>
        ...
        <body>
            ...
            $query = mysqli_query($conn,$sql); $rct_amt1 = 0;
            while($row = mysqli_fetch_assoc($query)){ $rct_amt1 = round($row['amount'],5); }
            ...
            $query = mysqli_query($conn,$sql); $rct_amt2 = 0; $bank_method = "";
            while($row = mysqli_fetch_assoc($query)){ $rct_amt2 = round($row['amount'],5); $bank_method = $row['method']; }

            ?>
            <div class="card border-secondary mb-3">
                <div class="card-header">Edit Multiple-Sales</div>
                <form action="chicken_modify_multiplesale1_ta.php" method="post" onsubmit="return checkval();">
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group" style="width:110px;">
                                <label for="date">Date<b style="color:red;">&nbsp;*</b></label>
                                <input type="text" name="date" id="date" class="form-control sale_datepickers" value="<?php echo date("d.m.Y",strtotime($date[$c])); ?>" style="width:100px;" onchange="fetch_tcds_per(this.id);" readonly />
                            </div>
                            ...
                            <div class="form-group" style="width:110px;">
                                ...
                                <input type="text" name="vehicle" id="vehicle" class="form-control" value="<?php echo $vehiclecode[$c]; ?>" style="width:100px;" />
                            </div>
                            <div class="form-group" style="width:110px;">
                                <label for="driver">Driver</label>
                                <input type="text" name="driver" id="driver" class="form-control" value="<?php echo $drivercode[$c]; ?>" style="width:100px;" />
                            </div>
                            ...
                            <table>
                                <thead>
                                    <tr>
                                        ...
                                        <th>Bank Method</th>
                                        <th>Bank Receipt</th>
                                        <th style="width:20px;visibility:hidden;">CS</th>
                                        <th>Remarks</th>
                                ...
                                <?php $incr = $c; for($c = 0;$c <= $incr;$c++){ ?>
                                    <tr id="row_no[<?php echo $c; ?>]">
                                        <td><select name="vcode[]" id="vcode[<?php echo $c; ?>]" class="form-control select2" style="width:180px;" onchange="fetch_customer_balance_master(this.id);fetch_paperrate_master(this.id);"><option value="select">-select-</option><?php foreach($cus_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($customercode[$c] == $scode){ echo "selected"; } ?>><?php echo $cus_name[$scode]; ?></option><?php } ?></select></td>
                                        <td><input type="text" name="bookinvoice[]" id="bookinvoice[<?php echo $c; ?>]" class="form-control" value="<?php echo $bookinvoice[$c]; ?>" style="width:100px;" /></td>
                                        ...
                                        <td style="width:50px;text-align:center;"><input type="checkbox" name="tcds_chk[]" id="tcds_chk[<?php echo $c; ?>]" onchange="calculate_total_amt();" <?php if((float)$tcdsamt[$c] > 0){ echo "checked"; } ?> /></td>
                                        ...
                                        <td><input type="text" name="rct_amt1[]" id="rct_amt1[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $rct_amt1; ?>" style="width:90px;" onkeyup="validate_num(this.id);calculate_total_amt();" onchange="validate_amount(this.id);" /></td>
                                        <td><select name="bank_method1[]" id="bank_method1[<?php echo $c; ?>]" class="form-control select2" style="width:180px;"><option value="select">-select-</option><?php foreach($bank_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($bank_method == $scode){ echo "selected"; } ?>><?php echo $bank_name[$scode]; ?></option><?php } ?></select></td>
                                        <td><input type="text" name="rct_amt2[]" id="rct_amt2[<?php echo $c; ?>]" class="form-control text-right" value="<?php echo $rct_amt2; ?>" style="width:90px;" onkeyup="validate_num(this.id);calculate_total_amt();" onchange="validate_amount(this.id);" /></td>
            ...
            <script>
                ...
                function create_row(a){
                    ...
                    html += '<td style="width:20px;visibility:hidden;"><input type="text" name="trnums[]" id="trnums['+d+']" class="form-control" style="width:20px;" readonly /></td>';
                    html += '</tr>';
