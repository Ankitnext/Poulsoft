...
if($access_error_flag == 0){
    ...
    $incr = 0; $prefix = $trnum = "";
    $trno_dt1 = generate_transaction_details($date,"generalsales8","CINV","display",$_SESSION['dbase']);
    ...
    $sql = "SELECT * FROM `extra_access` WHERE `field_name` LIKE 'Warehouse' AND `field_function` LIKE 'Show default selected' AND `user_access` = '$addedemp'"; $query = mysqli_query($conn,$sql);
    $defaultwarehouse = mysqli_num_rows($query); $warehousevalue = "";
    if($defaultwarehouse > 0){
        while($row = mysqli_fetch_assoc($query)){
            $warehousevalue = $row['field_value'];
        }
        ...
        <body>
            <div class="card border-secondary mb-3">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" class="btn btn-success btn-sm" onclick="open_new();" style="visibility:hidden;"><i class="fa-solid fa-plus"></i></button>
                </div>
                <form action="chicken_save_generalsales8.php" method="post" onsubmit="return checkval();">
                    <div class="card-body">
                        <div class="row justify-content-center align-items-center">
                            ...
                            <div class="form-group" style="width:290px;">
                                ...
                                <select name="warehouse" id="warehouse" class="form-control select2" style="width:280px;" onchange="fetch_customer_outstanding();">
                                    <option value="select">-select-</option>
                                    <?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($defaultwarehouse > 0 && $sector_name[$scode] == 'Ramtek'){ echo ' selected'; } ?>><?php echo $sector_name[$scode]; ?></option><?php } ?>
                                </select>
                            </div>
                            <?php if($gr_flag > 0){  $groups = $_POST['groups'] ?? $_GET['groups'] ?? 'SCG-0017';?>
                            <div class="form-group" style="width:290px;">
                                ...
                                <select name="groups" id="groups" class="form-control select2" style="width:280px;" onchange="fetch_customer_groups(this.id);">
                                    <option value="select">-select-</option>
                                    <?php foreach($grp_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($groups == $scode){ echo "selected"; } ?>><?php echo $grp_name[$scode]; ?></option><?php } ?>
                                </select>
                            </div> <?php } ?>
                            ...
                            <table align="center">
                                ...
                                <tbody id="row_body">
                                    ...
                                    <tr style="margin:5px 0px 5px 0px;">
                                        ...
                                        <?php if($gr_flag > 0){?>
                                        <td><textarea name="remarks2[]" id="remarks2[0]" class="form-control" data-row="0" data-col="<?php echo $colIndex++; ?>" style="height:25px;width:100px;"></textarea></td>
                                        <?php } ?>
                                        <td id="action[0]"><a href="javascript:void(0);" id="addrow[0]" onclick="create_row(this.id)" class="form-control" style="width:15px; height:15px;border:none;"><i class="fa fa-plus" style="color:green;"></i></a></td>
                                    </tr>
                                </tbody>
            ...
            <script>
                ...
                function open_new(){
                    window.open('chicken_add_generalsales8.php', '_blank');
                }
                function checkval(){
