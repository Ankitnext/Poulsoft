<?php
    ...
    if(!empty($_SESSION['saleorder'])){ $gp_id = $cid = $_SESSION['saleorder']; }
    else{
        $sql = "SELECT *  FROM `main_linkdetails` WHERE `href` LIKE '%$href%' AND `activate` = '1' ORDER BY `sortorder` ASC"; $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($query)){ $gp_id = $cid = $_SESSION['saleorder'] = $row['childid']; }
    }
...
<html>
	<body class="hold-transition skin-blue sidebar-mini">
		...
		<div align="right">
			<?php if($edt_flag == 1){ ?><button type="button" class="btn btn-info" id="editpage" value="chicken_edit_pursale2m.php" onclick="add_page(this.id)" ><i class="fa fa-align-left"></i> Edit-Multiple</button><?php } ?>
		...
		<section class="content">
			<div class="row">
				<div class="col-lg-19">
					<div class="box">
					<?php
                        $sql='SHOW COLUMNS FROM `salesorder`'; $query=mysqli_query($conn,$sql); $existing_col_names = array(); $i = 0;
                        while($row = mysqli_fetch_assoc($query)){ $existing_col_names[$i] = $row['Field']; $i++; }
                        if(in_array("active", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `salesorder` ADD `active` INT(11) NOT NULL DEFAULT 1 AFTER `isDelete`"; mysqli_query($conn,$sql); }
                        if(in_array("dflag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `salesorder` ADD `dflag` INT(11) NOT NULL DEFAULT 1 AFTER `active`"; mysqli_query($conn,$sql); }
                        if(in_array("flag", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `salesorder` ADD `flag` INT(11) NOT NULL DEFAULT 1 AFTER `dflag`"; mysqli_query($conn,$sql); }
						...
						$sql = "SELECT * FROM `salesorder` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `isDelete` = '0' ORDER BY `id` DESC";
						$query = mysqli_query($conn,$sql);
					?>
						<div class="box-body">
