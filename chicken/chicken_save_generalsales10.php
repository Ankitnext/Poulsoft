<?php
//chicken_save_generalsales10.php
...
$trno_dt1 = generate_transaction_details($date,"generalsales10","FDS","generate",$_SESSION['dbase']);
...
for($i = 0;$i < $dsize;$i++){
 //   if($jals[$i] == ""){ $jals[$i] = 0; }
  ...
  //  echo $sql;
    ...
    //   $sql = "INSERT INTO `customer_sales` (`incr`,`d`,`m`,`y`,`fy`,`date`,`invoice`,`bookinvoice`,`customercode`,`itemcode`,`nof_bags`,`bag_kg`,`netweight`,`itemprice`,`totalamt`,`transporter_code`,`freight_amount`,`tcdsper`,`tcds_type1`,`tcds_type2`,`tcdsamt`,`dressing_charge`,`roundoff_type1`,`roundoff_type2`,`roundoff`,`finaltotal`,`balance`,`drivercode`,`vehiclecode`,`warehouse`,`remarks`,`flag`,`active`,`tdflag`,`pdflag`,`trtype`,`trlink`,`addedemp`,`addedtime`,`updated`) 
    // VALUES ('$incr','$d','$m','$y','$pfx','$date','$invoice','$bookinvoice','$vcode','$itemcode[$i]','$nof_bags[$i]','$bag_kg[$i]','$kgs[$i]','$rate[$i]','$item_amt[$i]','$transporter_code','$freight_amt','$tcds_per','$tcds_type1','$tcds_type2','$tcds_amt','$dressing_charge','$roundoff_type1','$roundoff_type2','$roundoff_amt','$finaltotal','$finaltotal','$driver','$vehicle','$sectors[$i]','$remarks','$flag','$active','$tdflag','$pdflag','$trtype','$trlink','$addedemp','$addedtime','$addedtime')";
   // echo $sql;
