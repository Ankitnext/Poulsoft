...
if($access_database != ""){
    ...
    if($acs_conn){
        ...
        if(strtolower($acslink_type) == "display"){
            $acs_sql = "SELECT * FROM `main_linkdetails` WHERE `href` LIKE '%$ufile_name%' AND `activate` = '1'";
            ...
            if($acs_cnt > 0){
                ...
                if($acs_cid != ""){
                    ...
                    while($acs_row = mysqli_fetch_array($acs_query)){
                        $dlink = $acs_row['displayaccess']; $alink = $acs_row['addaccess']; $elink = $acs_row['editaccess']; $ulink = $acs_row['otheraccess'];
                    ...
                    $acs_sql = "SELECT * FROM `main_linkdetails` WHERE `parentid` LIKE '$acs_cid' AND `activate` = '1'";
                    $acs_query = mysqli_query($acs_conn,$acs_sql); $acs_cnt = mysqli_num_rows($acs_query);
                    while($acs_row = mysqli_fetch_array($acs_query)){
                        ...
                        else if(strtolower($acs_row['name']) == "delete" || strtolower($acs_row['name']) == "update"){ if(str_contains($ulink, $acs_row['childid'])){ $acs_delete_flag = 1; $acs_delete_url = $acs_row['href']; } }
