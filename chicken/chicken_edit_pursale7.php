...
if($access_error_flag == 0){
    ...
    $query = mysqli_query($conn,$sql); $sp_code = $sp_name = array();
...
?>
    <html>
        ...
        <body>
            <?php
            $ids = $_GET['trnum']; $customercode = $ltrnum = ""; $sid = $cus_prc = $cus_amt = $tcs_per = $tcs_amt = $roff_camt = $cus_famt = 0;
            if($ids != ""){
                $sql = "SELECT * FROM `customer_sales` WHERE `invoice` = '$ids' AND `flag` = '1' AND `tdflag` = '0' AND `pdflag` = '0' ORDER BY `date`,`id` ASC";
                $query = mysqli_query($conn,$sql);
                while($row = mysqli_fetch_assoc($query)){
                    $sid = $row['id'];
                    $customercode = $row['customercode'];
                    $ltrnum = $row['link_trnum'];
                    $cus_prc = round($row['itemprice'],2);
                    $cus_amt = round($row['totalamt'],2);
                    $tcs_per = round($row['tcdsper'],2);
                    $tcs_amt = round($row['tcdsamt'],2);
                    $roff_camt = round($row['roundoff'],2);
                    $cus_famt = round($row['finaltotal'],2);
                }
            }
            $date = $vendorcode = $supbrh_code = $bookinvoice = $itemcode = $warehouse = $vehiclecode = $drivercode = $remarks = ""; $jals = $birds = $tweight = $eweight = $nweight = $sup_prc = $sup_amt = $tds_per = $tds_amt = $roff_samt = $sup_famt = 0;
            if($ltrnum != ""){
                ...
                while($row = mysqli_fetch_assoc($query)){
                    ...
                    $tweight = round($row['totalweight'],2);
                    $eweight = round($row['emptyweight'],2);
                    $nweight = round($row['netweight'],2);
                    $sup_prc = round($row['itemprice'],2);
                    $sup_amt = round($row['totalamt'],2);
                    ...
                    $remarks = $row['drivercode'];
                }
            }
            ?>
            <div class="card border-secondary mb-3">
                <div class="card-header">Edit Purchase-sale</div>
                <form action="chicken_modify_pursale7.php" method="post" onsubmit="return checkval();">
                    <div class="ml-5 card-body">
                        <div class="row">
                            ...
                            <div class="form-group" style="width:210px;">
                                ...
                                <select name="warehouse" id="warehouse" class="form-control select2" style="width:180px;"><option value="select">-select-</option><?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($scode == $warehouse){ echo "selected"; } ?>><?php echo $sector_name[$scode]; ?></option><?php } ?></select>
                            </div>
                            <div class="form-group" style="width:210px;">
                                <label for="itemcode">Item</label>
                                <select name="itemcode" id="itemcode" class="form-control select2" style="width:200px;" onchange="update_all_ifields();"><option value="select">-select-</option><?php foreach($item_code as $icode){ ?><option value="<?php echo $icode; ?>" <?php if($icode == $itemcode){ echo "selected"; } ?>><?php echo $item_name[$icode]; ?></option><?php } ?></select>
                            </div>
                        </div>
                            ...
                            <table>
                                ...
                                <tbody id="row_body">
                                    <tr style="margin:5px 0px 5px 0px;">
                                        <td><select name="vendorcode[]" id="vendorcode[0]" class="form-control select2" style="width:180px;" onchange="fetch_sup_branches2(this.id);"><option value="select">-select-</option><?php foreach($sup_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($scode == $vendorcode){ echo "selected"; } ?>><?php echo $sup_name[$scode]; ?></option><?php } ?></select></td>
                                        <td><select name="supbrh_code[]" id="supbrh_code[0]" class="form-control select2" style="width:90px;"><option value="select">-select-</option><?php foreach($sp_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($scode == $supbrh_code){ echo "selected"; } ?>><?php echo $sp_name[$scode]; ?></option><?php } ?></select></td>
                                        <td><input type="text" name="bookinvoice[]" id="bookinvoice[0]" class="form-control" style="width:60px;" value="<?php echo $bookinvoice; ?>" /></td>
                                        <?php
                                        if((int)$jals_flag == 1){ echo '<td><input type="text" name="jals[]" id="jals[0]" class="form-control text-right" value="'.$jals.'" style="width:50px;" onkeyup="validate_count(this.id);calculate_tot_amt();" /></td>'; }
                                        if((int)$birds_flag == 1){ echo '<td><input type="text" name="birds[]" id="birds[0]" class="form-control text-right" value="'.$birds.'" style="width:50px;" onkeyup="validate_count(this.id);calculate_tot_amt();" /></td>'; }
                                        if((int)$tweight_flag == 1){ echo '<td><input type="text" name="tweight[]" id="tweight[0]" class="form-control text-right" value="'.$tweight.'" style="width:90px;" onkeyup="validate_num(this.id);calculate_tot_amt();" onchange="validate_amount(this.id);" /></td>'; }
                                        if((int)$eweight_flag == 1){ echo '<td><input type="text" name="eweight[]" id="eweight[0]" class="form-control text-right" value="'.$eweight.'" style="width:90px;" onkeyup="validate_num(this.id);calculate_tot_amt();" onchange="validate_amount(this.id);" /></td>'; }
                                        ?>
                                        <td><input type="text" name="nweight[]" id="nweight[0]" class="form-control text-right" value="<?php echo $nweight; ?>" style="width:90px;" onkeyup="validate_num(this.id);calculate_tot_amt();" onchange="validate_amount(this.id);" /></td>
                                        <td><input type="text" name="sup_prc[]" id="sup_prc[0]" class="form-control text-right" value="<?php echo $sup_prc; ?>" style="width:60px;" onkeyup="validate_num(this.id);calculate_tot_amt();" onchange="validate_amount(this.id);" /></td>
                                        <td style="text-align:center;"><input type="checkbox" name="tds_chk[]" id="tds_chk[0]" onchange="fetch_tcds_per(this.id);" <?php if((float)$tds_amt > 0){ echo "checked"; } ?> /></td>
                                        <td><input type="text" name="sup_famt[]" id="sup_famt[0]" class="form-control text-right" value="<?php echo $sup_famt; ?>" style="width:90px;" readonly /></td>
                                        <td><select name="customercode[]" id="customercode[0]" class="form-control select2" style="width:180px;" onchange="fetch_latest_customer_paperrate(this.id);"><option value="select">-select-</option><?php foreach($cus_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($scode == $customercode){ echo "selected"; } ?>><?php echo $cus_name[$scode]; ?></option><?php } ?></select></td>
                                        <td><input type="text" name="cus_prc[]" id="cus_prc[0]" class="form-control text-right" value="<?php echo $cus_prc; ?>" style="width:60px;" onkeyup="validate_num(this.id);calculate_tot_amt();" onchange="validate_amount(this.id);" /></td>
                                        <td style="text-align:center;"><input type="checkbox" name="tcs_chk[]" id="tcs_chk[0]" onchange="fetch_tcds_per(this.id);" <?php if((float)$tcs_amt > 0){ echo "checked"; } ?> /></td>
                                        <td><input type="text" name="cus_famt[]" id="cus_famt[0]" class="form-control text-right" value="<?php echo $cus_famt; ?>" style="width:90px;" readonly /></td>
                                        <!-- <td><input type="text" name="vehiclecode[]" id="vehiclecode[0]" class="form-control" value="<?php //echo $vehiclecode; ?>" style="width:130px;" /></td>
                                        <td><input type="text" name="drivercode[]" id="drivercode[0]" class="form-control" value="<?php //echo //$drivercode; ?>" style="width:130px;" /></td> -->
                                        <td><textarea name="remarks[]" id="remarks[0]" class="form-control" style="width:130px;height:23px;"><?php echo $remarks; ?></textarea></td>
                                        
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="roff_samt[]" id="roff_samt[0]" class="form-control text-right" value="<?php echo $roff_samt; ?>" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="roff_camt[]" id="roff_camt[0]" class="form-control text-right" value="<?php echo $roff_camt; ?>" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="sup_amt[]" id="sup_amt[0]" class="form-control text-right" value="<?php echo $sup_amt; ?>" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="cus_amt[]" id="cus_amt[0]" class="form-control text-right" value="<?php echo $cus_amt; ?>" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="tds_per[]" id="tds_per[0]" class="form-control text-right" value="<?php echo $tds_per; ?>" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="tds_amt[]" id="tds_amt[0]" class="form-control text-right" value="<?php echo $tds_amt; ?>" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="tcs_per[]" id="tcs_per[0]" class="form-control text-right" value="<?php echo $tcs_per; ?>" style="padding:0;width:20px;" readonly /></td>
                                        <td style="width:20px;visibility:hidden;"><input type="text" name="tcs_amt[]" id="tcs_amt[0]" class="form-control text-right" value="<?php echo $tcs_amt; ?>" style="padding:0;width:20px;" readonly /></td>
                                    </tr>
                                </tbody>
                                <tfoot>
                                    <tr>
                        ...
                        <div class="row" style="visibility:hidden;">
                            <div class="form-group" style="width:30px;">
                                <label>SID</label>
                                <input type="text" name="sid" id="sid" class="form-control" value="<?php echo $sid; ?>" style="width:20px;" readonly />
                            </div>
                            <div class="form-group" style="width:30px;">
                                <label>PID</label>
                                <input type="text" name="pid" id="pid" class="form-control" value="<?php echo $pid; ?>" style="width:20px;" readonly />
                            </div>
                            <div class="form-group" style="width:30px;">
                                <label>IN</label>
