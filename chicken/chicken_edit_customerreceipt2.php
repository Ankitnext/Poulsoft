<?php
//chicken_edit_customerreceipt2.php
...
?>
    <html>
        ...
        <body>
            ...
            $query = mysqli_query($conn,$sql); $discount = 0;
            while($row = mysqli_fetch_assoc($query)){ $discount = $row['amount']; }
            ?>
            <div class="card border-secondary mb-3">
                <div class="card-header">Edit Receipt</div>
                <form action="chicken_modify_customerreceipt2.php" method="post" onsubmit="return checkval();">
                    <div class="card-body">
                        <div class="row">
                            <table align="center">
                                <thead>
                                ...
                                <tbody id="row_body">
                                    <tr style="margin:5px 0px 5px 0px;">
                                        <td><input type="text" name="date" id="date" class="form-control pay_datepickers" value="<?php echo date("d.m.Y",strtotime($date)); ?>" style="width:100px;" onchange="fetch_tcds_per();" readonly /></td>
                                        ...
                                        <td><input type="text" name="amount1" id="amount1" class="form-control text-right" value="<?php echo $amount; ?>" style="width:90px;" onkeyup="validate_num(this.id);calculate_row_amt();" onchange="validate_amount(this.id);" /></td>
                                        ...
                                        <td><select name="sector" id="sector" class="form-control select2" style="width:180px;"><option value="select">-select-</option><?php foreach($sector_code as $scode){ ?><option value="<?php echo $scode; ?>" <?php if($warehouse == $scode){ echo "selected"; } ?>><?php echo $sector_name[$scode]; ?></option><?php } ?></select></td>
                                        <td style="width:90px;"><input type="text" name="discount" id="discount" class="form-control text-right" style="width:90px;" value="<?php echo $discount; ?>" /></td>
