<?php
//chicken_save_customerreceipt2.php
...
$file_name = "chicken_display_customerreceipt2.php"; $sms_type = "WappKey"; $wapp_ptrn = "Normal";
...
//
$sql = "SELECT * FROM `acc_coa` WHERE `description` = 'Cash In Hand' AND `active` = '1'"; $query = mysqli_query($conn,$sql);
while($row = mysqli_fetch_assoc($query)){ $coacode = $row['code']; }

//Payment Information
$date = $ccode = $mode = $code = $amount1 = $dcno = $sector = $remarks = $tcds_per = $tcds_amt = $amount = $discount = array();
...
$i = 0; foreach($_POST['discount'] as $discounts){ $discount[$i] = $discounts; $i++; }
$vtype = "C";
$flag = $active = 1; 
$tdflag = $pdflag = 0;

$trtype = "customerreceipt2";
$trlink = "chicken_display_customerreceipt2.php";
...
for($i = 0;$i < $dsize;$i++){
    ...
    $trno_dt1 = generate_transaction_details($date[$i],"customerreceipt2","CRI","generate",$_SESSION['dbase']);
    ...
    if($discount[$i] == ""){ $discount[$i] = 0; }
    if((float)$tcds_amt[$i] > 0){ } else{ $tcds_per[$i] = 0; }
    
    $sql = "INSERT INTO `customer_receipts` (`incr`,`prefix`,`trnum`,`date`,`ccode`,`docno`,`mode`,`method`,`amount1`,`tcds_per`,`tcds_amt`,`amount`,`vtype`,`warehouse`,`remarks`,`flag`,`active`,`tdflag`,`pdflag`,`trtype`,`trlink`,`addedemp`,`addedtime`,`updatedtime`) 
    ...
    else {//Fetch Out-standing balance
...
<tempid>'.$sms_tempid.'</tempid>
    ...
    if($discount[$i] > 0){
        ...
        while($row = mysqli_fetch_assoc($query)){ $dincr = $row['cuscredit']; } $cincr = (int)$dincr + 1;

        $sql = "UPDATE `master_generator` SET `cuscredit` = '$cincr' WHERE `fdate` <='$date[$i]' AND `tdate` >= '$date[$i]' AND `type` = 'transactions'";
        ...
        VALUES('CCN','$code','$trnum','$date[$i]','$ccode[$i]','$coacode','Cr','$discount[$i]','$discount[$i]','C','0','1','$addedemp','$addedtime')";
        mysqli_query($conn,$sql2);
    }
}
header('location:chicken_display_customerreceipt2.php?ccid='.$ccid);
