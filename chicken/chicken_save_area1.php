...
$sql = "SELECT MAX(incr) as incr FROM `main_areas`";
...
for($i = 0;$i < count($description);$i++){
    if($description[$i] != ""){
        ...
        $sql = "INSERT INTO `main_areas`(`incr`, `prefix`, `code`, `description`, `addedemp`, `addedtime`, `flag`, `active`, `dflag`) 
        VALUES ('$incr','$prefix','$code','$description[$i]','$addedemp','$addedtime','0','1','0')";
        if(!mysqli_query($conn, $sql)){ die("Error: ".mysqli_error($conn)); } else{ }
    }
}

header('location:chicken_display_area1.php?ccid='.$cid);
