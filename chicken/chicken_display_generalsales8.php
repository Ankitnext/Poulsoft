<?php
//chicken_display_generalsales8.php
...
if($access_error_flag == 0){
    ...
    if($acslist_error_flag == 0){
        ...
        if((int)$sec_aflag == 0){ $sec_fltr = " AND `warehouse` IN ('$sec_list')"; }
    ?>
    <html>
        <head>
                        ...
                        <div class="box-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <?php
                                    ...
                                    if(in_array("jali_no", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `customer_sales` ADD `jali_no` VARCHAR(300) NULL DEFAULT NULL AFTER `invoice`"; mysqli_query($conn,$sql); }
                                    if(in_array("remarks2", $existing_col_names, TRUE) == ""){ $sql = "ALTER TABLE `customer_sales` ADD `remarks2` VARCHAR(300) NULL DEFAULT NULL AFTER `remarks`"; mysqli_query($conn,$sql); }
                                    ...
                                    $sql = "SELECT * FROM `customer_sales` WHERE `date` >= '$fdate' AND `date` <= '$tdate' AND `tdflag` = '0' AND `pdflag` = '0'".$sec_fltr." AND `trlink` LIKE '$href' ORDER BY `date` ASC";
