...
<tempid>'.$sms_tempid.'</tempid>
						...
						if($wapp_ptrn == "Template"){
							...
							if((int)$wapp_nfmt1 > 0){
                                ...
                                if((int)$slt_wflag > 0 && $from != "" && $to != ""){
                                    $words = ['Sale Details', 'Dear', 'Date', 'Sale Amt', 'Old Balance', 'Balance: Rs.', 'Thank You']; if(!empty($msg_header)){ $words[] = $msg_header; } if(!empty($msg_footer)){ $words[] = $msg_footer; }
                                    $res_words = convert_language($conns, $from, $to, $words);

                                    $message = "*".$res_words[strtolower("Sale Details")]."*%0D%0A".$res_words[strtolower("Dear")].": ".$customer_name."%0D%0A".$res_words[strtolower("Date")].": ".date("d.m.Y",strtotime($date)).",%0D%0A".$item_dlt.",%0D%0A".$res_words[strtolower("Old Balance")].": ".number_format_ind($old_bamt).",%0D%0A".$res_words[strtolower("Sale Amt")].": ".$totalamount."/-,%0D%0A".$res_words[strtolower("Balance: Rs.")]." ".$bal."/-%0D%0A".$res_words[strtolower("Thank You")].",%0D%0A".$cdetails;
                                ...
                                if((int)$slt_wflag > 0 && $from != "" && $to != ""){
                                    $words = ['Sale Details', 'Dear', 'Date', 'Sale Amt', 'Balance: Rs.', 'Thank You']; if(!empty($msg_header)){ $words[] = $msg_header; } if(!empty($msg_footer)){ $words[] = $msg_footer; }
                                    $res_words = convert_language($conns, $from, $to, $words);

                                    $message = "*".$res_words[strtolower("Sale Details")]."*%0D%0A".$res_words[strtolower("Dear")].": ".$customer_name."%0D%0A".$res_words[strtolower("Date")].": ".date("d.m.Y",strtotime($date)).",%0D%0A".$item_dlt."%0D%0A".$res_words[strtolower("Sale Amt")].": ".$totalamount."/-%0D%0A".$res_words[strtolower("Balance: Rs.")]." ".$bal."/-%0D%0A".$res_words[strtolower("Thank You")].",%0D%0A".$cdetails;
                                    $message = str_replace(" ","+",$message);
                                }
                                else{
	...
	<script>
		...
		if(x == true){
			window.location.href = "cus_add_multisales1.php";
		}
		else if(x == false){
